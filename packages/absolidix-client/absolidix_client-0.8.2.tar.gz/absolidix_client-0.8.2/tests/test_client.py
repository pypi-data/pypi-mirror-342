...
from absolidix_client.helpers import absolidix_json_decoder
...
async def test_json_case(
    client: AbsolidixClient, data: dict[str, Any] | str, expected: dict[str, Any] | str
):
    "Test case converter"
    async with client.request(
        url=URL(PATH_JSON_CASE_CHECK_STATUS), method="POST", json=data
    ) as resp:
        body = await resp.json(loads=absolidix_json_decoder)
