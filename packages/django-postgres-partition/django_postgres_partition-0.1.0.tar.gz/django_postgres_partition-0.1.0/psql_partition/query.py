...
class PostgresQuerySet(models.QuerySet):
    ...
    def _build_insert_compiler(self, rows: Iterable[Dict], using: Optional[str] = None):
        ...
        using = using or self._db or router.db_for_write(self.model, **self._hints)
        ...
        for index, row in enumerate(chain([first_row], rows_iter)):
            ...
            objs.append(self._create_model_instance(row, using, apply_converters=False))
