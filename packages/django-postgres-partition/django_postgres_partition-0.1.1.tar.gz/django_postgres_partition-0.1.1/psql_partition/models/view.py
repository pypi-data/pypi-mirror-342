...
from psql_partition.type_assertions import is_query_set, is_sql, is_sql_with_params
from psql_partition.types import SQL, SQLWithParams

from .base import PostgresModel
from .options import PostgresViewOptions

if TYPE_CHECKING:
    from psql_partition.backend.schema import PostgresSchemaEditor
...
class PostgresViewModelMeta(ModelBase):
    ...
    def __new__(cls, name, bases, attrs, **kwargs):
        ...
        sql_with_params = cls._view_query_as_sql_with_params(new_class, view_query)
...
class PostgresMaterializedViewModel(PostgresViewModel, metaclass=PostgresViewModelMeta):
    ...
    @classmethod
    def refresh(cls, concurrently: bool = False, using: Optional[str] = None) -> None:
        ...
        with connections[conn_name].schema_editor() as schema_editor:
            cast("PostgresSchemaEditor", schema_editor).refresh_materialized_view_model(
                cls, concurrently
            )
