...
def verify_run_ray_add_udf_column(
    tmp_path: Path, shuffle_config, udf: UDFTestConfig, batch_applier=None
) -> None:
...
def struct_udf(a: pa.Array) -> pa.Array:  # is the output type correct?
...
ret_struct_udftest = UDFTestConfig(
    ...
    {
        "a": list(range(256)),
        "b": [{"lpad": f"{x:04d}", "rpad": f"{x}0000"[:4]} for x in range(256)],
    },
)
...
ret_nested_udftest = UDFTestConfig(
    ...
    {
        "a": list(range(256)),
        "b": [{"lpad": f"{val:04d}", "array": [val] * val} for val in range(256)],
    },
)
