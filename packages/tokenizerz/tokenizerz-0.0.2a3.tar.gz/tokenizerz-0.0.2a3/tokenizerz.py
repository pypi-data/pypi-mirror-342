...
class Tokenizer:
    ...
    def __call__(self, text, use_chat_template=False, pad_token_id=1, pad_position_id=1, **kwargs):
        ...
        return self.pad_token_sequences(tokens, pad_token_id, pad_position_id)
...
def demo():
    ...
    iids, pids, mask = tokenizer(["#write a quick sort algorithm", "#hello world"])
    print(f"Batch:\n{iids=}\n{pids=}\n{mask=}")
    messages = [
        {"role": "system", "content": "You are a helpful assistant."},
        {"role": "user", "content": "Hello!"},
    ]
    iids, pids, mask = tokenizer(messages, use_chat_template=True, add_generation_prompt=True, tools=None)
    print(f"Format:\n{iids=}\n{pids=}\n{mask=}")
