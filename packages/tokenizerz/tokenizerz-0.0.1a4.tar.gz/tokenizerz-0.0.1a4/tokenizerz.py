...
import ctypes
import sys
from ctypes import c_char_p, c_uint32, c_size_t, POINTER, c_void_p

class Tokenizer:
    def __init__(self, model_name="Qwen2.5-Coder-1.5B-4bit"):
        ...
        try:
            ...
            raise RuntimeError(f"Failed to load the tokenizer library at {lib_path}: {e}")
        self.lib.create_tokenizer.argtypes = [c_char_p]
        ...
        self.handle = self.lib.create_tokenizer(model_name.encode('utf-8'))
        if not self.handle:
    ...
    def decode(self, tokens):
        ...
        max_text_len = len(tokens) * 4 + 1  # +1 for null terminator
