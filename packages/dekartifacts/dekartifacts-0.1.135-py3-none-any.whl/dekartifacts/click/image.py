...
from dektools.file import read_lines, remove_path, write_file, read_text
from dektools.typer import command_mixin, multi_options_to_dict, annotation
from dektools.shell import shell_wrapper
from dektools.fetch import download_dir, download_file, download_content
...
@app.command()
def rm(images: List[str]):
    for image in images:
        DockerArtifact.remove(image)


@app.command()
def exports(path, images: List[str]):
...
@app.command()
def serve(path, images: List[str], safe: bool = False, port: int = _serve_fetch_port):
    DockerArtifact.exports(images, path)
...
@app.command()
def sync(path, images: List[str], force: Annotated[bool, typer.Option("--force/--no-force")] = True):
