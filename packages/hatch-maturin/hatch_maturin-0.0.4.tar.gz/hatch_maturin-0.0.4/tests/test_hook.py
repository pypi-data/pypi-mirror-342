...
from hatch_maturin.builder import build_maturin_command
...
from hatch_maturin.config import get_config_with_defaults
from hatch_maturin.utils import get_python_path
from hatch_maturin.builder import build_maturin_command, collect_wheels, extract_wheel
...
class MaturinHookTest(MockBuildHookInterface):
    ...
    def _setup_logging(self):
        self.logger = logging.getLogger("hatch_maturin")
    ...
    def _process_single_crate(self, config, build_data):
        ...
        if not crate_path.exists():
            raise RuntimeError(f"[hatch-maturin] crate path missing: {crate_path}")
        ...
        self.app.display_mini_status("hatch-maturin", f"Compiling Rust crate{module_info}…")
        
        if hasattr(self, "subprocess_run_results") and self.subprocess_run_results:
            result = self.subprocess_run_results.pop(0)
            if isinstance(result, Exception):
                raise RuntimeError(f"[hatch-maturin] Maturin build failed: {result}") from result
        ...
        if config.get("parallel_build", False):
            ...
            for module_name, crate_path_str in crates.items():
                ...
                try:
                    ...
                    future.set_exception(RuntimeError(f"[hatch-maturin] Maturin build failed: {e}"))
    ...
    def _build_crate(self, module_name, crate_path_str, config, build_data):
        ...
        if not crate_path.exists():
            raise RuntimeError(f"[hatch-maturin] crate path missing for {module_name}: {crate_path}")
    ...
    def _run_build_scripts(self, config, crate_path, script_key):
        ...
        if script:
            ...
            if not script_path.exists():
                raise RuntimeError(f"[hatch-maturin] {script_key} script missing: {script_path}")
            ...
            if hasattr(self, "script_results") and self.script_results:
                ...
                if isinstance(result, Exception):
                    raise RuntimeError(f"[hatch-maturin] {script_key} script failed: {result}") from result
...
def test_status_message_display(tmp_hook_env):
    ...
    assert any("hatch-maturin" in msg[0] for msg in app.status_messages)