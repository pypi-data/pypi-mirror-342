...
class DataFrameDiff:
    ...
    @classmethod
    def create_diff(cls, expected_df, generated_df, key_columns, diff_key, column_subset=None):
        ...
        if column_subset is not None:
            ...
            if missing_keys:
                raise ValueError(f"Key columns {list(missing_keys)} must be included in column_subset.")
            ...
            error_msgs = []
            if missing_expected:
                error_msgs.append(f"Columns missing in expected dataset: {list(missing_expected)}")
            if missing_generated:
                error_msgs.append(f"Columns missing in generated dataset: {list(missing_generated)}")
            if error_msgs:
        ...
        value_columns = cls.get_value_columns(processed_expected_df, processed_generated_df, key_columns)

        # Create the joined DataFrame after splitting on primary key uniqueness.
        joined_df = cls.create_joined_df(
            df1=cls.split_df_by_pk_uniqueness(processed_generated_df, key_columns=key_columns)[0],
            df2=cls.split_df_by_pk_uniqueness(processed_expected_df, key_columns=key_columns)[0],
