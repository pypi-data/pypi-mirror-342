...
def _convert_table_to_features(
    table: pa.Table,
    columns_to_features: Callable[[Sequence[str]], Mapping[str, Feature]],
):
    features = columns_to_features(table.column_names)
    restricted_schema = pa.schema([pa.field(k, v.converter.pyarrow_dtype) for (k, v) in features.items()])
    table = table.select(list(features.keys()))
    table = table.cast(restricted_schema)
    table = table.rename_columns([x.root_fqn for x in features.values()])
    return table
    ...
    def _execute_query_efficient(
        ...
        with safe_trace("athena.execute_query_efficient"):
            ...
            with self._pyathena_connection().cursor() as cursor:
                ...
                if len(objects_list_response["Contents"]) <= 1:
                    chalk_logger.info(f"One file or less was unloaded: choose to query without unloading")
                    _, simple_query_fut = cursor.execute(
                        operation=formatted_op,
                        parameters=execution_params,
                        paramstyle=paramstyle,
                    )
                    query_result = simple_query_fut.result()
                    assert isinstance(
                        query_result, AthenaArrowResultSet
                    ), "Expected athena query result to be AthenaArrowResultSet"
                    parsed_table = query_result.as_arrow()
                    parsed_table = _convert_table_to_features(parsed_table, columns_to_features)
                    if parsed_table.num_rows > 0 or query_execution_parameters.yield_empty_batches:
