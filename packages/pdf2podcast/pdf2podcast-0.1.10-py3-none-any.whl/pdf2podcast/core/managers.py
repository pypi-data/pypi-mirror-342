...
from .tts import AWSPollyTTS, GoogleTTS, AzureTTS
...
class TTSManager:
    ...
    def __init__(self, tts_provider: str, **kwargs: Dict[str, Any]):
        ...
        """
        # Validate configuration
        if tts_provider not in ["aws", "google", "azure"]:
    ...
    def get_tts(self) -> Optional[BaseTTS]:
        ...
        try:
            if self.tts_provider == "aws":
                ...
                if "subscription_key" not in self.config:
                    logger.warning(
                        "No subscription_key specified for Azure TTS, using default"
                    )
                if "region_name" not in self.config:
                    logger.warning(
                        "No region_name specified for Azure TTS, using default"
                    )
