...
class AdapterRegistry:
    ...
    @classmethod
    def register(
        cls, provider_name: str, adapter_class: Type[BaseProviderAdapter]
    ) -> None:
        """
        ...
        if provider_name in cls._registry:
            raise ValueError(
                f"Adapter for provider '{provider_name}' is already registered"
            )
    ...
    @classmethod
    def create_adapter(
        cls, provider_name: str, provider: Provider
    ) -> BaseProviderAdapter:
        """
