        ...
        if tool_results:
            ...
            for tool_id, result_data in tool_results.items():
                ...
                if result_data:
                    ...
                    if not isinstance(response_data, dict) and not isinstance(
                        response_data, list
                    ):
        ...
        for tool in tools:
            ...
            if "parameters" in schema and "properties" in schema["parameters"]:
                ...
                for param_name, param_def in schema["parameters"]["properties"].items():
                    ...
                    clean_param = (
                        param_def.copy() if isinstance(param_def, dict) else param_def
                    )
                ...
                if "required" in schema["parameters"]:
                    function_declaration["parameters"]["required"] = schema[
                        "parameters"
                    ]["required"]
        ...
        try:
            ...
            if not tool_calls and hasattr(response, "text"):
                ...
                if "non-text parts in the response: ['function_call']" in warning_text:
                    logger.debug(
                        "Detected non-text parts warning, checking candidates manually"
                    )
                    try:
                        # Try to extract function calls directly from candidates
                        if (
                            hasattr(response, "_candidate_dict")
                            and response._candidate_dict
                        ):
                            candidate_dict = response._candidate_dict
                            if (
                                "content" in candidate_dict
                                and "parts" in candidate_dict["content"]
                            ):
                        ...
                        logger.debug(
                            f"Error extracting function calls from candidate dict: {extract_err}"
                        )
