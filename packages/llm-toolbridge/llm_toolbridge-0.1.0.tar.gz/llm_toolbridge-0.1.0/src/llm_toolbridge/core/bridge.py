        ...
        **kwargs,
        ...
        if isinstance(self.provider_or_adapter, BaseProviderAdapter):
            ...
            return await self._execute_with_provider(
                prompt, resolved_tools, max_tool_calls, **kwargs
            )

    async def _execute_with_provider(
        self, prompt: str, tools: List[Tool], max_tool_calls: int, **kwargs
    ) -> LLMResponse:
        """
        ...
        while response.tool_calls and call_count < max_tool_calls:
            ...
            response = await provider.generate(
                prompt=prompt, tools=[], tool_results=tool_results, **kwargs
            )

        return response

    def _resolve_tools(
        self, tools: Optional[List[Union[Tool, str]]] = None
    ) -> List[Tool]:
        """
