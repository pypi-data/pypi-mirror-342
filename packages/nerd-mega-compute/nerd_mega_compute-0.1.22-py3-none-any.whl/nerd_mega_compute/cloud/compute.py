...
from .on_server.on_server import generate_cloud_code
...
def cloud_compute(cores=8, timeout=1800):
    ...
    def decorator(func):
        ...
        def wrapper(*args, **kwargs):
            ...
            # Print debugging information about the data formats
            for i, arg in enumerate(args):
                if isinstance(arg, dict) and "__nerd_data_reference" in arg:
                    debug_print(f"Arg {i} is a data reference: {arg['__nerd_data_reference']}, S3 URI: {arg.get('__nerd_s3_uri', 'None')}")

            for key, value in kwargs.items():
                if isinstance(value, dict) and "__nerd_data_reference" in value:
                    debug_print(f"Kwarg {key} is a data reference: {value['__nerd_data_reference']}, S3 URI: {value.get('__nerd_s3_uri', 'None')}")
            ...
            # Add cloudpickle to the import block if not already present
            if "import cloudpickle" not in import_block:
                import_block = "import cloudpickle\nimport base64\n" + import_block
            elif "import base64" not in import_block:
                import_block = "import base64\n" + import_block

            # Serialize the arguments for cloud execution
            def serialize_with_chunking(obj, spinner=None):
                try:
                    # Special handling for large binary data that should go to S3 directly
                    if isinstance(obj, bytes) and len(obj) > 5 * 1024 * 1024:  # > 5MB
                    ...
                    # Use our simplified cloudpickle serialization approach
                    # 1. Pickle the object directly
                    pickled = cloudpickle.dumps(obj)
                    # 2. Base64 encode the binary data
                    encoded = base64.b64encode(pickled).decode('utf-8')
                    return {'type': 'cloudpickle', 'value': encoded}
