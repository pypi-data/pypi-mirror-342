...
def fetch_large_file(data_id, api_key, max_retries=3, retry_delay=1):
    ...
    Returns:
        bytes: The raw downloaded data
    """
    if not data_id:
        raise ValueError("data_id is required to fetch a large file")

    spinner = Spinner(f"Fetching file (ID: {data_id})...")
    spinner.start()
    ...
    try:
        ...
        for attempt in range(max_retries):
            try:
                debug_print(f"Requesting download URL for dataId: {data_id}")
        ...
        spinner.stop()
        spinner = Spinner(f"Downloading data from S3...")
        spinner.start()
