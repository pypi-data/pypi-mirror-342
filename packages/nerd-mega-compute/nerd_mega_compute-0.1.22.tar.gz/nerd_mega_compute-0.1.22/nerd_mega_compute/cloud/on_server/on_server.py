...
def deserialize_object(obj):
    if isinstance(obj, dict) and 'type' in obj:
        if obj['type'] == 'cloudpickle':
            ...
            # Simple string value
            return obj['value']
        # Handle data references for astronomy tests
        elif '__nerd_data_reference' in obj:
            try:
                import boto3
                ...
                if s3_uri.startswith('s3://'):
                    ...
                    if len(parts) == 2:
                        ...
                        s3 = boto3.client('s3')
                        response = s3.get_object(Bucket=bucket, Key=key)
                        return pickle.loads(zlib.decompress(response['Body'].read()))
                return f"Could not load data reference: {{s3_uri}}"
            except Exception as e:
                print(f"Error retrieving data reference: {{e}}")
                traceback.print_exc()
                return None
        else:
            # Unknown type
        ...
        # Check if this might be a data reference
        if '__nerd_data_reference' in obj:
            # This is a special data reference, handle it
            try:
                import boto3
                s3_uri = obj.get('__nerd_s3_uri', '')
                if s3_uri.startswith('s3://'):
                ...
                traceback.print_exc()
                return obj
        # Recursively deserialize regular dictionaries
