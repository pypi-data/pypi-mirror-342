...
def deserialize_arg(arg_data):
    if isinstance(arg_data, dict):
        if arg_data.get('type') == 'data':
            ...
            # Return the reference as is - it will be handled by auto_reference_wrapper
            # which contains the proper S3 fetch implementation
            reference = arg_data['value']['data_reference']
            print(f"Resolving reference: {reference}")
            return arg_data
        else:
