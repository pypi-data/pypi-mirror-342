        ...
        if s3_uri.startswith("s3://"):
            ...
            if len(parts) == 2:
                ...
                try:
                    ...
                    try:
                        # First try to decompress then unpickle
                        ...
                        if isinstance(unpickled_data, dict) and 'sources' in unpickled_data and 'positions' in unpickled_data:
                            print("Detected sparse catalog data structure")
                            # No pre-processing needed - DataFrame creation happens in the function
                            
                        # Case 2: ML model
                        elif isinstance(unpickled_data, dict) and 'model' in unpickled_data:
                            print("Detected ML model data structure")
                            # If the model is bytes, try to unpickle it
                            if isinstance(unpickled_data['model'], bytes):
                                try:
                                    unpickled_data['model'] = pickle.loads(unpickled_data['model'])
                                    print(f"Successfully unpickled model of type: {type(unpickled_data['model']).__name__}")
                                except Exception as e:
                                    print(f"Error unpickling model: {e}")
                        
                        # Case 3: HEALPix map
                        elif isinstance(unpickled_data, dict) and 'pixels' in unpickled_data:
                            print("Detected HEALPix map data structure")
                            # Make sure we have pixel_values
                            if 'pixel_values' not in unpickled_data:
                                print("Adding pixel_values key with pixels data")
                                unpickled_data['pixel_values'] = unpickled_data['pixels']
