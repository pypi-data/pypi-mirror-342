        ...
        if arg_data['type'] == 'data':
            try:
                ...
                if isinstance(obj, dict):
                    # Handle sparse catalog
                    if 'sources' in obj and 'positions' in obj and 'fluxes' in obj:
                        print("Detected sparse catalog in deserialize_arg")
                        try:
                            from pandas import DataFrame
                            import numpy as np
                            
                            # Convert the sparse catalog to a proper DataFrame
                            df_dict = {
                                'source_id': obj['sources'],
                                'ra': obj['positions'][:, 0] if isinstance(obj['positions'], np.ndarray) else [p[0] for p in obj['positions']],
                                'dec': obj['positions'][:, 1] if isinstance(obj['positions'], np.ndarray) else [p[1] for p in obj['positions']],
                                'flux': obj['fluxes']
                            }
                            
                            # Create and return the DataFrame
                            print(f"Creating DataFrame with keys: {list(df_dict.keys())}")
                            catalog_df = DataFrame(df_dict)
                            print(f"Successfully created DataFrame with shape: {catalog_df.shape}")
                            return catalog_df
                    ...
                    # Handle ML model
                        ...
                        print("Detected ML model in deserialize_arg")
                        if isinstance(obj['model'], bytes):
                            try:
                    ...
                    # Handle HEALPix map
                        ...
                        print("Detected HEALPix map in deserialize_arg")
                        if 'pixel_values' not in obj:
                            obj['pixel_values'] = obj['pixels']
                            print("Added pixel_values key")
