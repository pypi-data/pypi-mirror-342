...
class GithubOauth:
	...
	def git_oauth(self) -> bool:
		...
		try:
			current_pasteboard = pyperclip.paste()
			pyperclip.copy(current_pasteboard)
			with_cp = True
		except:
			# copy past not permitted
			with_cp = False

		if with_cp:
			vraag = input(Css.normal(f"{Css.attention()} Your Github login code is: {Css.reset()} {req_params['user_code'][0]}\n{Css.attention()} Copy to clipboard and continue [Y/N] or Quit [Q]? {Css.reset()} "))
		else:
			vraag = input(Css.normal(f"{Css.attention()} Your Github login code is: {Css.reset()} {req_params['user_code'][0]}\n{Css.attention()} Continue or Quit [Y/Q]? {Css.reset()} "))

		if vraag.lower() == 'q':
			print(Css.warning(f"Leaving so soon?"))
			sys.exit(1)
		if vraag.lower() == 'y' and with_cp:
...
class MonApi:
	...
	def test(self):
		url = f'{self.gho.domain}/api/update_user'
		try:
	...
	def local_user(self, luser: dict):
		...
		try:
			...
			self.log.add(Ins.info(), str(e))
			print(Css.wrong(f"The check11 users API at DeeBeeBros.com is not available now. Try again later. {Ins.info()}"))
			return False
	...
	def update_check(self, check: dict) -> bool:
		...
		url = f"{self.gho.domain}/api/update_check"
		try:
			response = requests.post(url, json=jso)
			return response.status_code == 200
		except Exception as e:
			self.log.add(Ins.info(), str(e))
			print(Css.wrong(f"The check11 API at DeeBeeBros.com is not available now. Try again later.  {Ins.info()}"))
			return False
