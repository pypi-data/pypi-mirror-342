...
def _process_nested_type(
    ...
    max_depth: int,
    current_depth: int,
) -> tuple[str, list[None | tuple[str, list[None | tuple[str, list[None]]]]]]:
...
def build_nested_selection(
    ...
    max_depth: int,
    current_depth: int = 1,
) -> list[tuple[str, list[None | tuple[str, list[None | tuple[str, list[None]]]]]]]:
    ...
    if ds:
        ...
        for query_name, field in fields.items():
            ...
            for arg_name, arg in args_map.items():
                logger.info("Converting GraphQL type for %s: %s", arg_name, arg.type)
