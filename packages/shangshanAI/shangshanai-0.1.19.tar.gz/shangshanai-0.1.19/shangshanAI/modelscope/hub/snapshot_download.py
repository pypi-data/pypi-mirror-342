...
from shangshanAI.modelscope.hub.api import HubApi, ModelScopeConfig
from shangshanAI.modelscope.hub.constants import \
    MODELSCOPE_SHOW_INDIVIDUAL_PROGRESS_THRESHOLD
from shangshanAI.modelscope.hub.errors import InvalidParameter
from shangshanAI.modelscope.hub.file_download import (create_temporary_directory_and_cache,
                                          download_file, get_file_download_url)
from shangshanAI.modelscope.hub.utils.caching import ModelFileSystemCache
from shangshanAI.modelscope.hub.utils.utils import (get_model_masked_directory,
                                        model_id_to_group_owner_name)
from shangshanAI.modelscope.utils.constant import (DEFAULT_DATASET_REVISION,
                                       DEFAULT_MODEL_REVISION,
                                       DEFAULT_REPOSITORY_REVISION,
                                       REPO_TYPE_DATASET, REPO_TYPE_MODEL,
                                       REPO_TYPE_SUPPORT)
from shangshanAI.modelscope.utils.logger import get_logger
from shangshanAI.modelscope.utils.thread_utils import thread_executor
...
def _download_file_lists(
    ...
    filtered_repo_files = []
    for repo_data in repo_files:
        if repo_data['type'] == 'tree':
            continue
        file_path = repo_data['path']
        repo_file = api.get_file_metadata(repo_id, file_path)

        try:
            # processing patterns
            if ignore_patterns and any([
                    fnmatch.fnmatch(repo_file['X-Gitlab-File-Path'], pattern)
            ...
            if ignore_regex_pattern and any([
                    re.search(pattern, repo_file['X-Gitlab-File-Name']) is not None
            ...
            if cache.exists(repo_file):
                file_name = os.path.basename(repo_file['X-Gitlab-File-Name'])
                logger.debug(
    ...
    def _download_single_file(repo_file):
        if repo_type == REPO_TYPE_MODEL:
            ...
            #     revision=revision)
            url = api.get_model_file_url(
                model_id=repo_id,
                file_path=repo_file['X-Gitlab-File-Path'],
                revision=revision)
        # elif repo_type == REPO_TYPE_DATASET:
