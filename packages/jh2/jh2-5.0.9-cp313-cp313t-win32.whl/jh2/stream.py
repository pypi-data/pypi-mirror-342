...
try:
    from ._hazmat import Encoder  # noqa: F401

    ALTERNATIVE_HPACK = True
except ImportError:
    ALTERNATIVE_HPACK = False
...
_transitions = {
    ...
    (StreamState.IDLE, StreamInputs.SEND_HEADERS): (
        H2StreamStateMachine.request_sent,
        StreamState.OPEN,
    ),
    ...
    (StreamState.IDLE, StreamInputs.RECV_DATA): (
        H2StreamStateMachine.reset_stream_on_error,
        StreamState.CLOSED,
    ),
    ...
    (StreamState.IDLE, StreamInputs.RECV_ALTERNATIVE_SERVICE): (None, StreamState.IDLE),
    ...
    (StreamState.RESERVED_LOCAL, StreamInputs.SEND_HEADERS): (
        H2StreamStateMachine.response_sent,
        StreamState.HALF_CLOSED_REMOTE,
    ),
    ...
    (StreamState.RESERVED_LOCAL, StreamInputs.RECV_WINDOW_UPDATE): (
        H2StreamStateMachine.window_updated,
        StreamState.RESERVED_LOCAL,
    ),
    ...
    (StreamState.RESERVED_LOCAL, StreamInputs.RECV_RST_STREAM): (
        H2StreamStateMachine.stream_reset,
        StreamState.CLOSED,
    ),
    (StreamState.RESERVED_LOCAL, StreamInputs.SEND_ALTERNATIVE_SERVICE): (
        H2StreamStateMachine.send_alt_svc,
        StreamState.RESERVED_LOCAL,
    ),
    ...
    (StreamState.RESERVED_REMOTE, StreamInputs.RECV_ALTERNATIVE_SERVICE): (
        H2StreamStateMachine.recv_alt_svc,
        StreamState.RESERVED_REMOTE,
    ),
    # State: open
    ...
    (StreamState.OPEN, StreamInputs.RECV_DATA): (
        H2StreamStateMachine.data_received,
        StreamState.OPEN,
    ),
    ...
    (StreamState.HALF_CLOSED_REMOTE, StreamInputs.SEND_END_STREAM): (
        H2StreamStateMachine.send_end_stream,
        StreamState.CLOSED,
    ),
    ...
    (StreamState.HALF_CLOSED_LOCAL, StreamInputs.RECV_END_STREAM): (
        H2StreamStateMachine.stream_ended,
        StreamState.CLOSED,
    ),
    ...
    # State: closed
    (StreamState.CLOSED, StreamInputs.RECV_END_STREAM): (None, StreamState.CLOSED),
    ...
    (StreamState.CLOSED, StreamInputs.RECV_HEADERS): (
        H2StreamStateMachine.recv_on_closed_stream,
        StreamState.CLOSED,
    ),
    ...
    (StreamState.CLOSED, StreamInputs.RECV_WINDOW_UPDATE): (None, StreamState.CLOSED),
    (StreamState.CLOSED, StreamInputs.RECV_RST_STREAM): (None, StreamState.CLOSED),
...
class H2Stream:
    ...
    def __init__(self, stream_id, config, inbound_window_size, outbound_window_size):
    ...
    def upgrade(self, client_side):
        ...
        input_ = (
            StreamInputs.UPGRADE_CLIENT if client_side else StreamInputs.UPGRADE_SERVER
        )
    ...
    def send_headers(self, headers, encoder, end_stream=False):
        ...
        if (not self.state_machine.client) and is_informational_response(headers):
            if end_stream:
                raise ProtocolError("Cannot set END_STREAM on informational responses.")
        ...
        frames = self._build_headers_frames(headers, encoder, hf, hdr_validation_flags)
    ...
    def receive_push_promise_in_band(
        self, promised_stream_id, headers, header_encoding
    ):
        """
        ...
        """
        self.config.logger.debug(
            "Receive Push Promise on %r for remote stream %d", self, promised_stream_id
        )
    ...
    def receive_headers(self, headers, end_stream, header_encoding):
        ...
        if end_stream:
            es_events = self.state_machine.process_input(StreamInputs.RECV_END_STREAM)
    ...
    def reset_stream(self, error_code=0):
        ...
        """
        self.config.logger.debug("Local reset %r with error code: %d", self, error_code)
    ...
    def acknowledge_received_data(self, acknowledged_size):
        ...
        """
        self.config.logger.debug(
            "Acknowledge received data with size %d on %r", acknowledged_size, self
        )
    ...
    def _build_hdr_validation_flags(self, events):
        ...
        is_response_header = isinstance(
            events[0], (_ResponseSent, ResponseReceived, InformationalResponseReceived)
        )
    ...
    def _build_headers_frames(
        self, headers, encoder, first_frame, hdr_validation_flags
    ):
        """
        ...
        header_blocks = [
            ...
            for i in range(0, len(encoded_headers), self.max_outbound_frame_size)
        ]
    ...
    def _process_received_headers(
        self, headers, header_validation_flags, header_encoding
    ):
        """
