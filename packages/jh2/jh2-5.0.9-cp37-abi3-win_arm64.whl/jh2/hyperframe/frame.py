...
class Priority:
    ...
    def serialize_priority_data(self) -> bytes:
        return _STRUCT_LB.pack(
            self.depends_on + (0x80000000 if self.exclusive else 0), self.stream_weight
        )
...
class DataFrame(Padding, Frame):
    ...
    def parse_body(self, data: memoryview) -> None:
        ...
        self.data = data[padding_data_length : len(data) - self.pad_length].tobytes()
...
class PriorityFrame(Priority, Frame):
    ...
    def _body_repr(self) -> str:
        return "exclusive={}, depends_on={}, stream_weight={}".format(
            self.exclusive, self.depends_on, self.stream_weight
        )
    ...
    def parse_body(self, data: memoryview) -> None:
        if len(data) > 5:
            raise InvalidFrameError(
                "PRIORITY must have 5 byte body: actual length %s." % len(data)
            )
...
class RstStreamFrame(Frame):
    ...
    def parse_body(self, data: memoryview) -> None:
        if len(data) != 4:
            raise InvalidFrameError(
                "RST_STREAM must have 4 byte body: actual length %s." % len(data)
            )
...
class SettingsFrame(Frame):
    ...
    def serialize_body(self) -> bytes:
        return b"".join(
            [
                _STRUCT_HL.pack(setting & 0xFF, value)
    ...
    def parse_body(self, data: memoryview) -> None:
        if "ACK" in self.flags and len(data) > 0:
            raise InvalidDataError(
                "SETTINGS ack frame must not have payload: got %s bytes" % len(data)
            )
...
class PushPromiseFrame(Padding, Frame):
    ...
    def parse_body(self, data: memoryview) -> None:
        ...
        self.data = data[
            padding_data_length + 4 : len(data) - self.pad_length
        ].tobytes()
        self.body_len = len(data)

        if self.promised_stream_id == 0 or self.promised_stream_id % 2 != 0:
            raise InvalidDataError(
                "Invalid PUSH_PROMISE promised stream id: %s" % self.promised_stream_id
            )
...
class GoAwayFrame(Frame):
    ...
    def serialize_body(self) -> bytes:
        data = _STRUCT_LL.pack(self.last_stream_id & 0x7FFFFFFF, self.error_code)
...
class WindowUpdateFrame(Frame):
    ...
    def parse_body(self, data: memoryview) -> None:
        if len(data) > 4:
            raise InvalidFrameError(
                "WINDOW_UPDATE frame must have 4 byte length: got %s" % len(data)
            )
...
class HeadersFrame(Padding, Priority, Frame):
    ...
    def parse_body(self, data: memoryview) -> None:
        ...
        self.body_len = len(data)
        self.data = data[priority_data_length : len(data) - self.pad_length].tobytes()
...
class AltSvcFrame(Frame):
    ...
    def __init__(
        self, stream_id: int, origin: bytes = b"", field: bytes = b"", **kwargs: Any
    ) -> None:
        super().__init__(stream_id, **kwargs)
