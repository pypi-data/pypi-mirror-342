...
try:
    # We try to load optimized hpack encoder and decoder (rust module)
    # for the sake of speed...
    from ._hazmat import Decoder, Encoder, HPACKError, OversizedHeaderListError
...
from .settings import SettingCodes, Settings
...
class H2ConnectionStateMachine:
    ...
    _transitions = {
        ...
        (ConnectionState.CLIENT_OPEN, ConnectionInputs.RECV_ALTERNATIVE_SERVICE): (
            None,
            ConnectionState.CLIENT_OPEN,
        ),
        ...
        (ConnectionState.SERVER_OPEN, ConnectionInputs.SEND_ALTERNATIVE_SERVICE): (
            None,
            ConnectionState.SERVER_OPEN,
        ),
        (ConnectionState.SERVER_OPEN, ConnectionInputs.RECV_ALTERNATIVE_SERVICE): (
            None,
            ConnectionState.SERVER_OPEN,
        ),
        # State: closed
...
class H2Connection:
    ...
    def send_headers(
        ...
        priority_present = (
            ...
            or (priority_depends_on is not None)
            or (priority_exclusive is not None)
        )
    ...
    def send_data(self, stream_id, data, end_stream=False, pad_length=None):
        ...
        self.config.logger.debug(
            "Outbound flow control window size is %d", self.outbound_flow_control_window
        )
    ...
    def increment_flow_control_window(self, increment, stream_id=None):
        ...
        if stream_id is not None:
            ...
            self.config.logger.debug(
                "Increase stream ID %d flow control window by %d", stream_id, increment
            )
        else:
    ...
    def local_flow_control_window(self, stream_id):
        ...
        *smaller*.
        ...
        stream = self._get_stream_by_id(stream_id)
        return min(
            self.outbound_flow_control_window, stream.outbound_flow_control_window
        )
    ...
    def acknowledge_received_data(self, acknowledged_size, stream_id):
        ...
        if stream_id <= 0:
            raise ValueError(
                "Stream ID %d is not valid for acknowledge_received_data" % stream_id
            )
    ...
    def _receive_headers_frame(self, frame):
        ...
        frames, stream_events = stream.receive_headers(
            headers, "END_STREAM" in frame.flags, self.config.header_encoding
        )
    ...
    def _receive_push_promise_frame(self, frame):
        ...
        try:
            ...
            if (
                self._stream_closed_by(frame.stream_id)
    ...
    def _receive_settings_frame(self, frame):
        ...
        events.append(
            RemoteSettingsChanged.from_settings(self.remote_settings, frame.settings)
        )
    ...
    def _stream_is_closed_by_end(self, stream_id):
        ...
        """
        return self._stream_closed_by(stream_id) in (
            StreamClosedBy.RECV_END_STREAM,
...
def _decode_headers(decoder, encoded_header_block):
    ...
    try:
        ...
        # this branch only applies to the optimized (e.g. alt hpack encoder/decoder)
        # the underlying library does not handle well enough invalid payloads.
