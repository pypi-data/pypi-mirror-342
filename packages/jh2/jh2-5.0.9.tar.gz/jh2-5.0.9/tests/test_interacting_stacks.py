...
class TestCommunication(coroutine_tests.CoroutineTestCase):
    ...
    def test_basic_request_response(self):
        """
        A request issued by hyper-h2 can be responded to by hyper-jh2.
        """
        request_headers = [
            (b':method', b'GET'),
            (b':path', b'/'),
        ...
        def client():
            ...
            assert len(events) == 2
            assert isinstance(events[0], jh2.events.SettingsAcknowledged)
            assert isinstance(events[1], jh2.events.RemoteSettingsChanged)
            ...
            events = c.receive_data(data)
            assert len(events) == 2
            assert isinstance(events[0], jh2.events.ResponseReceived)
            assert events[0].stream_id == 1
            assert events[0].headers == response_headers
            assert isinstance(events[1], jh2.events.StreamEnded)
        ...
        def server():
            ...
            assert len(events) == 3
            assert isinstance(events[0], jh2.events.SettingsAcknowledged)
            assert isinstance(events[1], jh2.events.RequestReceived)
            assert events[1].stream_id == 1
            assert events[1].headers == request_headers
            assert isinstance(events[2], jh2.events.StreamEnded)
