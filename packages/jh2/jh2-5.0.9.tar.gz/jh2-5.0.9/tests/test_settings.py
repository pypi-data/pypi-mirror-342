...
class TestSettings(object):
    ...
    def test_can_set_initial_values(self, client):
        ...
        """
        overrides = {
            jh2.settings.SettingCodes.HEADER_TABLE_SIZE: 8080,
            jh2.settings.SettingCodes.MAX_FRAME_SIZE: 16388,
            jh2.settings.SettingCodes.MAX_CONCURRENT_STREAMS: 100,
            jh2.settings.SettingCodes.MAX_HEADER_LIST_SIZE: 2**16,
            jh2.settings.SettingCodes.ENABLE_CONNECT_PROTOCOL: 1,
        }
        s = jh2.settings.Settings(client=client, initial_values=overrides)
        ...
        assert s[jh2.settings.SettingCodes.ENABLE_CONNECT_PROTOCOL] == 1

    @pytest.mark.parametrize(
        'setting,value',
        [
            (jh2.settings.SettingCodes.ENABLE_PUSH, 2),
            (jh2.settings.SettingCodes.ENABLE_PUSH, -1),
            (jh2.settings.SettingCodes.INITIAL_WINDOW_SIZE, -1),
            (jh2.settings.SettingCodes.INITIAL_WINDOW_SIZE, 2**34),
            (jh2.settings.SettingCodes.MAX_FRAME_SIZE, 1),
            (jh2.settings.SettingCodes.MAX_FRAME_SIZE, 2**30),
            (jh2.settings.SettingCodes.MAX_HEADER_LIST_SIZE, -1),
            (jh2.settings.SettingCodes.ENABLE_CONNECT_PROTOCOL, -1),
        ]
    )
    def test_cannot_set_invalid_initial_values(self, setting, value):
        ...
        with pytest.raises(jh2.exceptions.InvalidSettingsValueError):
            jh2.settings.Settings(initial_values=overrides)
    ...
    def test_acknowledging_returns_the_changed_settings(self):
        ...
        s = jh2.settings.Settings(client=True)
        s[jh2.settings.SettingCodes.HEADER_TABLE_SIZE] = 8000
        s[jh2.settings.SettingCodes.ENABLE_PUSH] = 0

        changes = s.acknowledge()
        assert len(changes) == 2

        table_size_change = (
            changes[jh2.settings.SettingCodes.HEADER_TABLE_SIZE]
        )
        push_change = changes[jh2.settings.SettingCodes.ENABLE_PUSH]
    ...
    def test_acknowledging_only_returns_changed_settings(self):
        ...
        """
        s = jh2.settings.Settings(client=True)
        s[jh2.settings.SettingCodes.INITIAL_WINDOW_SIZE] = 70

        changes = s.acknowledge()
        assert len(changes) == 1
        assert list(changes.keys()) == [
            jh2.settings.SettingCodes.INITIAL_WINDOW_SIZE
        ]
    ...
    def test_deleting_values_deletes_all_of_them(self):
        ...
        with pytest.raises(KeyError):
            s[jh2.settings.SettingCodes.HEADER_TABLE_SIZE]
    ...
    def test_settings_getters(self):
        ...
        assert s.enable_push == s[jh2.settings.SettingCodes.ENABLE_PUSH]
    ...
    def test_settings_setters(self):
        ...
        s.acknowledge()
        assert s[jh2.settings.SettingCodes.HEADER_TABLE_SIZE] == 0
        assert s[jh2.settings.SettingCodes.ENABLE_PUSH] == 1
        assert s[jh2.settings.SettingCodes.INITIAL_WINDOW_SIZE] == 2
        assert s[jh2.settings.SettingCodes.MAX_FRAME_SIZE] == 16385
        assert s[jh2.settings.SettingCodes.MAX_CONCURRENT_STREAMS] == 4
        assert s[jh2.settings.SettingCodes.MAX_HEADER_LIST_SIZE] == 2**16
        assert s[jh2.settings.SettingCodes.ENABLE_CONNECT_PROTOCOL] == 1
    ...
    def test_cannot_set_invalid_values_for_enable_push(self, val):
        ...
        with pytest.raises(jh2.exceptions.InvalidSettingsValueError) as e:
            s[jh2.settings.SettingCodes.ENABLE_PUSH] = val
    ...
    def test_cannot_set_invalid_values_for_max_frame_size(self, val):
        ...
        if 2**14 <= val <= 2**24 - 1:
            ...
            with pytest.raises(jh2.exceptions.InvalidSettingsValueError) as e:
                s[jh2.settings.SettingCodes.MAX_FRAME_SIZE] = val
...
class TestSettingsEquality(object):
    ...
    SettingsStrategy = builds(
        jh2.settings.Settings,
        client=booleans(),
        initial_values=fixed_dictionaries({
            jh2.settings.SettingCodes.HEADER_TABLE_SIZE:
                integers(0, 2**32 - 1),
            jh2.settings.SettingCodes.ENABLE_PUSH: integers(0, 1),
            jh2.settings.SettingCodes.INITIAL_WINDOW_SIZE:
                integers(0, 2**31 - 1),
            jh2.settings.SettingCodes.MAX_FRAME_SIZE:
                integers(2**14, 2**24 - 1),
            jh2.settings.SettingCodes.MAX_CONCURRENT_STREAMS:
                integers(0, 2**32 - 1),
            jh2.settings.SettingCodes.MAX_HEADER_LIST_SIZE:
                integers(0, 2**32 - 1),
        })
    )
