...
import jh2.settings
import jh2.utilities

try:
    from jh2._hazmat import Encoder
    ALTERNATIVE_HPACK = True
except ImportError:
    ALTERNATIVE_HPACK = False

from jh2.hyperframe import frame
...
class TestInvalidFrameSequences(object):
    ...
    def test_pseudo_headers_rejected_in_trailer(self, frame_factory):
        ...
        with pytest.raises(jh2.exceptions.ProtocolError) as e:
...
class TestSendingInvalidFrameSequences(object):
    ...
    def test_headers_event_skipping_validation(self, frame_factory, headers):
        ...
        norm_headers = jh2.utilities.normalize_outbound_headers(headers, None, False)
...
class TestFilter(object):
    ...
    validation_functions = [
        jh2.utilities.validate_headers,
        jh2.utilities.validate_outbound_headers
    ]

    hdr_validation_combos = [
        jh2.utilities.HeaderValidationFlags(
    ...
    def test_invalid_pseudo_headers(self, hdr_validation_flags):
        ...
        with pytest.raises(jh2.exceptions.ProtocolError):
            list(jh2.utilities.validate_headers(headers, hdr_validation_flags))
    ...
    def test_matching_authority_host_headers(self,
        ...
        assert headers == list(jh2.utilities.validate_headers(
            headers, hdr_validation_flags
        ))
    ...
    def test_inbound_header_name_length(self, hdr_validation_flags):
        with pytest.raises(jh2.exceptions.ProtocolError):
            list(jh2.utilities.validate_headers([(b'', b'foobar')], hdr_validation_flags))
    ...
    def test_inbound_header_name_length_full_frame_decode(self, frame_factory):
        ...
        c = jh2.connection.H2Connection(config=jh2.config.H2Configuration(client_side=False))
        c.initiate_connection()
        c.receive_data(frame_factory.preamble())
        c.clear_outbound_data_buffer()

        with pytest.raises(jh2.exceptions.ProtocolError, match="Received header name with zero length."):
            c.receive_data(data)
...
class TestOversizedHeaders(object):
    ...
    def test_hpack_bomb_request(self, frame_factory):
        ...
        attack_frame = frame.HeadersFrame(stream_id=3)
        attack_frame.data = self.second_header_block
        attack_frame.flags.add('END_HEADERS')
        data = attack_frame.serialize()

        with pytest.raises(jh2.exceptions.DenialOfServiceError):
            c.receive_data(data)

        expected_frame = frame_factory.build_goaway_frame(
            last_stream_id=1, error_code=jh2.errors.ErrorCodes.ENHANCE_YOUR_CALM
        )
        assert c.data_to_send() == expected_frame.serialize()
    ...
    def test_hpack_bomb_response(self, frame_factory):
        ...
        expected_frame = frame_factory.build_goaway_frame(
            last_stream_id=0, error_code=jh2.errors.ErrorCodes.ENHANCE_YOUR_CALM
        )
        assert c.data_to_send() == expected_frame.serialize()
    ...
    def test_hpack_bomb_push(self, frame_factory):
        ...
        attack_frame = frame.PushPromiseFrame(stream_id=3)
    ...
    def test_reject_headers_when_list_size_shrunk(self, frame_factory):
        ...
        c.update_settings({jh2.settings.SettingCodes.MAX_HEADER_LIST_SIZE: 49})
        c.clear_outbound_data_buffer()
        f = frame_factory.build_headers_frame(
            stream_id=3,
            headers=self.request_header_block
        ...
        expected_frame = frame_factory.build_goaway_frame(
            last_stream_id=3, error_code=jh2.errors.ErrorCodes.ENHANCE_YOUR_CALM
        )
        assert c.data_to_send() == expected_frame.serialize()

    @pytest.mark.xfail(ALTERNATIVE_HPACK is True, reason="alternative hpack need a patch")
    def test_reject_headers_when_table_size_shrunk(self, frame_factory):
        ...
        # Now, send a settings change. It's un-ACKed at this time. A new
        # request arrives, also without incident.
        c.update_settings({jh2.settings.SettingCodes.HEADER_TABLE_SIZE: 128})
        c.clear_outbound_data_buffer()
        f = frame_factory.build_headers_frame(
            stream_id=3,
            headers=self.request_header_block
        ...
        expected_frame = frame_factory.build_goaway_frame(
            last_stream_id=3, error_code=jh2.errors.ErrorCodes.PROTOCOL_ERROR
        )
        assert c.data_to_send() == expected_frame.serialize()
