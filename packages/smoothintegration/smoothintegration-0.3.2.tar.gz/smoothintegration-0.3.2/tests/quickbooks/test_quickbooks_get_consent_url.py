...
class TestQuickbooksGetConsentUrl:
    ...
    def test_can_get_consent_url(self, mocked_responses, test_client):
        mocked_responses.add(
            responses.GET,
            "https://api.smooth-integration.com/v1/quickbooks/connect?company_id=d56b018d-42a3-4f47-b141-44d9d4d81878",
            json={
    ...
    def test_raises_error_on_unauthorized(self, mocked_responses, test_client):
        mocked_responses.add(
            responses.GET,
            "https://api.smooth-integration.com/v1/quickbooks/connect?company_id=invalid-company-id",
            json={"message": "Invalid 'X-Organisation' header"},
            status=401,
        )

        with pytest.raises(SIError) as excinfo:
            test_client.quickbooks.get_consent_url("invalid-company-id")
