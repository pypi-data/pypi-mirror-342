...
class DCAT_MODEL(SODAS_SDK_CLASS, ABC):
    ...
    async def populate_from_dto(self, dto: DCAT_MODEL_DTO) -> None:
        ...
        if dto.CreatedAt:
            self._CreatedAt = datetime.fromisoformat(dto.CreatedAt)
        if dto.UpdatedAt:
            self._UpdatedAt = datetime.fromisoformat(dto.UpdatedAt)
        ...
        *additional_args: Any,
        ...
        if "total" in data and isinstance(data["list"], list):
            ...
            for item in data["list"]:
                instance = cls()
                await instance.populate_from_dto(DCAT_MODEL_DTO(**item))
