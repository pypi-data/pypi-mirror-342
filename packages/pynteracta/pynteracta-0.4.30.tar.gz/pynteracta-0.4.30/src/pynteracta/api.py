...
from . import urls
from .constants import DEFAULT_LOGIN_HEADERS
...
from .schemas.models import BaseListPostsElement, Group
...
class InteractaApi(Api):
        ...
        **kwargs,
        ...
        if not params:
            params = {
                ...
                "loadMainAttachmentPreviewImageHiResLink": False,
                "loadMainAttachmentPreviewImageHiResAnimatedLink": False,
            }
    ...
    def list_all_posts(
        self, community_id: str | int, data: ListCommunityPostsFilteredIn | None = None, **kwargs
    ) -> list[BaseListPostsElement]:
        all_posts = []
        page_token = None
        data = data if data else ListCommunityPostsFilteredIn()
        while True:
            data.page_token = page_token
            posts = self.list_posts_filtered(community_id=community_id, data=data, **kwargs)
