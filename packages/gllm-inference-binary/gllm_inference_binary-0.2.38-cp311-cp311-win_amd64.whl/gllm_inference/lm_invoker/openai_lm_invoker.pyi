...
class OpenAILMInvoker(LangChainLMInvoker):
    """A language model invoker to interact with language models hosted through OpenAI API endpoints.

    The `OpenAILMInvoker` class is responsible for invoking a language model using the OpenAI API. It handles both
    standard and streaming invocation. Streaming mode is enabled if an event emitter is provided. It also supports both
    tool calling and structured output capabilities.

    Attributes:
        llm (ChatOpenAI): The LLM instance to interact with a language model hosted through OpenAI API endpoints.
