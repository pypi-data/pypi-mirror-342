...
// We use the Rust implementation for all data transport.
#define RUST_SOCKETS
...
// Misc
////////////////////////////////////////////////////////////////////////////////
static inline Timestamp Fprofile_Time(void) {
...
static PyObject* Fprofile_ThreadFunctionTrace(PyObject* obj, PyFrameObject* frame,
    ...
    // We previously called `Fprofile_FunctionTrace` to manually recorded this
    // call, but that's both not very interesting as well as causes crashes on
    // startup for some Python versions (due to insufficiently initialized
    // frames), so we don't do that anymore.
    Py_RETURN_NONE;
}
...
static void Fprofile_ThreadTeardown(void* key) {
    ...
    // TODO: We should tear down the socket to avoid leaking O(# threads)
    // resources.
}
...
PyMODINIT_FUNC PyInit__functiontrace(void) {
    ...
    if (PyModule_AddStringConstant(m, "__version__", PACKAGE_VERSION) == -1) {
      Py_XDECREF(m);
      m = NULL;
    }
...
static PyObject* Fprofile_StartTrace(PyObject* obj, PyObject* args, PyObject* kwargs) {
    ...
    if (moduleConfiguration.pid != 0) {
        // This can be triggered by running `functiontrace.begin_tracing()`
        // from inside a process that's already being traced.
        // We'll fail in this situation, but might as well print a mildly
        // helpful error message here.
        perror("Functiontrace doubly initialized! Aborting.");
        exit(-1);
    }
...
#if defined(RUST_SOCKETS)
        // NOTE: We only import this module once.  Importing while tracing is
        // live will be recorded, which exposes internals users don't care
        // about and breaks the invariant that we send a `RegisterThread`
        // message first when threads are traced.
        moduleConfiguration.rust = PyImport_ImportModule("_functiontrace_rs._lib");

        // Verify that the rust extension loads if we're using it.
        if (moduleConfiguration.rust == NULL) {
            perror("Failed to load internal Rust extension");
            exit(-1);
        }
#endif

        // Spawn the server.  We'll wait a bit for it to start accepting
        // sockets.
        pid_t server = fork();
        if (server == 0) {
...
#else
        {
            PyObject* msg_init = PyObject_GetAttrString(moduleConfiguration.rust, "message_initialization");
            assert(msg_init != NULL);

            PyObject* arg = PyLong_FromVoidPtr(&moduleConfiguration.socket.sun_path);
            PyObject* context = PyObject_CallFunctionObjArgs(msg_init, arg, NULL);
            assert(context != NULL);

            PyObject* msg_flush = PyObject_GetAttrString(moduleConfiguration.rust, "message_flush");
            assert(msg_flush != NULL);
            PyObject* mpack_flush = PyObject_CallFunctionObjArgs(msg_flush, NULL);
            assert(mpack_flush != NULL);

            mpack_writer_set_context(&initWriter, PyLong_AsVoidPtr(context));
            mpack_writer_set_flush(&initWriter, PyLong_AsVoidPtr(mpack_flush));
        }
#endif
...
#else
        {
            PyObject* msg_shutdown = PyObject_GetAttrString(moduleConfiguration.rust, "message_shutdown");
            assert(msg_shutdown != NULL);

            PyObject* socket = PyLong_FromVoidPtr(mpack_writer_context(&initWriter));
            assert(PyObject_CallFunctionObjArgs(msg_shutdown, socket, NULL) != NULL);
        }
#endif
    } else {
...
static ThreadState* Fprofile_CreateThreadState(void) {
    ...
    // Setup msgpack and a socket for this thread to report tracing info on.
    //
    // We need to do this before any allocations or Python function calls
    // occur, since consumers can't tell whether mpack is fully initialized or
    // not.
    ...
    // Configure Mpack to flush events to this thread's socket once the buffer
    // fills up.
...
#else
    {
        ...
        // TODO: Convert this to a shared Rust configuration object so we don't
        // need to pass it back and forth.
        PyObject* arg = PyLong_FromVoidPtr(&moduleConfiguration.socket.sun_path);
        PyObject* context = PyObject_CallFunctionObjArgs(msg_init, arg, NULL);
        assert(context != NULL);
        ...
        // Re-initialize msgpack, since we've done some allocations and Python
        // calls that we don't want to actually record.
        mpack_writer_init(&state->writer, (char*) &state->buffer, sizeof(state->buffer));
        mpack_writer_set_context(&state->writer, PyLong_AsVoidPtr(context));
        mpack_writer_set_flush(&state->writer, PyLong_AsVoidPtr(mpack_flush));
    }
#endif
