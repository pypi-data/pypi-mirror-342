...
# 对django_simple_api库的serialize文件提纯出来，可使models拥有to_json()转为字符串方法
class JDjangoSimpleApi:
    @staticmethod
    ...
    def serialize_queryset(self: models.QuerySet, excludes: List[str] = None) -> List[dict]:
        return [JSimpleSerialize.serialize_model(model, excludes) for model in self]
