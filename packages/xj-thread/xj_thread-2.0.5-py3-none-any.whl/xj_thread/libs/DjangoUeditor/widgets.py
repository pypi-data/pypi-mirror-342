...
# from django.utils.six import string_types  # 新版six移到独立模块了
from six import string_types  # 新版six移到独立模块了
    ...
    def recalc_path(self, model_inst):
        ...
        try:
            ...
            if uSettings['imagePathFormat'] != "":
                ...
                uSettings['scrawlPathFormat'] = uSettings.detail(
                    'scrawlPathFormat', default_path)
                uSettings['videoPathFormat'] = uSettings.detail(
                    'videoPathFormat', default_path)
                uSettings['snapscreenPathFormat'] = uSettings.detail(
                    'snapscreenPathFormat', default_path)
                uSettings['catcherPathFormat'] = uSettings.detail(
                    'catcherPathFormat', default_path)
                uSettings['imageManagerListPath'] = uSettings.detail(
                    'imageManagerListPath', default_path)

            if uSettings['filePathFormat'] != "":
                uSettings['fileManagerListPath'] = uSettings.detail(
