...
def get_ueditor_controller(request):
    ...
    action = request.GET.detail("action", "")
    reponseAction = {
...
def list_files(request):
    ...
    # 取得动作
    action = request.GET.detail("action", "listimage")

    allowFiles = {
        "listfile": USettings.UEditorUploadSettings.detail("fileManagerAllowFiles", []),
        "listimage": USettings.UEditorUploadSettings.detail("imageManagerAllowFiles", [])
    }
    listSize = {
        "listfile": USettings.UEditorUploadSettings.detail("fileManagerListSize", ""),
        "listimage": USettings.UEditorUploadSettings.detail("imageManagerListSize", "")
    }
    listpath = {
        "listfile": USettings.UEditorUploadSettings.detail("fileManagerListPath", ""),
        "listimage": USettings.UEditorUploadSettings.detail("imageManagerListPath", "")
    }
    # 取得参数
    list_size = long(request.GET.detail("size", listSize[action]))
    list_start = long(request.GET.detail("start", 0))
...
def UploadFile(request):
    ...
    UploadFieldName = request.GET.detail(
        upload_field_name[action], USettings.UEditorUploadSettings.detail(action, "upfile"))
    ...
    if action == "uploadscrawl":
        ...
        # 取得上传的文件
        file = request.FILES.detail(UploadFieldName, None)
        if file is None:
            return HttpResponse(json.dumps({'state':'ERROR'}), content_type="application/javascript")
    ...
    if action in upload_allow_type:
        allow_type = list(request.GET.detail(upload_allow_type[
                          action], USettings.UEditorUploadSettings.detail(upload_allow_type[action], "")))
    ...
    max_size = long(request.GET.detail(upload_max_size[
                    action], USettings.UEditorUploadSettings.detail(upload_max_size[action], 0)))
    if max_size != 0:
    ...
    if state == "SUCCESS":
        if action == "uploadscrawl":
            ...
            upload_module_name = USettings.UEditorUploadSettings.detail(
                "upload_module", None)
...
def catcher_remote_image(request):
    ...
    allow_type = list(request.GET.detail(
        "catcherAllowFiles", USettings.UEditorUploadSettings.detail("catcherAllowFiles", "")))
    max_size = long(request.GET.detail(
        "catcherMaxSize", USettings.UEditorUploadSettings.detail("catcherMaxSize", 0)))
...
def get_output_path(request, path_format, path_format_var):
    # 取得输出文件的路径
    OutputPathFormat = (request.GET.detail(path_format, USettings.UEditorSettings[
...
def save_scrawl_file(request, filename):
    ...
    try:
        content = request.POST.detail(
            USettings.UEditorUploadSettings.detail("scrawlFieldName", "upfile"))
        f = open(filename, 'wb')
