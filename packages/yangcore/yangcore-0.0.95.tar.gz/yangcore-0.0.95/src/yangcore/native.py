...
class NativeViewHandler(RouteHandler):
	...
	def __init__(A,_dal,_loop):
		N='certificates/certificate/cert-data';M='yangcore:plugins';A.dal=_dal;A.loop=_loop;A.fifolock=FifoLock();A.create_callbacks={};A.change_callbacks={};A.delete_callbacks={};A.subtree_change_callbacks={};A.somehow_change_callbacks={};A.leafref_callbacks={};A.periodic_callbacks={};A.onetime_callbacks={};A.plugins={};B=A.dal.handle_get_opstate_request('/ietf-yang-library:yang-library',{});O=A.loop.run_until_complete(B);P=yl_8525_to_7895(O);A.dm=yangson.DataModel(json.dumps(P),A.dal.module_paths)
	...
	async def _check_auth(B,request,data_path):
		P='No authorization required for fresh installs.';O='/yangcore:users/user';L='access-denied';K='failure';J='success';G='comment';E='outcome';D=request;assert data_path is not _A;A={};A['timestamp']=datetime.datetime.now(datetime.UTC);A['source-ip']=D.remote
