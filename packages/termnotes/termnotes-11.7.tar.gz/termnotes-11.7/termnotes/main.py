...
def list_folders():
  ...
  # Calculate half width for each panel
  half_width = (console.width - 4) // 2  # -4 for borders

  inner_panel = Panel(
    content,
    title="[bold blue]Folders[/bold blue]",
    expand=True,
    width=half_width,
    padding=(0, 1)
  )
  ...
  inner_columns = Columns(
    ...
    equal=True,
    expand=True,
    width=console.width - 2  # -2 for outer panel borders
  )

  outer_panel = Panel(
    inner_columns,
    title="[spring_green1 bold]Termnotes[/spring_green1 bold]",
    width=console.width
  )
...
def list_notes(folder):
  ...
  panel_title = f"[bold blue]{folder}[/bold blue]"
  folder_panel = Panel(
    content,
    title=panel_title,
    expand=True,
