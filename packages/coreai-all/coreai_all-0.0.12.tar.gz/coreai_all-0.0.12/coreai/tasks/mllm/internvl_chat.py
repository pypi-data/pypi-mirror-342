...
from ..api_schema import ChatCompletionRequest
...
if torch.cuda.get_device_capability()[0] < 8:
    ...
    # Replace the built-in import function with the mock import function
    builtins.__import__ = mock_import

    def mock_find_spec(name, *args, **kwargs):
        if name == "flash_attn":
            return None  # Simulate that the flash_attn package is not found
        return original_find_spec(name, *args, **kwargs)

    # Save the original find_spec function
...
def dynamic_preprocess(
    image, min_num=1, max_num=MAX_TILES, image_size=448, use_thumbnail=False
):
...
class VisionQnA(VisionQnABase):
    ...
    def __init__(
        ...
        if torch.cuda.get_device_capability()[0] < 8:
            with patch("importlib.util.find_spec", side_effect=mock_find_spec):
                result = import_utils.is_flash_attn_2_available()
                print(f"flashatten installed? {result} <- this is mocked.")
                config = AutoConfig.from_pretrained(
                    **self.params, trust_remote_code=True
                )
                config.vision_config.use_flash_attn = False
                # print(config.vision_config)
                # why config will force
    ...
    async def stream_chat_with_images(
        ...
        images = [
            load_image(image, max_num=self.max_tiles).to(self.model.dtype).cuda()
            for image in images
        ]
        if len(images) > 1:
        ...
        logger.info(f"crt images: {len(images)}")

        if pixel_values is not None:
            for img in images:
                image_tokens = (
                    "<img>"
                    + "<IMG_CONTEXT>" * self.model.num_image_token * img.size(0)
                    + "</img>"
                )
