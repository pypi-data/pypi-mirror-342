...
from coreai.serve.api_schema import (
    ...
    ChatMessageContentImageUrlModel,
    ChatMessageContentItemModel,
    ChatMessage,
)
...
class VisionQnABase:
    ...
    format: str = None
    revision: str = "main"
    vision_layers: List[str] = (
        []
    )  # "vision_model", "resampler", "vision", "vision_tower"]
    ...
    def __init__(
        ...
        if extra_params.get("load_in_4bit", False):
            ...
            load_in_8bit_params = {
                "quantization_config": BitsAndBytesConfig(
                    load_in_8bit=True, llm_int8_skip_modules=self.vision_layers,
                )
            }
    ...
    def loaded_banner(self):
        logger.info(
            f"Loaded {self._model_id} [ device: {self.model.device}, dtype: {self.model.dtype}, template: {self.format} ]"
        )
    ...
    def repack_message_content(
        self, request: ChatCompletionRequest
    ) -> ChatCompletionRequest:
        """Repack messages to remove string "content" messages and convert to List[ChatMessageContentItemModel]"""

        for m in request.messages:
            if isinstance(m.content, str):
                m.content = [ChatMessageContentItemModel(type="text", text=m.content)]

        return request

    # implement one or both of the stream/chat_with_images functions
    async def chat_with_images(self, request: ChatCompletionRequest) -> str:
    ...
    def get_generation_params(
        self, request: ChatCompletionRequest, default_params={}
    ) -> dict:
        params = {
            "top_k": None,
            "do_sample": False,
...
def threaded_streaming_generator(generate, tokenizer, generation_kwargs):
    streamer = TextIteratorStreamer(
        tokenizer, skip_special_tokens=True, skip_prompt=True, timeout=60
    )
...
def threaded_streaming_generator_bare(generate, tokenizer, generation_kwargs):
    streamer = TextIteratorStreamer(
        tokenizer, skip_special_tokens=True, skip_prompt=True, timeout=60
    )
...
async def images_hfmessages_from_messages(
    messages: list[ChatMessage], url_handler=url_to_image
):
    hfmessages = []
    images = []
...
async def phi15_prompt_from_messages(
    messages: list[ChatMessage], img_tok="<image>", img_end="", url_handler=url_to_image
):  # </image>
    prompt = ""
    images = []
        ...
        if m.role == "user":
            ...
            for c in m.content:
                if c.type == "image_url":
                    ...
                    p = (
                        img_tok.format(img_data) + p + img_end
                    )  # this is a bit strange, but it works for Monkey and filenames
                if c.type == "text":
                    p += f"{c.text}\n\n"  # Question:
            prompt += p
...
async def vicuna0_prompt_from_messages(
    messages: list[ChatMessage], img_tok="<image_placeholder>\n"
):
    prompt = ""
    images = []
...
async def vicuna_prompt_from_messages(messages: list[ChatMessage], img_tok="<image>\n"):
    prompt = ""
    images = []
    generation_msg = "ASSISTANT:"
...
async def llama2_prompt_from_messages(messages: list[ChatMessage], img_tok="<image>\n"):
    prompt = ""
    images = []
...
async def llama3_prompt_from_messages(messages: list[ChatMessage], img_tok="<image>"):
    prompt = ""
    images = []
...
async def chatml_prompt_from_messages(messages: list[ChatMessage], img_tok="<image>\n"):
    prompt = ""
    images = []
    ...
    for m in messages:
        if m.role == "user":
            ...
            img_tag = ""
            tag_prefix = "Frame" if len(images) > 4 else "Image-"
            for i, _ in enumerate(images, start=1):
                img_tag += f"{tag_prefix}{i}: {img_tok}"
...
async def gemma_prompt_from_messages(messages: list[ChatMessage], img_tok="<image>\n"):
    prompt = ""
    images = []
...
async def fuyu_prompt_from_messages(
    messages: list[ChatMessage], img_tok="", img_end=""
):
    prompt = ""
    images = []
...
async def emu_images_prompt_system_from_messages(
    messages: list[ChatMessage], img_tok="[<IMG_PLH>]"
):
    prompt = ""
    images = []
    system_message = None
...
# img_tok = "<|image_{}|>\n" is also ok
async def phi3_prompt_from_messages(messages: list[ChatMessage], img_tok="<image>\n"):
    n = 1
    prompt = ""
    images = []
    generation_msg = "<|assistant|>\n"
...
async def phintern_prompt_from_messages(
    messages: list[ChatMessage], img_tok="<image>\n"
):
    prompt = ""
    images = []
...
async def falcon_prompt_from_messages(messages: list[ChatMessage], img_tok="<image>\n"):
    prompt = ""
    images = []
...
async def prompt_history_images_system_from_messages(
...
async def glm4v_prompt_from_messages(
    messages: list[ChatMessage],
    img_tok="<|begin_of_image|><|endoftext|><|end_of_image|>",
    url_handler=url_to_image,
):
    prompt = "[gMASK]<sop>"
    images = []
...
async def florence_prompt_from_messages(
    messages: list[ChatMessage], url_handler=url_to_image
):
...
async def prompt_from_messages(messages: list[ChatMessage], format: str) -> str:
    known_formats = {
