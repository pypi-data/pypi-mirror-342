...
class Local:
    ...
    def __init__(
        ...
        Args:
            ...
            lmsys_api_key: LMSYS API key for authentication (preferred method)
        ...
        if lmsys_api_key:
            self._authenticate_with_lmsys(lmsys_api_key)
        elif api_keys:
            # Legacy method: Set API keys directly in environment
            self._set_api_keys(api_keys)
        else:
            # Try to read from environment
            if not (os.environ.get("OPENAI_API_KEY") or os.environ.get("ANTHROPIC_API_KEY")):
                raise ValueError("Either lmsys_api_key or api_keys must be provided, or OPENAI_API_KEY/ANTHROPIC_API_KEY must be set in environment")
    ...
    def _authenticate_with_lmsys(self, lmsys_api_key: str) -> None:
        """
        Authenticate with the LMSYS API server and get a session token.

        Args:
            lmsys_api_key: The LMSYS API key for authentication
    ...
    def _get_provider_keys(self) -> None:
        ...
        try:
            ...
            raise ValueError(f"Failed to fetch provider keys: {str(e)}")
    ...
    def code(
        ...
        # If using session token, make sure we have fresh provider keys
        if self.session_token:
            self._get_provider_keys()
    ...
    def _initialize_sandbox(self, sandbox_id=None, timeout=300):
        ...
        if self.session_token:
            try:
                ...
                print(f"Warning: Failed to get E2B API key from LMSYS API: {str(e)}")

        # Use our E2B API key from environment or fall back to default
