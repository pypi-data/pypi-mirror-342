...
from urllib.parse import urlparse, urlunparse, ParseResult, quote, quote_plus, unquote, unquote_plus, \
    _coerce_args
import redis
...
class Normalizer:
    def __init__(self, rds_host=None, rds_port=None, rds_password=None):
        ...
        self.cache_table = {}
        self.runner = Thread(target=self.sync_rules, daemon=True)
        self.runner.start()
    ...
    def get_manual_rule(self, url, domain):
    ...
    def canonicalize_url(self,
        ...
        try:
            ...
            if not all([scheme, netloc]):
                return url, rule, extra, ReturnType.url_is_invalid
            ...
            # 查询归一化人工规则
            if not data:
                rule = self.get_manual_rule(url, domain)
                if rule:
                    data = rule["data"]
            ...
            if data:
                ...
                [keyvals.remove((key, value)) for key, value in keyvals if key.decode() in black_keys]
            ...
            # 每个 part 都应该被安全编码过了
            return urlunparse((scheme, netloc, path, params, query, fragment)), rule, extra, ReturnType.normal
        except Exception as exc:
            logger.exception("url normalize error", exc)
            return url, rule, extra, ReturnType.normalize_error
    ...
    def manual_evaluate(self, url, rules=None, data=None):
        ...
        if link_filter_result.hit:
            new_url = self.canonicalize_url(url, keep_blank_values=True, data=data)
        else:
            new_url = self.canonicalize_url(url, keep_blank_values=True)
