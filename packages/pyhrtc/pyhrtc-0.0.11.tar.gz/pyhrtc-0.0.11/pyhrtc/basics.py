...
class Matching:
    ...
    def matched(self, agent: str | Agent | Couple) -> set[str]:
    ...
    def _is_stable_not_couple(self, left: Agent, right: Agent) -> bool:
        ...
        left_match = self.matched(left.ident)
        if not (
        ...
        # would reject someone to take left.
        right_match = self.matched(right.ident)
        if not (
    ...
    def _is_stable_couple(
        ...
        if not (
            left.could_admit(self.matched(left) | {right.ident})
            or any(
                left.prefers(right, d)
                and left.could_admit((self.matched(left) | {right.ident}) - {d})
                for d in self.matched(left)
            )
        ):
        ...
        if right[0] != right[1]:
            ...
            # CHH3
            if not (
                hosps[0].could_admit(self.matched(hosps[0]) | {left.first.ident})
                or any(
                    hosps[0].could_admit(
                        (self.matched(hosps[0]) | {left.first.ident}) - {d}
                    )
                    for d in self.matched(hosps[0])
            ...
            # CHH4
            if not (
                hosps[1].could_admit(self.matched(hosps[1]) | {left.second.ident})
                or any(
                    hosps[1].could_admit(
                        (self.matched(hosps[1]) | {left.second.ident}) - {d}
                    )
                    for d in self.matched(hosps[1])
            ...
            hosp_match = self.matched(hosps[0])
            ...
            if stability_type == STABILITY.BIS:
                ...
                for agent_ident in self.matched(hosps[0]):
                    ...
                    if other_couple is None:
                        ...
                        continue
                    if not (
                        other_couple.first in self.matched(hosps[0])
                        and other_couple.second in self.matched(hosps[0])
                    ):
                        continue
                    if not (
