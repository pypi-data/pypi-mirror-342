...
class ArgumentParserWithHelp(argparse.ArgumentParser):
    ...
    def format_help(self):
        ...
        for action_group in self._action_groups:
            if all(
                action not in subparsers_actions
                for action in action_group._group_actions
            ):
                self._extracted_from_format_help_41(formatter, action_group)
            else:
    ...
    # TODO Rename this here and in `format_help`
    @staticmethod
    def _extracted_from_format_help_41(formatter, action_group):
        formatter.start_section(action_group.title)
        formatter.add_text(action_group.description)
