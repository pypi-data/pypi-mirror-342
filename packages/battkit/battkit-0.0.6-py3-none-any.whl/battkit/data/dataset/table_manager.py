...
class TableManager():
	...
	def update_files_table(self, df_group_by:pd.DataFrame, tags:Dict[str,Any]=None, reload:bool=False):
		...
		# agg_dict = {
		# 	'CELL_ID': ('CELL_ID', lambda x: x.unique()[0]),
		# 	'PROTOCOL': ('PROTOCOL', 
		# 		lambda x: (
		...
		# 		)
		# 	)
		# df_files_summary_new = df_files_new.groupby('FILE_ID').agg(**agg_dict).reset_index()
		df_files_summary_new = df_files_new[['FILE_ID', 'CELL_ID', 'PROTOCOL', 'TEST_TYPE',] + [k for k in tags.keys()]].copy()
		...
		for t_name, new_data in {'Files':df_files_new, 'Files_Summary':df_files_summary_new}.items():
			...
			if ddf_prev is None or reload:
				self.create_table(name=t_name, data=dd.from_pandas(new_data), overwrite_existing=True)

			# Only append new data, don't regenerate entire table
			else:
				self._append_to_table(t_name, dd.from_pandas(new_data))
	...
	def update_test_type_summary_tables(self, df_summary:pd.DataFrame):
		...
		for test_type, df_t in df_summary.groupby('TEST_TYPE'):
			...
			# Otherwise, append this to the existing summary table
				...
				self._append_to_table(f"{test_type}_Summary", dd.from_pandas(new_sum_data))
	...
	def _load_existing(self):
		...
		if 'Files' in [f.name for f in existing_tables]:
			...
			self._tables['Files'] = dd.from_pandas(new_df_files)
