...
class BaseConnector:
    def __init__(self, redis_connector: RedisConnector | None = None, ttl: int = 3600 * 4, cache_prefix: None | str = None):
        self.redis_cache_connector: RedisConnector
        if redis_connector:
            cache.setup(redis_connector=redis_connector, ttl=ttl, prefix=cache_prefix)
    ...
    def update_one(self, obj: T, *, _use_cache: bool | None = None, _cache_ttl: int = DEFAULT_CACHE_TTL) -> T:
        """
    ...
    def delete_one(self, collection: type[T], *, id: int | str, _use_cache: bool | None = None, _cache_ttl: int = DEFAULT_CACHE_TTL) -> Any:
        """
    ...
    def delete_many(
        self, collection: type[T], *, ids: list[str | int], _use_cache: bool | None = None, _cache_ttl: int = DEFAULT_CACHE_TTL
    ) -> list[T]:
        """
...
class AsyncBaseConnector:
    ...
    async def delete_many(self, collection: type[T], *, ids: list[str | int]) -> list[T]:
        """
        Delete multiple objects from the data source.
        Args:
            collection:
