...
class AsyncOpensearchConnector(AsyncBaseConnector):
    ...
    async def find_one(self, collection: type[T], query: str | None = None, **kwargs: Any) -> T | None:
        if not self.client:
            raise RuntimeError("OpenSearch client is not connected.")
    ...
    async def semantic_search(
        self, collection: type[T], query_vector: list[float], top_k: int = 10, normalize: bool = True, **filters: Any
    ) -> list[T]:
        if not self.client:
            raise RuntimeError("OpenSearch client is not connected.")
    ...
    async def update_one(self, obj: T, attributes: Iterable[str] = None, exclude_fields: Iterable[str] = None) -> dict[str, Any] | None:
        if not self.client:
            raise RuntimeError("OpenSearch client is not connected.")
        try:
    ...
    async def delete_one(self, collection: type[T], *, id: str | int) -> dict[str, Any] | None:
        if not self.client:
            raise RuntimeError("OpenSearch client is not connected.")
        try:
