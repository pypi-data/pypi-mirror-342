...
from polycrud.connectors.base import DEFAULT_CACHE_TTL, BaseConnector, T
...
class OpensearchConnector(BaseConnector):
    ...
    @cache.cache()
    def delete_one(
        self, collection: type[T], *, id: str | int, _use_cache: bool | None = None, _cache_ttl: int = DEFAULT_CACHE_TTL
    ) -> dict[str, Any] | None:
        if not self.client:
            raise RuntimeError("OpenSearch client is not connected.")
        try:
