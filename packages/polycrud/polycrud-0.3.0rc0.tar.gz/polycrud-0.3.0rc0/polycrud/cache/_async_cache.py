...
def acache() -> Callable[[F], F]:
    def decorator(fn: F) -> F:
        ...
        async def wrapper(*args: Any, **kwargs: Any) -> Any:
            ...
            cache_key = f"{cls_name}:{model_class.__name__}:{key_hash}"
...
class AsyncRedisCache:
    ...
    async def get(self, key: str, model_cls: type[ModelT]) -> ModelT | None:
        try:
            return await self.redis_connector.get_object(model_cls, key=self._format_key(key))
        except Exception as e:
