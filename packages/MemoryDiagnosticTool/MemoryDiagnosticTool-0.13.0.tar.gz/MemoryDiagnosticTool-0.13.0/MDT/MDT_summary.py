...
from rrw_error_count_parser import rrw_error_count
from esid_complete_parser import check_esid_complete
from memtest_parser import check_memtest
from Trained_CSR_CSV import trained_csr_csv
from Trained_LOG_CSV import log_to_csv
from MR_readout_v1 import mr_readout
from dca_dcs_eye import scan_dcs_dca
from rd_wr_eye import Rd_Wr_Eye
from rd_wr_eye import Rd_Eye_Scan
from trained_phycsr_subset import phycsr_subset
from postcode import get_postcodefile
from MDT_html_template import parse_to_html
...
def MDT_summary_table(mdt_html, rrw_file, esid_file):
    ...
    try:
        ...
        if os.path.exists(esid_file):
            ...
            parse_to_html(mdt_html, pmu_train_result, summary_table)
...
def MDT_process(file_path):
    ...
    if file_path and os.path.exists(file_path):
        if os.path.isdir(file_path):
            ...
            if decoded_logs:
                ...
                if not os.path.exists(mdteye):
                    os.mkdir(mdteye)
                try:
                    log_to_csv(decoded_logs, jmp_logcsv, analysis_logcsv)
                except:
        ...
        MDT_summary_table(mdt_html, rrw_csv, memtest_csv)
