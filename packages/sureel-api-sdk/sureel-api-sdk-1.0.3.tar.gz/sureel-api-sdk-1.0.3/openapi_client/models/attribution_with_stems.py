...
class AttributionWithStems(BaseModel):
    """
    Attribution describes the influence of training data on content generated using AI.     When training an AI model with different collections of data, each collection has a certain amount of influence on the resulting model.     When generating new content with the resulting model, the influence each collection has on the new content is referred to as the attribution of the new content. The attribution is returned for the master audio as well as for each individual stem. Additionally, attribution is calculated for multiple segments of the entire song. If the duration and period are -1, it indicates that attribution for the entire song was calculated.
    """ # noqa: E501
    master: Optional[List[AttributionWithStemsMasterInner]] = None
    instrumentals: Optional[List[AttributionWithStemsMasterInner]] = None
    vocals: Optional[List[AttributionWithStemsMasterInner]] = None
    bass: Optional[List[AttributionWithStemsMasterInner]] = None
    drums: Optional[List[AttributionWithStemsMasterInner]] = None
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of AttributionWithStems from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of AttributionWithStems from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate({
            "master": [AttributionWithStemsMasterInner.from_dict(_item) for _item in obj["master"]] if obj.get("master") is not None else None,
            "instrumentals": [AttributionWithStemsMasterInner.from_dict(_item) for _item in obj["instrumentals"]] if obj.get("instrumentals") is not None else None,
            "vocals": [AttributionWithStemsMasterInner.from_dict(_item) for _item in obj["vocals"]] if obj.get("vocals") is not None else None,
            "bass": [AttributionWithStemsMasterInner.from_dict(_item) for _item in obj["bass"]] if obj.get("bass") is not None else None,
            "drums": [AttributionWithStemsMasterInner.from_dict(_item) for _item in obj["drums"]] if obj.get("drums") is not None else None
        })
        return _obj
