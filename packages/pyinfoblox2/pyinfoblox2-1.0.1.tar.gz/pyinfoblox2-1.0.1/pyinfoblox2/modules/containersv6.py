...
class Containerv6(IPHelpers):
    ...
    RECORD_TYPE = 'ipv6networkcontainer'

    def __init__(self, client, network, view='default', **kwargs):
        super(Containerv6, self).__init__(client)
    ...
    def to_json(self, original_format=False):
        ...
        """
        if original_format:
            return dict(network=str(self.network), _ref=self.ref, comment=self.description, view=self.view, networks=self.networks.to_json() if self.networks else None, options=self.options,
    ...
    @classmethod
    def add(cls, client, network, username, description=None, options=None, view='default', **extattrs):
        """
...
class Containersv6(BaseList):
    ...
    def __init__(self, client, **kwargs):
        super(Containersv6, self).__init__(client, **kwargs)
        self.items = list()  # type: List[Containerv6]
