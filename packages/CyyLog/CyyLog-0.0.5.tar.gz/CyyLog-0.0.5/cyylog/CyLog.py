...
class CyyLog:
    ...
    def _load_latest_output_data(self) -> Dict[str, List[Dict]]:
        """Load the most recent output data from file."""
        try:
            with open(self.output_path, 'r', encoding='utf-8') as f:
                return json.load(f)
        except (FileNotFoundError, json.JSONDecodeError):
    ...
    def _save_output_data(self) -> None:
        """Save current output data to file with synchronization across processes."""
        latest_data = self._load_latest_output_data()
        for key, entries in self.output_data.items():
            if key not in latest_data:
                latest_data[key] = []
            
            existing_ids = {entry.get("id") for entry in latest_data[key]}
            for entry in entries:
                if entry.get("id") not in existing_ids:
                    latest_data[key].append(entry)
                    existing_ids.add(entry.get("id"))
        self.output_data = latest_data
