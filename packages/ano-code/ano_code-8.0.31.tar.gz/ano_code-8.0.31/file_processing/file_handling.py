...
def process_directory(path, ignore)-> dict:
    
    # Walk through the directory and its subdirectories
    folders_to_ignore = {".pnpm-store", ".pytest_cache", "__pycache__", "node_modules", "documents", "dist", "ano_code.egg-info", "auto-code-env"}
    tree = {}
    try:
        with os.scandir(path) as it:
            ...
            for entry in it:
                if entry.is_dir(follow_symlinks=False):
                    if entry.name in folders_to_ignore.union(ignore):
                        continue
                    # Recurse into the directory and merge its result
                    sub_tree = process_directory(entry.path, folders_to_ignore.union(ignore))
                    tree.update(sub_tree)
                elif entry.is_file(follow_symlinks=False):
                    files.append(entry.name)
            tree[os.path.abspath(path)] = files
