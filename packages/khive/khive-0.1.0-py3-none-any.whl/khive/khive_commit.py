...
def git(cmd: List[str] | str, *, capture: bool = False, check: bool = True) -> subprocess.CompletedProcess[str] | int:
...
def ensure_identity():
    ...
    for what, key, default in (("user.name", "name", fallback_name), ("user.email", "email", fallback_email)):
        try:
            proc = git(["config", "--get", what], capture=True, check=False)
            have = proc.stdout.strip() if isinstance(proc, subprocess.CompletedProcess) else ""
        except Exception:
            have = ""
        if not have:
...
def working_tree_dirty() -> bool:
    ...
    return isinstance(rc, int) and rc == 1 or isinstance(rc, subprocess.CompletedProcess) and rc.returncode == 1


def staged_nothing() -> bool:
    rc = git(["diff", "--cached", "--quiet"], check=False)
    return isinstance(rc, int) and rc == 0 or isinstance(rc, subprocess.CompletedProcess) and rc.returncode == 0

# ────────── main workflow ──────────

def _cli() -> None:
    global verbose, DRY_RUN
    ap = argparse.ArgumentParser(description="Commit helper with Conventional-Commit linting")
    ...
    ap.add_argument("--amend", action="store_true", help="amend last commit instead of creating new")
    ...
    # push current branch
    branch = subprocess.check_output(["git", "branch", "--show-current"], text=True).strip() or "HEAD"
