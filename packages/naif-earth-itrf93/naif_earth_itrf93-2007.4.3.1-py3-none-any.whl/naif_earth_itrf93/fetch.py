...
FILE = os.path.join(os.path.dirname(__file__), "earth_assoc_itrf93.tf")
MD5_FILE = os.path.join(os.path.dirname(__file__), "earth_assoc_itrf93.md5")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Fetch the NAIF Earth ITRF93 file store its checksum."
    )
    parser.add_argument(
        "--url",
        type=str,
        default=URL,
        help="URL to fetch the NAIF Earth ITRF93 file from.",
    )
    parser.add_argument(
        "--file",
        type=str,
