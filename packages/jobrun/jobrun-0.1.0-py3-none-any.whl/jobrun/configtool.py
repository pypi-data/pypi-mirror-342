...
def main():
    ...
    subparser1 = subparsers.add_parser('setup')
    subparser1.set_defaults(func=setup)
    subparser2 = subparsers.add_parser('rebuild')
    subparser2.set_defaults(func=rebuild)
...
#        if (config_dir/'specfiles'/'package_specs'/specfile).is_file():
#            if json5_read(package_dir/'specfiles'/'package_specs'/specfile) != json5_read(config_dir/'specfiles'/'package_specs'/specfile):
...
#                    (package_dir/'specfiles'/'package_specs'/specfile).copy(config_dir/'specfiles'/'package_specs')
#        else:
#            (package_dir/'specfiles'/'package_specs'/specfile).copy(config_dir/'specfiles'/'package_specs')
#    for specfile in (package_dir/'specfiles'/'scheduler_specs').listdir():
#        if (config_dir/'specfiles'/'scheduler_specs'/specfile).is_file():
#            if json5_read(package_dir/'specfiles'/'scheduler_specs'/specfile) != json5_read(config_dir/'specfiles'/'scheduler_specs'/specfile):
#                prompt = _('¿Desea sobrescribir la configuración local del gestor de trabajos {queuename}?', queuename=specfile)
#                if complete_binary_choice(prompt, truthy_options, falsy_options):
#                    (package_dir/'specfiles'/'scheduler_specs'/specfile).copy(config_dir/'specfiles'/'scheduler_specs')
#        else:
#            (package_dir/'specfiles'/'scheduler_specs'/specfile).copy(config_dir/'specfiles'/'scheduler_specs')
    ...
    for package in package_names:
        if package in selected_packages:
            ...
            try:
                ...
                config.update(json5_read(package_dir/'specfiles'/'scheduler_specs'/config.schedspecfile))
                config.update(json5_read(package_dir/'specfiles'/'package_specs'/config.progspecfile))
