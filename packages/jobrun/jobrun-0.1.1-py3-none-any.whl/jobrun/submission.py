...
#            print_error_and_exit(_('El archivo de reinicio {path} no existe'), path=path)
    ...
    for key, value in options.parameteropts.items():
        if '/' in options.parameteropts[key]:
            print_error_and_exit(_('El nombre del conjunto de parámetros no es válido: parameteropts[{key}]={value}'), key=key, value=value)
    ...
    if options.interpolate:
        ...
        if options.interpolation.prefix:
            ...
            try:
                ...
                print_error_and_exit(_('El sufijo contiene variables de interpolación inválidas'), suffix=options.interpolation.suffix, key=e.args[0])
            except KeyError as e:
                print_error_and_exit(_('El sufijo contiene variables de interpolación indefinidas'), suffix=options.interpolation.suffix, key=e.args[0])
        else:
            if options.interpolation.mol:
                if len(options.interpolation.mol) == 1:
                    settings.prefix = molprefix
                else:
                    print_error_and_exit(_('Se debe especificar un prefijo o sufijo cuando se especifican múltiples archivos de coordenadas'))
            elif options.interpolation.trjmol:
                settings.prefix = molprefix
            else:
                print_error_and_exit(_('Se debe especificar un prefijo o sufijo para interpolar sin archivo coordenadas'))

    if not 'scratch' in config.defaults:
        print_error_and_exit(_('No se especificó el directorio de escritura por defecto'))
    ...
    if 'version' in options.common:
        if options.common.version not in config.versions:
            print_error_and_exit(_('La versión del programa solicitada no es válida'), version=options.common.version)
        settings.version = options.common.version
    elif 'version' in config.defaults:
        if not config.defaults.version in config.versions:
            print_error_and_exit(_('La versión del programa establecida por defecto no es válida'), version=config.defaults.version)
    ...
    for i, path in enumerate(config.parameterpaths):
        ...
        if logdict.logged_keys:
            ...
            if logdict.logged_keys:
                print_error_and_exit(_('La ruta {path} contiene variables de interpolación indefinidas'), path=path, keys=logdict.logged_keys)
        else:
            ...
            for part in AbsPath(path).parts:
                ...
                try:
                    ...
                    option_list = trunk.listglob(InterpolationTemplate(part).substitute(GlobDict()))
...
def submit_single_job(workdir, inputname, filtergroups):
    ...
    if 'outdir' in options.common:
        outdir = AbsPath(options.common.outdir, relto=workdir)
    else:
        outdir = AbsPath(jobname, relto=workdir)
                    ...
                    with open(srcpath, 'r') as f:
                        ...
                        if options.interpolate:
                            try:
                                ...
                                print_failure(_('El archivo {file} contiene variables de interpolación indefinidas'), file=srcpath, key=e.args[0])
                                return
                        else:
                            try:
                                ...
                                prompt = _('Parece que hay variables de interpolación en el archivo {file} ¿desea continuar sin interpolar?').format(file=srcpath)
...
#        targetfile.symlink_to(stagedir/jobname%config.fileopts[key])
    ...
    if options.remote.remote_host:
        ...
        remote_indir = paths.remotedir/names.user%names.host/'input'
        remote_outdir = paths.remotedir/names.user%names.host/'output'
        remote_args.gather(options.common)
        remote_args.flags.add('raw')
        remote_args.flags.add('job')
        remote_args.flags.add('move')
        remote_args.options['indir'] = remote_indir/reloutdir
        remote_args.options['outdir'] = remote_outdir/reloutdir
        ...
        if options.common.debug:
            ...
            try:
                check_output(['ssh', '-S', paths.socket, options.remote.remote_host, f"mkdir -p '{remote_indir}' '{remote_outdir}'"])
                check_output([f'rsync', '-e', "ssh -S '{paths.socket}'", '-qRLtz'] + filelist + [f'{options.remote.remote_host}:{remote_indir}'])
    ...
    for key in config.inputfiles:
        if (workdir/inputname%key).is_file():
