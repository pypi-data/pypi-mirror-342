...
async def mcp_dispatch_get_pending_dispatch_details() -> Dict:
...
class DispatchGroup(TypedDict):
    orderNo: str  # 订单号
    supplier: str  # 供应商名称
    dispatchType: str  # 指派类型


@mcp.tool(description="""批量外部派车，支持多组订单号、供应商和指派类型，参数示例 [
            {
                "orderNo": ["5000985910"],
                "supplier": "上海万创",
                "dispatchType": "提货"
...
async def batch_external_dispatch(
    ...
    Args:
        dispatch_groups: 派车信息列表，每组包含：
            - orderNo: 订单号，例如：["5000985910"]
            - supplier: 供应商名称，例如："上海万创"
            - dispatchType: 指派类型，例如："提货，送货，干线"
    ...
    for group in dispatch_groups:
        try:
            order_numbers = group.get("orderNo", [])
            supplier_name = group.get("supplier", "")
            dispatch_type = group.get("dispatchType", "")
