...
from highlighter.core.logging import get_default_logger
...
class HLPipelineImpl(aiko.PipelineImpl):
    ...
    def _process_stream_event(self, element_name, stream, stream_event, diagnostic, in_destroy_stream=False):
        ...
        if stream_event == aiko.StreamEvent.ERROR:
            ...
            if aiko.StreamState.STOP < most_recent_state:
                ...
                self.hl_task_status_info["message"] = diagnostic
            if self.on_agent_stop is not None:
    ...
    def process_frame(self, stream_dict, frame_data) -> bool:
        try:
            ...
            # TODO: This return value doesn't gel with aiko.PipelineImpl.process_frame
...
class HLAgent:
        ...
        try:
            while True:
                ...
                tasks = lease_tasks_from_steps(
                    HLClient.get_client(),
                    [step_id],
                    lease_sec=TASK_LEASE_DURATION_SECONDS,
                    count=1,
                )
                for task in tasks:
                    self._process_task(task)
                if len(tasks) == 0:
                    time.sleep(TASK_POLLING_PERIOD_SECONDS)
    ...
    def _process_task(self, task: Task):
        ...
        while True:
            try:
                stream_info, frame_data = stream_result_queue.get(timeout=PIPELINE_TIMEOUT_SECONDS)
            except Empty:
                raise ValueError(
                    "Timeout: Pipeline has not produced a value for more than "
            ...
            if stream_info["state"] == aiko.StreamEvent.ERROR:
                logger.error(f"Error in task {task.id}: {frame_data.get('diagnostic')}")
                update_task(
                    client=HLClient.get_client(),
                    task_id=task.id,
            ...
            if (
                ...
                # Re-lease task
                logger.info(f"Extending lease for task {task.id}")
                task = lease_task(
                    client=HLClient.get_client(),
                    task_id=task.id,
                    lease_sec=TASK_LEASE_DURATION_SECONDS,
