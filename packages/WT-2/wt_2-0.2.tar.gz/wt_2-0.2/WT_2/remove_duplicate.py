...
import numpy as np
import pandas as pd
from utils import weighted_dot_product_distance, split_mz_intensity_row, filter_intensity_neg, filter_unique_mz
import logging

class Deduplicator:
    def __init__(self, peak_result_dir, msdial_out_path):
        """
    ...
    def remove_msdial_duplicate(self):
        ...
        try:
            os.chdir(self.peak_result_dir)
            peak_group_files = [file for file in os.listdir(self.peak_result_dir) if file.endswith('peak_group_df.csv')]
            self.all_clean_p3 = pd.DataFrame()
            self.all_ms2_peak = pd.DataFrame()

            for file in peak_group_files:
                file_name = file.split('_peak_group_df')[0]
                print("processing", file_name)
                product_ion_peak = f'{file_name}_product_ion_peak_df.csv'
            ...
            self.all_clean_p3.to_csv(f"./all_p3.csv")
            self.all_ms2_peak.to_csv(f"./all_ms2_peak.csv")
            print("全部一二级匹配完成")
        except Exception as e:
    ...
    def filter_p3_group(self):
        ...
        try:
            ...
            valid_groups = set(self.all_clean_p3.groupby(['pepmass', 'index']).groups.keys())
    ...
    def all_remove_type1(self, Q1_file, raw_peak_group_file, ms2_peak_df, file_name):
        ...
        try:
            ...
            ms2_peak_group.to_csv(f"./{file_name}_p3.csv")
        ...
        try:
            ...
            for index, row in ms2_peak_group.iterrows():
                if index not in del_list:
                    query_peak_RT = row["RT"]
                    query_peak_index = row["index"]
                    query_peak_pepmass = row["pepmass"]
                    ...
                    condition = (
                            (abs(ms2_peak_group['RT'] - query_peak_RT) <= 0.05) &
                            (abs(ms2_peak_group['pepmass'] - query_peak_pepmass) <= 6)
                    )
                    son_df = ms2_peak_group[condition]

                    for i, target_row in son_df.iterrows():
                        target_peak1_pepmass = target_row["pepmass"]
                        target_peak1_index = target_row["index"]
    ...
    def match_ms1_ms2(self, ms2_peak_group, ms1_peak, ms2_peak):
        ...
        try:
            ...
            ms2_peak_group.columns = ["pepmass", "index", "RT"] + list(ms1_peak.columns)
...
# a = Deduplicator(r"D:\work\WT2.0\WT_2\test_data\result", r"D:\work\WT2.0\WT_2\test_data\result\CRL_SIF_1_Q1_peak_df.csv")
