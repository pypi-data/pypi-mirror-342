...
from utils import parse_mz_intensity, weighted_dot_product_distance
import logging

class MspGenerator:
    def __init__(self, p3_peak_group_df, msp_file_path):
        """
    ...
    def _write_msp(self):
        ...
        try:
            ...
            for row in self.p3_peak_group_df.itertuples():
                ...
                name_line = f"ID={i}|pepmass={pepmass}|group={int(group)}"
...
class MspFileLibraryMatcher:
    def __init__(self, query_msp_path, library_msp_path, out_path):
    ...
    def calculateCosineBoth(self, num=3):
        ...
        with open(self.out_path, "w", encoding='utf-8') as f:
            ...
            for q in query_spectrums:
                ...
                if 'precursor_mz' in q.metadata:
                    ...
                    scores = sorted(scores, key=lambda x: x[0], reverse=True)[:num]
                    for score in scores:
