...
class MultiprocessingManager:
    def __init__(self, outer_max_workers, inner_max_workers, mgf_folder, out_dir):
        """
        初始化多进程管理类
    ...
    def process_pepmass(self, pepmass, product_ion_db):
        ...
        intensity_df, mz_df, rt_df = self.getRTwindow(intensity_df, mz_df, rt_df, 30, 720)
        peak_processor = PeakProcessor(intensity_df, mz_df, rt_df, wid=6, sigma=2, min_noise=200, pepmass=pepmass)
        peak_df = peak_processor.find_peak()
        peak_df, peak_group_df = peak_processor.group_peak(peak_df, rt_df, 6, 0.5, pepmass)
