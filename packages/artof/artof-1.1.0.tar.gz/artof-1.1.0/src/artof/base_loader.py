...
from .data_process import get_axis_values, get_bin_edges, project_data
from .data_read import get_metadata_df, load_counts, load_file, read_metadata
...
class BaseLoader:
    ...
    def __init__(
        ...
        self.event_counts = {i: 0 for i in range(self.metadata.general.lensIterations)}
    ...
    def set_iter_interval(self, start_iter: int, stop_iter: int):
        ...
        self.progress_info["total"] = self.iterations * self.metadata.general.lensSteps

    def count_events(self, start_step: tuple = None, stop_step: tuple = None) -> dict:
        """
    ...
    def add_event_counts(self, event_counts_to_add: dict) -> None:
        ...
        """
        for it in event_counts_to_add.keys():
            self.event_counts[it] += event_counts_to_add[it]

    def load_and_transform(
        self, multithreading: bool, start_step: tuple = None, stop_step: tuple = None
    ) -> dict:
        """
        ...
        while True:
            if multithreading:
                thread = PropagatingThread(
                    target=self.__process_data, args=(cur_step[0], cur_step[1], data_pieces)
                )
                self.threads.append(thread)
                thread.start()
            else:
                self.__process_data(cur_step[0], cur_step[1], data_pieces)
            # break loop if last loaded step was the last step
            if is_last_step(cur_step[0], cur_step[1], self.stop_iter, self.lens_steps):
                break
            cur_step = get_next_step(cur_step[0], cur_step[1], self.lens_steps)
    ...
    def __process_data(self, it: int, step: int, data_pieces: dict):
        """
        ...
        try:
            ...
            time.sleep(0.5)
            raw_data = load_file(self.path, it, step)
        data_pieces[it].append(self.__transform_data(raw_data, step))
        self.progress_info["current"] += 1
        self.__print_progress()

    def __transform_data(self, raw_data: np.ndarray, step: int):
        """
        Load and transform single data file in given format (needed for multithreading).
        ...
        if self.acquisition_mode == "sweep":
            ...
            return self.transformer.transform(
                raw_data, self.transform_format, center_energy=center_energy
            )
    ...
    def export_to_csv(
        ...
        axes_values = get_axis_values(self.bin_edges, self.axes, proj_axes)
        ...
        for win_id, data in self.binned_data.items():
            ...
            # add axis to header
            for i, ax in enumerate(proj_axes):
    ...
    def export_counts_to_csv(self, path: str, iter_range: list = None, delimiter: str = ","):
        ...
        iterations, counts = self.get_iter_counts(iter_range=iter_range)

        df = pd.DataFrame({"Iteration": iterations, "Counts": counts})
        df.to_csv(f"{path}.csv", index=False, sep=delimiter)

    def get_iter_counts(self, iter_range: list = None) -> tuple[list | list]:
        """
