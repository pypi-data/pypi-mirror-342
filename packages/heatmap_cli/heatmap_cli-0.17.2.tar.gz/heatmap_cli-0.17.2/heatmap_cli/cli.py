...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        dest="cmap",
        default=None,  # Default is handled in main()
        action="append",
...
def main(args: Optional[Sequence[str]] = None) -> None:
    ...
    parser = build_parser()
    parsed_args = None  # Initialize in case parse_args fails early
    ...
    try:
        ...
        # Set default cmap if none provided
        if not parsed_args.cmap:
        ...
        if not parsed_args.demo and parsed_args.input_filename is None:
            parser.error(
                "the following arguments are required: CSV_FILENAME"
            )
        ...
        if parsed_args:
            ...
            # Fallback check if parsing failed before setting parsed_args.debug
            debug_active = True
