...
class SmokeTest(IsolatedAsyncioTestCase):
    ...
    async def test_connection_await(self):
        db = await aiolitesql.connect(self.db)
        self.assertIsInstance(db, aiolitesql.Connection)
    ...
    async def test_connection_context(self):
        async with aiolitesql.connect(self.db) as db:
            self.assertIsInstance(db, aiolitesql.Connection)
    ...
    async def test_connection_locations(self):
        ...
        async with aiolitesql.connect(locs[0]) as db:
        ...
        for loc in locs:
            async with aiolitesql.connect(loc) as db:
                cursor = await db.execute("select * from foo")
    ...
    async def test_multi_loop_usage(self):
        ...
        async with aiolitesql.connect(":memory:") as db:
    ...
    async def test_connection_properties(self):
        async with aiolitesql.connect(self.db) as db:
            ...
            async with db.cursor() as cursor:
                cursor.row_factory = aiolitesql.Row
                self.assertEqual(cursor.row_factory, aiolitesql.Row)
                await cursor.execute("select * from test_properties")
                row = await cursor.fetchone()
                self.assertIsInstance(row, aiolitesql.Row)
            ...
            self.assertEqual(db.row_factory, aiolitesql.Row)
    ...
    async def test_connect_error(self):
        ...
        with self.assertRaisesRegex(OperationalError, "unable to open database"):
            async with aiolitesql.connect(bad_db) as db:
                self.assertIsNone(db)  # should never be reached

        with self.assertRaisesRegex(OperationalError, "unable to open database"):
            await aiolitesql.connect(bad_db)
    ...
    async def test_connect_base_exception(self):
        ...
        connection = aiolitesql.Connection(lambda: sqlite3.connect(":memory:"), 64)
        with (
    ...
    async def test_backup_aiosqlite(self):
        ...
        async with (
            aiolitesql.connect(":memory:") as db1,
            aiolitesql.connect(":memory:") as db2,
        ):
    ...
    async def test_backup_sqlite(self):
        async with aiolitesql.connect(":memory:") as db1:
