...
class PerfTest(TestCase):
    ...
    async def test_connection_memory(self):
        while True:
            yield
            async with aiolitesql.connect(TEST_DB):
                pass
    ...
    async def test_connection_file(self):
        with tempfile.NamedTemporaryFile(delete=False) as tf:
            ...
            async with aiolitesql.connect(path) as db:
                await db.execute(
            ...
            while True:
                yield
                async with aiolitesql.connect(path):
                    pass

    @timed
    async def test_atomics(self):
        async with aiolitesql.connect(TEST_DB) as db:
