...
def main():
    ...
    # Check if speech-to-text mode is enabled
    if "--speech" in sys.argv:
        print(colorize("🎙️ Speech-to-text mode activated. Please speak your command...", Colors.OKCYAN))
        user_command = audio_to_text()
        if not user_command:
            print(colorize("❌ Could not process speech input. Please try again or use text mode.", Colors.FAIL))
            return
    else:
        # Read user command from command-line arguments
    ...
    print(colorize(f"📝 AI-generated shell command: {shell_command}", Colors.OKCYAN))

    # ✅ SAFETY CHECK using Gemini
    if security_check:
        loader = LoadingDots("🔒 Checking command safety with AI")
        loader.start()
        safety_result = is_command_safe(shell_command, gemini_api_key, gemini_model_config)
        loader.stop()

        print(colorize(f"🔍 Safety Analysis: {safety_result}", Colors.WARNING))

        if "not safe" in safety_result.lower() or "dangerous" in safety_result.lower():
            user_input = input(colorize("⚠️ The command may be unsafe. Do you want to run it anyway? (yes/no): ", Colors.FAIL)).strip().lower()
            if user_input != "yes":
                print(colorize("❌ Aborting command execution for safety.", Colors.FAIL))
                return
