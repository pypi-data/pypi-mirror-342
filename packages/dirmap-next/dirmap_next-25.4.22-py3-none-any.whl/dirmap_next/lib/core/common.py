...
from ..core.data import paths, payloads
from ..core.setting import BANNER
...
def genIP(ip_range: str) -> list[str]:
    '''
...
# 识别目标，转换成列表形式
def parseTarget(target: str) -> list[str]:
    lists=[]
        ...
        if parsed_url.scheme != 'http':
            ...
            if ipv4withmask_re.search(parsed_url.path):
                ...
                if flag in ('N', 'n', 'no', 'No', 'NO'):
                    ...
                    lists=list(map(str, ipaddress.ip_interface(target).network))
...
def intToSize(bytes: int) -> str:
    ...
    for i in a:
        if bytes >= b:
            return f'{float(bytes) / float(b):.2f}{i}b'
        b /= 1024
    return '0b'

def urlSimilarCheck(url: str) -> bool:
    '''
