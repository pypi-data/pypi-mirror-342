...
from ..core.common import intToSize, outputscreen, urlSimilarCheck
from ..core.data import bar, conf, paths, payloads, tasks, th
from ..utils.config import ConfigFileParser
        ...
        if conf.crawl_mode:
            ...
            if conf.request_headers:
                try:
                    ...
                    outputscreen.error(f"[x] Check personalized headers format: header=value,header=value.\n[x] error:{e}")
                    # sys.exit()
            #自定义ua
            ...
            try:
                ...
                outputscreen.error(f"[x] Crawler network connection error!plz check whether the target is accessible, error info:{e}")
                # sys.exit()
