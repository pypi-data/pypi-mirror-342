...
def parse_inputs(expr, symbols, name_result):
    # clean up initial symbols
    symbols = symbols_given_cleanup(expr, symbols)

    if isinstance(expr, str):
        expr = string_expr_cleanup(expr)
        expr, symbols, indexers, results = string_expr_to_sympy(expr, name_result, symbols)
    ...
    return expr, symbols, indexers, results, symbols_dummy


def parse_parallel_model(model_str=None):
    # TODO should this be merged with instance config? probably yes

    # begin with defaults (subject to change, likely warning for non-None)
    parallel_model = {
        "numba_parallel":    True,   # longer build times, but may benefit
        "allow_prange":      None,   # decide based upon data ndim
        "native_threadpool": False,  # FUTURE ignore numba parallelization
    }

    if model_str is None:
        # model_str = "native_threadpool"  # FUTURE prefer native as default [ISSUE 147]
        return parallel_model

    # expressly disable all parallization
    # won't lose errors in Numba threads and shorter build times
    if model_str == "disabled":
        parallel_model = {
            "numba_parallel":    False,
            "allow_prange":      False,
            "native_threadpool": False,
        }
        return parallel_model

    if model_str == "prange":
        parallel_model["numba_parallel"]    = True
        parallel_model["allow_prange"]      = True
        return parallel_model

    if model_str == "native_threadpool":  # pragma nocover [ISSUE 147]
        raise NotImplementedError("parallel_model should only be 'disabled' or 'prange' for now, a future version may support 'native_threadpool'")
        parallel_model["numba_parallel"]    = False
        parallel_model["allow_prange"]      = False
        parallel_model["native_threadpool"] = True
        return parallel_model

    raise ValueError(f"parallel_model must be one of (None,'disabled','prange'), but got '{model_str}'")
