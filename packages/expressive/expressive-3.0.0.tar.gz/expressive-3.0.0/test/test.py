...
@contextlib.contextmanager
def patch_config(field, new, debug=False):
    """ helper to avoid frustrating unittest.mock.patch.dict """
    refchain = field.split(".")
    value = expressive.config.CONFIG
    if debug:  # pragma nocover FUTURE show diffs
        print("original config")
        print(text_indent(json.dumps(expressive.config.CONFIG, sort_keys=False, indent=4), "    "))
    for field_name in refchain:
        outer = value              # keep a reference to the last dict in the chain
        value = value[field_name]  # next successive value (might be another dict)
    # `value` is now the final value in the chain (and probably not a dict, though it can be if `new` is one!)
    # and `outer` is the by-reference dict containing it (at `refchain[-1]`)
    if not isinstance(outer, dict):
        # if here, field is probably wrong and it's best to pack the whole value of upper field member
        raise ValueError(f"BUG: outer must be a dict, but got {type(outer)}: {outer}")
    field_final = refchain[-1]  # actually the name field_name should still be ok here
    outer[field_final] = new    # dict wrapping final value (by-reference)
    if debug:  # pragma nocover FUTURE show diffs
    ...
    try:
        ...
        outer[field_final] = value  # restore the original value
    if debug:  # pragma nocover FUTURE show diffs
...
class TestEqualityExtract(unittest.TestCase):
    ...
    def test_pass_name_result(self):
        ...
        with self.assertRaisesRegex(ValueError, re.escape("mismatch between name_result (b) and parsed symbol name (a)")):
            E = Expressive("a = x**2", name_result="b")
...
class TestGuess_dtype(unittest.TestCase):
    ...
    def test_signature_automatic(self):
        ...
        data, dtype_result_prepare, sig_prepare, result_passed, allow_prange = E._prepare(data, dtype_result=None)
...
class TestParallelizationModel(unittest.TestCase):
    ...
    def test_parallel_model_parse(self):
        ...
        # FUTURE special-case likely future feature "native_threadpool" [ISSUE 147]
        with self.assertRaisesRegex(NotImplementedError, r"disabled.*prange.*for now.*future.*native_threadpool"):
            expressive.parsers.parse_parallel_model("native_threadpool")
    ...
    def test_prange_ndim_guess(self):
        ...
        # FUTURE may choose threadpool [ISSUE 147]
        # unset (None) -> max .ndim 1 -> True
        E = Expressive("a + log(b) * c")
        _, _, _, _, allow_prange = E._prepare(data_ndim1, dtype_result=None)
        ...
        with must_warn(r"mixed dimensions may not broadcast correctly"):
            with self.assertRaisesRegex(ValueError, r"couldn't determine result dimensions.*provide a result array"):
                _, _, _, _, allow_prange = E._prepare(data_ndim2, dtype_result=None)

        data_ndim2["result"] = numpy.zeros_like(data_ndim2["a"], dtype="float32")  # create a result array to avoid error
        with must_warn(r"mixed dimensions may not broadcast correctly"):
            data, _, _, _, allow_prange = E._prepare(data_ndim2, dtype_result=None)
...
class TestSumCompiler(unittest.TestCase):
    ...
    def test_basic(self):
        ...
        if VERSION_SYMPY <= (1, 7):  # pragma nocover (path only used in older versions)
            ...
            _expr = expr.simplify()
            if str(_expr) == str(expr):
                warnings.warn(f"this block may no longer be needed for symyp v{VERSION_SYMPY}", RuntimeWarning)
            expr = _expr
    ...
    def test_result_passed(self):
        ...
        ref = data["r"]
        # don't simplify
        with patch_config("translate_simplify.build.sum.try_algebraic_convert", False):
    ...
    def test_warn_thread(self):
        ...
        with patch_config("translate_simplify.build.sum.threaded_timeout_warn", timeout):
            with must_warn([
                # re.escape(r"Sum() has no non-dummy symbols"),
                re.escape(f"failed to simplify Sum() in expr after {timeout}s (may be hung)"),
            ]):
                with unittest.mock.patch("sympy.simplify", wraps=sympy.simplify) as mock:
