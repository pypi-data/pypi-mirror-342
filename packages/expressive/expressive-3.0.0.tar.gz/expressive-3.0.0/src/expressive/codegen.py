...
from .config import (
    CONFIG,  # FIXME only needed for `Sum()` extras, ideally fixed by [ISSUE 147]
    DTYPES_FILLER_HINT,
    SYMPY_ATOMS_FP,
)
...
def loop_function_template_builder(expr, symbols, indexers, results, result_passed, dtype_result, data, allow_prange=False):
    ...
    if not indexers:
        ...
        range_sequence_outer = "prange" if allow_prange else "range"
        ...
        def expressive_wrapper({block_args}):
            ...
            {block_result}
            for {indexer} in {range_sequence_outer}({start}, {end}):
                {expr.lhs} = {expr.rhs}
            return {name_result}
        """
    else:
