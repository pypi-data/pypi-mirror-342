...
def test_check_source_success(tmp_path, monkeypatch, dummy_kvle, dummy_kvlr):
    ...
    monkeypatch.setattr(
        kvl_instance, "search_python_files_in_dir", lambda x: [str(file_path)]
    )
...
def test_check_source_errors(
    tmp_path, monkeypatch, dummy_kvlr_nok, dummy_kvle_source_nok
):
    """
    ...
    monkeypatch.setattr(
        kvl_instance, "search_python_files_in_dir", lambda target: [str(file_path)]
    )
    # NOTE: Patch get_executer and get_reporter to return dummy objects that simulate errors.
    monkeypatch.setattr(
        kvl_instance, "get_executer", lambda **kwargs: dummy_kvle_source_nok()
    )
    ...
    class DummyFlake8Report:
        ...
        _application = type(
            ...
            (),
            {
                "file_checker_manager": type(
                    "DummyFcm", (), {"results": [("file", 1, 2)]}
                )
            },
        )()

    dummy_style_guide = type(
        "DummyStyleGuide", (), {"check_files": lambda self, files: DummyFlake8Report()}
    )()
    monkeypatch.setattr(
        "flake8.api.legacy.get_style_guide", lambda: dummy_style_guide()
    )
