...
def test_dump_test_results_report_json_success(tmp_path):
    ...
    assert (
        kvlr_instance._dump_test_results_report_json(formatted_results, str(dump_file))
        is True
    )
...
def test_dump_test_results_report_json_failure(monkeypatch):
    ...
    with pytest.raises(WriteFailureException):
        kvlr_instance._dump_test_results_report_json(
            formatted_results, "dummy_report.json"
        )
...
def test_generate_report_no_dump(monkeypatch):
    ...
    kvlr_instance = KVLR()
    monkeypatch.setattr(
        kvlr_instance, "check_preconditions", lambda *results, dump=False: True
    )
    monkeypatch.setattr(
        kvlr_instance,
    ...
    report = kvlr_instance.generate_report(
        {"source": {"ok": ["file1.py"], "nok": []}}, dump=False
    )
...
def test_generate_report_with_dump(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        kvlr_instance, "_format_validation_report", lambda *results: dummy_formatted
    )
    report = kvlr_instance.generate_report(
        {"rules": {"ok": ["rule1"], "nok": []}}, dump=True
    )
