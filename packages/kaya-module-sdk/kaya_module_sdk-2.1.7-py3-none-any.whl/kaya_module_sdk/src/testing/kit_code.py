...
class KIT:
    ...
    def run(
        ...
        def decorator(func: Callable) -> Callable:
            ...
            def wrapper(*args: Any, **kwargs: Any) -> Any:
                ...
                module_endpoint = (
                    module if (module and package) else class_name.replace("Args", "")
                )
                ...
                ret_type_hints = {
                    ...
                    for key, val in get_type_hints(
                        list(method_defaults.items())[0][1]
                    ).items()
                }
                if response["exit"] == 0:
                    for key, val in response.copy()["response"].items():
                        ...
                        try:
                            if response["response"][key] and matrix_flag:
                                casted_return = [
                                    [
                                        cls._type_cast(item, expected_key_type)
                                        for item in row
                                    ]
                                ...
                                casted_return = [
                                    cls._type_cast(item, expected_key_type)
                                    for item in response["response"][key]
                                ]
                            else:
                                casted_return = cls._type_cast(
                                    response["response"][key], expected_key_type
                                )
                ...
                if rets_constructor is None:
                    raise KITFailureException(
                        "No result function specified in test method signature."
                    )
    ...
    def check_package_installed(cls, package: str, module: str) -> bool:
        ...
        try:
            ...
            raise ModuleNotFoundException(
                f"Package {package} does not have a spec!"
            ) from e
        return True
    ...
    @classmethod
    def shell_cmd(
        cls, command: str, user: Optional[str] = None
    ) -> tuple[str, str, int]:
        """
