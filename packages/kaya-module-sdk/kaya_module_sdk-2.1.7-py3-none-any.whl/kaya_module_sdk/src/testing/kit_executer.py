...
class KITE:
    ...
    * Collects all test results in a easy to handle manner
    ...
    # @pysnooper.snoop()
    def module_request(
        self, test: dict, request_type: str, request_body: str | dict
    ) -> dict:
    ...
    def _type_cast(self, value: str, target_type: type | None) -> Any:
        ...
        try:
            ...
            raise ValueError(
                f"Failed to cast '{value}' to {target_type.__name__}! Details: {e}"
            ) from e
    ...
    def _execute(self, *tests: dict) -> dict:
        ...
        #     "float": float,
        #     "str": str,
        #     "bool": lambda x: x.strip().lower() in {"true", "1", "1.0", "yes"},
        # }
        for test in tests:
            ...
            for key in result["response"]:
                if key in ("error", "errors") or (
                    ...
                    and not len(result["response"][key])
                ):
                    continue
                ...
                # return_key_type = type(
                #     None if not len(result["response"][key])
                #     else (
                #         result["response"][key][0] if isinstance(result['response'][key], list)
                #         else result['response'][key]
                #     )
                # )
                try:
                    if result["response"][key] and isinstance(
                        result["response"][key], list
                    ):
                        casted_return = [
                            self._type_cast(item, expected_key_type)
                            for item in result["response"][key]
                        ]
                    else:
                        casted_return = self._type_cast(
                            result["response"][key], expected_key_type
                        )
                except Exception:
                    nok.update(test_record)
                    break
                if (
                    ...
                    or casted_return != test["expected"]["return"][key]["value"]
                ):
                    nok.update(test_record)
                    break
            if test["name"] in nok:
