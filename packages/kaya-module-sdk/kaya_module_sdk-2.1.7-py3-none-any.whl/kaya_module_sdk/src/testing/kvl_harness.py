...
class KVL:
    ...
    def load_module_package(self, module_name: str) -> dict:
        ...
        strategy_class = getattr(pkg, "KayaStrategyModule")
    ...
    # @pysnooper.snoop()
    def check_rules(
        self, module: str, dump_report: bool = False, **kwargs: dict
    ) -> list:
        """
        ...
        try:
            ...
            report = self.get_reporter(**self.__dict__).generate_report(
                {"rules": results}, dump=dump_report
            )
    ...
    # @pysnooper.snoop()
    def check_source(
        self, file_path: str = "", dump_report: bool = False, **kwargs: dict
    ) -> list:
        """
        ...
        target = file_path or "."
        files = (
            self.search_python_files_in_dir(target)
        ...
        try:
            ...
            report = self.get_reporter(**self.__dict__).generate_report(
                {"source": results}, dump=dump_report
            )
        except Exception as e:
