...
def test_import_subclasses(mock_module):
    ...
    mock_mod.__package__ = (
        mock_package  # Simulate a real module with a __package__ attribute.
    )
    with patch("importlib.import_module", return_value=mock_mod):
        result = mock_module.import_subclasses()
        assert (
            result == mock_module.subclasses
        )  # Verify that it returns the subclasses list.
...
def test_manifest_property(mock_module):
    ...
    with patch.object(
        mock_module, "_extract_manifest", return_value={"moduleName": "TestModule"}
    ):
