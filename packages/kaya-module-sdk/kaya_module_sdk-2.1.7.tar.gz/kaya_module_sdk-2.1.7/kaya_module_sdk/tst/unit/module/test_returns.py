...
def test_property_getters():
    ...
    assert (
        str(r.errors[0]) == "Test error"
    ), "Expected the first error to be 'Test error'."
...
def test_metadata():
    ...
    assert hasattr(
        results_annotations, "__args__"
    ), "Annotations for '_results' should have '__args__'."
    ...
    if len(results_annotations.__args__) == 1:
        ...
        ), "Expected DisplayName annotation for '_results'."
        assert (
            display_name._data.get("name") == "Result"
        ), "DisplayName should match the expected value."
        assert (
            display_description is not None
        ), "Expected DisplayDescription annotation for '_results'."
        assert (
            display_description._data.get("description")
            == "Module computation results."
...
def test_results_and_errors():
    ...
    assert [str(e) for e in r.errors] == [
        "Error 1"
    ], "Expected _errors to be ['Error 1']."
