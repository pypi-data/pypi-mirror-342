...
def find_rates(
    ...
    matching_rates = filter_rates(config.all_rates, rate_type.id, category_id, tool_id, area_id, consumable_id)
    if not rate_type.can_have_rate_time():
        ...
        if not matching_rates or not any([rate for rate in matching_rates if not rate.time]):
            raise NoRateSetException(rate_type, category, tool=tool, area=area, consumable=consumable)
        return matching_rates


def filter_rates(rates: List[Rate], rate_type_id, category_id, tool_id, area_id, consumable_id) -> List[Rate]:
    matching_rates = []
    for rate in rates:
...
def process_billables(item, project, configuration: InvoiceConfiguration, raise_no_rate=True) -> List[BillableItem]:
    ...
    try:
        ...
        if not billable.amount:
            ...
            rates = find_rates(
                billable.rate_type, billable.project, configuration, billable.tool, billable.area, billable.consumable
            )
            billables: List[BillableItem] = []
