...
class DataManager:
    ...
    def get_average_cop(self):
        filters_for_cop = ['MV_COMMISSIONED','PLANT_COMMISSIONED','DATA_LOSS_COP','SYSTEM_MAINTENANCE','SYSTEM_TESTING']
    ...
    def generate_daily_stats_query(self):
        data_stat_table = self.get_attribute_for_site("data_stats_table")
        if data_stat_table is None:
            data_stat_table = f"{self.day_table}_stats"
        summary_query = f"SELECT * FROM {data_stat_table} "
    ...
    def apply_event_filters_to_df(self, df : pd.DataFrame, events_to_filter : list, exclude_ongoing : list = []):
        ...
        if len(events_to_filter) > 0:
            filtered_df = df.copy()
            query = f"SELECT start_time_pt, end_time_pt, event_type FROM site_events WHERE site_name = '{self.selected_table}' AND event_type IN ("
            query = f"{query}'{events_to_filter[0]}'"
            for filter_event_type in events_to_filter[1:]:
                query = f"{query},'{filter_event_type}'"
            query = f"{query})"
            ...
            time_ranges = [(pd.to_datetime(start_time), pd.to_datetime(end_time) if not end_time is None else None, event_type) 
                           for start_time, end_time, event_type in time_ranges]

            # Remove points in the DataFrame whose indexes fall within the time ranges
            for start_time, end_time, event_type in time_ranges:
                if event_type == 'MV_COMMISSIONED' or event_type == 'PLANT_COMMISSIONED':
                    # filter out data befor commissioned event
                    if end_time is None:
                        filtered_df = filtered_df.loc[filtered_df.index >= start_time]
                    else:
                        filtered_df = filtered_df.loc[filtered_df.index >= end_time]
    ...
    def get_site_events(self, filter_by_date : bool = True, event_types : list = [], start_date : str = None, end_date : str = None,
                        include_ids : bool = True) -> pd.DataFrame:
        """
        Parameters
        ----------
        filter_by_date : bool
        ...
        end_date : str
            String representation for the start date of the timeframe
        include_ids : bool
            includes event ids in the returned df if True.
        ...
        if include_ids:
            query += "id, "
        query += f"start_time_pt, end_time_pt, event_type, event_detail FROM site_events WHERE site_name = '{self.selected_table}'"
