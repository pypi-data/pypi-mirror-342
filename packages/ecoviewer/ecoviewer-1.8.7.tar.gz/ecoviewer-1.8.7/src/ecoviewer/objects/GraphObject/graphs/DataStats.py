...
class DataStats(GraphObject):
    ...
    def create_graph(self, dm : DataManager):
        ...
        try:
            daily_stats_df = dm.get_df_from_query(query)
        except:
        ...
        total_missing = daily_stats_df.filter(regex='_missing_mins$').rename(columns=lambda x: x.replace('_missing_mins', ''))
        max_gap_missing = daily_stats_df.filter(regex='_max_gap$').rename(columns=lambda x: x.replace('_max_gap', ''))
        avg_gap_missing = daily_stats_df.filter(regex='_avg_gap$').rename(columns=lambda x: x.replace('_avg_gap', ''))
