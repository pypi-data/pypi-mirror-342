...
class BaseSheetInterface(object):
    def __init__(self, model_cls, spreadsheet_id, sheet_name=None, data_range=None, model_id_field=None,
                 sheet_id_field=None, batch_size=None, max_rows=None, max_col=None, sheet_headers = None, **kwargs):
        """
    ...
    def sheet_headers(self):
        ...
        lg(f'self._sheet_headers = {self._sheet_headers}')
    ...
    def column_index(self, field_name):
        ...
        lg(f'{field_name} está em {self.sheet_headers} ?? ')
        return self.sheet_headers.index(str(field_name))
    ...
    def writeout_batch(self, ranges, data):
        ...
        print(f'data {data}')
        print(f'len(ranges) != len(data) {len(ranges) != len(data)}')
...
class SheetPushInterface(BaseSheetInterface):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # ['id', 'nome', 'tom', 'link', 'partes_da_musica', 'informacoes', 'acordes']
    ...
    def upsert_table(self):
        ...
        for i, obj in enumerate(queryset):
            if i > 0 and i % self.batch_size == 0:
                ...
                # p('writeout_range_start_row, writeout_range_end_row')
                # p(writeout_range_start_row, writeout_range_end_row)
                ...
                # p('writeout_range1', writeout_range)
                # p('writeout_data1', writeout_data)
        ...
        if last_writeout < len(queryset):
            ...
            # lg(f'writeout_range2 =  {writeout_range}')
            # lg(f'self.sheet_data =  {self.sheet_data}')
...
class SheetFindInterface(BaseSheetInterface):
    ...
    @decorators.backoff_on_exception(decorators.expo, HttpError)
    def read_range(self, tipo, valor):
        """
        ...
        if tipo == 'linha':
            range_to_read = f'{self.sheet_name}!{valor}:{valor}'  # Exemplo: 'Sheet1!2:2'
        elif tipo == 'coluna':
            range_to_read = f'{self.sheet_name}!{valor}:{valor}'  # Exemplo: 'Sheet1!B:B'
        elif tipo == 'celula':
            range_to_read = f'{self.sheet_name}!{valor}'          # Exemplo: 'Sheet1!E2'
        elif tipo == 'range':
            range_to_read = f'{self.sheet_name}!{valor}'          # Exemplo: 'Sheet1!A1:C5'
        else:
            raise ValueError("Invalid type specified. Must be 'linha', 'coluna', 'celula', or 'range'.")

        try:
            # Make the API request to get the values from the range
            request = self.api.spreadsheets().values().get(
            ...
            logger.debug(f'Got response {response} reading {tipo} with value {valor}')
            ...
            if tipo == 'linha' or tipo == 'coluna':
                ...
                # Determina estrutura do dicionário
                num_rows = len(data)
                num_cols = max(len(row) for row in data)

                result = {}
                if num_rows < num_cols:
                    # Linhas > Colunas: usa os valores da primeira coluna como chaves
                    for row in data:
                        if len(row) > 0:  # Verifica se há dados na linha
                            key = row[0]  # Elemento da primeira coluna
                            result[key] = row[1:]  # Valores restantes da linha
                else:
                    # Colunas >= Linhas: usa os valores da primeira linha como chaves
                    header = data[0]
                    for col_idx, key in enumerate(header):
                        result[key] = [row[col_idx] if col_idx < len(row) else None for row in data[1:]]
    ...
    def All_datas(self):
        ...
        try:
            ...
            self._sheet_data = response.get('values', [])
            self._sheet_headers = self._sheet_data[0]
            # remove the headers from the data
            self._sheet_data = self._sheet_data[1:]
    ...
    @decorators.backoff_on_exception(decorators.expo, HttpError)
    def find_word_and_get_row(self, palavra):
    ...
    @decorators.backoff_on_exception(decorators.expo, HttpError)
    def Criar_worksheet(self):
        # Obter informações sobre a planilha
        sheet_headers = self.push_fields[1:]
        spreadsheet = self.api.spreadsheets().get(spreadsheetId=self.spreadsheet_id).execute()

        # Lista de abas existentes
        ...
        if self.sheet_name not in existing_sheets:
            ...
            print("Aba criada:", response)


            # Inserir os nomes das colunas na primeira linha da aba criada
            colunas =  [self.sheet_id_field]+sheet_headers
            lg(f'colunas {colunas}')
            range_name = f"'{self.sheet_name}'!A1:{chr(64 + len(colunas))}1"  # Exemplo: 'NovaAba'!A1:D1 para 4 colunas
            values = [colunas]  # Insere os nomes das colunas em uma única linha
            data = {
                "range": range_name,
                "majorDimension": "ROWS",
                "values": values
            ...
            print(f"Os nomes das colunas foram adicionados à aba '{self.sheet_name}'.")
    ...
    def up_data(self, linha,data):
        ...
        self.writeout_batch([writeout_range], [[data]])
