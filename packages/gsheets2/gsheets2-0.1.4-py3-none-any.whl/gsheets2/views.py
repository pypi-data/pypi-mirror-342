...
class AuthorizeView(TemplateView):
    def get(self, request, *args, **kwargs):
        ...
        # error.
        # print('gsheets_settings.OAUTH_REDIRECT_HOSTNAME', gsheets_settings.OAUTH_REDIRECT_HOSTNAME, '\n')

        flow.redirect_uri = get_oauth_cb_url(request, cb_hostname=gsheets_settings.OAUTH_REDIRECT_HOSTNAME)
        # flow.redirect_uri = "https://localhost:8000/gsheets/auth-success"


        # flow.redirect_uri = "https://127.0.0.1:8000/gsheets/authorize"

        printar('flow redirect URI:', get_oauth_cb_url(request, cb_hostname=gsheets_settings.OAUTH_REDIRECT_HOSTNAME))
...
class OAuthSuccessView(TemplateView):
    def get(self, request, *args, **kwargs):
        ...
        printar("verificar pq a request.session['state'] não está armazenando o valor de state")
        state = st
        ...
        flow.redirect_uri = get_oauth_cb_url(request, cb_hostname=gsheets_settings.OAUTH_REDIRECT_HOSTNAME)
        # flow.redirect_uri = "http://localhost:8000/gsheets/auth-success"


        # Use the authorization server's response to fetch the OAuth 2.0 tokens.
        authorization_response = ensure_https(request.build_absolute_uri())

        # printar('authorization_response', authorization_response)
