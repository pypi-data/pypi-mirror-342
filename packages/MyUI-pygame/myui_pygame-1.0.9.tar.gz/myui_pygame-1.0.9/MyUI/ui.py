...
class Text:
    ...
    """
    def __init__(self, font: pygame.font.Font, pos: InputVector, size: InputVector, text_color: InputColor, border_color: InputColor, cursor_color: InputColor = (150, 150, 150, 200), flags: int = 0):
        ...
        self.cursor_color: pygame.Color = pygame.Color(cursor_color)
...
class Surface:
    ...
    def toggle_active(self, enabled: bool = None) -> None:
        """
        Toggle if the window should be drawn or not. 
        If not `enabled` is provided, it defaults to the opposite of the current value.
        """
        if enabled is None: enabled = not self.active
        self.active = enabled
...
class Group:
    """Easily seperate components by different groups. Now you know which are which!"""
    def __init__(self):
        self.event_handlers: List[EventCallback] = []
        self.on_render: List[RenderCallback] = []
    ...
    def handle_event(self, event: pygame.event.Event, offset: InputVector = (0, 0)) -> Optional[bool]:
        """Handle events for all components in the group."""
    ...
    def render(self, surface: pygame.Surface) -> None:
        """Render the group of content."""

        if not self.active: return

        for render in self.on_render: render(surface)
...
component = Union[Text, Button, Slider, DropdownMenu, Tooltip, Dropbox, Window, Surface, Group, HotbarItem, Hotbar, App]
