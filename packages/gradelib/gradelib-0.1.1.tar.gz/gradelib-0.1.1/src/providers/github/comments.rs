...
async fn fetch_repo_comments(
    ...
    let types = comment_types.unwrap_or_else(|| vec![
        CommentType::Issue,
        CommentType::Commit,
    ...
    if types.iter().any(|t| matches!(t, CommentType::ReviewComment)) {
        let owner_clone = owner.clone();
        let repo_clone = repo.clone();
        let client_clone = client.clone();
...
async fn fetch_issue_comments(
    ...
    if !issues_response.status().is_success() {
        return Err(format!("GitHub API error for issues: {}", issues_response.status()));
    }
    ...
    for issue in issues {
        ...
        match fetch_issue_comments_for_number(client, &comments_url, issue.number).await {
            ...
            Err(e) => eprintln!("Warning: Failed to fetch comments for issue #{}: {}", issue.number, e),
        }
    }
...
async fn fetch_issue_comments_for_number(
    ...
    if !response.status().is_success() {
        return Err(format!("GitHub API error for issue comments: {}", response.status()));
    }
...
async fn fetch_pr_comments(
    ...
    if !prs_response.status().is_success() {
        return Err(format!("GitHub API error for PRs: {}", prs_response.status()));
    }
    ...
    for pr in pull_requests {
        ...
        match fetch_pr_comments_for_number(client, &comments_url, pr.number).await {
            Ok(comments) => all_comments.extend(comments),
            Err(e) => eprintln!("Warning: Failed to fetch comments for PR #{}: {}", pr.number, e),
        }
    }
...
async fn fetch_pr_comments_for_number(
    ...
    if !response.status().is_success() {
        return Err(format!("GitHub API error for PR comments: {}", response.status()));
    }
...
async fn fetch_review_comments(
    ...
    if !response.status().is_success() {
        return Err(format!("GitHub API error for review comments: {}", response.status()));
    }
    ...
    for comment in comments {
        ...
        let pr_number = comment.pull_request_url
            .split('/')
            .last()
...
async fn fetch_commit_comments(
    ...
    if !response.status().is_success() {
        return Err(format!("GitHub API error for commit comments: {}", response.status()));
    }
