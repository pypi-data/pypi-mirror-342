...
def generate_structures(
    ...
    while len(final_pool) < config.n_per_structure + len(existing_pool):
        ...
        s = select_structure(final_pool, kT, config.beta, rand)
        ...
        dx = rand.randn(len(rattled.positions), 3) * sigma
        dx -= np.mean(dx, axis=0)  # subtrac c.o.m.
        rattled.positions += dx
        rattled.info["sigma"] = sigma

        rattled.info["parent"] = s.info["id"]
        rattled.info["level"] = s.info["level"] + 1
        rattled.info["id"] = unique_id()

        # skip if already too similar
        if too_similar(rattled.positions @ restoring_matrix, final_pool):
            continue

        # relax the structure
        s = label(rattled, calc)
        s.info["relax_steps"] = 0
        rejected = False
        prev_s = s.copy()
        ...
        for i in range(1, 21):
            ...
            ΔE = (s.info["energy"] - prev_s.info["energy"]) / len(s)
            prob = np.exp(-ΔE / kT)
            prob = min(0.25, prob)
            if max_force(s) < 30 and rand.uniform() < prob:
                break
...
def main():
    ...
    for i, s in enumerate(starting_structures):
        print(f"Augmenting structure {i} of {len(starting_structures)}")
