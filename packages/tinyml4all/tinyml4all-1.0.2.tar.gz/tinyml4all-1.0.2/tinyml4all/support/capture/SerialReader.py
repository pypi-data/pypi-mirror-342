...
class SerialReader:
    ...
    def __enter__(self):
        ...
        try:
            ...
            if "resource busy" in str(e):
                raise RuntimeError(f"{self.port} port is busy. Maybe you have the Arduino Serial Monitor open?")
    ...
    def resolve_port(self, port: str) -> str:
        ...
        for entry in list_ports.comports():
            if pattern.search(entry.name) is not None or pattern.search(entry.device) is not None:
                return entry.device
