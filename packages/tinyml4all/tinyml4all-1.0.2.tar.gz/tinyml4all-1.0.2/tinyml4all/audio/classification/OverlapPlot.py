    ...
    def __call__(self,
        ...
        def make_chart(label: str, files: List[WAV]):
            ...
            for file, color in zip(files, colors):
                ...
                chart.add_dataset(label=file.path, data=data, borderColor=color, backgroundColor=color, pointRadius=0)
        ...
        charts = list(make_chart(label, list(files)) for label, files
                  in groupby(sorted(self.album.files, key=lambda file: file.dirname),
