...
class PowerTransform(ProcessingBlock):
    ...
    def transform(self, dataset, *args, **kwargs):
        ...
        """
        df = dataset.df.copy()
        df[self.columns] = self.power.transform(self.preprocess(df[self.columns].to_numpy()))
    ...
    def get_configurables(self) -> List[ConfigurableAttribute]:
        ...
        """
        return [
            Hidden(name="lambdas", value=self.power.lambdas_),
        ]
    ...
    def hydrate_from_dict(cls, attributes: dict) -> Self:
        ...
        if lambdas := attributes.get("lambdas", {}).get("value", []):
            instance.power.lambdas_ = lambdas
