...
class ScatterPlot:
    ...
    def __call__(self, *args, column: str = None, orientation: str = "vertical", **kwargs):
        """
        Draw plot
        :param args:
        :param kwargs:
        ...
        columns = [column] if column is not None else numeric(self.table.df, boolean=False).columns.tolist()

        for column in columns:
            chart = Chartjs(type="scatter", ax=column, ay="Ground truth", title=f"{column} vs. ground truth")
