...
class RandomForest(Estimator):
    ...
    """
    def __init__(self, n_estimators: int = 15, *, max_depth: int = 10, min_samples_leaf: int = 5, **kwargs):
        Estimator.__init__(self)

        self.estimator = RandomForestClassifier(n_estimators=n_estimators, max_depth=max_depth, min_samples_leaf=min_samples_leaf, **kwargs)
    ...
    def get_configurables(self) -> List[ConfigurableAttribute]:
        ...
        return [
            Numeric(
                ...
                value=self.estimator.n_estimators,
                min=1,
                max=100,
                step=1
            ),
            Numeric(
                ...
                value=self.estimator.max_depth,
                min=1,
                max=20,
                step=1
            )
    ...
    def hydrate_from_dict(cls, attributes: dict) -> Self:
        ...
        :return:
        """
        n_estimators = int(attributes.get("n_estimators", {}).get("value", 10) or 10)
        max_depth = int(attributes.get("max_depth", {}).get("value", 10) or 10)
