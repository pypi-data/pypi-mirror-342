...
class Table(HasSources, IsPickleable):
    ...
    def __getitem__(self, item: Union[str, int, List[str], Array, slice]) -> Union["Table", dict, Series]:
        """
        Array-like access
        :param item:
        :return:
        ...
        assert is_iterable(item, str), 'item must be a list of strings to select columns'
    ...
    def get_column(self, column: str) -> Series:
        ...
        :return:
        """
        return Series(np.concatenate([source.df[column] for source in self.sources]), name=column)
    ...
    def get_rows(self, rows: Tuple[int|None, int|None]|Array) -> "Table":
        ...
        for i in rows:
            ...
            if new_sources[source_index] is None:
                ...
                new_sources[source_index] = Source(source.source_name, index=source.index, data=empty)
    ...
    def get_columns(self, columns: List[str]) -> "Table":
        ...
        :return:
        """
        return type(self)(sources=[source.get_columns(columns) for source in self.sources])

    def set_targets(self, *, column: str = None, values: Array = None, rows: tuple = None) -> Self:
        """
        Set target values
        ...
        assert rows is None or isinstance(rows, tuple), "rows must be a tuple (start, end)"
