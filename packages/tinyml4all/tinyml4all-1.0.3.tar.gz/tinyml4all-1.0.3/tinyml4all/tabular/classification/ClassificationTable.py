...
class ClassificationTable(Table, HasClassmap):
    ...
    def __init__(self, sources: List[Source]):
        ...
        self.set_targets(values=[str(y) if y is not None else None for y in self.Y_true])
    ...
    def Y_true_indices(self) -> np.ndarray:
        ...
        return np.asarray([classmap.get(y_true, None) for y_true in self.Y_true], dtype=int)
    ...
    def classification_report(self) -> str:
        ...
        :return:
        """
        # assert not any(y is None for y in self.Y_pred), 'Some predictions are missing'
        Y_pred = [y for y in self.Y_pred if y is not None]
        Y_true = [y for y, pred in zip(self.Y_true, self.Y_pred) if pred is not None]
