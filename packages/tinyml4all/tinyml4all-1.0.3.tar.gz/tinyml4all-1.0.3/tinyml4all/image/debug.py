...
def decode_gray(data: bytes, w: int, h: int) -> Image:
    ...
    :return:
    """
    return Image.fromarray(np.asarray([int(x) for x in data], dtype=np.uint8).reshape((h, w)))
...
def decode_rgb(data: bytes, w: int, h: int) -> Image:
    ...
    return Image.fromarray(np.dstack([np.asarray(ch).reshape((h, w)) for ch in [r, g, b]]).astype(np.uint8))
...
def decode_bboxes(data: str) -> list[dict]:
    ...
    matches = [re.match(r"^(.+?) at coordinates (\d+), (\d+) \(confidence (\d+\.\d+)\)$", line) for line in lines]
