            ...
            with open(save_to, "a") as output:
                ...
                while not progress.expired():
                    ...
                    if 0 < len(headings) != len(values):
                        userwarn(f"Inconsistent number of values ({len(values)}). {len(headings)} expected based on headings")
                        continue

                    # transform values
                    if transform is not None:
                        kv = {heading: transform.get(heading, lambda x: x)(value) for heading, value in zip(headings, values)}
