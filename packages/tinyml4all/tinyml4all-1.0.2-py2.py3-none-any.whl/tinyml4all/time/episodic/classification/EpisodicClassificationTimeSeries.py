...
class EpisodicClassificationTimeSeries(TimeSeries, HasClassmap, Convertible):
    ...
    def __init__(self, sources: List[Source], event_durations: Dict[str, pd.Timedelta] = None):
        """
        Constructor
        :param sources:
        """
        ...
        with open(path, "r") as f:
            for event in json.load(f):
                ...
                # event
                    ...
                    self.add_label(event["label"], event["t"], height=event.get("height", 0))
                else:
    ...
    def split_by_events(self, test_size: float = 0.2) -> Tuple[Self, Self]:
        ...
        for label, label_events in groupby(events, lambda event: event.label):
            ...
            train_indices = np.random.choice(count, int(count * (1 - test_size)), replace=False)
            train_events.extend([ev for i, ev in enumerate(label_events) if i in train_indices])
            test_events.extend([ev for i, ev in enumerate(label_events) if i not in train_indices])

        def mask_events(ts: EpisodicClassificationTimeSeries, include_events: List[Event], exclude_events: List[Event]):
            T = ts.T
            df = ts.df
            ...
            return EpisodicClassificationTimeSeries(sources=[source], event_durations=self.event_durations)
