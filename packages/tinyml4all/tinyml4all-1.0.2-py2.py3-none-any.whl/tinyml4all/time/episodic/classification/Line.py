...
class Line(Convertible):
    ...
    def draw_lines(self, chart: Chartjs, df: pd.DataFrame, labels: Array, line_palette: str, normalize: bool, clip: Tuple[float, float], **kwargs):
        """
        Draw lines of data
        :return:
        """
        ...
        for i, (col, color) in enumerate(zip(df.columns, line_colors)):
            ...
            if normalize:
                values = MinMaxScaler(feature_range=(-1, 1)).fit_transform(values[:, np.newaxis]).flatten()
    ...
    def draw_events(self, chart: Chartjs, point_palette: str, T: Array, labels: List[str], **kwargs):
        """
        Draw events
        :param chart:
        :param point_palette:
        ...
        point_colors = {
            label: color
            for label, color in zip(self.ts.unique_labels, hex_colors(point_palette, n_colors=len(self.ts.unique_labels)))
        }
        scatter_data = []
