...
class ContinuousClassificationTimeSeries(TimeSeries, HasClassmap):
    ...
    @property
    def groups(self) -> Generator[Tuple[Source, str, pd.Timestamp, pd.Timestamp, pd.DataFrame], None, None]:
        """
    ...
    def as_windows(self, duration: str, shift: str = None) -> Tuple[np.ndarray, np.ndarray]:
        """
        ...
        window_indices = np.lib.stride_tricks.sliding_window_view(np.arange(len(self.df)), window_len)[::shift_len]
        ...
        return np.asarray([X[idx] for idx in window_indices]), np.asarray(
            [mode(Y[idx]).mode for idx in window_indices]).astype(int)
