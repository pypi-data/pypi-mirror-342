...
from ..providers.utils import hash_file, accept_file
...
class RetailerProvider(ReboticsBaseProvider):
    ...
    @remote_service('/api/v1/supervised-learning/scans/{id}/')
    def get_processed_scan_details(self, scan_id, **kwargs):
    ...
    @remote_service('/api/v1/realograms/voting-data-export/')
    def get_voting_data(self, scan_ids: Sequence[int], all_data: bool) -> List[dict]:
        return self.session.post(json={
            'scans': scan_ids,
            'filter_code_from_user': not all_data,
        })
