...
class EmpiricalBinningCalibrator(CalibratorBase):
    ...
    def predict(self, predictions):
        ...
        try:
            ...
            bin_indices = np.digitize(predictions, self.bin_boundaries, right=False) - 1
            bin_indices = np.clip(bin_indices, 0, self.n_bins - 1)

            calibrated_probs = np.zeros_like(predictions)

            for class_idx in range(predictions.shape[1]):  # Process each class separately
                calibrated_probs[:, class_idx] = self.bin_probabilities[bin_indices[:, class_idx], class_idx]

            # Ensure monotonicity is preserved per class
            calibrated_probs = np.maximum.accumulate(calibrated_probs, axis=1)


            row_sums = np.sum(calibrated_probs, axis=1, keepdims=True)
            valid_rows = row_sums > 0  # Avoid division by zero
            ...
            if not np.allclose(np.sum(calibrated_probs, axis=1), 1, atol=1e-6):
                raise ValueError("Probabilities do not sum to 1 after monotonicity adjustment")
