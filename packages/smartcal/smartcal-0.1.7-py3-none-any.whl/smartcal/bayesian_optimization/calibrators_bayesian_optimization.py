...
class CalibrationOptimizer:
    ...
    def optimize_calibrator(self, calibrator_name: str, X_cal, y_cal, n_iterations: int) -> Dict:
        """
    ...
    def run_optimization(self, dataset_results: Dict, total_iterations: int = 30):
        ...
        try:
                ...
                if n_iterations > 0:
                    try:
                        optimization_result = self.optimize_calibrator(
                            calibrator_name, X_cal, y_cal, n_iterations
                        )
                        ...
                        try:
                            ...
                            if cal_ece < best_ece:
                                ...
                                results["cal_f1_score_macro"] = float(f1_score(y_cal, cal_probs.argmax(axis=1), average='macro'))
                                results["cal_f1_score_micro"] = float(f1_score(y_cal, cal_probs.argmax(axis=1), average='micro'))
                                results["cal_calibration_curve_mean_predicted_probs"] = cal_metrics['calibration_curve_mean_predicted_probs']
                                results["cal_calibration_curve_true_probs"] = cal_metrics['calibration_curve_true_probs']
                                results["cal_calibration_curve_bin_counts"] = cal_metrics['calibration_curve_bin_counts']
                                
                                results["confidence_score"] = float(confidence)
                                results["allocated_iterations"] = int(n_iterations)
                                results["bayesian_optimization_time"] = optimization_result.get('bayesian_optimization_time', None)
            ...
            if best_calibrator is not None:
                try:
                    ...
                    results["test_calibration_curve_mean_predicted_probs"] = test_metrics_full['calibration_curve_mean_predicted_probs']
