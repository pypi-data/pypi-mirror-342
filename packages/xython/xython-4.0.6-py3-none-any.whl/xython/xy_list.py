...
class xy_list(object):
	...
	def filter_by_nth_style(self, input_list, nth_style, nth_only=""):
		...
		if nth_style == "even":
			...
			for one in c_list:
				aaa = self.rex.search_all_by_xsql("[+-:0~1][숫자:1~][~:1~1][+-:0~1][숫자:1~]", one)  # 3~5
				bbb = self.rex.search_all_by_xsql("[+-:0~1][숫자:1~][+-:1~1][숫자:1~]", one)  # 3+5,-3-5
				ccc = self.rex.search_all_by_xsql("[+-:0~1][숫자:1~]", one)  # 숫자만 분리
		...
		nth_jf = None
		nth_full = self.rex.search_all_by_xsql("[+-:0~1][숫자:1~]n[+-:0~1][숫자:1~]", nth_style)
		if nth_full:
			nth_jf = nth_full[0][0]
