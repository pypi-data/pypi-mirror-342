...
import xy_re, xy_color, xy_util, xy_time, xy_list, xy_common
import ctypes
...
class xy_excel:
	...
	def get_activeworkbook_object(self):
	...
	def change_56color_to_color_name(self, input_56color):
		result = self.colorx.change_56color_to_color_name(input_56color)
		return result
	...
	def change_value_in_range_by_xsql(self, sheet_name, xyxy, iy, xsql, new_value):
		...
		for index, list_1d in enumerate(list_2d):
			try:
				aa = self.rex.replace_with_xsql(xsql, new_value, list_1d[iy])
				if aa == list_1d[index]:
					pass
				else:
	...
	def check_same_data_for_two_range(self, xyxy1, xyxy2):
		...
		base_l1d = self.utilx.change_list_2d_to_list_1d(base_l2d)  # 비교를 위하여 1차원자료로 만든것
		same_data = []
	...
	def conditional_format_by_cell_value(self, sheet_name, xyxy, input_xcolor, start_xy, input_value):
		"""
	...
	def conditional_format_by_empty_value(self, sheet_name, xyxy, input_xcolor, start_xy):
		"""
	...
	def conditional_format_by_not_none_value(self, sheet_name, xyxy, input_xcolor, start_xy, cf_no=1):
		"""
		선택한 영역의 n 번째 값이 입력값과 같으면, 전체 가로줄에 색칠하기
	...
	def conditional_format_for_data_bar_style(self, sheet_name="", xyxy="", input_xcolor="red80"):
		"""
		조건부서식 : 바타입
		만약 형태를 바꾸고 싶으면 setup을 먼저 이용해서 형태를 설정합니다
	...
	def conditional_format_for_min_number(self, sheet_name, xyxy, input_xcolor="red"):
		"""
		그라데이션으로 색을 칠하는 것
		최저값에 색을 칠하는 것
	...
	def delete_same_value_in_range(self, sheet_name="", xyxy=""):
		...
		for x in range(x1, x2 + 1):
			for y in range(y1, y2 + 1):
				value = str(self.sheet_object.Cells(x, y).Value2)
				if value == "" or value == None:
					...
					len_new = len(set_a)
					if len_old == len_new:
						self.sheet_object.Cells(x, y).Value = ""
	...
	def delete_xsql_from_found_to_end_for_selection(self, xsql):
		"""
	...
	def delete_all_number_in_range(self, sheet_name="", xyxy=""):
		...
		for x in range(x1, x2 + 1):
			for y in range(y1, y2 + 1):
				...
				changed_value = self.rex.delete_number_including_comma(value)
	...
	def delete_yrange_in_range_for_empty_yrange(self, sheet_name="", xyxy=""):
		"""
		현재 선택된 영역의 각x영역이 비어있으면, 전체 x라인을 삭제하는것
		"""
		sheet_obj = self.check_sheet_name(sheet_name)
		...
		for y in range(y2, y1, -1):
			...
			if num != 0:
				char_y = self.change_num_to_char(y)
				changed_address = str(char_y) + ":" + str(char_y)
	...
	def filter_xsql_style_in_input_text(self, xsql, input_value):
		"""
		입력으로 들어온 텍스트에서 정규표현식과 맞는 것을 갖고오는 것
		"""
		result = self.check_ok_or_no(xsql, input_value)
		return result
	...
	def insert_chart_1(self, sheet_name, chart_type, pxywh, source_xyxy):
		...
		self.varx["r1c1"] = self.change_xyxy_to_r1c1([x1, y1, x2, y2])
		range_object = sheet_obj.Range(self.varx["r1c1"])
	...
	def insert_data_in_list_2d(self, sheet_name, xyxy, xy, input_value):
		...
		list_1d = self.utilx.change_list_2d_to_list_1d(reverse_list_2d)
		list_1d.insert(insert_position, input_value)
		result = self.utilx.change_list_1d_to_list_2d_group_by_step(list_1d, len_x)
		return result
	...
	def new_shape(self, sheet_name, xy="", size=[25, 25], shape_style="circle", input_xcolor="", input_value=""):
		"""
	...
	def new_shape_for_number_circle_by_setup(self, sheet_name, xy="", input_no=1):
		...
		rgb_list_1d = self.colorx.change_xcolor_to_rgb(self.varx["shape"]["color"])
		rgb_int = self.colorx.change_rgb_to_rgbint(rgb_list_1d)
		...
		rgb2_list_1d = self.colorx.change_xcolor_to_rgb(self.varx["shape_font"]["color"])
		rgb2_int = self.colorx.change_rgb_to_rgbint(rgb2_list_1d)
	...
	def draw_detail_line_in_range(self, **input):
		...
		# 기본자료에 입력받은값을 update하는것이다
		sheet_object = self.get_sheet_object(base_data["sheet_name"])
	...
	def make_dict_by_first_value_in_range(self, sheet_name="", xyxy=""):
		...
		result = {}
		list_2d = self.read_range(sheet_name, xyxy)
		list_2d = self.utilx.change_tuple_2d_to_list_2d(list_2d)
		list_2d_changed = self.utilx.delete_empty_xline_in_list_2d(list_2d)
	...
	def make_file_as_same_group(self, sheet_name, xyxy, line_index, first_is_title_or_not, folder_name):
		...
		grouped_data = self.utilx.group_list_2d_by_index(list_2d, line_index)  # 4
		startx = 1
		count = 1
	...
	def make_text_with_chain_char(self, sheet_name, xyxy, chain_char="tab"):
		...
		list_2d = self.read_value_in_range(sheet_name, xyxy)
		result = self.utilx.change_list_2d_to_text_with_chain_word(list_2d, chain_char)
		return result
	...
	def multi_vlookup(self, input_value1, input_value2):
		...
		:param input_value1: [자료의영역, 같은것이있는위치, 결과값의위치]
		:param input_value2: 
		:return: 
		"""
	...
	def paint_2n_xline_for_range(self, sheet_name, xyxy, input_xcolor="gra80"):
		"""
		짝수열을 색칠하는 것
		...
		:param sheet_name: 시트이름, ""은 현재 활성화된 시트이름을 뜻함
		:param xyxy: [1,1,2,2], 가로세로셀영역 (""은 현재 선택영역을 뜻함)
		:param input_xcolor: xcolor형식의 색이름
		:return:
		"""
		sheet_obj = self.check_sheet_name(sheet_name)
		x1, y1, x2, y2 = self.check_address_value(xyxy)
	...
	def paint_cell(self, sheet_name, xyxy, input_xcolor):
		"""
	...
	def paint_cell_as_gradation_by_color_n_position(self, input_style, input_object, input_bg_color, input_list_2d):
		...
		input_object.Fill.ForeColor.RGB = self.colorx.change_xcolor_to_rgbint(input_bg_color)
		...
		for index, list_1d in enumerate(input_list_2d):
			rgbint = self.colorx.change_xcolor_to_rgbint(list_1d[0])
	...
	def paint_cell_by_xcolor(self, sheet_name, xyxy, input_xcolor):
		"""
	...
	def paint_cell_for_sheet_tab(self, sheet_name, input_xcolor):
		"""
		시트탭의 색을 넣는것
	...
	def paint_cell_for_specific_text_in_range(self, sheet_name, xyxy, input_list, input_xcolor):
		"""
	...
	def paint_cell_in_range_by_same_with_input_text(self, sheet_name, xyxy, input_value, input_xcolor="gra50"):
		"""
	...
	def paint_cell_in_range_by_specific_text(self, sheet_name, xyxy, input_value, input_xcolor):
		"""
		영역안에 입력받은 글자와 같은것이 있으면 색칠하는것
	...
	def paint_cell_in_range_for_max_value_in_each_xline(self, sheet_name="", xyxy="", input_xcolor="yel++"):
		"""
		한줄에서 가장 큰 값에 색칠하는 것
	...
	def paint_cell_in_range_for_min_value_in_each_xline(self, sheet_name="", xyxy="", input_xcolor="red++"):
		"""
	...
	def paint_cell_in_range_for_same_value_by_xcolor(self, sheet_name, xyxy, input_xcolor="gray"):
		"""
	...
	def paint_cell_in_range_for_space_cell_by_xcolor(self, sheet_name, xyxy, input_xcolor="gra50"):
		"""
	...
	def paint_cell_with_sheet_object(self, sheet_obj, xy, input_xcolor):
		"""
		셀의 배경색을 칠하는 것
	...
	def paint_data_bar_in_range(self, sheet_name, xyxy, input_xcolor="red"):
		"""
	...
	def paint_range(self, sheet_name, xyxy, input_xcolor="gra"):
		"""
		선택 영역에 색깔을 넣는다
	...
	def paint_range_by_xcolor(self, sheet_name, xyxy, input_xcolor):
		"""
		선택 영역에 색을 칠한다
	...
	def paint_selection_by_xcolor(self, input_xcolor):
		"""
	...
	def paint_sheet_tab_by_xcolor(self, sheet_name, input_xcolor="gra50"):
		"""
		시트탭의 색을 넣는것
	...
	def read_datas_for_cell(self, sheet_name, yx=[7, 7]):
		...
		:return: 
		"""
		basic_cell = xy_common.basic_cell_class()
		sheet_obj = self.check_sheet_name(sheet_name)
	...
	def resize_list_2d_by_xyxy(self, input_list_2d, xyxy):
		...
		"""
		x1, y1, x2, y2 = self.check_address_value(xyxy)
		result = self.utilx.resize_list_2d(input_list_2d, 0, 0, (x2 - x1), (y2 - y1))
		return result
	...
	def reverse_list_2d_top_n_bottom(self, sheet_name="", xyxy=""):
		...
		# list_2d = self.utilx.change_tuple_to_list_2d(t2d)
	...
	def search_xsql_for_selection_with_new_sheet(self, xsql):
		...
		result_list_2d_1 = self.rex.match_for_list_2d_by_xsql(xsql, list_2d)

		result_list_2d = self.change_list_2d_over_to_list_2d(result_list_2d_1)
		self.new_sheet()
		self.write_list_2d_from_cell("", [1, 1], result_list_2d)

	def search_value_by_xsql_in_range_with_paint(self, sheet_name, xyxy, xsql, input_xcolor="yel"):
		"""
		엑셀의 영역에서 값을 찾으면, 셀에 색칠하기
		...
		for x in range(x1, x2 + 1):
			for y in range(y1, y2 + 1):
				...
				if found_or_not:
					self.paint_cell_with_sheet_object(sheet_obj, [x, y], input_xcolor)

	def search_value_in_range_with_paint(self, sheet_name, xyxy, input_list, input_xcolor="yel"):
		"""
		엑셀의 영역에서 값을 찾으면, 셀에 색칠하기
		"""
		sheet_obj = self.check_sheet_name(sheet_name)
	...
	def set_font_color_in_cell(self, sheet_name, xyxy, input_xcolor):
		"""
		셀의 폰트 색을 돌려주는것
	...
	def set_font_color_in_range(self, sheet_name, xyxy, input_xcolor):
		"""
		draw_range_fontcolor(sheet_name, xyxy, input_xcolor)
		영역에 글씨체를 설정
		"""
		sheet_obj = self.check_sheet_name(sheet_name)
		x1, y1, x2, y2 = self.check_address_value(xyxy)
	...
	def set_font_color_in_range_by_xcolor(self, sheet_name, xyxy, input_xcolor=""):
		"""
	...
	def set_pen_color_style_thickness_for_object(self, target_drawing="", xcolor="bla", style=4, thickness=5):
		"""
	...
	def setup_gradation_for_color_n_position(self, in_style, in_object, bg_xcolor, in_list_2d):
		"""
		...
		in_object.Fill.ForeColor.RGB = self.colorx.change_xcolor_to_rgbint(bg_xcolor)
	...
	def write_list_1d_from_cell_as_step(self, sheet_name, xyxy, input_list_1d, input_step):
		...
		:return:
		"""
		self.utilx.change_list_1d_to_list_2d_group_by_step(input_list_1d, input_step)
	...
	def write_list_in_range(self, sheet_name, xyxy, input_list):
		...
		list_2d = self.utilx.change_any_data_to_list_2d(input_list)
	...
	def write_searched_value_at_special_position(self, xyxy, value_line_no, changed_value_line_no, result_line_no, input_xsql):
		"""
	...
	def write_searched_value_at_special_position_ver2(self, xyxy, value_line, changed_value_line, result_start_no, input_xsql):
		"""
	...
	def write_value_in_range(self, sheet_name, xyxy, input_any_type):
		...
		checked_list_2d = self.utilx.change_any_data_to_list_2d(input_any_type)  # 1

		self.set_screen_update_off()  # 2
		for index, list_1d in enumerate(checked_list_2d):
			self.varx["r1c1"] = self.change_xyxy_to_r1c1([x1 + index, y1, x1 + index, y1 + len(list_1d) - 1])
			sheet_obj.Range(self.varx["r1c1"]).Value = list_1d
		self.set_screen_update_on()
	...
	def write_value_in_range_by_range_priority(self, sheet_name, xyxy, input_value):
		...
		sheet_obj = self.check_sheet_name(sheet_name)
		x1, y1, x2, y2 = self.check_address_value(xyxy)
		list_2d = self.utilx.change_any_data_to_list_2d(input_value)
		...
		for index, list_1d in enumerate(list_2d):
			if index >= x2 - x1 + 1:
				...
				if len(list_1d) > y2 - y1 + 1:
					...
					sheet_obj.Range(self.varx["r1c1"]).Value = list_1d[:y2 - y1 + 1]
				else:
					self.varx["r1c1"] = self.change_xyxy_to_r1c1([x1 + index, y1, x1 + index, y1 + len(list_1d) - 1])
					sheet_obj.Range(self.varx["r1c1"]).Value = list_1d
	...
	def write_value_in_range_by_value_priority(self, sheet_name, xyxy, input_any_type):
		...
		sheet_obj = self.check_sheet_name(sheet_name)
		x1, y1, x2, y2 = self.check_address_value(xyxy)
		list_2d = self.utilx.change_any_data_to_list_2d(input_any_type)
	...
	def write_value_in_range_for_speed(self, sheet_name, xyxy, input_list_2d):
		...
		if x1 == x2 and y1 == y2:
			...
			self.varx["r1c1"] = self.change_xyxy_to_r1c1([x1, y1, x1 + len(input_list_2d) - 1, y1 + len(input_list_2d[0]) - 1])
	...
	def make_many_excel_file_group_by_yline_value_for_xyxy(self, sheet_name, xyxy, base_yline_no, file_name="D:\\__test\\save_file_"):
		...
		l2d_group = self.utilx.group_list_2d_by_index(l2d, base_yline_no - 1)
		no = 0
	...
	def get_cell_screen_coordinates(self, excel_hwnd, sheet_obj, row, col):
		# 셀의 좌표와 크기를 가져옵니다
	...
	def get_visible_xyxy(self, excel_obj):
		"""
	...
	def get_sheet_pxyxy(self, sheet_hwnd):
	...
	def client_to_screen(self, hwnd, client_rect):
	...
	def cell_size(self, x, y, excel_cell_start_pxy,sheet_obj, input_factor, monitor_dpi):
		cell_target = sheet_obj.Cells(x, y)
	...
	def move_line_2(self, line_obj, width, height):
	...
	def draw_rectangle_by_ltwh(self, sheet_name, top, left, width, height):
		"""
		sheet: 엑셀 시트 객체 (win32com.client.Dispatch('Excel.Application').ActiveSheet)
		top: 사각형의 상단 위치 (픽셀)
		left: 사각형의 왼쪽 위치 (픽셀)
		width: 사각형의 너비 (픽셀)
		height: 사각형의 높이 (픽셀)
		fill_transparent: 채우기 투명 여부 (True: 투명, False: 채우기 색 적용)
		"""
		sheet_obj = self.check_sheet_name(sheet_name)
	...
	def add_textbox_at_pxyxy(self, sheet_name, x, y, width, height, text):
		sheet_obj = self.check_sheet_name(sheet_name)
	...
	def draw_rectangle_by_pxyxy(self, sheet_name, pxyxy):
		# 사각형을 그려주는 기능
		sheet_obj = self.check_sheet_name(sheet_name)
		px, py, pw, ph = self.change_pxyxy_to_pxywh(pxyxy)
	...
	def calc_cxy_by_angle_n_length(self, px, py, angle_degree, length):
	...
	def move_line_obj(self, line_obj, angle, option=""):
		if not option:
			line_obj.IncrementLeft(70)
			line_obj.IncrementTop(-50)
			line_obj.IncrementRotation(angle)

	def setup_new_xyxy(self, line_obj, x1, y1, x2, y2):
		print("Error: Shape is not a line. Shape type: (line_obj.Type)")
		line_obj.Vertices.ltem(1).x = x1
		line_obj.Vertices.ltem(1).y = y1
		line_obj.Vertices.ltem(2).x = x2
		line_obj.Vertices.Item(2).y = y2

	def move_line_1(self, line_obj):
		line_obj.Select()
