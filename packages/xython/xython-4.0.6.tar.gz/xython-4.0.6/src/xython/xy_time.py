...
class xy_time():
	...
	def change_any_text_time_to_dt_obj(self, input_time=""):
		...
		resql_result = self.rex.search_all_by_xsql("[+-:1~1][숫자:2~2]:[숫자:2~2]", dt_string)
		if resql_result:
		...
		resql_result = self.rex.search_all_by_xsql(resql, dt_string)
		...
		dt_string = dt_string.strip()
		old_dt_string = dt_string
		resql_result = self.rex.search_all_by_xsql("[숫자:2~2]:[숫자:2~2]:[숫자:2~2]", dt_string)
		...
		dt_string = dt_string.strip()
		old_dt_string = dt_string
		if self.rex.search_all_by_xsql("[숫자:4~4]-[숫자:1~2]-[숫자:1~2]", dt_string):
			resql_result = self.rex.search_all_by_xsql("[숫자:4~4]-[숫자:1~2]-[숫자:1~2]", dt_string)
		...
		dt_string = dt_string.strip()
		old_dt_string = dt_string
		resql_result = self.rex.search_all_by_xsql("[숫자:1~2]-[숫자:1~2]-[숫자:1~4]", dt_string)
		if resql_result:
			temp = resql_result[0][0].split("-")
			result["yea"] = temp[2]
			result["mon"] = temp[1]
		...
		if old_dt_string != "":
			...
			dt_string = dt_string.strip()
			old_dt_string = dt_string
			resql_result = self.rex.search_all_by_xsql("(20|19)[숫자:6~6]", dt_string)
		if resql_result:
		...
		dt_string = dt_string.strip()
		old_dt_string = dt_string
		resql_result = self.rex.search_all_by_xsql("[숫자:6~6][공백:1~1][숫자:6~6]", dt_string)
		if resql_result:
		...
		dt_string = dt_string.strip()
		old_dt_string = dt_string
		resql_result = self.rex.search_all_by_xsql("([영어:3~10])[공백&,.:0~3]([숫자:1~2])[공백&,.:1~3]([숫자:1~4])", dt_string)
		...
		dt_string = dt_string.strip()
		old_dt_string = dt_string
		resql_result = self.rex.search_all_by_xsql("[숫자:1~2][영어:0~3][공백&,:1~3][영어:3~9][공백&,:0~3][숫자:4~4]", dt_string)
		if resql_result:
			found_text = resql_result[0][0]

			bbb = self.rex.search_all_by_xsql("[영어:3~9]", found_text)
			for num in self.varx["no_vs_month"].keys():
				if bbb[0][0] in self.varx["no_vs_month"][num]:
					result["mon"] = num
			found_text = found_text.replace(bbb[0][0], "")

			ccc = self.rex.search_all_by_xsql("[숫자:4~4]", found_text)
			result["yea"] = ccc[0][0]
			found_text = found_text.replace(ccc[0][0], "")

			ddd = self.rex.search_all_by_xsql("[숫자:1~2]", found_text)
			result["day"] = ddd[0][0]

			dt_string = dt_string.replace(resql_result[0][0], "")
			dt_string = dt_string.strip()

		resql_result = self.rex.search_all_by_xsql("\.[숫자:6~6]", dt_string)
		if resql_result:
			dt_string = dt_string.replace(resql_result[0][0], "")
			# .586525
			# 초단위 이하의 자료
		...
		resql_result = self.rex.search_all_by_xsql("[숫자:4~4]", dt_string)
		if resql_result:
			# print(resql_result)
		...
		resql_result = self.rex.search_all_by_xsql("[숫자:2~2]", dt_string)
		if resql_result:
			result["day"] = resql_result[0][0]
			dt_string = dt_string.replace(resql_result[0][0], "")
			dt_string = dt_string.strip()

		resql_result = self.rex.search_all_by_xsql("pm[또는]am", dt_string)
		if resql_result:
			# print(resql_result)
	...
	def change_anytime_to_text_time_with_format(self, input_time, input_time_format='%Y-%m-%d'):
		"""
		입력시간을 입력된 형태로 바꾸는 것
	...
	def change_dt_obj_as_input_hms_list(self, input_dt, input_hms):
		"""
	...
	def change_dt_obj_to_dic_time(self, input_dt=""):
		...
		result = self.get_all_infomation_for_dt_obj_as_dic(input_dt)
		return result
	...
	def change_dt_obj_to_dt_timestamp(self, input_dt=""):
		...
		result = self.get_all_infomation_for_dt_obj_as_dic(input_dt)["timestamp"]
		return result
	...
	def get_sec_from_dt_obj(self, input_dt=""):
		"""
		시간 객체를 초로 나타내는 자료형으로 만드는 것
		"""
		input_dt = self.check_dt_obj(input_dt)
		result = self.get_all_infomation_for_dt_obj_as_dic(input_dt)["sec"]
		return result

	def get_sec_set_from_dt_obj(self, input_dt=""):
		"""
		시간 객체를 초로 나타내는 자료형으로 만드는 것
	...
	def change_dt_obj_to_text_time_with_format(self, input_dt="", input_format="%Y-%m-%d %H:%M:%S"):
		"""
	...
	def get_total_day_from_dt_obj(self, input_dt=""):
		"""
		total_day : 시간 객체를 날수로 계산
		"""
		input_dt = self.check_dt_obj(input_dt)
		result = self.get_all_infomation_for_dt_obj_as_dic(input_dt)["day"]
		return result
	...
	def get_weekno_from_dt_obj(self, input_dt=""):
		"""
	...
	def change_dt_obj_to_ymd_list(self, input_dt=""):
		...
		"""
		input_dt = self.check_dt_obj(input_dt)
		result = self.get_all_infomation_for_dt_obj_as_dic(input_dt)["ymd_list"]
		return result
	...
	def change_dt_obj_to_ymdhms_list(self, input_dt=""):
		...
		"""
		input_dt = self.check_dt_obj(input_dt)
		result = self.get_all_infomation_for_dt_obj_as_dic(input_dt)["ymdhms_list"]
		return result
	...
	def change_dt_obj_to_yyyy_mm_dd(self, input_dt=""):
		...
		"""
		input_dt = self.check_dt_obj(input_dt)
		temp = self.get_all_infomation_for_dt_obj_as_dic(input_dt)["ymd_list"]
		result = str(temp[0]) + "-" + str(temp[1]) + "-" + str(temp[2])
		return result

	def change_excel_time_to_dt_obj(self, input_excel_float):
		"""
	...
	def change_formatted_text_time_to_dt_obj_with_format(self, input_time, input_format):
		"""
		datetime는 utc local
	...
	def change_lunar_day_to_solar_day_1(self, input_value):
		"""
		음력을 양력으로 만들어 주는것
		...
		for one in input_value:
			if one[3] == "양":
				...
				if one[2] == "말일":
					result.append(one)
					input_value[1] = self.check_lunar_last_day_for_lunar_ym_list([one[0], one[1:]])
					self.lunar_calendar.setLunarDate(one[0], input_value[0], input_value[1], False)
					moon_day_1 = self.lunar_calendar.SolarIsoFormat()
	...
	def change_utc_to_timedic(self, input_utc=""):
		...
		result = self.get_all_infomation_for_dt_obj_as_dic(dt_obj)
		return result
	...
	def change_utc_to_ymd_dash(self, input_utc):
		...
		"""
		dt_obj = self.change_utc_to_dt_obj(input_utc)
		all_dic = self.get_all_infomation_for_dt_obj_as_dic(dt_obj)
	...
	def change_utc_to_time_dic(self, input_utc=""):
		"""
		입력된 시간에 대한 왠만한 모든 형식의 날짜 표현을 사전형식으로 돌려준다
	...
	def change_ymd_list_to_sec(self, input_ymd_list):
		"""
		[년, 월, 일] => 초
		...
		:return:
		"""
		total_sec = int(input_ymd_list[0]) * self.varx["year_sec"] + int(input_ymd_list[1]) * self.varx["month_sec"] + int(
			input_ymd_list[2]) * self.varx["day_sec"]
		return total_sec

	def change_ymd_list_to_utc(self, input_ymd_list):
		"""
		입력 : [년, 월, 일]
		출력 : utc값
		...
		:return:
		"""
		atime = arrow.get(int(input_ymd_list[0]), int(input_ymd_list[1]), int(input_ymd_list[2]))
	...
	def change_ymd_list_to_yearweekno(self, input_ymd_list=""):
		...
		if input_ymd_list == "":
			...
			dt_obj = self.change_anytime_to_dt_obj(input_ymd_list)
	...
	def check_text_time_with_xsql(self, input_time, xsql):
		...
		:return:
		"""
		result = self.rex.search_all_by_xsql(input_time, xsql)
		return result
	...
	def get_7_day_list_for_dt_obj(self, input_dt=""):
		...
		:return:
		"""
		dic_data = self.get_all_infomation_for_dt_obj_as_dic(input_dt)
	...
	def get_all_infomation_for_dt_obj_as_dic(self, input_dt=""):
		"""
		입력된 시간에 대한 왠만한 모든 형식의 날짜 표현을 사전형식으로 돌려준다
	...
	def get_all_information_for_input_time_as_dic(self, input_time=""):
		dt_obj = self.change_anytime_to_dt_obj(input_time)
		result = self.get_all_infomation_for_dt_obj_as_dic(dt_obj)
		return result
	...
	def get_day_set_for_now(self):
		...
		:return:
		"""
		all_dic = self.get_all_infomation_for_dt_obj_as_dic("")
		return [all_dic["day2"], all_dic["day3"]]
	...
	def get_dic_time_for_dt_obj(self, input_dt=""):
		...
		:return:
		"""
		all_dic = self.get_all_infomation_for_dt_obj_as_dic(input_dt)
		return all_dic
	...
	def get_holiday_list_between_day1_and_day2(self, input_anytime1, input_anytime2):
		...
		dic_time_1 = self.get_all_infomation_for_dt_obj_as_dic(input_anytime1)
		dic_time_2 = self.get_all_infomation_for_dt_obj_as_dic(input_anytime2)
	...
	def get_nea_random_date_between_two_date(self, dt_obj_1, dt_obj_2, nea):
		"""
	...
	def get_yyyy_mm_dd_style_for_today(self):
		"""
		...
		dt_obj = self.change_anytime_to_dt_obj("")
		all_dic = self.get_all_infomation_for_dt_obj_as_dic(dt_obj)
		return all_dic["yyyy-mm-dd"]

	def get_yyyy_mm_dd_style_for_now(self):
		"""
		2025-03-01의 형식으로 오늘 날짜를 주는 것
	...
	def get_weekno_from_utc(self, input_utc=""):
		"""
		시간이 들어온면
		입력값 : 년도, 위크번호
		한 주의 시작은 '월'요일 부터이다
	...
	def get_ymd_dash_from_utc(self, input_value=""):
		"""
		utc시간을 YYYY-MM-DD 형태의 시간으로 바꾸는 것
		:param input_value:
		:return:
		"""
		lt = self.change_anytime_to_utc(input_value)
		atime = arrow.get(lt)
	...
	def get_ymd_list_for_today(self):
		...
		all_dic = self.get_all_information_for_input_time_as_dic(dt_obj)
		return all_dic["ymdlist"]
	...
	def get_yyyymmdd_for_timestamp(self):
		...
		dt_obj = datetime.datetime.now()
		temp = self.get_all_infomation_for_dt_obj_as_dic(dt_obj)["ymd_list"]
	...
	def minus_two_date(self, input_date1, input_date2):
		...
		:return:
		"""
		start_timestampl, start_dayl, start_secl = self.get_sec_set_from_dt_obj(input_date1)
		start_timestamp2, start_day2, start_sec2 = self.get_sec_set_from_dt_obj(input_date2)
	...
	def overlap_area_for_two_dt_range(self, input_dt2_11, input_dt2_12):
		"""
		겹치는 시간 부분을 리스트로 돌려줌
		...
		start_timestamp, start_day, start_sec = self.get_sec_set_from_dt_obj(start_dt_obj)
		end_timestamp, end_day, end_sec = self.get_sec_set_from_dt_obj(end_dt_obj)
	...
	def replace_holiday_for_sunday(self, input_value):
		"""
		대체공휴일의 날짜를 확인하는 것이다
	...
	def split_range_time_per_day(self, input_dt1, input_dt2):
		...
		sec_datas_1 = self.get_sec_set_from_dt_obj(input_dt1)
		sec_datas_2 = self.get_sec_set_from_dt_obj(input_dt2)
