...
def foundation_model(image,dict_coords,checkpoint,model_cfg):
    ...
    for ix in L:
        ...
        n,height,width = output_pred.shape
        resized_masks = []
        for i in range(n):
...
# checkpoint = os.path.join(get_base_dir(),"segtree","sam-hq","sam-hq2","checkpoints","sam2.1_hq_hiera_large.pt")
# model_cfg = "configs/sam2.1/sam2.1_hq_hiera_l.yaml"
checkpoint = os.path.join(get_base_dir(),"segtree","sam2.1_hq_hiera_large.pt")
model_cfg = os.path.join(get_base_dir(),"segtree","sam2.1_hq_hiera_l.yaml")
def codes(rgb_data,data_msk):
    if data_msk is None or len(np.unique(data_msk))==1:
