...
def convert_invoice_type(data_field_value, params):
    ...
    best_match = process.extractOne(
        ...
        keywords,
        scorer=fuzz.WRatio,
        score_cutoff=params["fuzzy_threshold_invoice_classification"],
    )
    if best_match:
...
async def format_label(
    ...
    if isinstance(entity_value, list):
        format_tasks = await asyncio.gather(
            *[
                format_label(
                    entity_k, sub_v, embed_manager, document_type_code, params
                )
        ...
        for line_item in aggregated_data.get("lineItem", []):
            if line_item.get("lineItemDescription", None) is not None:
                line_item["itemCode"] = associate_forto_item_code(
                    embed_manager, line_item["lineItemDescription"]["formattedValue"], params
                )
...
def find_best_fuzzy_match(
    ...
    """
    Performs fuzzy matching for the query against the keys of the mapping dict.

    Args:
        query: The input string to search for.
        mapping: The dictionary where keys are potential matches and values are results.
        threshold: The minimum similarity score (0-100) required.
        scorer: The RapidFuzz scoring function to use (e.g., fuzz.Ratio, fuzz.WRatio).

    Returns:
        The corresponding *value* from the mapping if a key is matched above
        the threshold, otherwise None. Returns None if mapping is empty.
    """
...
def associate_forto_item_code(embed_manager, input_string, params):
    ...
    """
    # 1. Try fuzzy matching using the helper function
    forto_item_code = find_best_fuzzy_match(
        query=input_string,
        mapping=params["lookup_data"]["item_code"],
    ...
    if forto_item_code is None:
        ...
        embeddings_dict = embed_manager.embeddings_dict
        forto_item_code = embed_manager._find_most_similar_option(input_string, *embeddings_dict["item_codes_label"])

    result = {
        "documentValue": input_string,
        "formattedValue": forto_item_code
    }
    return result