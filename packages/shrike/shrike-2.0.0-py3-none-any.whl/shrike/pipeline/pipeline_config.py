...
from shrike.pipeline.module_helper_base import module_loader_config, module_manifest
...
class pipeline_cli_config:  # pylint: disable=invalid-name
    ...
    - `create_schedule` (bool): set to True for creating a schedule for the pipeline (default to False)
    ...
    - `publish_component` (bool): set to True to publish the pipeline as a component, the component name is the set pipeline name (default to False)
    - `publish_component_version` (Optional[str]): version of the component to publish, if not set, the version will be automatically generated from AML SDK
    - `publish_component_registry` (Optional[str]): name of the registry to publish the component to, if not set, the component will be published to the workspace
    ...
    - `schedule_name` (Optional[str]): name of the schedule to create for the pipeline, must be set if `create_schedule` is set to True
    - `schedule_description` (Optional[str]): description of the schedule to create for the pipeline
    ...
    publish: bool = False
    publish_component: bool = False
    publish_component_version: Optional[str] = None
    publish_component_registry: Optional[str] = None
...
@dataclass
class recurrence_pattern_config:
    """
    Recurrence pattern configuration, used when `recurrence_frequency` and `recurrence_interval` are set in the pipeline CLI config

    Parameters:
    - `hours` (int | List[int]): hours for the schedule
    - `minutes` (int | List[int]): minutes for the schedule
    - `week_days` (Optional[str | List[str]]): week days for the schedule ("monday", "tuesday", etc...)
    - `month_days` (Optional[int | List[int]]): month days for the schedule
    """

    hours: List[int] = MISSING
    minutes: List[int] = MISSING
    week_days: Optional[List[str]] = None
    month_days: Optional[List[int]] = None


@dataclass
class schedule_trigger_config:
    """
    Schedule trigger configuration, used when `schedule_name` is set in the pipeline CLI config
    Can be either recurrence or cron

    Parameters:
    - `cron_expression` (Optional[str]): cron expression for the schedule, either this or `frequency` and `interval` must be set
    - `recurrence_frequency` (Optional[str]): frequency for the schedule, either this or `cron_expression` must be set
    - `recurrence_interval` (Optional[int]): interval for the schedule, either this or `cron_expression` must be set
    - `recurrence_pattern` (Optional[recurrence_pattern_config]): recurrence pattern for the schedule, used when `recurrence_frequency` and `recurrence_interval` are set
    - `end_time` (Optional[str]): end time for the schedule
    - `time_zone` (Optional[str]): time zone for the schedule
    """

    cron_expression: Optional[str] = None
    recurrence_frequency: Optional[str] = None
    recurrence_interval: Optional[int] = None
    recurrence_pattern: Optional[recurrence_pattern_config] = None
...
def default_config_dict():
    ...
    return {
        ...
        "schedule_trigger": schedule_trigger_config,
    }
