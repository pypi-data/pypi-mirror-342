...
class Mimir:
    ...
    def _setupValidation(self, widget, validation):
        ...
        if not hasattr(widget, "_mimir_validation_meta"):
            widget._validationMeta  = []
        ...
        for rule in validation:
            ...
            def makeValidator(func, severity, on_invalid, on_valid, styling):
                def validator(event=None):
                    try:
                        ...
                        if func(value):
                            ...
                            widget._validationMeta.append({"func": func, "result": True})
                        else:
                            if styling == "regularMimir":
                            ...
                            if severity == "error":
                                widget._validationMeta.append({"func": func, "result": False})

                    except Exception as e:
                        print("Validator raised exception:", e)
                return validator

            validator = makeValidator(func, severity, on_invalid, on_valid, styling)
            ...
            if validate:
                for child in parent.winfo_children():
                    for grandchild in child.winfo_children():
                        grandchild.event_generate("<FocusOut>")
