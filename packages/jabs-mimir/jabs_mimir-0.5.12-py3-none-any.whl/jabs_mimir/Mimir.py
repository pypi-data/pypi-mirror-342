...
class Mimir:
    ...
    def _setupValidation(self, widget, validation):
        """
        Attach one or more validation rules to a widget.

        This method supports both legacy single-validator format and new multiple-validator structures.
        Each validation is stored in the widget's `_validationMeta["validations"]` list for later batch evaluation.

        Supported validation formats:
        - Callable: Direct validation function
        - Dict: {
            "func": function or string,
            "severity": "error" | "warn" | ...,
            "onInvalid": callback,
            "onValid": callback,
            "styling": "regularMimir" | None
        }

        Parameters:
            widget (tk.Widget): The widget to validate.
            validation (Callable | Dict | List): The validation rule(s).
        """
        if not validation:
            return
        ...
        for rule in validations:
            ...
            def validate(event=None, func=func, meta=meta):
                value = widget.get()
                result = func(value)
                meta["failed"] = not result

                if result:
                    if meta["styling"] == "regularMimir":
                        self.clearInvalid(widget)
                    if callable(meta["on_valid"]):
                        meta["on_valid"](widget, value)
                else:
                    if meta["styling"] == "regularMimir":
                        self.markInvalid(widget)
                    if callable(meta["on_invalid"]):
                        meta["on_invalid"](widget, value)

            widget.bind("<FocusOut>", validate)
            self._manualValidations.append((widget, validate))
            widget._validationMeta["validations"].append(meta)
