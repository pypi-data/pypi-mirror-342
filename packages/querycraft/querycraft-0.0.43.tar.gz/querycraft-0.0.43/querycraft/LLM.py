    ...
    def run(self, erreur, sql_attendu, sql_soumis):
        try:
            ...
            return (f"{GREEN}"+response.message.content + f"{RESET}\n---\n"
                    ...
                    + f"{BLUE}Attention, Ollama/{self.modele} ne garantit pas la correction de la réponse. Veuillez vérifier la réponse et vous rapprocher de vos enseignants si nécessaire.{RESET}")
    ...
    def run(self, erreur, sql_attendu, sql_soumis):
        try:
            ...
            return (f"{GREEN}"+chatbot.chat(self.prompt).wait_until_done() + f"{RESET}\n---\n"
                    ...
                    + f"{BLUE}Attention, HuggingChat/Mia-DB ne garantit pas la correction de la réponse. Veuillez vérifier la réponse et vous rapprocher de vos enseignants si nécessaire.{RESET}")
        except Exception as e:
