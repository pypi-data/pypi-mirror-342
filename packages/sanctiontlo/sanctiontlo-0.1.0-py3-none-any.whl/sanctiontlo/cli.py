...
from .api import TLOApi
import os

app = typer.Typer(help="SanctionTLO CLI - A command-line interface for TLO searches")
console = Console()

def get_admin_key() -> str:
    key_file = os.path.expanduser("~/.sanctiontlo")
...
def search(
    ...
    with console.status("[bold green]Searching..."):
        try:
            ...
            if not key:
                console.print("[red]Error: Admin key not provided. Either use --admin-key or run 'sanctiontlo config' first.")
                raise typer.Exit(1)
