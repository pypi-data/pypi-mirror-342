...
from yta_core.enums.field import SegmentBuildingField, SegmentField, MusicField, MusicBuildingField
...
from yta_core.builder.voice_narration import VoiceNarrationConcept
...
class Segment:
    ...
    def voice_narration(
        self
    ) -> Union[VoiceNarrationJson, None]:
        return (
            self.data.voice_narration
            if self.data else
            None
        )
    ...
    def audio_narration_filename(
        ...
        """
        if self.voice_narration is not None:
            self.data.voice_narration.filename = value

    @property
    def music(
        self
    ) -> Union[MusicJson, None]:
        return (
            self.data.music
            if self.data else
            None
    ...
    def music_filename(
        ...
        return (
            self.music.filename
            if (
                self.music is not None and
                self.music.filename is not None and
                self.music.filename != ''
            ) else
            None
        )
    ...
    def music_url(
        ...
        return (
            ...
            if (
                self.music is not None and
                self.music.url is not None and
                self.music.url != ''
            ) else
            None
        )
    ...
    def music_engine(
        self
    ) -> Union[MusicEngine, None]:
        return (
            MusicEngine.to_enum(self.music.engine)
            if (
                self.music is not None and
                self.music.engine is not None
            ) else
            None
        )
    ...
    def music_keywords(
        ...
        return (
            ...
            if (
                self.music is not None and
                self.music.keywords is not None and
                self.music.keywords != ''
            ) else
            None
        )
    ...
    def music_filename_processed(
        ...
        return (
            self.music.filename_processed
            if (
                self.music is not None and
                self.music.filename_processed is not None and
                self.music.filename_processed != ''
            ) else
            None
        )
    ...
    def music_filename_processed(
        ...
        # TODO: What if no 'self.music' (?)
        self.music.filename_processed = value
    ...
    def __init__(
        ...
        self.audio_narration_filename = (
            data[SegmentField.MUSIC.value][MusicField.FILENAME.value]
            if data.get(SegmentField.MUSIC.value, None) is not None else
            None
        )
    ...
    def build(
        ...
        if DoShouldBuildNarration.is_satisfied(configuration, self.data.as_dict):
            ...
            if not self.audio_narration_filename:
    ...
    def _create_narration(
        ...
        This method is called only when the voice
        narration is needed.
        """
        # 1. Check if voice_narration 'filename' exist
        # 2. Check if voice_narration 'filename_processed' exist
        # 3. Create narration and store in 'filename_processed'
        if self.voice_narration.filename:
            if FileValidator.file_is_audio_file(self.voice_narration.filename):
                # TODO: Maybe this has to be persisted or something
                segment_part_filename = self._create_segment_file(f'narration.{get_file_extension(self.voice_narration.filename)}')
                FileHandler.copy_file(self.voice_narration.filename, segment_part_filename)
                ...
                # This has been verified previously but the file
                # can be not available after that verification
                # TODO: What do we do in this case (?)
                raise Exception('No valid voice narration "filename"')
        else:
            # The voice narration must be created
            segment_part_filename = self._create_segment_file('narration.wav')
            self._set_audio_filename(
                VoiceNarrationConcept.get(
                    voice_narration = self.data.voice_narration,
