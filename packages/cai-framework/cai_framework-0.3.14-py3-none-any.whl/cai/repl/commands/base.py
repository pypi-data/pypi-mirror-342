...
def handle_command(
    ...
    if cmd:
        # Check the number of parameters the handle method accepts
        if hasattr(cmd, 'handle'):
            # Pass only the arguments that the method can accept
            if messages is not None:
                try:
                    return cmd.handle(args, messages)
                except TypeError:
                    # If it fails with both args, try with just args
                    try:
