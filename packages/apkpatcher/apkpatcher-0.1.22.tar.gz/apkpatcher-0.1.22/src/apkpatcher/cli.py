...
def main() -> int:
    ...
    parser.add_argument('--add-permissions',
                        nargs='+',
                        help='Add permissions in android app')
    ...
    if len(sys.argv) == 1 or (
            not (args.apk and args.only_unpack) \
            and not (args.apk and args.sdktools)):
    ...
    if args.only_unpack:
        patcher.extract_apk(patcher.apk, Path(args.only_unpack))
        return 0
    ...
    if args.only_repack:
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            patcher.final_dir = Path(tmp_dir)
