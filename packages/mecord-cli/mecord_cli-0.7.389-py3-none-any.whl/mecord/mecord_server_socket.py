...
class MecordLongConnectThread(Thread):
    ...
    def receive_GetTask(self, body):
        ...
        if len(rsp.list) > 0:
            ...
            if self.wait_flag:
                ...
                logger.info(f'{self.name} === no task (+{self.last_receive_no_task}) -t {get_task_time}')
    ...
    def GetTask(self):
        if not os.path.exists(stop_thread_file):
            ...
            if limit <= 0:
                return
            if self.last_gettask_time - self.last_receive_time > 15000:
                remake_file = os.path.join(thisFileDir, f"remake-{self.service_country}")
                with open(remake_file, 'w') as f:
                    f.write('')

                logger.info('LongConnection ---  getTask wait time more than 15s')
                taskUtils.notifyServerError('', self.service_country, 'LongConnection ---  getTask more than 15s')
                return
