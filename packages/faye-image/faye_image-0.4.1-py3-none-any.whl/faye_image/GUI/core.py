...
CallbackResult = ImageIntermediate # Could be PIL Image, numpy array, etc.
...
class FyGUI:
    ...
    def __init__(self,
        ...
        self.result_texture_internal_format: Optional[int] = None # 新增：存储纹理格式用于比较
    ...
    def _update_result_texture(self, result_data: Optional[CallbackResult]):
        ...
        if result_data is None:
            # Release texture if there's no new data
            if self.result_texture_id is not None:
                try:
                    release_texture(texture_id=self.result_texture_id)
                except Exception as e:
                    print(f"Warning: Error releasing texture when result is None: {e}")
                finally:
                    self.result_texture_id = None
                    self.result_texture_width = 0
                    self.result_texture_height = 0
                    self.result_texture_internal_format = None
            self.callback_result = None
            return
        
        assert isinstance(result_data, ImageIntermediate), "Result data must be an ImageIntermediate object."
        ...
        # We assume that:
        # - Channels number:
        #   - If the texture is 3 channels, it is RGB
        #   - If the texture is 4 channels, it is RGBA
        #   - If the texture is 1 channel, it is grayscale (L)
        #   - If the texture is 2 channels, it is not supported, but we can convert it to RGB
        # - Texture format
        #   - If the texture is in float, it is 32F
        #   - If the texture is in uint8, it is UNSIGNED_BYTE
        #   - If the texture is not unsigned int but a integer, it is 32I
        # try:
        ...
        try:
            ...
            # --- Get data properties ---
            numpy_data = result_data.GetData()
            if numpy_data.shape[0] == 0: # Handle empty batch
                 # Release texture if data is empty
                if self.result_texture_id is not None:
                    try:
                        release_texture(texture_id=self.result_texture_id)
                    except Exception as e:
                        print(f"Warning: Error releasing texture for empty data: {e}")
                    finally:
                        self.result_texture_id = None
                        self.result_texture_width = 0
                        self.result_texture_height = 0
            ...
            img_data_chw = numpy_data[0] # Shape: [C, H, W]
            channels = result_data.GetNumOfChannels()
            height, width = result_data.GetResolution()
            dtype = img_data_chw.dtype

            # --- Determine OpenGL formats ---
            ...
            if dtype == np.float32:
                ...
                pixel_type = gl.GL_UNSIGNED_BYTE
                if channels == 1:
                    internal_format = gl.GL_R8 # Use R8 for single channel uint8
                    gl_format = gl.GL_RED
                elif channels == 3:
                    ...
                    gl_format = gl.GL_RGBA
            # Add more dtype/channel combinations if needed

            if internal_format is None or gl_format is None or pixel_type is None:
                raise TypeError(f"Unsupported image format: {channels} channels, dtype {dtype}")

            # --- Prepare data for OpenGL (transpose C,H,W -> H,W,C and ensure contiguous) ---
            img_data_hwc = np.ascontiguousarray(img_data_chw.transpose(1, 2, 0))

            # --- Check if texture needs recreation ---
            needs_recreation = (
                self.result_texture_id is None or
                self.result_texture_width != width or
                self.result_texture_height != height or
                self.result_texture_internal_format != internal_format
            )

            try:
                if needs_recreation:
                    print(f"Recreating result texture: Size={width}x{height}, Format={internal_format}")
                    if self.result_texture_id is not None:
                        # Assuming release_texture handles its own errors or we catch them above
                        release_texture(texture_id=self.result_texture_id)

                    self.result_texture_id = gl.glGenTextures(1)
                    self.result_texture_width = width
                    self.result_texture_height = height
                    self.result_texture_internal_format = internal_format

                    gl.glBindTexture(gl.GL_TEXTURE_2D, self.result_texture_id)
                    gl.glTexParameteri(gl.GL_TEXTURE_2D, gl.GL_TEXTURE_MIN_FILTER, gl.GL_LINEAR)
                    gl.glTexParameteri(gl.GL_TEXTURE_2D, gl.GL_TEXTURE_MAG_FILTER, gl.GL_LINEAR)
                    gl.glTexParameteri(gl.GL_TEXTURE_2D, gl.GL_TEXTURE_WRAP_S, gl.GL_CLAMP_TO_EDGE)
                    gl.glTexParameteri(gl.GL_TEXTURE_2D, gl.GL_TEXTURE_WRAP_T, gl.GL_CLAMP_TO_EDGE)
                    gl.glPixelStorei(gl.GL_UNPACK_ALIGNMENT, 1)
                    gl.glTexImage2D(gl.GL_TEXTURE_2D, 0, internal_format, width, height, 0,
                                      gl_format, pixel_type, img_data_hwc)
                    gl.glBindTexture(gl.GL_TEXTURE_2D, 0)
                    print(f"New result texture created: ID={self.result_texture_id}")
                ...
                error_code = gl.glGetError() # Check error code *after* exception occurred
                error_string = gluErrorString(error_code) if error_code != gl.GL_NO_ERROR else f"GLError: {e}"
                if isinstance(error_string, bytes): error_string = error_string.decode('utf-8', errors='ignore')
                # Raise a new exception or handle it
                raise RuntimeError(f"OpenGL Error during texture update/creation: {error_string}") from e

        except ImportError:
             # This might occur if NumPy or PyOpenGL is missing
             self.callback_error = "Required library (NumPy or PyOpenGL) not found."
             print(f"Error: {self.callback_error}")
             # Clean up texture state if error occurs
             if self.result_texture_id is not None:
                 try:
                     release_texture(texture_id=self.result_texture_id)
                 except Exception as e:
                     print(f"Warning: Error releasing texture during ImportError handling: {e}")
                 finally:
                     self.result_texture_id = None
                     self.result_texture_width = 0
                     self.result_texture_height = 0
             ...
             if self.result_texture_id is not None:
                 try:
                     ...
                     print(f"Warning: Error releasing texture during general exception handling: {e}")
                 finally:
                     self.result_texture_id = None
                     self.result_texture_width = 0
                     self.result_texture_height = 0
    ...
    def run(self, callback: Callable[[Any, FyContext], CallbackResult]):
        ...
        try:
            ...
            release_all_textures() # Releases widget textures (from cache)
            # Release the result texture specifically
            if self.result_texture_id is not None:
                 try:
                     # Use the utility function which might handle context issues
                     release_texture(texture_id=self.result_texture_id)
                     print(f"Released final result texture: ID={self.result_texture_id}")
                 except Exception as e:
                     print(f"Warning: Error releasing result texture during shutdown: {e}")
                 finally:
                     # Ensure these are reset even if release fails
                     self.result_texture_id = None
                     self.result_texture_width = 0
                     self.result_texture_height = 0
                     self.result_texture_internal_format = None
...
if __name__ == '__main__':
    ...
    from .. import Stash, PIL_IMAGE_RT
