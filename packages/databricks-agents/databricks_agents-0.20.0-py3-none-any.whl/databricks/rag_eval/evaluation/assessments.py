...
class BuiltInAssessmentRunner(AssessmentRunner):
    ...
    ) -> Optional[entities.AssessmentResult]:
    ...
    def run(self, eval_item: entities.EvalItem) -> List[entities.AssessmentResult]:
        ...
        with self.rate_limiter:
            return self.client.get_assessment(eval_item, self.config)
...
def _get_assessments_for_configs(
    ...
    for assessment_conf in assessment_configs:
        if isinstance(assessment_conf, assessment_config.BuiltinAssessmentConfig):
            ...
            results.append(
                CustomAssessmentRunner(
                    config=assessment_conf, rate_limiter=rate_limiter
                )
            )
    return results
...
def _get_assessments_to_run(
    ...
    if (
        ...
        enabled_metrics -= (
            evaluation_config.metrics_requiring_ground_truth_or_expected_facts()
        )

    # If request is not in a first-class format, disable all built-in assessments
    if not input_output_utils.is_valid_input(
        eval_item.raw_request or eval_item.question
    ):
        enabled_metrics -= set(assessment_config.builtin_assessment_names())

    # If response is not in a first-class format, disable all built-in assessments requiring response
    if not input_output_utils.is_valid_output(
        eval_item.raw_response or eval_item.answer
    ):
    ...
    global_guideline_adherence_name = (
        assessment_config.GLOBAL_GUIDELINE_ADHERENCE.user_facing_assessment_name
    )

    guidelines_exist = (
        eval_item.named_guidelines is not None or config.global_guidelines is not None
    )
    ...
    if (
        ...
        and guideline_adherence_name in enabled_metrics
    ):
    ...
    minimum_assessments_to_run = [
        ...
        for assessment in assessments_to_run
        if (
            isinstance(assessment, BuiltInAssessmentRunner)
...
def generate_llm_assessments(
    *,
    ...
    guideline_assessments = [
        ...
        if assessment.config.assessment_name
    ...
    with ThreadPoolExecutor(max_workers=len(assessments)) as executor:
        ...
        for assessment in guideline_assessments:
            ...
            user_facing_assessment_name = (
                assessment.config.user_facing_assessment_name
                or assessment.config.assessment_name
            )
            ...
            if (
                assessment_config.GLOBAL_GUIDELINE_ADHERENCE.user_facing_assessment_name
                in user_facing_assessment_name
            ):
                ...
                guidelines_eval_item["guidelines"] = config.global_guidelines[
                    guidelines_key
                ]
                futures.append(
                    executor.submit(
                ...
                guidelines_eval_item["guidelines"] = eval_item.named_guidelines[
                    guidelines_key
                ]
                futures.append(
                    executor.submit(
