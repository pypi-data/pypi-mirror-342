...
def invoke_model(
    ...
    if model_result.trace is None:
        if model_result.error_message:
            model_result.trace = traces.create_minimal_error_trace(
                request=model_input, response=model_result.error_message
            )
        else:
            model_result.trace = traces.create_minimal_trace(
