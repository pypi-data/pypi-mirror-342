...
_ROOT_CAUSE_ASSESSMENT = "root_cause_assessment"
_ROOT_CAUSE_RATIONALE = "root_cause_rationale"
...
class OverallAssessment:
    """Overall assessment for an eval item."""

    rating: Rating
    """Rating item to represent the overall assessment."""

    root_cause_assessment: Optional[str] = None
    """Root cause assessment for when overall assessment is failing."""

    root_cause_rationale: Optional[str] = None
    """Root cause rationale for when overall assessment is failing."""

    suggested_action: Optional[str] = None
    """Action items  for when overall assessment is failing."""
...
class MetricResult:
    ...
    metric_value: mlflow_eval.Assessment
    legacy_metric: bool = False

    @staticmethod
    def make_legacy_metric(metric_name, metric_value):
        """
    ...
    def to_mlflow_assessment_v3(
        ...
        mlflow_eval.Assessment objects for feedback and internalize all of the conversions here.
        ...
        """
        if (
            self.metric_value.source is None
            or self.metric_value.source.source_type
            == mlflow_entities.AssessmentSourceType.SOURCE_TYPE_UNSPECIFIED
        ):
            source = mlflow_entities.AssessmentSource(
                source_type=mlflow_entities.AssessmentSourceType.SOURCE_TYPE_UNSPECIFIED,
                source_id=self.metric_value.name,
            )
        else:
            source_type = {
                mlflow_entities.AssessmentSourceType.SOURCE_TYPE_UNSPECIFIED: mlflow_entities.AssessmentSourceType.SOURCE_TYPE_UNSPECIFIED,
                mlflow_entities.AssessmentSourceType.LLM_JUDGE: mlflow_entities.AssessmentSourceType.LLM_JUDGE,
...
class EvalResult:
    ...
    def assessments(self) -> List[mlflow_entities.Assessment]:
        ...
        if self.overall_assessment is None:
            ...
            overall_assessment_rating = self.overall_assessment.rating
            overall_assessment_metadata = {
                _ROOT_CAUSE_ASSESSMENT: self.overall_assessment.root_cause_assessment,
                _ROOT_CAUSE_RATIONALE: self.overall_assessment.root_cause_rationale,
                _SUGGESTED_ACTION: self.overall_assessment.suggested_action,
            }
            overall_assessment = [
                mlflow_entities.Assessment(
                    trace_id=trace_id,
                    ...
                    source=mlflow_entities.AssessmentSource(
                        source_type=mlflow_entities.AssessmentSourceType.LLM_JUDGE,
                        source_id=AssessmentSource.builtin().source_id,
                    ),
                    feedback=Feedback(
                        value=overall_assessment_rating.categorical_value
                        or overall_assessment_rating.double_value
                    ),
                    rationale=overall_assessment_rating.rationale,
                    metadata={
                        key: value
                        for key, value in overall_assessment_metadata.items()
                        if value is not None
                    },
                    # These two fields, plus assessment_id, are created service-side.
                    create_time_ms=0,
    ...
    def get_metrics_dict(self) -> Dict[str, Any]:
        ...
        metrics: Dict[str, Any] = {
            metric.metric_value.name: metric.metric_value.value
            for metric in self.metric_results
    ...
    def get_overall_assessment_dict(self) -> Dict[str, schemas.ASSESSMENT_RESULT_TYPE]:
        ...
        overall_assessment_rating = self.overall_assessment.rating
        if overall_assessment_rating.categorical_value is not None:
            result[schemas.OVERALL_ASSESSMENT_RATING_COL] = (
                overall_assessment_rating.categorical_value
            )
        if overall_assessment_rating.rationale is not None:
            result[schemas.OVERALL_ASSESSMENT_RATIONALE_COL] = (
                overall_assessment_rating.rationale
            )
        return result
