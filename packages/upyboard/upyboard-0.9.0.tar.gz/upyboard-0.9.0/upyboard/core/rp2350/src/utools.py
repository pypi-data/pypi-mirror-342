...
def get_cpu_info() -> tuple:
    ...
    cpu_volt = machine.ADC(3)
    adc_value = cpu_volt.read_u16()
    voltage = (adc_value * 3.3 / 65535) * 3  
    
    cpu_temp = machine.ADC(4)
    adc_value = cpu_temp.read_u16()
    temp = 27 - ((adc_value * 3.3 / 65535) - 0.706) / 0.001721
    
    return freq, voltage, temp
...
class ReplSerial:
    ...
    def __read(self) -> bytes :
        ...
        if usys.stdin.buffer in uselect.select([usys.stdin.buffer], [], [], 0)[0]:
            return usys.stdin.buffer.read(1)
        else:
            return b''
    
    def __read_timeout(self, size:int=-1, timeout:int=10) -> bytes:
        """
        Reads data from the REPL UART with a timeout.
    ...
    def read(self, size:int=1) -> bytes: 
        ...
        if self.timeout is None:
            ...
            return self.__read_timeout(size)
