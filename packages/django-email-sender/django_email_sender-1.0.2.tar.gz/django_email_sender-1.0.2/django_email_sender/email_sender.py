...
EMAIL_TEMPLATES_DIR  = join(TEMPLATES_DIR, "emails_templates")
...
class EmailSender:
    ...
    def with_html_template(self, template_name: str, folder_name: str = None) -> "EmailSender":
        ...
        Returns:
            EmailSender: The current instance for chaining.
        """
        self.html_template = self._create_path(folder_name, template_name)
        return self
    ...
    def _does_template_path_exists(self, email_path):
        ...
        if not exists(TEMPLATES_DIR):
           raise TemplateDirNotFound(f"The template directory wasn't found. Got template with path - {TEMPLATES_DIR}")
        if not exists(EMAIL_TEMPLATES_DIR):
           raise EmailTemplateDirNotFound(f"The emails_templates folder wasn't found. Got email path - {EMAIL_TEMPLATES_DIR}")
        if not exists(email_path):
            raise EmailTemplateNotFound(f"The email template wasn't found. Got email path - {email_path}")
