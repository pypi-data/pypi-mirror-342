...
class EmailSender:
    ...
    def _does_template_path_exists(email_path):
        """
        ...
        if not exists(TEMPLATES_DIR):
           raise TemplateDirNotFound("The template directory wasn't found")
        if not exists(EMAIL_TEMPLATES_DIR):
           raise EmailTemplateDirNotFound("The emails_templates folder wasn't found")
        if not exists(email_path):
            raise EmailTemplateNotFound("The email template wasn't found")
