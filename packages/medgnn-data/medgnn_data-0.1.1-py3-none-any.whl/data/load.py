...
class load:
  ...
  def datasets(data_path: str = DATA_PATH):
    ...
    return {
      'allaml': lambda: load.allaml(data_path),
      'colon': lambda: load.colon(data_path),
      'gli': lambda: load.gli(data_path),
      'leukemia': lambda: load.leukemia(data_path),
      'prostate': lambda: load.prostate(data_path),
      'smk': lambda: load.smk(data_path),
    }