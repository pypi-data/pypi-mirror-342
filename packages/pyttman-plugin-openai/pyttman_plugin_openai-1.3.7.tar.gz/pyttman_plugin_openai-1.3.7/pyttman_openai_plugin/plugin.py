...
class OpenAIPlugin(PyttmanPlugin):
    ...
    detect_memory_prompt = ("You are a filtering layer in a multi-modal AI system. Your "
                            ...
                            "memory string. Do not use relative terms in stored memories. "
                            ...
                            "elaborate. Only output `[MEMORY]: ...` or `0`. You are not ChatGPT. "
                            "You do not answer. You only filter and summarize if needed. Current "
                            "date time information: {DATE_TIME_DATA}")
    ...
    def create_memory_if_applicable(self, message) -> str or None:
        ...
        memory_detect_prompt = self.detect_memory_prompt.format(DATE_TIME_DATA=self.time_awareness_prompt)
