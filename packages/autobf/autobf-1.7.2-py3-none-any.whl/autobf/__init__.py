...
#     237,496,color=(255, 255, 255)
    ...
    time.sleep(1)
    click(455,587)
def switchSever(join='navy',repeat=5):
    for i in range(repeat):
        row(1252,67,dy=1)
...
def resetUntilOK():
    while True:
        ...
        if check:
            print('黃色問號在左前，港口在左')
            bf.slide('w',3)
            if not bf.close2Color():continue
            if isGetMission:bf.getMission(1)
            bf.press('d',4);bf.slide('w',4);bf.press('a',2);bf.press('w',5)
            return
        check=bf.checkColorByXY(581,108,color=(241, 234, 55))
        if check:
            print('黃色問號在正前，港口在左')
            continue
#             bf.slide('d');bf.slide('w',3);bf.slide('a');bf.press('a',5)
#             bf.close2Color()
#             if isGetMission:bf.getMission(1)
#             bf.press('d',4);bf.slide('w',4);bf.press('a',2);bf.press('w',5)
#             bf.press('a',2)
#             return
        ...
        if check:
            print('先滑一步後黃色問號和港口在左前前')
            bf.press('a',8);bf.slide('w',3);bf.press('a',5);bf.press('w',4)
            if not bf.close2Color():continue
            if isGetMission:bf.getMission(1)
            bf.slide('d',4);bf.slide('w',2);bf.press('d',2);bf.press('s',1)
            return
        check=bf.checkColorByXY(426,111,color=(239, 220, 41))
        if check:
            print('先滑一步後黃色問號和港口在左左前')
            bf.slide('w');bf.slide('a');bf.slide('w',2);bf.slide('a');bf.press('d')
            if not bf.close2Color():continue
            if isGetMission:bf.getMission(1)
            bf.slide('d',2);bf.slide('w',2);bf.slide('d');bf.press('d',5);bf.slide('w')
            return
while True:
    for i in range(50):
        resetUntilOK()
        fight()
    bf.switchSever(join='navy')
'''
code['沙漠']='''
import autobf as bf
bf.setCenterXY(x=639,y=530);bf.click(center=True)
def resetUntilOK():
    while True:
        ...
        bf.openPopUp();bf.click(66,563)
        check=bf.checkColorByXY(40,549,color=(0, 254, 0))
        bf.closePopUp()
        if check:
            print('島上較靠近任務點')
            bf.zoom2max()
            bf.slide('w',3);bf.slide('d',3);bf.press('w',4)
            if not bf.close2Color():continue
            if isGetMission:bf.getMission(1)
            bf.jumpSlide('w',4,2)
            bf.press('s',3);bf.press('d',2)
            return
        check=bf.checkColorByXY(528,496,color=(0, 223, 71))
        if check:
            print('船商站靠近左邊')
            bf.zoom2max()
            bf.jumpSlide('d',4,4);bf.slide('s',3);bf.jumpSlide('d',4,2)
            bf.press('s',4);bf.press('d',5)
            if not bf.close2Color():continue
            if isGetMission:bf.getMission(1)
            bf.slide('d');bf.press('d',4);bf.press('w',3)
            return
def fight():
    bf.switchOn('1')
    for i in range(6):
...
def getCode(location):
    global code
    try:    pyperclip.copy(code[location])
    except: print(location,'目前無程式碼')
