...
def load_and_preprocess_data(file_path, attri):
    ...
    std_scaler.fit(origin_data.iloc[:, :attri])
    origin_data = std_scaler.transform(origin_data.iloc[:, :attri])
    origin_data = pd.DataFrame(10 * origin_data, index=y_true.index,
