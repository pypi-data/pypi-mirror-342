...
class Tilemap(Object):
    ...
    def render_grid(self) -> None:
        cell_w, cell_h = self.tilesize, self.tilesize

        start_x = self.location[0] // self.tilesize
        end_x = (self.location[0] + (self.sizet[0] * self.tilesize)) // self.tilesize
        
        start_y = self.location[1] // self.tilesize
        end_y = (self.location[1] + (self.sizet[1] * self.tilesize)) // self.tilesize

        for gx in range(int(start_x), int(end_x)):
            x = gx * cell_w
            x -= x % self.tilesize
            pg.draw.line(self.image, [125, 125, 125], [x, start_y * cell_h], [x, end_y * cell_h], 1)
        
        for gy in range(int(start_y), int(end_y)):
            y = gy * cell_h
            y -= y % self.tilesize
            pg.draw.line(self.image, [125, 125, 125], [start_x * cell_w, y], [end_x * cell_w, y], 1)
# ------------------------------------------------------------ #
