...
def compute_amplitude_spectrum(signal, sampling_rate, lowcut=None, highcut=None,
    ...
    """
    Вычисляет амплитудный спектр сигнала с возможностью фильтрации и наложения окна.
    ...
    :return: Кортеж (frequencies, amplitude_spectrum), где:
    ...
    if lowcut is not None and highcut is not None:
        signal = filter_signal(signal, (lowcut, highcut), sampling_rate, 'bandpass')
    ...
    # Вычисляем БПФ и амплитудный спектр
    spectrum = np.abs(fft(signal))[:n_fft // 2] / n_original  # Нормализация по исходной длине
