...
from zut import (Header, Literal, RowMixin, TabularDumper, cached_property,
                 files, format_csv_value, get_logger)
...
def split_excel_path(path: str|os.PathLike, *, default_table_name: str = None) -> tuple[Path,str]:
    """
...
class ExcelWorkbook:
    ...
    def __init__(self, path: str|os.PathLike, *, formula = _UNSET, archivate: bool|str|os.PathLike = False, save_as: str|os.PathLike|bool|None = None):
        """
    ...
    @classmethod
    def get_instance(cls, path: str|os.PathLike, *, formula = _UNSET, archivate: bool|str|os.PathLike = False, save_as: str|os.PathLike|bool|None = None) -> ExcelWorkbook:
...
class ExcelTable:
    ...
    def __init__(self, pyxl_table: Table|str, pyxl_worksheet: Worksheet, workbook: ExcelWorkbook, no_headers: bool = None):
        ...
        if isinstance(pyxl_table, str):
            ...
            self.headers = [Header(name) for name in self.pyxl_table.column_names] 
    ...
    def add_column(self, name: str):
        ...
        name = str(name)
        if any(h.name == name for h in self.headers):
        ...
        self.headers.append(Header(name)) # implies self.col_count += 1
        self._column = None # invalidate columns
    ...
    def redefine(self):
        ...
        for i in range(0, self.col_count):
            if self.has_headers:
                ...
                name = self.headers[i].name if i < len(self.headers) else None
...
class ExcelRow(RowMixin):
    ...
    def values(self) -> list[Any]:
        if self._values is None:
            self._values = list(self.provider.get_value(self.index, col_index) for col_index in range(0, self.provider.col_count))
            self._must_refresh = None

        elif self._must_refresh:
            for col_index in range(0, self.provider.col_count):
                if self._must_refresh[col_index]:
                    self._values[col_index] = self.provider.get_value(self.index, col_index)
            self._must_refresh = None
    ...
    def __getitem__(self, key: int|str):
        if not isinstance(key, int):
            key = self.provider.get_column_index(key)
            
        if self._values is None:
            self._values = [ _UNSET ] * self.provider.col_count
            self._must_refresh = {index: True for index in range(0, self.provider.col_count)}

            value = self.provider.get_value(self.index, key)
    ...
    def __setitem__(self, key: int|str, value):
        ...
        if not self.readonly:
            self.provider.set_value(self.index, key, value)


class ExcelDumper(TabularDumper[Union[str,os.PathLike]]):
    atexit = True
    ...
    def _convert_value(self, value):
        return format_csv_value(value, decimal_separator='.', no_tz=self.no_tz, no_microseconds=True, visual=True)
