"""
File: cplotting_tools.py
Author: Matthias Wolff, Florian Eilers, Xiaoyi Jiang; original author: Arturo Mena López
Description: Modified copy of cplotting_tools.py by Arturo Mena López
             (see https://github.com/artmenlope/complex-plotting-tools/blob/master/cplotting_tools.py ; 17.01.2025)
             Changes include (limited to method 'complex_plot3D', everything else remains unchanged):
                - Added support to draw directly into figure given as parameter instead of creating new figure
                - added support to return an image of the plot
                - made fontsize a parameter
                - introduced a flag whether colormap should be drawn
                - added support to return all drawn matplotlib.pyplot LineSet objects (to later toggle their
                  transparency in KanPlot()
...
def complex_plot3D(x, y, f, 
    ...
    Arguments:
        ...
        for Matplotlib.
    """
    plotted_objects = []  # list to store LineSet objects (to later adjust transparency)
    if log_mode == True:
    ...
    if fig_ax is None:
        ...
        # use existing figure and axis given as parameter
        fig, ax = fig_ax
    # set axis labels
    ax.set_xlabel(xlabel, fontsize=fontsize, labelpad=fontsize//2)
    ax.set_ylabel(ylabel, fontsize=fontsize, labelpad=fontsize//2)
    ...
    surf = ax.plot_surface(x, y, abs_f, linewidth=0, alpha=0.7,
                    cstride=1, rstride=1,
                    facecolors=fcolors)
    plotted_objects.append(surf)
    ...
    # Draw the colorbar
    if draw_colorbar:
        cbar = plt.colorbar(s_m, ticks=[0, np.pi/2, np.pi, 3*np.pi/2, 2*np.pi], ax=ax)
        cbar.ax.set_yticklabels(["$0$", "$\\frac{\\pi}{2}$", "$\\pi$", "$\\frac{3\\pi}{2}$", "$2\\pi$"], fontsize=fontsize)
        cbar.ax.set_ylabel("Arg f(z)", fontsize=fontsize)
        plotted_objects.append(cbar)
    if fig_ax is None:
        if return_image:
        ...
        # append everything that's plotted to plotted_objects (se transparency can be adjusted later
        # for everything in this plot)
        plotted_objects.append(ax.xaxis.pane)
        plotted_objects.append(ax.yaxis.pane)
        plotted_objects.append(ax.zaxis.pane)

        plotted_objects.append(ax.xaxis.line)
        plotted_objects.append(ax.yaxis.line)
        plotted_objects.append(ax.zaxis.line)

        for axis in [ax.xaxis, ax.yaxis, ax.zaxis]:
            for ls in [axis.get_ticklabels(), axis.get_ticklines()]:
                for l in ls:
                    plotted_objects.append(l)
        plotted_objects.append(ax.xaxis.get_label())
        plotted_objects.append(ax.yaxis.get_label())
        plotted_objects.append(ax.zaxis.get_label())
        ax.grid(False)
        # data has been plotted into specified figure and axis
        # plotted lines will additionally be returned as a list (to later adjust transparency)
