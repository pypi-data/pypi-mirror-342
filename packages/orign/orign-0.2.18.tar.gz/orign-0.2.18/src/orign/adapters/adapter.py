...
class Adapter:
    ...
    def __init__(
        ...
        if not namespace:
            ...
            namespace = user_profile.handle or user_profile.email.replace(
                "@", "-"
            ).replace(".", "-")
        ...
        if not self.adapter:
            ...
            # Follow buffer pattern: Update if relevant fields are provided in init
            update_payload = V1AdapterUpdateRequest(
                uri=uri,  # Update URI if provided
                lora=lora,  # Update lora if provided
                learning_rate=learning_rate,  # Update lr if provided
                epochs_trained=epochs_trained,  # Update epochs_trained if provided
                examples_trained=examples_trained,  # Update examples_trained if provided
                last_trained=last_trained,  # Update last_trained if provided
                # labels=labels, # V1AdapterUpdateRequest doesn't currently support labels update via PATCH in __init__? Or maybe it does? Keep commented for now based on original code.
            ).model_dump(exclude_unset=True)  # Only include fields explicitly set
            ...
            # else:
            # print(f"No updates specified during init for adapter {self.adapter.metadata.name}")
