...
cdef double _scipy_callback_lambda_real(int n, double *x, void *user_data) noexcept:
    cdef symengine.LambdaRealDoubleVisitor* lamb = <symengine.LambdaRealDoubleVisitor *>user_data
    cdef double result
    deref(lamb).call(&result, x)
    return result

cdef void _ctypes_callback_lambda_real(double *output, const double *input, void *user_data) noexcept :
    cdef symengine.LambdaRealDoubleVisitor* lamb = <symengine.LambdaRealDoubleVisitor *>user_data
    deref(lamb).call(output, input)

cdef double _scipy_callback_llvm_real(int n, double *x, void *user_data) noexcept :
    cdef symengine.LLVMDoubleVisitor* lamb = <symengine.LLVMDoubleVisitor *>user_data
    cdef double result
    deref(lamb).call(&result, x)
    return result

cdef void _ctypes_callback_llvm_real(double *output, const double *input, void *user_data) noexcept :
    cdef symengine.LLVMDoubleVisitor* lamb = <symengine.LLVMDoubleVisitor *>user_data
    deref(lamb).call(output, input)
