...
def transcribe_audio_file(audio_path=None, json_data=None,video_path=None, chunk_length_ms=50000,chunk_length_diff=10000,renew=False):
    ...
    full_text = ""
    chunk_path = os.path.join(video_directory,"temp_chunk.wav")
    ...
    for i, chunk in enumerate(chunks):
        ...
        if result is True:
            logger.info(f"block {start_time} - {end_time} is already transcribed")
            continue
        else:
            logger.info(f"transcribing block {start_time} - {end_time}")
            ...
            with sr.AudioFile(chunk_path) as source:
                ...
                try:
                    ...
                    text = ""
                text = stitch_text(last_text,text)
...
def export_srt(audio_text, output_path):
    ...
    with open(output_path, 'w') as f:
        for i, entry in enumerate(audio_text, 1):
            ...
            f.write(f"{i}\n{start} --> {end}\n{entry['text']}\n\n")
def transcribe_audio_file(audio_path=None, json_data=None,video_path=None, chunk_length_ms=10000,chunk_length_diff=75,renew=False):
    logger.info(f"transcribe_audio_file: {audio_path}")
    json_data = json_data or {}
    audio_path = audio_path or json_data.get('audio_path')
    video_path = video_path or json_data.get('video_path')
    ...
    for i, chunk in enumerate(chunks):
        ...
        end_time = min((i + 1) * chunk_length_ms, len(audio))
        if i != 0:
            start_time=get_time_diff(start_time,end_time,chunk_length_diff)
        result = start_time,end_time
        if renew == False:
            result = check_if_transcribed_block(json_data['audio_text'], start_time, end_time)
        if result is True:
            ...
            with sr.AudioFile(chunk_path) as source:
                ...
                try:
                    ...
                    text = ""
                text = stitch_text(last_text,text)
                logger.info(f"trancription: {text}")
            json_data['audio_text'].append({
                "start_time": format_timestamp(updated_start),
                "end_time": format_timestamp(updated_end),
                "text": text
