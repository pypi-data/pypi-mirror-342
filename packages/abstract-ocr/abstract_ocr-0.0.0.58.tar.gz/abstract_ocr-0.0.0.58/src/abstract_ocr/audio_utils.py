...
def save_voice(voice, voice_txt_path):
    ...
    if not os.path.isfile(voice_txt_path):
        write_to_file(contents = '',file_path = voice_txt_path)
    ...
    try:
        if voice:
            ...
            logging.info(f"voice not a valid value {voice_txt_path}")
        
    except Exception as e:
        logging.info(f"Error saving text to {voice_txt_path}: {e}")
