...
def transcribe_audio_file(audio_path, json_path, chunk_length_ms=60000):
    ...
    try:
        ...
        # Save plain text
        text_result= save_voice("", text=full_text)
...
def transcribe_all_video_paths(directory=None,output_dir=None,remove_phrases=None):
    ...
    for video_path in video_paths:
        transcribe_audio(video_path,output_dir=output_dir,remove_phrases=remove_phrases)
def transcribe_audio(video_path,output_dir=None,remove_phrases=None):
    remove_phrases=remove_phrases or []
    ...
    args_tuple = (video_path, video_directory, video_json_path, [], '', filename,remove_phrases)
    ...
    info['video_text_json']=video_text_path
    info['video_json']=video_json_path
    extract_audio_from_video(video_path=info['video_path'],audio_path=info['audio_path'])
    info.update(transcribe_audio_file(audio_path=info['audio_path'],json_path=info['video_text_json']))
