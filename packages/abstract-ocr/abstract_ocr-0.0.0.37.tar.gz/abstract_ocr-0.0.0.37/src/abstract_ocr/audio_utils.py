...
def transcribe_all_video_paths(directory=None,output_directory=None):
    ...
    output_directory = output_directory if output_directory else make_dirs(abs_dirname,'text_dir')
    paths = glob.glob(path_join(abs_dirname, '**', '**'), recursive=True)
    paths = [file_path for file_path in paths if confirm_type(file_path,media_types=get_media_types(['video']))]
    video_paths = get_all_file_types(directory=abs_dirname,types='video') or get_all_file_types(directory=abs_dirname,types='videos')
