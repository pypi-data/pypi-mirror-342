...
def analyze_video_text(video_path=None,output_dir=None,json_data=None,remove_phrases=None):
    json_data = json_data or {}
    video_path = video_path or json_data.get('video_path')
    if not video_path or not os.path.isfile(video_path):
        return json_data
    video = VideoFileClip(video_path)
    thumbnails_directory = json_data.get('thumbnails_directory')
    output_dir = output_dir or json_data.get('info_directory') or thumbnails_directory or os.path.dirname(video_path)
    if not output_dir or not os.path.isdir(output_dir):
         output_dir = make_dirs(os.path.getcwd(),'thumbnails')
    if os.path.basename(output_dir) != 'thumbnails':
        output_dir = make_dirs(output_dir,'thumbnails')    
    thumbnails_directory = output_dir
    json_data['thumbnails_directory']=thumbnails_directory
    duration = video.duration
    json_data = create_key_value(json_data,'remove_phrases',remove_phrases or [])
    dirname = os.path.dirname(video_path)
    basename = os.path.basename(video_path)
    filename,ext = os.path.splitext(basename)
    frame_interval = 1  # Adjust as needed
    for t in range(0, int(duration), frame_interval):
        frame_path = os.path.join(thumbnails_directory,f"{filename}_frame_{t}.jpg")
    ...
    for frame_file in os.listdir(thumbnails_directory):
        if frame_file.endswith(".jpg"):
            if not is_frame_analyzed(frame_file,json_data['video_text']):
                image_path = os.path.join(thumbnails_directory,frame_file)
