...
def check_if__block(audio_text, start_time, end_time):
...
def get_audio_text_data(i,
    ...
    if isinstance(result, tuple):
        logging.info(f"transcribing audio chunk {i}, start_time:{result[0]}, end_time:{result[1]}")
        ...
        logging.info(f"audio text already transcribed for chunk {i}")
...
def get_audio_text_jsons(audio_path,
    ...
    for i in range(len_chunks):
        ...
        logging.info(f"chunk {i} of {len_chunks} processed")
    return json_data
...
def transcribe_audio_file(audio_path=None, json_data=None, video_path=None, chunk_length_ms=8000, chunk_length_diff=-5, renew=False):
    ...
    if not audio_path and video_path:
        ...
        logging.info(f"audio path derived from video_path:{video_path}")
    ...
    if os.path.isfile(temp_file_path):
        try:
            ...
            logging.info(f"temp_info_path transfered to info_path:{file_path}")
        except OSError as e:
