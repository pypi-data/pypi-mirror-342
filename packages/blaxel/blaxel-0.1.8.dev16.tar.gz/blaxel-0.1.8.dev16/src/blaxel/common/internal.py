...
def get_alphanumeric_limited_hash(input_str, max_size):
    """
    Generates a SHA-256 hash of the input string, converts to base64, 
    removes non-alphanumeric characters, and limits to max_size.
    Args:
        input_str: The input string to hash
        max_size: The maximum length of the output hash
        
    Returns:
        A string containing the limited alphanumeric hash
    """
    ...
    # IMPORTANT: We only keep lowercase letters and numbers
    # This matches the TypeScript regex /[^a-z0-9]+/g
    alphanumeric = re.sub(r'[^a-z0-9]', '', hash_base64)
