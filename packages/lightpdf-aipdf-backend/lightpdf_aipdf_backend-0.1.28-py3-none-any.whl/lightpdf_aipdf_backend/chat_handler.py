...
async def process_messages(session_id: str, messages: List[Message]) -> List[Dict]:
    ...
    # 定义一个内部函数来处理添加文件链接
    def add_file_links_to_message(message_dict: Dict, links: List[str]) -> Dict:
        """将文件链接添加到消息内容中
    ...
    # 定义一个内部函数来处理直接的文件信息
    def get_file_info_links(file_infos: List) -> List[str]:
        """从直接文件信息获取链接列表
        ...
        for file_info in file_infos:
            links.append(f"[{file_info.filename}]({urllib.parse.quote(file_info.path)})")
        return links
    ...
    for msg in messages:
        ...
        # 如果消息包含直接的文件信息
        if msg.file_infos:
            file_urls = get_file_info_links(msg.file_infos)
            message_dict = add_file_links_to_message(message_dict, file_urls)
...
async def generate_chat_response(session_id: str, messages: List[Dict], instructions: Optional[str] = None) -> AsyncGenerator:
    """生成聊天响应
    ...
    try:
        ...
        # 如果提供了instructions参数，添加系统提示词
        if instructions:
            # 先检查消息列表中是否已经有系统提示词
            has_system_message = any(msg.get("role") == "system" for msg in api_messages)
            if has_system_message:
                # 如果已有系统提示词，则更新它而不是添加新的
                for i, msg in enumerate(api_messages):
                    if msg.get("role") == "system":
                        api_messages[i] = {"role": "system", "content": instructions}
                        break
            else:
                # 在消息列表开头添加系统提示词
        ...
        while True:
            ...
            if finish_reason == 'tool_calls' and tool_calls_data:
                ...
                # 更新会话历史，过滤掉系统提示词
                session_messages = [msg for msg in api_messages if msg.get("role") != "system"]
                update_user_session(session_id, session_messages)
