...
FILE = os.path.join(os.path.dirname(__file__), "latest_leapseconds.tls")
MD5_FILE = os.path.join(os.path.dirname(__file__), "latest_leapseconds.md5")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Fetch the NAIF leapseconds kernel file store its checksum."
    )
    parser.add_argument(
        "--url",
        type=str,
        default=URL,
        help="URL to fetch the NAIF leapseconds kernel file from.",
    )
    parser.add_argument(
        "--file",
        type=str,
