...
def update_package_info(executable: str, pkg: PackageInfo, source_folder: str):
    ...
    if "setup.py" in files:
        ...
        if folder:
            ...
            # Handle the case where no .dist-info folder is found
            pkg.name = "unknown" if not pkg.name else pkg.name
            pkg.version = "unknown" if not pkg.version else pkg.version
