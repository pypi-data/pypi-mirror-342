...
async def prepare_venv(venv_num: int, package_info: PackageInfo, package_number: int, github_token: str, ado_token: str):
    ...
    if package_info.install_type != package_info.InstallType.SOURCE_CODE or package_info.skip_install_deps != True:
        # Skip build if the package is source code and skip_install_deps is set to True
        await pip_utils.install_in_exe_async(get_venv_exe(venv_num), package_name, options)
    else:
        await pip_utils.install_in_exe_async(get_venv_exe(venv_num), package_name, options + ['--no-deps'])
