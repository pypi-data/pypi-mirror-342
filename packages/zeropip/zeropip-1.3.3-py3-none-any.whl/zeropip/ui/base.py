...
class ZeropipUI:
    ...
    def _render_ui(self):
        ...
        self.output_box = widgets.Textarea(layout=widgets.Layout(width='100%', height='400px'), disabled=False)
        self.info_box = widgets.Textarea(layout=widgets.Layout(width='100%', height='400px'), disabled=False)
    ...
    def _on_submit_click(self, b):
        try:
            ...
            self.info_box.value = result.get("정보", "")
        except Exception as e:
            self.output_box.value = ""
            self.info_box.value = f"[오류] {e}"
    ...
    def _on_upload(self, change):
        for filename, fileinfo in self.upload_button.value.items():
            ...
            self.info_box.value = f"📂 {filename} 업로드됨"

    def _on_clear_cache(self, b):
        self.input_box.value = ""
        self.output_box.value = ""
        self.info_box.value = "캐시/입력 내용이 초기화되었습니다"
        self.file_cache = None
    ...
    def _on_download_click(self, b):
        ...
        self.info_box.value = f"📥 결과가 파일로 다운로드되었습니다"
