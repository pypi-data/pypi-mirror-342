...
class Series(FireDucksPandasCompat):
    ...
    def __getitem__(self, key):
        ...
        return self._fallback_call(
            "__getitem__", args=[_unwrap(key)], reason=reason
        )
    ...
    def aggregate(self, func=None, axis=0, *args, **kwargs):
        ...
        if irutils._is_scalar_or_list_or_tuple_of(func, str):
            ...
            is_supported_agg = utils.is_supported_df_agg_funcs(targets)
        elif is_dict_like(func):
            for k, v in func.items():
                index_names.append(k)
                targets.append(v)
            is_supported_agg = irutils._is_list_or_tuple_of(
                targets, str
            ) and utils.is_supported_df_agg_funcs(targets)
        else:
            reason = "'func' is neither string, nor a list/tuple of strings, nor a dict-like"

        if not is_supported_agg:
            reason = f"all or few agg functions are not supported: {targets}"
        ...
        if isinstance(func, str):
            ...
            func = irutils.make_vector_or_scalar_of_str(targets)
            value = ir.aggregate(self._value, func, axis=0)
            ret = Series._create(value)
            if len(index_names) > 0:
                ret._set_axis(index_names, _inplace_index_setter=True)
            return ret
    ...
    def to_numpy(self, *args, **kwargs):
        if self._fireducks_meta.is_cached():
            return self._fireducks_meta.get_cache().to_numpy(*args, **kwargs)
