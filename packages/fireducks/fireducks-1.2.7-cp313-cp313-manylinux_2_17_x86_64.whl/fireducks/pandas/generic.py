...
class FireDucksMetadata:
    ...
    def set_ir_metadata_wrapper(self, meta):
        self.ir_metadata_wrapper = meta

    def invalidate_ir_metadata_wrapper(self):
        self.ir_metadata_wrapper = None
...
class FireDucksPandasCompat(
    ...
    def _rebind(self, value, *, invalidate_cache=False):
        ...
        if invalidate_cache:
            ...
            self._fireducks_meta.invalidate_ir_metadata_wrapper()
    ...
    def _get_metadata(self):
        ...
        if self._fireducks_meta.ir_metadata_wrapper is None:
            value = ir.get_metadata(self._value)
            metadata = fireducks.core.evaluate([value])[0]
            self._fireducks_meta.set_ir_metadata_wrapper(
                IRMetadataWrapper(metadata)
            )
        return self._fireducks_meta.ir_metadata_wrapper
    ...
    def _take(
        ...
        if input_indices is None:
            ...
            return self._fallback_call(
                "take", args=[indices, axis], reason=reason
            )
    ...
    def sum(self, *args, **kwargs):
        ...
        if reason is not None:
            return self._fallback_call("sum", args, kwargs, reason=reason)
