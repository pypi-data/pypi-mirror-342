...
def prepare_training(sh_degree: int, source: str, destination: str, device: str, load_ply: str = None, load_camera: str = None, with_depth=False, with_scale_reg=False, configs={}, init=None, init_configs={}) -> Tuple[CameraDataset, GaussianModel, AbstractTrainer]:
    gaussians = CameraTrainableGaussianModel(sh_degree).to(device)
    if init:  # initialize
        initialized_cameras, initialized_point_cloud = initialize(initializer=init, directory=source, with_depth=True, configs=init_configs, device=device)
        ...
        dataset = (TrainableCameraDataset.from_json(load_camera, load_depth=True) if load_camera else ColmapTrainableCameraDataset(source, load_depth=True)).to(device)
    ...
    trainer = {
        True: {False: Trainer, True: ScaleRegularizeTrainer},
        False: {False: BaseTrainer, True: BaseScaleRegularizeTrainer},
    }[with_depth][with_scale_reg](
        gaussians,
