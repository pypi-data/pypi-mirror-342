...
class AnalysisContext:
    ...
    def get_qc_status(self, analysis=False):
        ...
        if not analysis:
            data_sessions = self.__comm.send_request(
                "patient_manager/get_patient_list", req_headers={"X-Range": "items=0-9999"},
                req_data={"pars_patient_secret_name": f"string;{self.analysis_data['patient_secret_name']}"}
            )
            ...
            for session in data_sessions:
                ssid = session["ssid"]
                self.logger.info("Matches QC for {} / {}?".format(self.analysis_data['patient_secret_name'], ssid))
        ...
        if analysis:
            data_analyses = self.__comm.send_request(
                "analysis_manager/get_analysis_list", req_headers={"X-Range": "items=0-9999"},
                req_data={"secret_name": self.analysis_data["patient_secret_name"]}
            )
            try:
                for analysis in data_analyses:
                    if analysis["_id"] == self.analysis_id:
                        self.logger.info(f"Found analysis / {analysis}, getting QC.")
                        qa_data = analysis["qa_data"]["new_value"]
                        qa_status = qa_data.split("<b>")[1].split("</b>")[0]  # `fail` or `pass`
    ...
    def set_metadata_value(self, key, value, title=None, readonly=False, patient_secret_name=None, ssid=None):
        ...
        if result:
            raise Exception(f"Special characters not accepted: {', '.join(result)}")
        if not isinstance(value, (int, list, str, float)):
            raise Exception("Parameter 'value' must be of type 'int', 'list', 'str', or 'float'.")
    ...
    def get_metadata_value(self, key=None, patient_secret_name=None, ssid=None):
        """
    ...
    def get_session_owner(self, input_id):
        ...
        return self.__comm.send_request(
            "file_manager/get_container_files", self.analysis_data["settings"][input_id]
        )["owner"]
