...
def gui_tkinter():
    ...
    def perform_selection_check(values):
        try:
            raise_if_false(isinstance(values["code"], str), "Tool ID must be a string.")
            raise_if_false(values["code"] != "", "Tool ID must be defined.")
            raise_if_false(" " not in values["code"], "Tool ID can't have spaces.")
            values["code"] = values["code"].lower()  # must be lowercase
            values["short_name"] = values["code"].lower().replace(" ", "_")  # must be lowercase

            raise_if_false(isinstance(values["name"], str), "Tool name must be a string.")
            raise_if_false(values["name"] != "", "Tool name must be defined.")

            raise_if_false(values["version"] != "", "Tool version must be defined.")
            raise_if_false(re.search(r"^(\d+\.)?(\d+\.)?(\*|\d+)$", values["version"]), "Version format not valid.")

            raise_if_false(values["cores"], "Number of cores must be defined.")
            raise_if_false(values["cores"].isnumeric(), "Number of cores must be an integer.")
            raise_if_false(
                MAX_CORES >= int(values["cores"]) > MIN, f"Number of cores must be between {MIN} and {MAX_CORES}."
            )

            raise_if_false(values["memory"], "RAM must be defined.")
            raise_if_false(values["memory"].isnumeric(), "RAM must be an integer.")
            raise_if_false(MAX_RAM >= int(values["memory"]) > MIN, f"RAM must be {MIN} and {MAX_RAM}.")
    ...
    # image
    logo = Image.open(os.path.join(os.path.dirname(__file__), "templates_tool_maker", "qmenta.png")).resize((500, 110))
    ...
    # title
    title_label = ttk.Label(master=window, text="(*) indicates mandatory field.")
    title_label.pack(padx=10, pady=10)
    ...
    folder_tool_label = ttk.Label(master=folder_tool_frame, text="Select folder where the tool is stored.*")
    folder_tool_label.pack(side="left")
    folder_tool_button = ttk.Button(master=folder_tool_frame, text="Browse Folder", command=browse_folder)
    ...
    tool_id_label = ttk.Label(master=tool_id_frame, text="Specify the tool ID.*")
    ...
    tool_version_label = ttk.Label(master=tool_version_frame, text="Specify the tool version.*")
    ...
    tool_name_label = ttk.Label(master=tool_name_frame, text="Specify the tool name.*")
    ...
    num_cores_label = ttk.Label(
        master=num_cores_frame, text=f"How many cores does the tool require? (integer, max. {MAX_CORES})"
    )
    ...
    memory_label = ttk.Label(
        master=memory_frame, text=f"How many GB of RAM does the tool require? (integer, max. {MAX_RAM})"
    )
    ...
    image_name_label = ttk.Label(master=image_name_frame, text="Docker image name")
    ...
    title_label = ttk.Label(
        ...
        "Credentials need to be valid in order to be able to pull the Docker image from the registry.",
    )
    title_label.pack(padx=10, pady=10)

    docker_url_frame = ttk.Frame(master=window)
    docker_url_label = ttk.Label(master=docker_url_frame, text="Docker registry URL")
    ...
    docker_user_label = ttk.Label(master=docker_user_frame, text="Docker registry user")
    ...
    docker_password_label = ttk.Label(master=docker_password_frame, text="Docker registry password*")
    ...
    button = ttk.Button(
        master=action_frame, text="Publish in QMENTA Platform", command=submit_form, bootstyle="primary"
    )
    ...
    ttk.Button(action_frame, text="Quit", command=quit_tool_creator, bootstyle="danger").pack(side="left", padx=10)
...
def main():
    ...
    try:
        auth = platform.Auth.login(
            username=user, password=password, base_url="https://platform.qmenta.com", ask_for_2fa_input=False
        )
    except Needs2FAError as needs2faerror:
        messagebox.showinfo(
            "Message", str(needs2faerror) + " Please check the terminal to add the code sent to your phone."
        )
        auth = platform.Auth.login(
            username=user, password=password, base_url="https://platform.qmenta.com", code_2fa=input("Input 2-FA code:")
        )

    # Get information from the advanced options file.
    advanced_options = "settings.json"
    raise_if_false(os.path.exists(advanced_options), "Settings do not exist! Run the local test to create it.")
    ...
    content_build["results_configuration"] = json.dumps(results_config).replace("{}", "")
    ...
    res = platform.post(auth, "analysis_manager/upsert_user_tool", data=content_build)
