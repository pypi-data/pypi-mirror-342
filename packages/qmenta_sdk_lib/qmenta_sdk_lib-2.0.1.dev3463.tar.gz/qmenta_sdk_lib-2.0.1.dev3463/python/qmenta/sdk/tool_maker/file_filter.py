...
def make_tag(p: Union[List[Tag]]) -> str:
    ...
    str
    """
    return ",".join([f"t'{tag}'" if not tag.startswith("!") else f"!t'{tag[1:]}'" for tag in p])


def make_modality(p: Union[List[Modality], Modality]) -> str:
    return "|".join([f"m'{mod.value}'" for mod in p]) if (isinstance(p, list) and len(p) > 1) else f"m'{p.value}'"
...
def parse_property(filters: List[Property_type]) -> str:
    ...
    for _filter in filters:
        ...
        parsed.append("(" + pf + ")") if ("," in pf or "|" in pf) and not isinstance(_filter, str) else parsed.append(
            pf
        )
    return "(" + ",".join(parsed) + ")" if len(filters) > 1 else ",".join(parsed)
