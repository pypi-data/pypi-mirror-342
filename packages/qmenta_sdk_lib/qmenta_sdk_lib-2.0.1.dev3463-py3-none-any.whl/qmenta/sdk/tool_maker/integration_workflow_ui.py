...
def gui_tkinter():
    ...
    def perform_selection_check(values):
        ...
        try:
            ...
            raise_if_false(os.path.exists(values["folder"]), "Folder must exist.")

            # Ensure tool ID has no spaces and create a short name
            if " " in values["code"]:
                print("WARNING: Tool code can't have spaces, replacing with under score.")
                values["short_name"] = values["code"].lower().replace(" ", "_")  # Create short name
    ...
    tool_id_label = ttk.Label(master=tool_id_frame, text="Specify the Workflow ID.*")
    ...
    button = ttk.Button(master=action_frame, text="Publish in QMENTA Platform", command=submit_form)
    ...
    ttk.Button(master=action_frame, text="Quit", command=quit_tool_creator, bootstyle="danger").pack(
        side="left", padx=10
    )
...
def main():
    ...
    res = platform.post(auth, "a_administrator/upsert_analysis_tool", data=content_build)
