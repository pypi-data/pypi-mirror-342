...
def run_docker(
    ...
    c_values_path = os.path.join("/", "qmenta", "local_exec_settings_values.json")
    ...
    if not values:
        ...
        values_content = (
            '{"input":[\n' + ",\n".join(['   {"path": "' + f + '"}' for f in files if not isdir(f)]) + "]\n}"
        )
    ...
    run_command(["docker", "cp", abspath(settings), c_name + ":" + c_settings_path], verbose=False)
    run_command(["docker", "cp", abspath(values), c_name + ":" + c_values_path], verbose=False)
    ...
    if resources:
        run_command(["docker", "cp", abspath(resources), c_name + ":" + c_res_path], verbose=False)
