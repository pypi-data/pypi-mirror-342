...
from qmenta.sdk.tool_maker.make_files import (
    ...
    build_script,
    build_test,
    build_description, raise_if_false,
)
...
def gui_tkinter():
    ...
    def perform_selection_check(values):
        try:
            raise_if_false(isinstance(values["tool_id"], str), "Tool ID must be a string.")
            raise_if_false(values["tool_id"] != "", "Tool ID must be defined.")
            raise_if_false(" " not in values["tool_id"], "Tool ID can't have spaces.")
            values["tool_id"] = values["tool_id"].lower()  # must be lowercase
            values["folder"] = os.getcwd()
            if os.path.exists(os.path.join(values["folder"], values["tool_id"])):
    ...
    button = ttk.Button(master=action_frame, text="Create", command=submit_form, bootstyle="primary")
