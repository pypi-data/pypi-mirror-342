...
from modelbase2.experimental import model_fn_to_sympy
...
def test_unary_plus() -> None:
    ...
    a = sympy.Symbol("a")
    assert model_fn_to_sympy(fn, [a]) == a
...
def test_binary_add() -> None:
    ...
    assert model_fn_to_sympy(fn, [left, right]) == left + right
...
def test_single_assignment() -> None:
    ...
    assert model_fn_to_sympy(fn, [a, b]) == a * b
...
def test_condition() -> None:
    ...
    a = sympy.Symbol("a")
    assert model_fn_to_sympy(fn, [a]) == sympy.Piecewise(
        (a, a > 1),
        (a**2, True),
    )
...
def test_fn_call_power() -> None:
    ...
    a = sympy.Symbol("a")
    assert model_fn_to_sympy(fn, [a]) == a**2 * 2
