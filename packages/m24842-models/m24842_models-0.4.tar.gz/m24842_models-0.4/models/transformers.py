...
# Transformers with ConvPE
class CausalConv(nn.Module):
    def __init__(self, dim, len, bias=True):
        super().__init__()
        self.dim = dim
        self.len = len
        self.conv = nn.Conv1d(dim, dim, kernel_size=len, groups=dim, padding=len-1, bias=bias)
    ...
    def forward(self, x):
        ...
        Returns:
            torch.Tensor: Output sequence of shape (batch_size, seq_len, dim).
        """
        bsz, seq_len, dim = x.size()
        x = x.transpose(1, 2)
        x = self.conv(x)  # (bsz, dim, seq_len)
        x = x[:, :, :seq_len]
        x = x.transpose(1, 2)  # (bsz, seq_len, dim)
        return x

class ConvMultiheadAttention(nn.Module):
    def __init__(self, dim, num_heads, d_conv, dropout=0.0, bias=True, add_bias_kv=False, 
                 add_zero_attn=False, batch_first=False):
        """
        Initialize the MultiheadAttention module.
        ...
        Args:
            dim: Total dimension of the model
            num_heads: Number of parallel attention heads
            d_conv: Dimension of the convolutional positional encoding
            dropout: Dropout probability on attention weights
            bias: Add bias to input projections
            add_bias_kv: Add bias to the key and value sequences
            add_zero_attn: Add a new batch of zeros to the key and value sequences
        ...
        # Linear projections for query, key, and value
        self.conv = CausalConv(self.head_dim, len=d_conv, bias=bias)
        self.q_proj = nn.Linear(dim, dim, bias=bias)
        self.k_proj = nn.Linear(dim, dim, bias=bias)
        self.v_proj = nn.Linear(dim, dim, bias=bias)
        self.out_proj = nn.Linear(dim, dim, bias=bias)
...
class ConvTransformer(nn.Module):
    def __init__(self, emb_dim, output_dim, d_conv, n_layers=1, n_heads=1, mlp_dim=None, vocab_size=10, dropout=0.0, causal=True):
        super().__init__()
        self.emb_dim = emb_dim
        self.output_dim = output_dim
        self.causal = causal
        ...
        self.layers = nn.ModuleList([
            nn.ModuleDict(
                dict(
                    norm1=nn.LayerNorm(emb_dim),
                    dropout1=nn.Dropout(dropout),
                    attention=ConvMultiheadAttention(emb_dim, self.n_heads, d_conv, bias=True, batch_first=True),
                    norm2=nn.LayerNorm(emb_dim),
                    dropout2=nn.Dropout(dropout),
                    feedforward=nn.Sequential(
                        nn.Linear(emb_dim, self.mlp_dim, bias=True),
    ...
    def forward(self, x):
        ...
        for layer in self.layers:
            x = layer.norm1(x)
            a_out, _ = layer.attention(x, attn_mask=mask)
            x = layer.norm2(x + layer.dropout1(a_out))
            ff_out = layer.feedforward(x)
            x = x + layer.dropout2(ff_out)
        x = self.norm_f(x)
...
class ConvLinearMultiheadAttention(nn.Module):
    def __init__(self, dim, num_heads, d_conv, bias=True):
        super().__init__()
        self.dim = dim
        self.num_heads = num_heads
        self.head_dim = dim // num_heads
...
class ConvLinearTransformer(nn.Module):
    def __init__(self, emb_dim, output_dim, d_conv, n_layers=1, n_heads=1, mlp_dim=None, vocab_size=10, dropout=0.0, causal=True):
        super().__init__()
        self.emb_dim = emb_dim
        self.output_dim = output_dim
        ...
        self.layers = nn.ModuleList([
            nn.ModuleDict(
                dict(
                    norm1 = nn.LayerNorm(emb_dim),
                    dropout1 = nn.Dropout(dropout),
                    attention=ConvLinearMultiheadAttention(emb_dim, self.n_heads, d_conv, bias=True),
                    norm2 = nn.LayerNorm(emb_dim),
                    dropout2 = nn.Dropout(dropout),
                    feedforward=nn.Sequential(
                        nn.Linear(emb_dim, self.mlp_dim, bias=True),
    ...
    def forward(self, x):
        ...
        for layer in self.layers:
            x = layer.norm1(x)
            a_out = layer.attention(x, causal=self.causal)
            x = layer.norm2(x + layer.dropout1(a_out))
            ff_out = layer.feedforward(x)
            x = x + layer.dropout2(ff_out)
        x = self.norm_f(x)
...
class ConvOrthoLinearAttention(nn.Module):
    """
    Orthogonal Linear Attention:
    A derivative of linear attention that orthogonalizes queries and keys for each head to reduce crossterm interference.
    ...
    An optimal choice for head dimension is 3.
    """
    def __init__(self, d_model: int, n_heads: int, d_conv: int, bias: bool = True):
        super().__init__()
        self.d_model = d_model
        self.n_heads = n_heads
        self.d_conv = d_conv
        ...
        self.conv = CausalConv(self.d_head, len=d_conv, bias=bias)
        self.beta = nn.Parameter(torch.zeros(1))
        self.q_proj = nn.Linear(d_model, d_model, bias=bias)
        self.k_proj = nn.Linear(d_model, d_model, bias=bias)
        self.v_proj = nn.Linear(d_model, d_model, bias=bias)
    ...
    def forward(self, x: torch.Tensor, causal: bool = True) -> torch.Tensor:
        ...
        """
        bsz, seq_len, d_model = x.size()
        q = rearrange(self.q_proj(x), 'b s (h d) -> (b h) s d', h=self.n_heads).contiguous()
        k = rearrange(self.k_proj(x), 'b s (h d) -> (b h) s d', h=self.n_heads).contiguous()
        v = rearrange(self.v_proj(x), 'b s (h d) -> (b h) s d', h=self.n_heads).contiguous()
...
class ConvOrthoLinearTransformer(nn.Module):
    def __init__(self, emb_dim, output_dim, d_conv, n_layers=1, n_heads=1, mlp_dim=None, vocab_size=10, dropout=0.0, causal=True):
        super().__init__()
        self.emb_dim = emb_dim
        self.output_dim = output_dim
        ...
        self.layers = nn.ModuleList([
            nn.ModuleDict(
                dict(
                    norm1 = nn.LayerNorm(emb_dim),
                    dropout1 = nn.Dropout(dropout),
                    attention=ConvOrthoLinearAttention(emb_dim, self.n_heads, d_conv, bias=True),
                    norm2 = nn.LayerNorm(emb_dim),
                    dropout2 = nn.Dropout(dropout),
                    feedforward=nn.Sequential(
                        nn.Linear(emb_dim, self.mlp_dim, bias=True),
