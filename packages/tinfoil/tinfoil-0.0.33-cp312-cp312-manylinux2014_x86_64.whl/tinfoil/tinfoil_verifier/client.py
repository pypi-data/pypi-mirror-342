...
class Slice_x509_PolicyMapping(go.GoClass):
	...
	def __init__(self, *args, **kwargs):
		...
		if len(kwargs) == 1 and 'handle' in kwargs:
			...
			self.handle = _client.Slice_x509_PolicyMapping_CTor()
			_client.IncRef(self.handle)
			if len(args) > 0:
				if not isinstance(args[0], _collections_abc.Iterable):
	...
	def __del__(self):
		_client.DecRef(self.handle)
	def __str__(self):
		s = 'client.Slice_x509_PolicyMapping len: ' + str(len(self)) + ' handle: ' + str(self.handle) + ' ['
		if len(self) < 120:
			s += ', '.join(map(str, self)) + ']'
		return s
	def __repr__(self):
		return 'client.Slice_x509_PolicyMapping([' + ', '.join(map(str, self)) + '])'
	def __len__(self):
		return _client.Slice_x509_PolicyMapping_len(self.handle)
	def __getitem__(self, key):
		if isinstance(key, slice):
			if key.step == None or key.step == 1:
				st = key.start
				...
				if st == None:
					st = 0
				if ed == None:
					ed = _client.Slice_x509_PolicyMapping_len(self.handle)
				return Slice_x509_PolicyMapping(handle=_client.Slice_x509_PolicyMapping_subslice(self.handle, st, ed))
			return [self[ii] for ii in range(*key.indices(len(self)))]
		elif isinstance(key, int):
			if key < 0:
				key += len(self)
			if key < 0 or key >= len(self):
				raise IndexError('slice index out of range')
			return go.x509_PolicyMapping(handle=_client.Slice_x509_PolicyMapping_elem(self.handle, key))
		else:
			raise TypeError('slice index invalid type')
	def __setitem__(self, idx, value):
		if idx < 0:
			idx += len(self)
		if idx < len(self):
			_client.Slice_x509_PolicyMapping_set(self.handle, idx, value.handle)
			return
		raise IndexError('slice index out of range')
	def __iadd__(self, value):
		if not isinstance(value, _collections_abc.Iterable):
	...
	def __next__(self):
		if self.index < len(self):
			rv = go.x509_PolicyMapping(handle=_client.Slice_x509_PolicyMapping_elem(self.handle, self.index))
			self.index = self.index + 1
			return rv
		raise StopIteration
	def append(self, value):
		_client.Slice_x509_PolicyMapping_append(self.handle, value.handle)
	def copy(self, src):
		""" copy emulates the go copy function, copying elements into this list from source list, up to min of size of each list """
		mx = min(len(self), len(src))
		for i in range(mx):
...
class GroundTruth(go.GoClass):
	...
	@property
	def PublicKey(self):
		return _client.client_GroundTruth_PublicKey_Get(self.handle)
	@PublicKey.setter
	def PublicKey(self, value):
		if isinstance(value, go.GoClass):
			_client.client_GroundTruth_PublicKey_Set(self.handle, value.handle)
		else:
			_client.client_GroundTruth_PublicKey_Set(self.handle, value)
	@property
	def Digest(self):
...
class TLSBoundRoundTripper(go.GoClass):
	...
	def __init__(self, *args, **kwargs):
		...
		if len(kwargs) == 1 and 'handle' in kwargs:
			...
			_client.IncRef(self.handle)
			if  0 < len(args):
				self.ExpectedPublicKey = args[0]
			if "ExpectedPublicKey" in kwargs:
				self.ExpectedPublicKey = kwargs["ExpectedPublicKey"]
	def __del__(self):
		_client.DecRef(self.handle)
	def __str__(self):
		pr = [(p, getattr(self, p)) for p in dir(self) if not p.startswith('__')]
	...
	def __repr__(self):
		...
		return sv + ')'
	@property
	def ExpectedPublicKey(self):
		return _client.client_TLSBoundRoundTripper_ExpectedPublicKey_Get(self.handle)
	@ExpectedPublicKey.setter
	def ExpectedPublicKey(self, value):
		if isinstance(value, go.GoClass):
			_client.client_TLSBoundRoundTripper_ExpectedPublicKey_Set(self.handle, value.handle)
		else:
			_client.client_TLSBoundRoundTripper_ExpectedPublicKey_Set(self.handle, value)
