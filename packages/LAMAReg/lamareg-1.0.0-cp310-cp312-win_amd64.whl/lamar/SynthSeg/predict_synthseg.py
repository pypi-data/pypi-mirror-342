...
from lamar.SynthSeg.predict import write_csv, get_flip_indices
from lamar.SynthSeg.model_handler import ensure_model_file_exists
...
def predict(
    ...
    if topology_classes is not None:
        topology_classes = utils.load_array_if_path(
            topology_classes, load_as_numpy=True
        )[unique_idx]
        ...
        if compute[i]:
            ...
            try:
                ...
                if path_posteriors[i] is not None:
                    utils.save_volume(
                        posteriors, aff, h, path_posteriors[i], dtype="float32"
                    )
    ...
    if robust:
        ...
        print(
            "Robust machine learning segmentation for large-scale analysis of heterogeneous clinical brain MRI "
            "datasets"
        )
    ...
    if gt_folder is not None:
        ...
        if compute_distances:
            ...
            path_hausdorff = path_hausdorff_99 = path_hausdorff_95 = (
                path_mean_distance
            ) = None
...
def prepare_output_files(
    path_images, out_seg, out_posteriors, out_resampled, out_volumes, out_qc, recompute
):
    ...
    # path_images is an image
        ...
        # input images
        assert os.path.isfile(path_images), (
    ...
    recompute_list = [
        recompute | re_seg | re_post | re_res | re_vol | re_qc
        for (re_seg, re_post, re_res, re_vol, re_qc) in zip(
            recompute_seg,
            recompute_post,
            recompute_resampled,
            recompute_volume,
...
def preprocess(
    ...
    if n_dims == 2 and 1 < n_channels < 4:
        raise Exception(
            "either the input is 2D with several channels, or is 3D with at most 3 slices. "
    ...
    if crop is not None:
        ...
        crop_shape = [
            utils.find_closest_number_divisible_by_m(s, 2**n_levels, "higher")
            for s in crop
        ]
...
def build_model(
    ...
    # Ensure all model files exist by reassembling them if necessary
    if not ensure_model_file_exists(path_model_segmentation):
        raise FileNotFoundError(
            f"Could not find or reassemble model file: {path_model_segmentation}"
        )

    if do_parcellation and not ensure_model_file_exists(path_model_parcellation):
        raise FileNotFoundError(
            f"Could not find or reassemble parcellation model: {path_model_parcellation}"
        )

    if do_qc and not ensure_model_file_exists(path_model_qc):
        raise FileNotFoundError(
            f"Could not find or reassemble QC model: {path_model_qc}"
        )
    ...
    if robust:
        ...
        if sigma_smoothing > 0:
            last_tensor = net.output
            if hasattr(last_tensor, "get_shape"):
        ...
        if flip_indices is not None:
            ...
            last_tensor = KL.Lambda(lambda x: tf.concat(x, -1), name="concat")(
                reordered_channels
            )
    ...
    if do_parcellation:
        ...
        last_tensor = layers.ConvertLabels(
            np.arange(n_labels_seg), labels_segmentation
        )(last_tensor)
        parcellation_masking_values = np.array(
            [1 if ((ll == 3) | (ll == 42)) else 0 for ll in labels_segmentation]
        )
        last_tensor = layers.ConvertLabels(
            labels_segmentation, parcellation_masking_values
        )(last_tensor)
        last_tensor = KL.Lambda(
...
class MakeShape(KL.Layer):
    ...
    def build(self, input_shape):
        ...
        self.cropping_shape = np.array(
            utils.reformat_to_list(self.target_shape, length=self.n_dims)
        )
        self.built = True
