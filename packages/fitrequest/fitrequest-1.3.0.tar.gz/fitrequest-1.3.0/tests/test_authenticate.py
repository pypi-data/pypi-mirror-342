...
def test_authenticate_on_init(config_with_url):
    default_auth = Auth(username='default_user', password='default_password')

    config = ConfigWithURL(auth=default_auth)
    assert config.auth == default_auth
    assert config.session.raw_auth == default_auth

    # No authentication made from pydantic configuration
    assert config.session.synchronous.auth is None
    assert config.session.asynchronous.auth is None

    # No configuration made during class and methods generation
    new_class = config.fit_class
    assert new_class.session == config.session
    assert new_class.session.synchronous.auth is None
    assert new_class.session.asynchronous.auth is None

    # Authentication configured to httpx clients once the fitrequest client is initialized
    client = new_class()
    assert client.session == new_class.session
    assert client.session.synchronous.auth == default_auth.authentication
    assert client.session.asynchronous.auth == default_auth.authentication
...
def test_instance_serialization_custom_auth():
    class AwesomeAuth(httpx.Auth): ...

    assert Auth(custom=AwesomeAuth()).model_dump(exclude_none=True) == {'custom': 'AwesomeAuth'}
...
def test_authenticate_instance_serialization_tokens():
    ...
    ).model_dump(exclude_none=True) == {'header_token': '**********'}
    ...
    ).model_dump(exclude_none=True) == {'params_token': '**********'}
