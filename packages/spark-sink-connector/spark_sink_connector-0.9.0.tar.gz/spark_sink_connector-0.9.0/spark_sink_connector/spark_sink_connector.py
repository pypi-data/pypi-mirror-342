...
class SparkSinkConnector:
    ...
    def _log_configurations(self):
        ...
        self.logger.info("Hoodie Options:")
        self.logger.info(self.config.hoodie_options)
        self.logger.info("Table Properties:")
        self.logger.info(self.config.table_properties)
        self.logger.info("S3 Configurations:")
        ...
        self.logger.info("Schema Registry Configurations:")
    ...
    def _apply_table_properties(self):
        ...
        if table_properties is not None:
            formatted_properties = [
                f"'{key}' = '{value}'" for key, value in self.config.delta_table_properties.items()
            ]
            alter_table_command = f"ALTER TABLE {self.config.database}.{self.config.table} SET TBLPROPERTIES ({', '.join(formatted_properties)})"
