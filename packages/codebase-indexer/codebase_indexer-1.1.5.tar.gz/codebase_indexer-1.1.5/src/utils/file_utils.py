...
# Limited set of code file extensions to include
CODE_EXTENSIONS = [
    # Python
    "*.py",
...
def load_documents(codebase_path: str, 
    ...
    if not documents:
        ...
        try:
            # Create a list of allowed globs based on our CODE_EXTENSIONS
            allowed_globs = ["**/" + ext.replace("*", "") if ext.startswith("*") else "**/" + ext for ext in CODE_EXTENSIONS]
            
            all_docs = []
            for glob_pattern in allowed_globs:
                try:
                    logger.info(f"Trying DirectoryLoader with pattern: {glob_pattern}")
                    ...
                    logger.info(f"Found {len(docs)} documents with pattern {glob_pattern}")
                except Exception as pattern_error:
                    logger.warning(f"Error with DirectoryLoader pattern {glob_pattern}: {pattern_error}")
...
def chunk_documents(documents, chunk_size=CHUNK_SIZE, chunk_overlap=CHUNK_OVERLAP):
    """Split documents into chunks using basic text splitter.
    ...
    # Use simple recursive character text splitter with reasonable defaults
