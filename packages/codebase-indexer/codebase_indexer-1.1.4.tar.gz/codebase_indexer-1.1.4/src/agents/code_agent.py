...
class CodeAgent:
    ...
    def query(self, 
        ...
        Args:
            ...
            use_mmr: Whether to use Maximum Marginal Relevance for retrieval. MMR helps
                     balance relevance and diversity in search results, avoiding redundant
                     information by considering both document relevance to the query and
                     uniqueness compared to already selected documents.
            mmr_diversity: Diversity parameter for MMR ranging from 0 to 1. Higher values
                           (closer to 1.0) increase result diversity at the expense of 
                           relevance. Lower values (closer to 0.0) prioritize relevance
                           over diversity. Default is 0.3 for a good balance.
        ...
        try:
            ...
            if use_mmr:
                # Maximum Marginal Relevance (MMR) retrieval balances relevance with diversity
                # MMR works by:
                # 1. First retrieving a larger set of documents based on similarity to the query
                # 2. Then selecting a diverse subset of those documents by considering both:
                #    - Similarity to the query (relevance)
                #    - Dissimilarity to already selected documents (diversity)
                # This helps avoid redundant code snippets and provides broader context
                #
                # The mmr_diversity parameter (0-1) controls the balance:
                ...
                # This returns the k most similar documents to the query without considering diversity
                retriever = self.vectorstore.as_retriever(
