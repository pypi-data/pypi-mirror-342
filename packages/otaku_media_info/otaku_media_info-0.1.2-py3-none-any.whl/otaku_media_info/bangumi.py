...
from .models import BangumiMediaInfo, RawBangumiMediaInfo
...
def clean_sub(sub: Union[str, None]) -> Union[str, None]:
    if sub is None:
        return sub
...
def raw_parser(raw: str) -> Union[None, Episode]:
    ret = process(raw)
    if ret is None:
...
def parse(path: Path, *, lang="zh") -> BangumiMediaInfo:
    try:
        episode = raw_parser(path.stem)
    except Exception as e:
        raise ParseError(f"Cannot parse {path.stem}") from e

    if episode is None:
        raise ParseError(f"Cannot parse {path.stem}")

    title = episode.title_zh or episode.title_en or episode.title_jp
    if lang == "zh" and episode.title_zh:
        title = episode.title_zh
    elif lang == "en" and episode.title_en:
        title = episode.title_en
    elif lang == "jp" and episode.title_jp:
        title = episode.title_jp

    return BangumiMediaInfo(
        raw=RawBangumiMediaInfo(
            raw_path=path,
            raw_name=path.stem,
            raw_size=path.stat().st_size,
            raw_mtime=path.stat().st_mtime,
            raw_ctime=path.stat().st_ctime,
            raw_title=", ".join(
                filter(
                    bool,
                    [
            ...
            raw_author="",  # not supported
            raw_season=episode.season_raw,
        ),
        title=title,
        author=episode.group,
        type="bangumi",
        page=episode.episode,
        total=None,  # not supported
        language="zh" if episode.sub else None,
        categories=None,  # not supported
        season=episode.season,
        sub=episode.sub,
        sub_group=episode.group,
        resolution=episode.resolution,
        capture_source=episode.source,
    )


if __name__ == "__main__":
    title = "[jibaketa合成&音頻壓制][TVB粵語]愛犬訊號 / Dog Signal - 15 [粵語+內封繁體中文字幕][WEB 1920x1080 AVC AAC SRT TVB YUE CHT]"
