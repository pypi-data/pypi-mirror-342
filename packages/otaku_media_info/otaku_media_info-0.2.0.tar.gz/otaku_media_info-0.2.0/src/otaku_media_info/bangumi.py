...
from otaku_media_info.api.bgm import (
    Bangumi,
    BangumiEpisode,
    query_bangumi,
    query_episode,
)
from otaku_media_info.exceptions import ParseError
...
@dataclass
class BangumiEpisodeInfo(Episode):
    bangumi: Bangumi
    bangumi_episode: BangumiEpisode
...
def parse(
    ...
    bangumi = query_bangumi(title)
    bangumi_episode = query_episode(bangumi.id, episode.episode)

    return BangumiEpisodeInfo(
        **asdict(episode), bangumi=bangumi, bangumi_episode=bangumi_episode
    )
