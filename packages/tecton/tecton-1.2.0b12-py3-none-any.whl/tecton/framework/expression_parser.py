...
class _ToAbstractSyntaxTree(Transformer):
    ...
    def try_strptime(self, values: List[AbstractSyntaxTreeNode]) -> AbstractSyntaxTreeNode:
        ...
        if timestamp_ast.dtype.type != DataTypeEnum.DATA_TYPE_STRING:
            msg = (
                f"Error parsing expression: TRY_STRPTIME requires a string timestamp value, {self.original_expression}"
            )
            raise ExpressionParsingError(msg, self.calculation_name)
        ...
        if timestamp_ast.HasField("literal_value"):
            ...
            msg = f"Error parsing expression: TRY_STRPTIME requires an ISO 8601 formatted UTC timestamp string, {self.original_expression}"
            try:
    ...
    def _validate_try_strptime_format(self, format_ast: AbstractSyntaxTreeNode):
        if format_ast.dtype.type != DataTypeEnum.DATA_TYPE_STRING:
            msg = f"Error parsing expression: TRY_STRPTIME requires a string format value, {self.original_expression}"
            raise ExpressionParsingError(msg, self.calculation_name)
        format_str = format_ast.literal_value.string_value
        if "%" not in format_str:
            msg = (
                f"Error parsing expression: no format directives specified in TRY_STRPTIME format string: {format_str}"
            )
            raise ExpressionParsingError(msg, self.calculation_name)
        ...
        if match:
            msg = f"Error parsing expression: Invalid format directive '{match.group()}' at position {match.start()} in '{format_str}'"
            raise ExpressionParsingError(msg, self.calculation_name)

        # check for incompatibilities
        if "%V" in format_str and "%Y" in format_str:
            msg = f"Error parsing expression: %V is incompatible with year directive %Y. Use the ISO year %G instead in {format_str}"
            raise ExpressionParsingError(msg, self.calculation_name)
        ...
        if "%g" not in format_str and "%n" not in format_str:
            try:
                ...
                msg = f"Error parsing expression: format failed test parsing, invalid format string: {format_str}"
                raise ExpressionParsingError(msg, self.calculation_name)

        # ensure format value is ISO 8601
        # TODO: This is a temporary gate. Remove in the future.
        if format_str != ISO_8601_FORMAT_DUCKDB:
            msg = f"Error parsing expression: TRY_STRPTIME currently only supports ISO 8601 format ('{ISO_8601_FORMAT_DUCKDB}'), {self.original_expression}"
            raise ExpressionParsingError(msg, self.calculation_name)
