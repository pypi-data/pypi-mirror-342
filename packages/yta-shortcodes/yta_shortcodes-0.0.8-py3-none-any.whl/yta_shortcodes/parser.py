...
class ShortcodeParser:
    ...
    def __init__(
        ...
        """
        if any(
            not PythonValidator.is_subclass(shortcode_tag, ShortcodeTag)
        ...
        # handlers overrided
        def register_shortcode_handler(self, shortcode_tag):
            """
            Register the shortcode handler specifically and
            individually for each shortcode.
            """
            def handler(pargs, kwargs, context, content = None):
    ...
    def parse(self, text: str):
        ...
        def sanitize_text(text: str):
            ...
            """
            return fix_excesive_blank_spaces(fix_ellipsis(fix_unseparated_periods(fix_separated_parenthesis(fix_separated_square_brackets(add_missing_spaces_before_and_after_parenthesis(add_missing_spaces_before_and_after_square_brackets(text)))))))
