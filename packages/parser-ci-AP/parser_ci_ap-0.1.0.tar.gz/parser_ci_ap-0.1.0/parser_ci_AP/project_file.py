...
class AbstractProjectFile(ABC):
    ...
    def _detect_encoding(self) -> str:
        ...
        try:
            ...
            print(f"[!] Не удалось определить кодировку файла {self.path}: {e}")
            return "utf-8"
...
class ProjectFile(AbstractProjectFile):
    ...
    def _count_lines_with_comments(self) -> int:
        ...
        try:
            ...
            print(f"[!] Ошибка при подсчёте строк с комментариями в файле {self.path}: {e}")
            return 0
    ...
    def _count_lines_without_comments(self) -> int:
        ...
        try:
            ...
            print(f"[!] Ошибка при подсчёте строк без комментариев в файле {self.path}: {e}")
            return 0
    ...
    def _parse_with_clang(self) -> cindex.TranslationUnit | None:
        ...
        try:
            ...
            print(f"Ошибка при анализе {self.path}: {e}")
            return None
        ...
        def visit(cursor):
            if cursor.kind == CursorKind.MACRO_DEFINITION and cursor.spelling:
                try:
                    ...
                    print(f"[!] Не удалось обработать define '{cursor.spelling}': {e}")
    ...
    def __str__(self) -> str:
        ...
        lines.append(f"  Includes: {', '.join(self.includes)}")
        return "\n".join(lines)