...
class EXAFS_Analysis:
    ...
    def larch_score(self, verbose=True):
        ...
        path, yTotal, best, loss, best_Fit_r, arr = larch_score.fitness(self.exp, self.bestFit_mat, self.paths,
                                                                        self.params, return_r=True, verbose=verbose)
        self.return_str += arr
    ...
    def plot(self, title='Temp', fig_gui=None, show=False):
        ...
        xftf(self.best.k, self.best.chi, kmin=self.params['Kmin'], kmax=self.params['Kmax'], dk=4, window='hanning',
             kweight=self.kweight, group=self.best)

        if fig_gui == None:
            fig, ax = plt.subplots(1, 2, figsize=(15, 5))

            ax[0].plot(self.g.k, self.g.chi * self.g.k ** self.kweight, 'b--', label="Experiment Data")
            ax[0].plot(self.path.k[SMALL:BIG], self.yTotal[SMALL:BIG] * self.path.k[SMALL:BIG] ** self.kweight, 'r-',
            ...
            ax.plot(self.path.k[SMALL:BIG], self.yTotal[SMALL:BIG] * self.path.k[SMALL:BIG] ** self.kweight, 'r-',
                    label="Genetic Algorithm")
            ax.legend()
    ...
    def individual_fit(self, plot=False, fig_gui=None):
        ...
        path, yTotal, best, loss, export_paths = larch_score.fitness_individual(self.exp, self.bestFit_mat, self.paths,
                                                                                self.params, export=True, plot=plot,
                                                                                fig_gui=fig_gui)
    ...
    def export_files(self, header='test', dirs='', igor_true=False):
        ...
        BIG = self.big
        self.header = header
        larch_score.write_bestFit_csv(self.g.k, self.g.chi, self.path.k, self.yTotal, SMALL, BIG, name=file_name_k,
                                      header_base=header)
        # larch_score.write_result_n_err(full_mat_var,err,name=file_name_best,header_base=header)
        larch_score.write_individual_csv(self.g.k, self.g.chi, self.path.k, self.yTotal, SMALL, BIG,
                                         self.ind_export_paths, self.paths, name=file_name_ind, header_base=header)
    ...
    def export_igor_individual(self, file_paths='export_ind.ipf'):
        ...
        if len(full_paths) < 3:
            if len(full_paths) == 1:
                f.write('•ModifyGraph offset(path_' + str(full_paths[0]) + '_' + self.header + '_chi2)={0,5}');
                f.write('\n')
            elif len(full_paths) == 2:
                f.write('•ModifyGraph offset(path_' + str(full_paths[0]) + '_' + self.header + '_chi2)={0,5}');
                f.write('\n')
                f.write('•ModifyGraph offset(path_' + str(full_paths[1]) + '_' + self.header + '_chi2)={0,10}');
                f.write('\n')
        else:
            f.write('•ModifyGraph offset(path_' + str(full_paths[0]) + '_' + self.header + '_chi2)={0,5}');
            f.write('\n')
            f.write('•ModifyGraph offset(path_' + str(full_paths[1]) + '_' + self.header + '_chi2)={0,10}');
            f.write('\n')
            f.write('•ModifyGraph offset(path_' + str(full_paths[2]) + '_' + self.header + '_chi2)={0,12.5}');
            f.write('\n')
    ...
    def create_legend(self, f):
        ...
        for i in range(len(full_paths)):
            if int(self.nleg_arr[i]) > 2:
                ...
                addition = ' '
            paths_arr = str(full_paths[i]) + '_' + self.header + '_chi2) Path ' + str(full_paths[i]) + addition + \
                        self.label_arr[i] + r'";DelayUpdate'
    ...
    def adjust_color(self, f, color_map=plt.cm.jet_r):
        ...
        Args:
            f (writer): writer
            color_map (matplotlib color_map, optional): color map. Defaults to plt.cm.jet_r.
        """
        full_paths = larch_score.flatten_2d_list(self.paths)
