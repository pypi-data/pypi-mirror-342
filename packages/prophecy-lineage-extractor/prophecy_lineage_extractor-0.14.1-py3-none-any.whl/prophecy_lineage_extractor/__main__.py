...
class PipelineProcessor:
    ...
    def end_ws(self):
        try:
            ...
            logging.info(f"Excel report generated at {output_file}")
            if self.send_email:
                send_email(self.project_id, output_file, self.pipeline_id_list)
                logging.info(f"Excel report sent for project_id = {self.project_id} ")
            else:
                logging.info(f"Excel report not generated")
