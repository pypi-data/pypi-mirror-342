...
void APIBridge::load_plugins(const QDir& plugin_dir) {
  try {
    ...
    py::dict loaded_backend_plugins = iprm_util_plugins.attr("load_backends")(dir);
    for (const auto& [plugin_name, plugin_klass] : loaded_backend_plugins) {
      ...
      const auto icon_path_str = QString::fromStdString(
    ...
    for (const auto& [plugin_name, plugin_klass] : loaded_object_plugins) {
      ...
      if (!svg_icon_path.is_none()) {
        const auto icon_path_str = QString::fromStdString(
            svg_icon_path.attr("__str__")().cast<std::string>());
        plugin_object_icons_[name] = AssetCache::svg_icon(icon_path_str);
      }
    }
  } catch (const py::error_already_set& e) {
