...
# TODO: This builder should go away and just be a method on the main CppThirdParty class that passes in
#  a archive file. As one can see, every other method here is just directly going on the main target to begin
#  with
class SourceArchiveBuilder(ArchiveBuilder):
    ...
    def requires(self, *targets: tuple[str]):
        self._target.requires(*targets)

    def build(self):
        super().build()
        target_name = self._target.name
        if not self._target.is_header:
            self._target.properties['build_target'] = f'{target_name}_objects'
        else:
            self._target.properties['build_target'] = target_name
