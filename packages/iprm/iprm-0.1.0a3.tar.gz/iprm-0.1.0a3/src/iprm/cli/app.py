...
from iprm.generator.generator import Generator
...
def _native_loader_main(command, **kwargs):
    ...
    if command == GENERATE_CMD:
        backend_class = supported_backends[kwargs.pop('backend')]
        loader.backend = backend_class.__name__
        backend = backend_class(loader, **kwargs)
        generator = Generator(backend, plugin_dir)
