...
FileView::FileView(const FileData& file_data, QWidget* parent)
  ...
  auto gui_view = new QSplitter(Qt::Vertical, this);
  objects_view_ = new ObjectsTableView(this);
  connect(objects_view_, &ObjectsTableView::object_selected, this,
          &FileView::on_object_selected);
  objects_view_->load(saved_file_data_.platform_file);
  gui_view->addWidget(objects_view_);

  object_properties_view_ = new ObjectsView(this);
  gui_view->addWidget(object_properties_view_);
  gui_view->setSizes({200, 400});
