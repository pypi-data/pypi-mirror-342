...
APIBridge::APIBridge(const QDir& iprm_src_dir, QObject* parent)
    : QObject(parent) {
  qRegisterMetaType<APIError>();
  qRegisterMetaType<PlatformFile>();
  qRegisterMetaType<Dir>();
  qRegisterMetaType<CurrentSourceDir>();
  qRegisterMetaType<SourceDir>();
  qRegisterMetaType<RootRelativeSourceDir>();
  qRegisterMetaType<CurrentBinaryDir>();
  qRegisterMetaType<BinaryDir>();
  qRegisterMetaType<RootRelativeBinaryDir>();

  py::gil_scoped_acquire acq;
  try {
    py::module sys = py::module::import("sys");
    const std::string iprm_src_dir_path =
        iprm_src_dir.absolutePath().toStdString();
    sys.attr("path").cast<py::list>().append(iprm_src_dir_path);
  } catch (const py::error_already_set&) {
    // TODO: Add console log infra for errors that occurr before MainWindow has
    //  been created
  }
}
...
const QHash<QString, QList<Object>>& APIBridge::objects(
    const QString& platform) const {
  static const QHash<QString, QList<Object>> s_no_platform;
...
void APIBridge::load_plugins(const QDir& plugin_dir) {
  try {
    ...
    for (const auto& [plugin_name, plugin_klass] : loaded_backend_plugins) {
      ...
      //  supported commands instead of only exposing Generate
...
void APIBridge::init_sess() {
  ...
  try {
    ...
    iprm_util_dir_klass_ = py::module::import("iprm.util.dir").attr("Dir");
...
void APIBridge::load_project() {
  ...
  const py::dict public_utility_api = iprm_namespace.attr("UTILITY_CATEGORY");
  populate_api(public_utility_api, public_utility_api_);
  public_api_.insert(public_utility_api_);

  const py::dict public_objects_api = iprm_namespace.attr("OBJECT_CATEGORIES");
  ...
  for (const py::list supported_platforms =
    ...
    platform.native_loader_ =
        iprm_util_loader.attr("Loader")(dir, plat, plugin_dir);
...
void APIBridge::process_objects(Platform& platform,
  ...
  for (const auto& [key, value] : py_objects) {
    ...
    for (const auto& obj : obj_list) {
      objects.push_back(make_object(normalized_file_path, obj));
    }
...
Object APIBridge::make_object(const QString& file_path,
                              const py::handle& py_obj) const {
  auto obj_name =
      QString::fromStdString(py_obj.attr("name").cast<std::string>());
  const auto obj_type_name = QString::fromStdString(
  ...
  const py::dict py_obj_properties = py_obj.attr("properties");
  const auto obj_properties = make_object_properties(py_obj_properties);

  if (static_cast<bool>(obj_type_flags & TypeFlags::SUBDIR)) {
    obj_name = obj_properties["dir_name"].toString();
  }

  const auto obj_hex_colour =
      QString::fromStdString(py_obj.attr("hex_colour").cast<std::string>());
  const auto obj_shape_type =
      QString::fromStdString(py_obj.attr("shape_type").cast<std::string>());
  ...
  const auto obj_icon = [this, &obj_type_name, &obj_type_flags]() {
    const auto obj_icon_itr = plugin_object_icons_.find(obj_type_name);
    if (obj_icon_itr != plugin_object_icons_.end()) {
      return obj_icon_itr.value();
    }
    return AssetCache::object_type_icon(obj_type_flags);
  }();
  ...
  return Object{
      ...
      .hex_colour = obj_hex_colour,
      .shape_type = obj_shape_type,
      .project_rel_dir_path = proj_relative_dir_path,
      .icon = obj_icon,
  };
}

QVariantHash APIBridge::make_object_properties(
    const py::dict& py_obj_properties) const {
  auto make_dir_value = [](const py::handle& dir_obj) -> QVariant {
    const auto dir_type =
        dir_obj.get_type().attr("__name__").cast<std::string>();
    const auto dir_path = QString::fromStdString(
        dir_obj.attr("path").attr("__str__")().cast<std::string>());
    if (dir_type == "CurrentSourceDir") {
      return QVariant::fromValue(CurrentSourceDir{});
    } else if (dir_type == "SourceDir") {
      return QVariant::fromValue(SourceDir{dir_path});
    } else if (dir_type == "RootRelativeSourceDir") {
      return QVariant::fromValue(RootRelativeSourceDir{dir_path});
    } else if (dir_type == "CurrentBinaryDir") {
      return QVariant::fromValue(CurrentBinaryDir{});
    } else if (dir_type == "BinaryDir") {
      return QVariant::fromValue(BinaryDir{dir_path});
    } else if (dir_type == "RootRelativeBinaryDir") {
      return QVariant::fromValue(RootRelativeBinaryDir{dir_path});
    }
    return QVariant{};
  };

  QVariantHash obj_properties;
  obj_properties.reserve(static_cast<qsizetype>(py_obj_properties.size()));
  for (const auto& [py_prop_name, py_prop_value] : py_obj_properties) {
    const auto prop_name =
        QString::fromStdString(py_prop_name.cast<std::string>());
    if (py::isinstance<py::str>(py_prop_value)) {
      obj_properties[prop_name] =
          QString::fromStdString(py_prop_value.cast<std::string>());
    } else if (py::isinstance<py::list>(py_prop_value)) {
      const auto py_prop_list_value = py_prop_value.cast<py::list>();
      QVariantList prop_list_value;
      prop_list_value.reserve(
          static_cast<qsizetype>(py_prop_list_value.size()));
      for (const auto& py_prop_elem : py_prop_list_value) {
        if (py::isinstance<py::str>(py_prop_elem)) {
          prop_list_value.append(
              QString::fromStdString(py_prop_elem.cast<std::string>()));
        } else if (iprm_util_dir_klass_.has_value() &&
                   py::isinstance(py_prop_elem, iprm_util_dir_klass_.value())) {
          const auto dir_value = make_dir_value(py_prop_elem);
          if (!dir_value.isNull()) {
            prop_list_value.append(dir_value);
          }
        }
      }
      obj_properties[prop_name] = prop_list_value;
    } else if (py::isinstance<py::dict>(py_prop_value)) {
      const auto py_prop_dict_value = py_prop_value.cast<py::dict>();
      QVariantList prop_dict_value;
      for (const auto& [py_prop_elem_key, py_prop_elem_value] :
           py_prop_dict_value) {
        if (iprm_util_dir_klass_.has_value() &&
            py::isinstance(py_prop_elem_key, iprm_util_dir_klass_.value())) {
          const auto dir_value = make_dir_value(py_prop_elem_key);
          if (!dir_value.isNull()) {
            QVariantList prop_dir_list_value;
            if (py::isinstance<py::list>(py_prop_elem_value)) {
              const auto py_prop_elem_list_value =
                  py_prop_elem_value.cast<py::list>();
              prop_dir_list_value.reserve(
                  static_cast<qsizetype>(py_prop_elem_list_value.size()));
              for (const auto& py_prop_elem : py_prop_elem_list_value) {
                if (py::isinstance<py::str>(py_prop_elem)) {
                  prop_dir_list_value.append(
                      QString::fromStdString(py_prop_elem.cast<std::string>()));
                }
              }
            }
            prop_dict_value.append(QVariant::fromValue(
                QVariantPair{dir_value, prop_dir_list_value}));
          }
        }
      }
      obj_properties[prop_name] = prop_dict_value;
    }
  }
  ...
  for (const auto& objects : platform_ctx.objs_) {
    for (const auto& obj : objects) {
      if (static_cast<bool>(obj.type & TypeFlags::TARGET)) {
        ...
        gv_node_icons[obj.name] = obj.icon;
  ...
  for (auto& [platform_name, platform_ctx] : platforms_) {
    ...
    try {
      ...
      for (const auto& py_obj : py_objects) {
        objects.push_back(make_object(file_path, py_obj));
      }
...
APIBridgeThread::APIBridgeThread(const QDir& iprm_src_dir)
    : QThread(nullptr), interp_(), bridge_(iprm_src_dir) {
