...
ObjectsTableView::ObjectsTableView(QWidget* parent)
    : QTreeView(parent),
  ...
  connect(this, &ObjectsTableView::doubleClicked, this,
...
void ObjectsTableView::showEvent(QShowEvent* event) {
  header()->setStretchLastSection(false);
  header()->setSectionResizeMode(0, QHeaderView::ResizeMode::Stretch);
  header()->setSectionResizeMode(1, QHeaderView::ResizeMode::Stretch);
}

void ObjectsTableView::dragEnterEvent(QDragEnterEvent* event) {
  if (event->mimeData()->hasFormat("application/x-apiitem")) {
    event->acceptProposedAction();
  }
}

void ObjectsTableView::dragMoveEvent(QDragMoveEvent* event) {
  if (event->mimeData()->hasFormat("application/x-apiitem")) {
    event->acceptProposedAction();
  }
}

void ObjectsTableView::dropEvent(QDropEvent* event) {
...
void ObjectsTableView::load(const QHash<QString, PlatformFile>& file) {
