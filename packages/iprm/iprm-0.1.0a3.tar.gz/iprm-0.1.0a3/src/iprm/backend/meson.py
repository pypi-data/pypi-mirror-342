...
class Meson(ProjectModel):
    ...
    def _generate_project(self, project: Project):
        ...
        native_file_path.write_text(f'{self.current_generated_file_header}\n')
        ...
        if project.cpp_compiler_flag() == EMSCRIPTEN:
            ...
            cross_file_path.write_text(f'{self.current_generated_file_header}\n')
    ...
    def _generate_cpp_first_party(self, cpp: CppTarget):
        ...
        header_files = self.cpp_get_header_src_file_paths(cpp)
        source_files = self.cpp_get_source_src_file_paths(cpp)
        ...
        # TODO: Now that Qt content is generated via the ObjectGenerator plugin infra, need to determine if
        #  Meson can attached more source files after the initial declaration of the target. And if not,
        #  ObjectGenerator infrastructure will need to be refactored to be invoked inline backend infra so content can
        #  be placed in the backend-specific location
...
'''"""
    ...
    def _generate_cpp_third_party_git(self, cpp: CppTarget):
        ...
        if target_type_name == 'GTestThirdParty' and tag == 'v1.15.2':
            ...
            gtest_dbwrap.write_text(f"""{self.current_generated_file_header}
[wrap-file]
...
""")
            ...
            pybind11_dbwrap.write_text(f"""{self.current_generated_file_header}
[wrap-file]
