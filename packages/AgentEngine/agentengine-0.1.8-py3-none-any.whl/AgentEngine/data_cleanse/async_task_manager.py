...
logger = logging.getLogger("data_cleanse.async_task_manager")
...
class AsyncTaskManager:
    def __init__(self, task_store: TaskStore, worker_pool: 'CleanseWorkerPool'):
        """
    ...
    async def stop(self) -> None:
        ...
        logger.info("Async Task Manager stopped", 
                   extra={'task_id': ' ' * 36, 'stage': 'SHUTDOWN', 'source': 'async'})
        ...
        while True:
            try:
                ...
                logger.error(f"Error processing status update: {str(e)}", 
                           extra={'task_id': task_id if 'task_id' in locals() else ' ' * 36, 
                                 'stage': 'ERROR', 'source': 'async'})
