...
from AgentEngine.core.utils.tools_common_message import SearchResultTextMessage
from AgentEngine.core.utils.image_filter import AsyncImageProcessor, LabelSet
...
class EXASearchTool(Tool):
    ...
    def __init__(self, exa_api_key, observer: MessageObserver=None,
                    max_results=5,
                    is_model_summary=False,
                    lang='zh',
                    image_filter=False,
                    image_filter_model_path: Optional[str] = None,
    ...
    def forward(self, query: str) -> str:
        ...
        if self.image_filter and images_list_url:
            thread = threading.Thread(target=self._filter_images, args=(images_list_url, query, self.image_filter_model_Path))
            thread.daemon = True
            thread.start()
        else:
            if self.observer:
        ...
        try:
            ...
            async def filter_process():
                try:
                    ...
                    label_sets = [
                        LabelSet(
                            negative="logo or banner or background or advertisement or icon or avatar",
                            positive=query
                        ),
                    ]
                    ...
                    # 初始化图片处理器
                    try:
                        processor = AsyncImageProcessor(label_sets=label_sets,
                                                    model_path=modle_path_clip,
...
if __name__ == "__main__":
    try:
        o1 = MessageObserver()
        tool = EXASearchTool(exa_api_key="",
                             observer=o1,
                             max_results=5,
                             is_model_summary=False,
                             lang='zh',
                             image_filter=True,
                             image_filter_model_path=r"D:/clip-vit-base-patch32")
