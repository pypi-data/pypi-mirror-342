...
class AsyncImageProcessor:
    ...
    @staticmethod
    def _check_image_size(width: int, height: int, min_width: int = 200, min_height: int = 200) -> bool:
        """检查图片尺寸是否满足最小要求
    ...
    async def load_image(self, session: aiohttp.ClientSession, path: str) -> Optional[Image.Image]:
        ...
        try:
            ...
            if path.startswith('data:image'):
                ...
                if not self._check_image_size(image.size[0], image.size[1]):
                    return None
    ...
    async def download_batch(self, urls: List[str]) -> List[Tuple[str, Optional[Image.Image]]]:
        ...
        async with aiohttp.ClientSession(connector=connector, trust_env=True) as session:
            tasks = [self.load_image(session, url) for url in urls]
        ...
        for i, (url, image) in enumerate(zip(urls, images)):
            ...
            for label_set in self.label_sets:
                inputs = self.processor(
                    images=image, 
                    text=[label_set.negative, label_set.positive], 
                    return_tensors="pt", 
                    padding=True
                )
