...
class FinalAnswerFormatTool(Tool):
    ...
    inputs = {"query": {"type": "string", "description": "输入用户提问"},
              "search_result": {"type": "array", "description": "一个包含多个字符串的列表，每个字符串表示之前检索到的**原始信息**，直接给入检索之后的变量，不要擅自总结。"}}
    output_type = "string"
    ...
    def forward(self, query: str, search_result: List[str]) -> str:
        ...
        for search_str in search_result:
            search_result_json.extend(item for item in json.loads(search_str))

        search_result_concat = json.dumps(search_result_json, ensure_ascii=False)
