...
class DataBloomContext:
    ...
    def create_sqlalchemy_engine(self, source: Optional[str] = None, database: Optional[str] = None):
        ...
        if source is None:
            ...
            connection_string = f"trino://{engine['username']}:{engine['password']}@{engine['host']}:{engine['port']}/{engine['catalog']}"
    ...
    def read_data(self, 
        ...
        try:
            ...
            if query:
                ...
                # Read from table
                result = pd.read_sql_table(table_name=table_name, con=engine, columns=columns)
                engine.dispose()
                return result

        except Exception as e:
            logger.error(f"Error reading data from {database}.{table_name}: {str(e)}")
            raise
