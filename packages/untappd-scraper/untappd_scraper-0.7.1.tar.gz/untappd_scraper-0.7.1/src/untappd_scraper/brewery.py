...
from untappd_scraper.structs.web import WebActivityBeer, WebBreweryDetails
...
class Brewery:
    ...
    _brewery_details: WebBreweryDetails = field(init=False, repr=False)
    _activity_details: list[WebActivityBeer] = field(
    ...
    def __post_init__(self) -> None:
        ...
        self._brewery_details = brewery_details(self._soup, brewery_id=self.brewery_id)

        self._activity_details = brewery_activity(self._soup)
    ...
    def activity(self) -> list[WebActivityBeer]:
        ...
        """
        if not self._activity_details:  # pragma: no cover
            msg = "Brewery incomplete, no activity details"
    ...
    def __getattr__(self, name: str) -> Any:  # noqa: ANN401
        ...
        Returns:
            Any: attribute value
        """
        return getattr(self._brewery_details, name)
...
def brewery_details(soup: BeautifulSoup, *, brewery_id: int) -> WebBreweryDetails:
    ...
    # Got to remove the "Show Less" text
    desc_el = soup.select_one("div.desc div.beer-descrption-read-less")
    if desc_el:
...
def brewery_activity(soup: BeautifulSoup) -> list[WebActivityBeer]:
    ...
    for beer in soup.select("div.activity div.item"):
        ...
        purchased_el = beer.select_one("p.purchased")
