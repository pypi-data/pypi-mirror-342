...
from dlt.cli import echo as fmt, CliCommandException, pipeline_files as files_ops
from dlt.cli.init_command import (
    _clone_and_get_verified_sources_storage,
    init_pipeline_at_destination,
    DEFAULT_VERIFIED_SOURCES_REPO,
)
from dlt.common.destination.reference import DestinationReference
from dlt.cli.config_toml_writer import write_values
...
def is_verified_source(source_type: str) -> bool:
    # todo better named as this actually fetches?
    verified_sources_storage = _clone_and_get_verified_sources_storage(
        DEFAULT_VERIFIED_SOURCES_REPO
    )
    return source_type in files_ops.get_sources_names(
...
def add_source(
    ...
    # trying to import?
    source_category = None  # local, core, verified, template
    ...
    if source_category != "local" and source_category != "template":
        # handle core and verified sources with oss-init
        files_to_write, required_config, required_secrets, source_category = (
            init_pipeline_at_destination(
        ...
        try:
            source_module_path = SourceReference.find(
            ...
            source_module_path = next(
                (s for s in SourceReference.SOURCES.keys() if s.startswith(source_type)),
            )

    if source_category == "core" and eject_source:
        source_module_path = strip_base_path(source_module_path)

        # and emit warning that pipeline example script is not pointint there yet

    multiple_sources_available_message(source_module_path, project_state, source_type)

    project_yaml = project_state.dlt_yaml
    project_yaml["sources"] = project_yaml.get("sources") or {}
    project_state.dlt_yaml["sources"][source_name] = {"type": source_module_path}

    # add secrets
    # the oss-init command doesnt know about the source_name
    replace_source_name(required_secrets, source_name)
    replace_source_name(required_config, source_name)

    write_values(project_state.secrets_provider._config_toml, required_secrets.values(), False)
    write_values(project_state.config_provider._config_toml, required_config.values(), False)

    return source_name


def replace_source_name(
    config_values: Dict[str, WritableConfigValue], new_source_name: str
) -> None:
    """
    Adjusts the sections tuple in WritableConfigValue objects by replacing the last entry
    with the provided new_source_name.

    Args:
        config_values (dict_values): A collection of WritableConfigValue objects.
        new_source_name (str): The value to replace the last entry in the sections tuple.

    Returns:
        list: A list of updated WritableConfigValue objects with modified sections.
    """
    for config_name, config_value in config_values.items():
        if config_value.sections[0] == "sources" and len(config_value.sections) > 1:
            sections = list(config_value.sections)
            sections[1] = new_source_name
            config_values[config_name] = WritableConfigValue(
                config_value.name, config_value.hint, config_value.default_value, tuple(sections)
            )
...
def add_a_local_source(
    ...
    multiple_sources_available_message(source_type, project_state, source_type)
...
def strip_base_path(module_path: str, base_path: str = "dlt.sources.") -> str:
    """
...
def multiple_sources_available_message(
...
def get_available_source_templates(package: str = SOURCES_TEMPLATES_PATH) -> Dict[str, str]:
    ...
    for source_template in resources.contents(package=package):
