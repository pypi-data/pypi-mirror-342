...
class Tensor(object):
    ...
    def Sparsity(self):
        ...
        if o != 0:
            x = self._tab.Indirect(o + self._tab.Pos)
            from circle_schema.v0_9.circle.SparsityParameters import SparsityParameters
    ...
    def VariantTensors(self, j):
        ...
        if o != 0:
            ...
            from circle_schema.v0_9.circle.VariantSubType import VariantSubType
            obj = VariantSubType()
            obj.Init(self._tab.Bytes, x)
            return obj
        return None
...
def AddCompressionType(builder, compressionType): builder.PrependInt8Slot(10, compressionType, 0)
def TensorAddCompressionType(builder, compressionType):
    """This method is deprecated. Please switch to AddCompressionType."""
    return AddCompressionType(builder, compressionType)
def End(builder): return builder.EndObject()
def TensorEnd(builder):
    """This method is deprecated. Please switch to End."""
    return End(builder)
import circle_schema.v0_9.circle.QuantizationParameters
import circle_schema.v0_9.circle.SparsityParameters
import circle_schema.v0_9.circle.VariantSubType
try:
...
class TensorT(object):
    ...
    def __init__(self):
        ...
        self.quantization = None  # type: Optional[circle_schema.v0_9.circle.QuantizationParameters.QuantizationParametersT]
        self.isVariable = False  # type: bool
        self.sparsity = None  # type: Optional[circle_schema.v0_9.circle.SparsityParameters.SparsityParametersT]
        self.shapeSignature = None  # type: List[int]
        self.hasRank = False  # type: bool
        self.variantTensors = None  # type: List[circle_schema.v0_9.circle.VariantSubType.VariantSubTypeT]
        self.compressionType = 0  # type: int
    ...
    def _UnPack(self, tensor):
        ...
        if tensor.Quantization() is not None:
            self.quantization = circle_schema.v0_9.circle.QuantizationParameters.QuantizationParametersT.InitFromObj(tensor.Quantization())
        self.isVariable = tensor.IsVariable()
        if tensor.Sparsity() is not None:
            self.sparsity = circle_schema.v0_9.circle.SparsityParameters.SparsityParametersT.InitFromObj(tensor.Sparsity())
        ...
        if not tensor.VariantTensorsIsNone():
            ...
            for i in range(tensor.VariantTensorsLength()):
                if tensor.VariantTensors(i) is None:
                    ...
                    variantSubType_ = circle_schema.v0_9.circle.VariantSubType.VariantSubTypeT.InitFromObj(tensor.VariantTensors(i))
                    self.variantTensors.append(variantSubType_)
        self.compressionType = tensor.CompressionType()
    ...
    def Pack(self, builder):
        ...
        if self.shapeSignature is not None:
            if np is not None and type(self.shapeSignature) is np.ndarray:
                ...
                StartShapeSignatureVector(builder, len(self.shapeSignature))
        ...
        if self.variantTensors is not None:
            ...
            StartVariantTensorsVector(builder, len(self.variantTensors))
        ...
        if self.quantization is not None:
            AddQuantization(builder, quantization)
        AddIsVariable(builder, self.isVariable)
        if self.sparsity is not None:
            AddSparsity(builder, sparsity)
        if self.shapeSignature is not None:
            AddShapeSignature(builder, shapeSignature)
        AddHasRank(builder, self.hasRank)
        if self.variantTensors is not None:
            AddVariantTensors(builder, variantTensors)
        AddCompressionType(builder, self.compressionType)
        tensor = End(builder)
        return tensor
