...
def extract_natorbs_func(uargs, save=True) -> None:
    '''
    ...
    data = oe.LoewdinCompositionExtractor.extract(uargs.output_file)
...
def read_args(arg_list=None):
    ...
    print_natorbs = print_parser.add_parser(
        ...
        usage=ut.cstring('orto print natorbs <output_file> [options]', 'cyan'),
        formatter_class=argparse.RawTextHelpFormatter
    )
    print_natorbs._positionals.title = 'Mandatory Arguments'

    print_natorbs.set_defaults(
        func=lambda x: extract_natorbs_func(x, save=False)
    )
