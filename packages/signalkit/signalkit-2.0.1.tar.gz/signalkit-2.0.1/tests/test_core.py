...
def test_handler_receives_correct_args_kwargs() -> None:
    ...
    def handler(
        sender: Any, pos1: str, *args: Any, kw1: Optional[str] = None, **kwargs: Any
    ) -> str:
        nonlocal received_args, received_kwargs
        received_args = (pos1,) + args
        received_kwargs = {"kw1": kw1, **kwargs}
        return "received"

    # act
    signal.connect(handler)
    response = signal.send(signal, "p1_val", "p2_val", kw1="kw1_val", kw2="kw2_val")
...
async def test_async_handler_receives_correct_args_kwargs() -> None:
    ...
    response = await signal.send(
        signal, "p1_val", "p2_val", kw1="kw1_val", kw2="kw2_val"
    )
    # assert
    assert response == "received"
    assert received_args == ("p1_val", "p2_val")
