...
class CoolSignal(_CoolSignalBase):
    ...
    def _wrap_handler(func: _ReceiverCallable) -> _ReceiverCallable:
        ...
        def wrapper(sender: Any, **kwargs: Any) -> Tuple[Optional[int], Any]:
            ...
            try:
                ...
                call_pattern = (
                    "func(sender, *call_args, **final_kwargs)"
                    if has_sender_param
                    else "func(*call_args, **final_kwargs)"
                )
                err_msg = (
    ...
    def send(
        self, sender: Optional[Any] = None, *args: Any, **kwargs: Any
    ) -> Optional[Any]:
    ...
    def _wrap_handler(func: _ReceiverCallable) -> _ReceiverCallable:
        ...
        async def wrapper(sender: Any, **kwargs: Any) -> Tuple[Optional[int], Any]:
            ...
            try:
                ...
                if has_sender_param:
                    arg: Any = await func(sender, *call_args, **final_kwargs)
                else:
                    arg: Any = await func(*call_args, **final_kwargs)

            except TypeError as e:
                call_pattern = (
                    "await func(sender, *call_args, **final_kwargs)"
                    if has_sender_param
                    else "await func(*call_args, **final_kwargs)"
                )
                err_msg = (
                    f"Error calling handler '{getattr(func, '__name__', repr(func))}': {e}. "
                    f"Handler Signature: {sig}. Is bound: {is_bound_method}. Expects sender: {has_sender_param}. "
