...
class OpenDicCatalog(Catalog):
    ...
    def pretty_print_result(self, result: dict):
        ...
        def format_columns(df: pd.DataFrame):
            """
            Format dictionary columns in the dataframe
            """
            for col in df.columns:
                # Format dictionary columns to pretty JSON strings
                if df[col].apply(lambda x: isinstance(x, dict)).any():
                    df[col] = df[col].apply(lambda x: f"\n{json.dumps(x, indent=2)}" if isinstance(x, dict) else x)

                # Format multiline strings to preserve structure
                elif df[col].dtype == "object":
                    df[col] = df[col].apply(lambda x: f"\n{x}" if isinstance(x, str) and "\n" in x else x)
        ...
        if isinstance(response, list) and all(isinstance(item, dict) for item in response):
            ...
            list_expanded_df: pd.DataFrame = format_columns(list_response_df)
            return list_expanded_df

        elif isinstance(response, dict):
            dict_response_df: pd.DataFrame = pd.DataFrame([response])
            dict_expanded_df: pd.DataFrame = format_columns(dict_response_df)
