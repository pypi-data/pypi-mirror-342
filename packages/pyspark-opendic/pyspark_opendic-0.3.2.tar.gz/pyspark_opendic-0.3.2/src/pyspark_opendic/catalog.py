...
class OpenDicCatalog(Catalog):
    ...
    def sql(self, sqlText: str):
        ...
        show_mapping_for_object_and_platform_match = re.match(
            opendic_show_mapping_for_object_and_platform_pattern, query_cleaned, re.IGNORECASE
        )
    ...
    def pretty_print_result(self, result: dict):
        ...
        def expand_columns(df: pd.DataFrame):
            """
            Expand dictionary columns in the dataframe so they are not wrapped in a single column
            """
            COMPLEX_COLS = ["props", "properties"]
            for col in COMPLEX_COLS:
                if col in df.columns:
                    expanded_props = pd.json_normalize(df[col].tolist())
                    expanded_props.columns = [f"{col}_{col}" for col in expanded_props.columns]
                    df = pd.concat([df.drop(col, axis=1), expanded_props], axis=1)
            return df

        # Polaris-spec-compliant "good" responses, so objects or lists of objects
        if isinstance(response, list) and all(isinstance(item, dict) for item in response):
            list_response_df: pd.DataFrame = pd.DataFrame(response)
            # Expand dictionary columns in the dataframe. So they are not wrapped in a single column
            list_expanded_df: pd.DataFrame = expand_columns(list_response_df)
            return list_expanded_df

        elif isinstance(response, dict):
            dict_response_df: pd.DataFrame = pd.DataFrame([response])
            # Expand dictionary columns in the dataframe.
            dict_expanded_df: pd.DataFrame = expand_columns(dict_response_df)
            return dict_expanded_df
