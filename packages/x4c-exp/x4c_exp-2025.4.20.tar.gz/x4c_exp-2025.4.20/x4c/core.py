...
class XDataArray:
    ...
    def plot(self, title=None, figsize=None, ax=None, latlon_range=None, add_clabels=False, clevels=None, clabel_kwargs=None,
             ...
             coastline_zorder=99, coastline_width=1, site_markersizes=100, df_sites=None, colname_dict=None,
        ...
        if ndim == 2 and 'lat' in da.coords and 'lon' in da.coords:
            ...
            if add_clabels:
                ...
                #     'colors': 'k',
                # }
                # im = self.da.plot.contour(ax=ax, **_contour_kws)
            ...
            if 'ylabel' not in kws:
                ...
                if 'depth' in str(ylabel):
                    ax.invert_yaxis()
                    if 'z_t' in self.da.coords:
                        if self.da['z_t'].units == 'centimeters':
                            ax.set_yticks([0, 2e5, 4e5])
                        elif self.da['z_t'].units == 'km':
                            ax.set_yticks([0, 2, 4])
                    else:
                        ax.set_yticks([0, 2e5, 4e5])
