...
from hestia_earth.utils.tools import flatten, list_sum, safe_parse_float, safe_parse_date, get_dict_key
...
def validate_list_min_max_lookup(node: dict, list_key: list, list_key_field='value'):
    def validate(values: tuple):
        ...
        mininum = safe_parse_float(get_table_value(lookup, 'termid', term_id, column_name('minimum')), None)
        maximum = safe_parse_float(get_table_value(lookup, 'termid', term_id, column_name('maximum')), None)
...
def _get_term_recalculated_max_delta(term: dict):
    ...
    return safe_parse_float(
        get_table_value(download_lookup(f"{term.get('termType')}.csv"), 'termid', term_id, column_name(col_name)),
        5
    ) / 100
