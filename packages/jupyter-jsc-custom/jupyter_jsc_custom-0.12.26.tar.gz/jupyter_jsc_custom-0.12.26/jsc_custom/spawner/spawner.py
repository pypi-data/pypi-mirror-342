...
from .utils import recursive_share_replace
...
async def custom_user_options(spawner, user_options):
    ...
    if "share_id" in user_options.keys():
        original_data = UserOptionsShares.find_by_share_id(
            spawner.user.db, user_options["share_id"]
        )
        user_options = await recursive_share_replace(
            user_options.get("secret_keys", []),
            user_options,
            original_data.user_options,
        )
