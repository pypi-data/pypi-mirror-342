...
def check_answer(user_ans, correct_ans):
    tokens_a = user_ans.lower().split()
    tokens_b = correct_ans.lower().split()
    similarity = 1 - jaccard_distance(set(tokens_a), set(tokens_b))
    similarityArray.append(similarity)

for i in range(1, CountofQuestions + 1):
    check_answer(UserAnswer[str(i)], CorrectAnswer[str(i)])

print(f"Your score is {sum(similarityArray):.2f}")
''')
...
while True:
    ...
    if user_input.lower() == "q":
        break
    check_ans = input("Compare data: ")
    similarity = SequenceMatcher(None, user_input, check_ans).ratio()
...
while True:
    user_input = input("Enter product to add (or 'q' to quit): ").lower()
    if user_input == "q":
        break
...
total = sum(available_prod[prod] for prod in shop_cart)
if total > wallet_bal:
...
print(f"Remaining Balance: {wallet_bal}")
''')
...
def find_nearby_attractions(location): 
    attractions = {
        ...
        "Tokyo": ["Tokyo Tower", "Senso-ji Temple", "Shinjuku Gyoen"],
    }
...
def find_nearby_restaurants(location): 
    restaurants = {
        ...
        "Paris": ["Le Comptoir", "La Tour", "L'Ambroisie"],
        "Tokyo": ["Sushi Dai", "Ichiran", "Ginza Kojyu"],
    }
...
def get_weather_info(location):
    weather = {
        ...
        "London": {"temperature": "20°C", "description": "Cloudy"},
        "Paris": {"temperature": "25°C", "description": "Clear"},
...
if __name__ == "__main__":
    while True:
        location = input("Enter location (or 'q' to quit): ")
        if location.lower() == 'q':
            break
        ...
        if info:
            ...
            print("Attractions:", ", ".join(find_nearby_attractions(location)))
            print("Restaurants:", ", ".join(find_nearby_restaurants(location)))
            weather = get_weather_info(location)
            print("Weather:", weather.get("temperature"), "-", weather.get("description"))
        else:
...
def print_timetable(timetable):
    for day in days:
        ...
        for time in timings:
            ...
            print(f"{time} - {slot['teacher']} - {slot['subject']}")
