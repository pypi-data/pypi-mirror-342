...
from ColorTransferLib.Algorithms.IIC.util.image_pool import ImagePool
from ColorTransferLib.Algorithms.IIC.util import util
from ColorTransferLib.Algorithms.IIC.models.base_model import BaseModel
...
class FusionModel(BaseModel):
    ...
    def get_current_imgs(self):
        out_img = torch.clamp(util.lab2rgb(torch.cat((self.full_real_A.type(torch.cuda.FloatTensor), self.fake_B_reg.type(torch.cuda.FloatTensor)), dim=1), self.opt), 0.0, 1.0)
        out_img = np.transpose(out_img.cpu().data.numpy()[0], (1, 2, 0))
        return img_as_ubyte(out_img)

    def setup_to_test(self, model_file_paths, fusion_weight_path):

        # GF_path = 'checkpoints/{0}/latest_net_GF.pth'.format(fusion_weight_path)
        GF_path = model_file_paths[fusion_weight_path + "_latest_net_GF.pth"]
        ...
        G_path = model_file_paths[fusion_weight_path + "_latest_net_G.pth"]
        ...
        GComp_path = model_file_paths[fusion_weight_path + "_latest_net_GComp.pth"]
