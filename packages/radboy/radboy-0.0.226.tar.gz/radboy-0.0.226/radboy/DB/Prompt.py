...
{Fore.light_red}Tax=(({Fore.cyan}CRV+{Fore.light_blue}Price)*{Fore.light_magenta}(Sales Tax Rate(0.0925))){Style.reset}
                ...
                with db.Session(db.ENGINE) as session:
                    ...
                    for num,i in enumerate(results):
                        ...
                        estimated_price=0
                        if i.Tax == 0 and i.CRV > 0:
                            estimated_price=(total*i.Price)+(i.CRV*total)
                            estimated_total+=estimated_price
                            msgx=f"""
No Sales Tax was used, but a CRV was present
Estimated_Price({estimated_price})=[Total({total})*Price({i.Price})]+[CRV({i.CRV})*Total({total})]
                            """
                            print(msgx)
                        else:
                            if i.Tax != 0 and i.CRV != 0:
                                estimated_price=((total*i.Price)+(i.Tax*total))
                                estimated_total+=estimated_price
                                msgx=f"""
    Estimated_Price({estimated_price})=[Total({total})*Price({i.Price})]+[Tax({i.Tax})*Total({total})]
    This is because CRV was calculated into the Tax
                                """
                                print(msgx)
                            else:
                                estimated_price=(total*i.Price)
                                msgx=f"""
    Estimated_Price({estimated_price})=[Total({total})*Price({i.Price})] 
    This is because there was No CRV and No Tax
                                """
                                print(msgx)
                    
                        msg+=f"""{estimated_total}:{Fore.light_magenta} |-|{Fore.light_green} Total = {Fore.light_sea_green}{round(total,3)}
...
{Fore.light_green}Estimated Total:{Fore.slate_blue_1}{round(estimated_total,3)}
                    {Style.reset}"""
                    print(msg)
