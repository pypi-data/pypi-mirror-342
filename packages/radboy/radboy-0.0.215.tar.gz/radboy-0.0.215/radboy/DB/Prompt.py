...
{Fore.light_green}Total Product Value Taxed w/ CRV({master_total_tax_crv}):{Fore.slate_blue_1}{round(master_total_tax_crv+master_total,3)}
                    {Style.reset}"""
                    print(msg)
            elif cmd.lower() in ["bldls2","build2","buildls2","build list 2","bld ls 2",'lsbld2','list build 2','ls bld2','bld2']:
                with db.Session(db.ENGINE) as session:
                    results_query=session.query(db.Entry).filter(db.Entry.InList==True)
                    location_fields=["Shelf","BackRoom","Display_1","Display_2","Display_3","Display_4","Display_5","Display_6","ListQty","SBX_WTR_DSPLY","SBX_CHP_DSPLY","SBX_WTR_KLR","FLRL_CHP_DSPLY","FLRL_WTR_DSPLY","WD_DSPLY","CHKSTND_SPLY","Distress"]
                    z=Prompt.mkfield_list(None,location_fields)
                    ...
                    for num,i in enumerate(results):
                        ...
                        tax=i.Tax
                        if tax == 0 and i.CRV > 0:
                            tax=i.CRV
                        final_price=((i.Price*total))+tax
                        print(final_price,i.seeShort())
                        master_total+=final_price
                    print(master_total)
