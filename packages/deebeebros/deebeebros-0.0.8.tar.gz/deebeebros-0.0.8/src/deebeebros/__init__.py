...
def print_help():
	print(f"How to use DeeBeeBros when your experiment is called {Css.bold()}my_exp.py{Css.reset()}: "
	      f"\n\t{Css.bold()}deebeebros -h{Css.reset()} for help"
	      f"\n\t{Css.bold()}deebeebros my_exp.py{Css.reset()} to run the test"
	      f"\n\t{Css.bold()}deebeebros my_exp.py -c{Css.reset()} to run the test, and clear terminal first"
	      f"\n\t{Css.bold()}deebeebros my_exp.py -v{Css.reset()} to run the test, with more info about errors"
	      f"\n\t{Css.bold()}deebeebros my_exp.py -t{Css.reset()} to run the test, with traceback for errors"
	      f"\n\t{Css.bold()}deebeebros my_exp.py -c -t -v{Css.reset()} to run the test, combining arguments"
      )
...
def run():
	...
	if experiment is None:
		print(f"{Css.wrong()}No valid experiment in command line args. Use: {Css.reset()}python deebeebros.py -h{Css.wrong()} for help{Css.reset()}")
		print_help()
		sys.exit(1)
	cwd = os.getcwd()
