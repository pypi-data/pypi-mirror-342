...
#   -----------------------------------------------------------------
    ...
    def itm_exchange_worker(self):
        while True:
            ...
            elif self.itm_meas_request and self.itm_request_channels and eval('self.pushButton_start_stop_'+ self.measuring_mode +'.text()') == 'Стоп':
    ...
    def start_measuring(self):
        ...
        self.counter_of_measurements = int(self.db_query("SELECT option_value FROM options WHERE option_name='counter_of_measurements'")[0][0])
        while self.db_query(f"SELECT COUNT(*) FROM measurements WHERE measurement_id={self.counter_of_measurements}")[0][0]>0: self.counter_of_measurements+=1
        ...
        if rowCount > 0:                                            # если в таблице уставки есть хоть одна уставка, то обрабатываем её.
            ...
            match self.measuring_mode:
                case 'auto':
                    ...
                    if self.measuring_mode == 'auto': self.auto_measuring_step_buttons(True)
                    return
                case 'manual':
    ...
    def do_measuring_step_body(self):
        ...
        if self.itm_meas_result:
            if self.itm_meas_result.find('Err')>-1: return
            for chan in self.itm_request_channels:
                t = extract_val_param(self.itm_meas_result, "valch"+str(chan))
                print('channel:', chan, self.itm_meas_result, t)
                if t == "errval=":
                    exec('self.textEdit_log_'+ self.measuring_mode +'.append("Нет связи с ИТМ:'+str(self.itm_meas_result)+'")')
                    t = -271    #random()*4+27
                ...
                r_or_e = extract_val_param(self.itm_meas_result, "dopch"+str(chan))
                if r_or_e == "errval=":
                    exec('self.textEdit_log_'+ self.measuring_mode +'.append("Нет связи с ИТМ:'+str(self.itm_meas_result)+'")')
            ...
            if self.measuring_mode == 'auto':
                ...
                if self.auto_measuring_step == 8:
                    self.timer_ustavka.stop()   #т.к. сложно точно вычислить время завешения измерений в авто. режиме, то переключаем уставку по счетчику измерений
                    self.do_ustavka_step()
        # if self.measuring_mode == 'auto':
        #     if self.auto_measuring_mode.find('TS')>-1:
        #         match self.auto_measuring_step%4:
        #             case 0:
        #                 result = exchange_data_itm("W,rewerseI=FF", self.portname_itm, self)
        #                 self.textEdit_log_auto.append('')
        #                 self.textEdit_log_auto.append("Измерение с прямым током: "+str(result))
        #             case 2:
        #                 result = exchange_data_itm("W,rewerseI=RW", self.portname_itm, self)
        #                 self.textEdit_log_auto.append('')
        #                 self.textEdit_log_auto.append("Измерение с обратным током: "+str(result))
    ...
    def progressbar_show_heating(self):
        ...
        exec("self.progressBar_"+str(self.ustavka_step)+".setValue("+str(self.timer_ustavka.interval())+")")
    ...
    def add_sensor_to_measuring_tab(self, selected_row, selected_col):
        ...
        sh_set_nsh_tp = self.db_query('SELECT option_value FROM options WHERE option_name="sh_set_nsh_tp"')
        sh_set_nsh_tr = self.db_query('SELECT option_value FROM options WHERE option_name="sh_set_nsh_tr"')
        for sensor_number in range(sensors_qty):
        ...
        if ok:
            ...
            sensor = self.db_query(f'SELECT * FROM sensors WHERE sensor_sn="{sensor_sn}"')
            match table.objectName():
                case "tableWidget_manual_channels_settings":
            ...
            if ish_set[0][0].find(sensor[0][4])>=0:                             # если выбранный датчик является эталонным
                ...
                for coef in range(len(coef_set)):
                    coef_data = self.db_query(f'SELECT ish_data_{coef_set[coef]} FROM ish_data WHERE ish_data_sensor="{sensor[0][0]}"')[0][0]
                    if coef_data:
            ...
            for measurement in range(eval(f'self.tableWidget_ustavka_{self.measuring_mode}.rowCount()')-1):
                ...
                if len(self.measured_data_channel[chan][0])>0:
                    ...
                    if chan != self.etalon_channel:
                        ...
                        sensor_type = self.db_query(f'SELECT sensor_type FROM sensors WHERE sensor_sn="{sensor_sn}"')[0][0]
                        sensor_class = self.db_query(f'SELECT sensor_class FROM sensors WHERE sensor_sn="{sensor_sn}"')[0][0]
                        print('sensor_type = ' + sensor_type)
                        gost_6616_id = int(self.db_query(f'SELECT gost_6616_id FROM gost_6616 WHERE gost_6616_sensor_type="{sensor_type}"')[0][0])
                        print('gost_6616_id = ' + str(gost_6616_id))
                        t_range = self.db_query(f'SELECT gost_6616_data_t_min, gost_6616_data_t_max FROM gost_6616_data WHERE gost_6616_data_gost_6616_id="{gost_6616_id}" AND gost_6616_data_class ="{sensor_class}"')
                        if self.auto_measuring_mode == 'verification_TS':
                            ...
                            U_0 = float(self.db_query(f'SELECT U0_data_val FROM U0_data WHERE U0_data_chan="{chan}" AND U0_data_R="{U0_data_R}" AND U0_data_t="{data_t}"')[0][0])
                            C1 = float(self.db_query(f'SELECT C1_data_val FROM C1_data WHERE C1_data_sensor_type="{sensor_type}" AND C1_data_t="{data_t}"')[0][0])
                            C2 = float(self.db_query(f'SELECT C2_data_val FROM C2_data WHERE C2_data_sensor_type="{sensor_type}" AND C2_data_t="{data_t}"')[0][0])
                        ...
                        for t in t_range:
                            if self.result[chan][measurement][0] > t[0] and self.result[chan][measurement][0] <= t[1]:
                                formula = self.db_query(f'SELECT gost_6616_data_formula FROM gost_6616_data WHERE gost_6616_data_gost_6616_id="{gost_6616_id}" AND gost_6616_data_class ="{sensor_class}" AND gost_6616_data_t_min ="{t[0]}" AND gost_6616_data_t_max ="{t[1]}"')
                                print(formula)
    ...
    def save_measurement_data(self):
        ...
        if self.db_query(f"SELECT * FROM measurements WHERE measurement_id='{str(self.counter_of_measurements)}'"): # если это измерение уже существует, то мы обновляем поля
            exec(f'self.pushButton_save_data_{self.measuring_mode}.setText("Обновление...")')
            table_fields = ('measurement_id', 'measurement_type', 'measurement_datetime', 'measurement_customer', 'measurement_operator', 'measurement_megaohmmeter_model', 'measurement_megaohmmeter_sn', 'measurement_t', 'measurement_p', 'measurement_h')
            for i in range(len(data_tuple)-4):  # 4 - т.к. количество уставок тоже не меняется (последнее поле), то заменяем поля с 4-го по предпоследнее
                self.db_query(f"UPDATE measurements SET {table_fields[i+3]}='{str(data_tuple[i+3])}' WHERE measurement_id='{str(self.counter_of_measurements)}';", "write")
            ...
            self.db_query("INSERT INTO measurements (measurement_id, measurement_type, measurement_datetime, measurement_customer, measurement_operator, measurement_megaohmmeter_model, measurement_megaohmmeter_sn, measurement_t, measurement_p, measurement_h, measurement_ustavlka_count) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?);", "write", data_tuple)
            measurement_data_id = int(self.db_query("SELECT option_value FROM options WHERE option_name='counter_of_measurements_data'")[0][0])
            while self.db_query(f"SELECT COUNT(*) FROM measurements_data WHERE measurement_data_id={measurement_data_id}")[0][0]>0: measurement_data_id+=1
            measurement_result_id = int(self.db_query("SELECT option_value FROM options WHERE option_name='counter_of_measurements_result'")[0][0])
            while self.db_query(f"SELECT COUNT(*) FROM measurements_result WHERE measurement_result_id={measurement_result_id}")[0][0]>0: measurement_result_id+=1
            for chan in range (8):
                if len(self.measured_data_channel[chan][0]):
                    ...
                    for i in range(len(self.measured_data_channel[chan][0])):
                        ...
                        self.db_query("INSERT INTO measurements_data (measurement_data_id, measurement_data_measurement, measurement_data_sensor, measurement_data_datetime, measurement_data_t, measurement_data_r_or_e, measurement_ustavka_number) VALUES (?, ?, ?, ?, ?, ?, ?);", "write", data_tuple)
                    if self.measuring_mode == 'auto':
                        ...
                        for measurement in range(eval('self.tableWidget_ustavka_' + self.measuring_mode + '.rowCount()')-1):
                            ...
                            data_tuple = (  measurement_result_id,
                                            ...
                                            ext_inspection_notes,                                                                       # ext_inspection_notes
                                            dielectric_strenght,                                                                        # dielectric_strenght
                                            r_isolation,                                                                                # r_isolation
                                            r_pins,                                                                                     # r_pins
                                            hk,                                                                                         # hk компенсация холодных концов
                                            result                                                                                      # result
                                            )
                            self.db_query("INSERT INTO measurements_result (measurement_result_id, measurement_result_measurement_id, measurement_result_sensor_id, measurement_result_t_mid, measurement_result_r_or_e_mid, measurement_result_delta_t_etalon, measurement_result_delta_t_dop, measurement_result_u_itm, measurement_result_t_ustavka, measurement_result_ustavka_number, measurement_result_ext_inspection_notes, measurement_result_dielectric_strenght, measurement_result_r_isolation, measurement_result_r_pins, measurement_result_hk, measurement_result) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?);", "write", data_tuple)
            self.db_query(f"UPDATE options SET option_value='{str(measurement_data_id)}' WHERE option_name='counter_of_measurements_data'", "write")
            self.textEdit_log_manual.append(f'Измерение {self.counter_of_measurements} сохранено.')
            self.db_query(f"UPDATE options SET option_value='{str(measurement_result_id)}' WHERE option_name='counter_of_measurements_result'", "write")
            self.db_query(f"UPDATE options SET option_value='{str(self.counter_of_measurements)}' WHERE option_name='counter_of_measurements'", "write")
    ...
    def make_protocol(self):
        ...
        protocol_file_name = shutil.copyfile(f'{os.path.dirname(__file__)}\\templates\\{template}', f'{os.path.dirname(__file__)}\\{template}')
        protocol = Document(protocol_file_name)
        if self.measuring_mode == 'manual': measuring_mode = self.measuring_mode
        else: measuring_mode = self.auto_measuring_mode
        measurement = self.db_query(f'SELECT * FROM measurements WHERE measurement_id="{self.counter_of_measurements}"')[0]
        print (measurement)
        measurement_result = self.db_query(f'SELECT * FROM measurements_result WHERE measurement_result_measurement_id="{self.counter_of_measurements}"')[0]
        print (measurement_result)

        # replace_data = ['measurement_id',,
        #                 'measurement_date',]
    ...
    def coef_edit(self):
        ...
        coef_val=self.db_query("SELECT ish_data_"+coef+" FROM ish_data WHERE ish_data_sensor="+str(ish_data_sensor))[0][0]
        if coef_val is None:
            coef_val = 0
        else:
        ...
        if ok:
            self.db_query("UPDATE ish_data SET ish_data_"+coef+"="+str(coef_val)+" WHERE ish_data_sensor="+str(ish_data_sensor), "write")
...
class Confidence_error_dialog(QDialog):
    ...
    def accept_data(self):
        self.MainWindow.db_query(f'DELETE FROM confidence_error WHERE confidence_error_sensor_id="{str(self.ish_data_sensor)}";', 'write')
        ...
        for row in range (rowCount):
            ...
            self.MainWindow.db_query(f'INSERT INTO confidence_error (confidence_error_sensor_id, confidence_error_t, confidence_error_delta_ets, confidence_error_instability_ets) VALUES (?, ?, ?, ?);', 'write', data)
        self.close()
    ...
    def setupUi(self, Dialog):
        ...
        rowCount = self.MainWindow.db_query(f'SELECT COUNT(*) FROM confidence_error WHERE confidence_error_sensor_id="{str(self.ish_data_sensor)}"')[0][0]
        self.tableWidget_confidence_error.setRowCount(rowCount)
        if rowCount > 0:
            delta_data = self.MainWindow.db_query(f'SELECT * FROM confidence_error WHERE confidence_error_sensor_id="{str(self.ish_data_sensor)}"')
            for row in range(rowCount):
                for col in range(3):
...
class Sensor_edit_dialog(QDialog):
    def __init__(self, MainWindow, selected_row):
        ...
        types = []
        for sh in all_types:
            self.count_of_types.append(len(MainWindow.db_query(f"SELECT option_value FROM options WHERE option_name='{sh}'")[0][0].split(',')))
            for i in MainWindow.db_query(f"SELECT option_value FROM options WHERE option_name='{sh}'")[0][0].split(','):
                types.append(i)
        ...
        if selected_row >= 0:
            ...
            self.sensor = MainWindow.db_query("SELECT * FROM sensors WHERE sensor_sn='"+sensor_sn+"'")
    ...
    def change_sensor_classes(self):
        ...
        classes = []
        for i in MainWindow.db_query(f"SELECT gost_6616_classes FROM gost_6616 WHERE gost_6616_sensor_type='{sensor_type}'")[0][0].split(','): classes.append(i)
    ...
    def chanhe_sensor_range(self):
        sensor_gost_id = MainWindow.db_query(f"SELECT gost_6616_id FROM gost_6616 WHERE gost_6616_sensor_type='{self.comboBox_sensor_type.currentText()}'")[0][0]
        sensor_class = self.comboBox_sensor_class.currentText()
        if not sensor_class: return
        # print(sensor_gost_id)
        # print(f"SELECT gost_6616_data_t_min FROM gost_6616_data WHERE gost_6616_data_gost_6616_id='{sensor_gost_id}' AND gost_6616_data_class='{sensor_class}'")
        class_gost_t_min = MainWindow.db_query(f"SELECT gost_6616_data_t_min FROM gost_6616_data WHERE gost_6616_data_gost_6616_id='{sensor_gost_id}' AND gost_6616_data_class='{sensor_class}'")[0][0]
        class_gost_t_max = MainWindow.db_query(f"SELECT gost_6616_data_t_max FROM gost_6616_data WHERE gost_6616_data_gost_6616_id='{sensor_gost_id}' AND gost_6616_data_class='{sensor_class}'")[0][0]
    ...
    def delete_data(self, selected_row, sensor_id):
        ...
        MainWindow.db_query(f'DELETE from sensors WHERE sensor_id ="{str(sensor_id)}";','write')
        if self.comboBox_sensor_type.currentText() == "ЭТС" or self.comboBox_sensor_type.currentText() == "ППО" or self.comboBox_sensor_type.currentText() == "ПРО":
