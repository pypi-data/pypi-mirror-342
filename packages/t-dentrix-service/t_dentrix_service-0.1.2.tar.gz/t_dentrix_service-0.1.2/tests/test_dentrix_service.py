...
class TestDentrixService:
    ...
    def test_gets_unattached_procedures_correctly(self, mock_get_problem_data: MagicMock):
        ...
        mock_get_problem_data.return_value = {"procedures": []}
        test_ds = self._generate_dentrix_service_instance()
    ...
    def test_gets_unsent_claims_correctly(self, mock_get_problem_data: MagicMock):
        ...
        mock_get_problem_data.return_value = {"claims": []}
        test_ds = self._generate_dentrix_service_instance()
    ...
    def test_get_schedules(self, mocked_location: MagicMock, mocked_schedules: MagicMock):
        """Test for get_schedules method."""
        TEST_LOC_ID = 9000000003333
        EPOCH_TIMESTAMP = 0

        def epoch_check_side_effect(timestamp, location_id):
            if timestamp == EPOCH_TIMESTAMP and location_id == TEST_LOC_ID:
        ...
        beginning_of_epoch = date(1970, 1, 1)

        ds = self._generate_dentrix_service_instance()
        result = ds.get_schedules(beginning_of_epoch)

        assert result == MockData.SCHEDULES
        mocked_location.assert_called_once()
        mocked_schedules.assert_called_once_with(EPOCH_TIMESTAMP, TEST_LOC_ID)
    ...
    def test_get_patient_ledger_transactions_guarantor_view(self, mock_get_ledger_list):
        ...
        result = test_ds.get_patient_ledger_transactions(view, patient_id)
