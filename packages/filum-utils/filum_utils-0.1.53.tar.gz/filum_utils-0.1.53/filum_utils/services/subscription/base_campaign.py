...
class BaseCampaignSubscriptionService(SubscriptionService):
    def __init__(
        ...
        self.action = action
        self.smart_distribution = glom(self.subscription, "data.smart_distribution", default={}) or {}
    ...
    def _validate_segment_user_file_name(self, segment_user_file_name: str):
        expected_segment_user_file_name = self.subscription_data.get("segment_user_file_name")
        if segment_user_file_name == expected_segment_user_file_name:
            return

        raise BaseError(
            message=ErrorMessage.MISMATCH_SEGMENT_USER_FILE_NAME
        )
    ...
    def _handle_initial_segment_trigger(
        ...
        if is_smart_config_enabled:
            ...
            self._handling_upload_metadata_groups_file(google_storage_client, metadata_groups)

        segment_user_file_name = self._handling_upload_segment_users_file(google_storage_client, users, properties)
    ...
    def _handling_upload_segment_users_file(
        ...
        google_storage_client.upload_csv_file(
            file_name=segment_user_file_name,
            headers=properties,
            rows=users
        )
    ...
    def _handling_upload_metadata_groups_file(
        ...
        """
        keys = list(metadata_groups.keys())

        google_storage_client.upload_csv_file(
            file_name=self._get_csv_file_name("metadata_groups"),
            headers=keys,
            rows=[metadata_group for metadata_group in metadata_groups.values()]
        )
    ...
    def _handle_publish_subscription(
        ...
        update_subscription_data_error_message = self._update_subscription_data_with_try_except(request_data)
