...
from thinknet_variable_validator import validate_and_check_format_datetime

def test_validate_and_check_format_datetime():
    dt = "2024-01-01T07:00:00Z"
    result = validate_and_check_format_datetime(dt)
    assert result == dt
...
def test_non_str_variable():
    ...
    with pytest.raises(ApplicationSpecificException) as excinfo:
        validate_and_check_format_datetime(dt)
...
def test_wrong_datetime_format_variable():
    ...
    assert excinfo.value.error_code == "UTV03"
