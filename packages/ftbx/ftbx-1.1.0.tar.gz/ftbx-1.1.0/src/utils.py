...
def merge_script_in_object_config(script_path: str, object_config: dict) -> None:
    ...
    with open(script_path, "r") as groovy_file:
        script_content = re.sub(r"import\s+.*?PluginCommand\n", "", groovy_file.read()).strip()
        ...
        script_content = script_content[: last_char - 1].replace(
...
def update_toolbox_resources():
    ...
    try:
        ...
        if "bitbucket.org" in result.stdout:
            ...
            temp_known_hosts = os.path.join(os.path.dirname(known_hosts_path), "temp_known_hosts")
            ...
            if hashed_fingerprints not in existing_content:
                with open(known_hosts_path, "a") as known_hosts_file:
                    ...
                    logger.info("Added hashed bitbucket.org fingerprints to SSH known hosts.")
            else:
                logger.info("bitbucket.org fingerprints are already in SSH known hosts.")
        ...
        # flex-icons
        shutil.copytree(
            os.path.join(os.path.expanduser("~"), ".ftbx", "flex-toolbox", "flex-icons"),
            os.path.join(os.path.expanduser("~"), ".ftbx", "flex-icons"),
            dirs_exist_ok=True,
        )
        logger.info("flex-icons have been updated successfully ('~/.ftbx/flex-icons/'). ")
        ...
        logger.info("flex-templates have been updated successfully ('~/.ftbx/templates/').")
        ...
        logger.warning(f"Could not fetch icons nor templates due to an error: {e}. This will prevent you from rendering workflow graphs as images and using the `ftbx create` commands.")
