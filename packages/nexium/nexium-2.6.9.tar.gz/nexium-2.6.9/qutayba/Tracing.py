...
from qutayba.__past__ import (  # pylint: disable=redefined-builtin
    BrokenPipeError,
    raw_input,
)
from qutayba.utils.Utils import isWin32Windows
...
class OurLogger(object):
    ...
    def _warnMnemonic(mnemonic, style, output_function):
        if mnemonic.startswith("http"):
            ...
            url = "https://qutayba.net/info/%s.html" % mnemonic
    ...
    def sysexit(
        ...
        from qutayba.Progress import closeProgressBar
...
general = OurLogger("nexium")
plugins_logger = OurLogger("nexium-Plugins")
recursion_logger = OurLogger("nexium-Inclusion")
progress_logger = OurLogger("nexium-Progress", quiet=True)
memory_logger = OurLogger("nexium-Memory")
metadata_logger = OurLogger("nexium-Metadata")
optimization_logger = FileLogger("nexium-Optimization")
pgo_logger = FileLogger("nexium-PGO")
code_generation_logger = OurLogger("nexium-CodeGen")
inclusion_logger = FileLogger("nexium-Inclusion")
scons_logger = OurLogger("nexium-Scons")
scons_details_logger = OurLogger("nexium-Scons")
postprocessing_logger = OurLogger("nexium-Postprocessing")
options_logger = OurLogger("nexium-Options")
unusual_logger = OurLogger("nexium-Unusual")
data_composer_logger = OurLogger("nexium-DataComposer")
onefile_logger = OurLogger("nexium-Onefile")
tools_logger = OurLogger("nexium-Tools")
wheel_logger = OurLogger("nexium-Wheel", base_style="blue")
cache_logger = OurLogger("nexium-Cache")
reports_logger = OurLogger("nexium-Reports")
offsets_logger = OurLogger("nexium-Offsets")
