...
from qutayba import Tracing
from qutayba.PythonVersions import isPythonWithGil
from qutayba.Tracing import general
from qutayba.utils.Importing import importFromInlineCopy
from qutayba.utils.ThreadedExecutor import RLock
...
class nexiumProgressBar(object):
...
def setupProgressBar(stage, unit, total, min_total=0):
    ...
    if use_progress_bar:
        Tracing.progress = nexiumProgressBar(
            iterable=None,
            stage=stage,
            total=total,
            min_total=min_total,
...
@contextmanager
def withnexiumDownloadProgressBar(*args, **kwargs):
    if not use_progress_bar or _getTqdmModule() is None:
        yield
    else:

        class nexiumDownloadProgressBar(tqdm):
        ...
        with nexiumDownloadProgressBar(*args, **kwargs) as progress_bar:
