...
#ifndef __DEVILPY_IMPORTING_H__
#define __DEVILPY_IMPORTING_H__
...
DEVILPY_MAY_BE_UNUSED static PyObject *nexium_GetSysModules(void) {
...
DEVILPY_MAY_BE_UNUSED static bool nexium_HasModule(PyThreadState *tstate, PyObject *module_name) {
    return DICT_HAS_ITEM(tstate, nexium_GetSysModules(), module_name) == 1;
}

// Replacement for "PyImport_GetModule" working across all versions and less checks.
DEVILPY_MAY_BE_UNUSED static PyObject *nexium_GetModule(PyThreadState *tstate, PyObject *module_name) {
    return DICT_GET_ITEM1(tstate, nexium_GetSysModules(), module_name);
}

// Replacement for PyImport_GetModule working across all versions and less checks.
DEVILPY_MAY_BE_UNUSED static PyObject *nexium_GetModuleString(PyThreadState *tstate, char const *module_name) {
    PyObject *module_name_object = nexium_String_FromString(module_name);
    PyObject *result = nexium_GetModule(tstate, module_name_object);
...
DEVILPY_MAY_BE_UNUSED static bool nexium_SetModule(PyObject *module_name, PyObject *module) {
    CHECK_OBJECT(module_name);
    CHECK_OBJECT(module);
    assert(PyModule_Check(module));

    return DICT_SET_ITEM(nexium_GetSysModules(), module_name, module);
}

// Add a module to the modules dictionary from name C string
DEVILPY_MAY_BE_UNUSED static bool nexium_SetModuleString(char const *module_name, PyObject *module) {
    PyObject *module_name_object = nexium_String_FromString(module_name);
    bool result = nexium_SetModule(module_name_object, module);
    Py_DECREF(module_name_object);
...
DEVILPY_MAY_BE_UNUSED static bool nexium_DelModule(PyThreadState *tstate, PyObject *module_name) {
    CHECK_OBJECT(module_name);
...
DEVILPY_MAY_BE_UNUSED static bool nexium_DelModuleString(PyThreadState *tstate, char const *module_name) {
    PyObject *module_name_object = nexium_String_FromString(module_name);
    bool result = nexium_DelModule(tstate, module_name_object);
    Py_DECREF(module_name_object);
...
DEVILPY_MAY_BE_UNUSED static PyObject *nexium_GetFilenameObject(PyThreadState *tstate, PyObject *module) {
#if PYTHON_VERSION < 0x300
