...
#ifndef __DEVILPY_ALLOCATOR_H__
#define __DEVILPY_ALLOCATOR_H__
...
#if PYTHON_VERSION < 0x270 && !defined(__DEVILPY_NO_ASSERT__)
...
extern void initnexiumAllocators(void);

// Our version of "PyObject_Malloc".
DEVILPY_MAY_BE_UNUSED static void *nexiumObject_Malloc(size_t size) { return python_obj_malloc(python_obj_ctx, size); }

// Our version of "PyMem_Malloc".
DEVILPY_MAY_BE_UNUSED static void *nexiumMem_Malloc(size_t size) { return python_mem_malloc(python_mem_ctx, size); }

// Our version of "PyMem_Calloc".
DEVILPY_MAY_BE_UNUSED static void *nexiumMem_Calloc(size_t nelem, size_t elsize) {
    return python_mem_calloc(python_mem_ctx, nelem, elsize);
}

#ifndef Py_GIL_DISABLED
// Our version of "PyMem_Realloc".
DEVILPY_MAY_BE_UNUSED static void *nexiumMem_Realloc(void *ptr, size_t new_size) {
    return python_mem_realloc(python_mem_ctx, ptr, new_size);
}
#else
DEVILPY_MAY_BE_UNUSED static void nexiumMem_Free(void *ptr) { python_mem_free(python_mem_ctx, ptr); }
#endif

#else
#define nexiumObject_Malloc(size) PyObject_MALLOC(size)
#define nexiumMem_Malloc(size) PyMem_MALLOC(size)
#define nexiumMem_Calloc(elem, elsize) PyMem_Calloc(elem, elsize)
#ifndef Py_GIL_DISABLED
#if defined(_WIN32)
// On Windows, mixing different runtime DLLs can cause issues at
// release, so we need to go through the API to get the proper
// DLL runtime.
#define nexiumMem_Realloc(ptr, new_size) PyMem_Realloc(ptr, new_size)
#else
#define nexiumMem_Realloc(ptr, new_size) PyMem_REALLOC(ptr, new_size)
#endif
#else
#define nexiumMem_Free(ptr) PyMem_Free(ptr)
#endif
#endif

#if PYTHON_VERSION >= 0x380 && PYTHON_VERSION < 0x3c0
// Need to make Py_DECREF a macro again that doesn't call an API
static inline void _nexium_Py_DECREF(PyObject *ob) {
...
#undef Py_DECREF
#define Py_DECREF(ob) _nexium_Py_DECREF((PyObject *)(ob))

// Need to make Py_XDECREF a macro again that doesn't call an API
static inline void _nexium_Py_XDECREF(PyObject *ob) {
    if (ob != NULL) {
...
#undef Py_XDECREF
#define Py_XDECREF(ob) _nexium_Py_XDECREF((PyObject *)(ob))
...
#define Py_CLEAR(op)                                                                                                   \
    ...
    !defined(Py_GIL_DISABLED) && !defined(_DEVILPY_EXPERIMENTAL_DISABLE_PY_DECREF_OVERRIDE)
...
#elif defined(_DEVILPY_DEBUG_DEBUG_IMMORTAL)
...
static inline void nexium_Py_NewReferenceNoTotal(PyObject *op) { Py_SET_REFCNT(op, 1); }
static inline void nexium_Py_NewReference(PyObject *op) {
#ifdef Py_REF_DEBUG
#if PYTHON_VERSION < 0x3c0
    _Py_RefTotal++;
#else
...
#else
#define nexium_Py_NewReference(op) _Py_NewReference(op)
#endif

static inline int nexium_PyType_HasFeature(PyTypeObject *type, unsigned long feature) {
    return ((type->tp_flags & feature) != 0);
}

#if PYTHON_VERSION >= 0x3b0

static inline size_t nexium_PyType_PreHeaderSize(PyTypeObject *tp) {
    return _PyType_IS_GC(tp) * sizeof(PyGC_Head) +
           nexium_PyType_HasFeature(tp, Py_TPFLAGS_MANAGED_DICT) * 2 * sizeof(PyObject *);
}

extern void nexium_PyObject_GC_Link(PyObject *op);

static PyObject *nexium_PyType_AllocNoTrackVar(PyTypeObject *type, Py_ssize_t nitems) {
    ...
    const size_t pre_size = nexium_PyType_PreHeaderSize(type);
    assert(pre_size == sizeof(PyGC_Head));

    char *alloc = (char *)nexiumObject_Malloc(size + pre_size);
    assert(alloc);
    ...
    if (nexium_PyType_HasFeature(type, Py_TPFLAGS_HEAPTYPE)) {
        Py_INCREF(type);
    }

    nexium_Py_NewReference(obj);

    return obj;
}

static PyObject *nexium_PyType_AllocNoTrack(PyTypeObject *type) {
    // TODO: This ought to be static for all our types, so remove it as a call.
    const size_t pre_size = nexium_PyType_PreHeaderSize(type);

    char *alloc = (char *)nexiumObject_Malloc(_PyObject_SIZE(type) + pre_size);
    assert(alloc);
    PyObject *obj = (PyObject *)(alloc + pre_size);
...
DEVILPY_MAY_BE_UNUSED static void *nexium_GC_NewVar(PyTypeObject *type, Py_ssize_t nitems) {
    assert(nitems >= 0);
    ...
    nexium_Py_NewReference((PyObject *)op);

    return op;
#else
    // TODO: We ought to inline this probably too, no point as a separate function.
    PyObject *op = nexium_PyType_AllocNoTrackVar(type, nitems);
#endif
    assert(Py_SIZE(op) == nitems);
    return op;
}

DEVILPY_MAY_BE_UNUSED static void *nexium_GC_New(PyTypeObject *type) {
#if PYTHON_VERSION < 0x3b0
...
#else
    // TODO: We ought to inline this probably too, no point as a separate function.
    PyObject *op = nexium_PyType_AllocNoTrack(type);
#endif
    return op;
}

static bool inline nexium_GC_IS_TRACKED_X(PyObject *object) {
