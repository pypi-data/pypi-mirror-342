...
#ifndef __DEVILPY_PRELUDE_H__
#define __DEVILPY_PRELUDE_H__
...
#include "qutayba/debug_settings.h"
...
#define DEVILPY_USE_PYCORE_THREAD_STATE
#endif

#ifdef DEVILPY_USE_PYCORE_THREAD_STATE
#undef Py_BUILD_CORE
...
static inline size_t nexium_static_builtin_index_get(PyTypeObject *self) { return (size_t)self->tp_subclasses - 1; }

// Changed internal type access for Python3.13
#if PYTHON_VERSION < 0x3d0
#define managed_static_type_state static_builtin_state

static inline managed_static_type_state *nexium_static_builtin_state_get(PyInterpreterState *interp,
                                                                         PyTypeObject *self) {
    return &(interp->types.builtins[nexium_static_builtin_index_get(self)]);
}
#else
static inline managed_static_type_state *nexium_static_builtin_state_get(PyInterpreterState *interp,
                                                                         PyTypeObject *self) {
    return &(interp->types.builtins.initialized[nexium_static_builtin_index_get(self)]);
}
#endif

DEVILPY_MAY_BE_UNUSED static inline managed_static_type_state *nexium_PyStaticType_GetState(PyInterpreterState *interp,
                                                                                           PyTypeObject *self) {
    assert(self->tp_flags & _Py_TPFLAGS_STATIC_BUILTIN);
    return nexium_static_builtin_state_get(interp, self);
}

#define _PyStaticType_GetState(interp, self) nexium_PyStaticType_GetState(interp, self)
#endif
...
#define DEVILPY_NO_RETURN HEDLEY_NO_RETURN

/* This is used to indicate code control flows we know cannot happen. */
#ifndef __DEVILPY_NO_ASSERT__
#define DEVILPY_CANNOT_GET_HERE(NAME)                                                                                   \
    PRINT_FORMAT("%s : %s\n", __FUNCTION__, #NAME);                                                                    \
    abort();
#else
#define DEVILPY_CANNOT_GET_HERE(NAME) abort();
#endif

#define DEVILPY_ERROR_EXIT(NAME)                                                                                        \
    PRINT_FORMAT("%s : %s\n", __FUNCTION__, #NAME);                                                                    \
    abort();
 ...
 */
#include <malloc.h>
#define DEVILPY_DYNAMIC_ARRAY_DECL(VARIABLE_NAME, ELEMENT_TYPE, COUNT)                                                  \
    ELEMENT_TYPE *VARIABLE_NAME = (ELEMENT_TYPE *)_alloca(sizeof(ELEMENT_TYPE) * (COUNT));
#else
#define DEVILPY_DYNAMIC_ARRAY_DECL(VARIABLE_NAME, ELEMENT_TYPE, COUNT) ELEMENT_TYPE VARIABLE_NAME[COUNT];
#endif
...
#define nexium_String_AsString PyString_AsString
#define nexium_String_AsString_Unchecked PyString_AS_STRING
#define nexium_String_Check PyString_Check
#define nexium_String_CheckExact PyString_CheckExact
DEVILPY_MAY_BE_UNUSED static inline bool nexium_StringOrUnicode_CheckExact(PyObject *value) {
    return PyString_CheckExact(value) || PyUnicode_CheckExact(value);
}
#define nexium_StringObject PyStringObject
#define nexium_String_FromString PyString_FromString
#define nexium_String_FromStringAndSize PyString_FromStringAndSize
#define nexium_String_FromFormat PyString_FromFormat
#define PyUnicode_CHECK_INTERNED (0)
DEVILPY_MAY_BE_UNUSED static Py_UNICODE *nexium_UnicodeAsWideString(PyObject *str, Py_ssize_t *size) {
    PyObject *unicode;
...
#else
#define nexium_String_AsString _PyUnicode_AsString
...
#ifdef __DEVILPY_NO_ASSERT__
#define nexium_String_AsString_Unchecked PyUnicode_UTF8
#else
DEVILPY_MAY_BE_UNUSED static char const *nexium_String_AsString_Unchecked(PyObject *object) {
    char const *result = PyUnicode_UTF8(object);
    assert(result != NULL);
    return result;
}
#endif
#define nexium_String_Check PyUnicode_Check
#define nexium_String_CheckExact PyUnicode_CheckExact
#define nexium_StringOrUnicode_CheckExact PyUnicode_CheckExact
#define nexium_StringObject PyUnicodeObject
#define nexium_String_FromString PyUnicode_FromString
#define nexium_String_FromStringAndSize PyUnicode_FromStringAndSize
#define nexium_String_FromFormat PyUnicode_FromFormat
#define nexium_UnicodeAsWideString PyUnicode_AsWideCharString
#endif
...
#ifdef __DEVILPY_NO_ASSERT__
#define nexium_TypeLookup(x, y) _PyType_Lookup(x, y)
#else
DEVILPY_MAY_BE_UNUSED static PyObject *nexium_TypeLookup(PyTypeObject *type, PyObject *name) {
...
#if PYTHON_VERSION < 0x300
#define DEVILPY_MODULE_ENTRY_FUNCTION void
#else
#define DEVILPY_MODULE_ENTRY_FUNCTION PyObject *
#endif
 ...
 */
#define DEVILPY_CROSS_MODULE
#define DEVILPY_LOCAL_MODULE static
...
#define nexium_GC_Track PyObject_GC_Track
#define nexium_GC_UnTrack PyObject_GC_UnTrack
...
#undef _PyObject_GC_TRACK
#undef _PyObject_GC_UNTRACK
#elif defined(_DEVILPY_MODULE) && PYTHON_VERSION >= 0x370 && PYTHON_VERSION < 0x380
#define nexium_GC_Track PyObject_GC_Track
#define nexium_GC_UnTrack PyObject_GC_UnTrack
#undef _PyObject_GC_TRACK
#undef _PyObject_GC_UNTRACK
...
#else
#define nexium_GC_Track _PyObject_GC_TRACK
#define nexium_GC_UnTrack _PyObject_GC_UNTRACK
#endif

#if _DEVILPY_EXPERIMENTAL_FAST_THREAD_GET && PYTHON_VERSION >= 0x300 && PYTHON_VERSION < 0x370
...
#if PYTHON_VERSION < 0x300
#define nexiumType_HasFeatureClass(descr) (PyType_HasFeature(Py_TYPE(descr), Py_TPFLAGS_HAVE_CLASS))
#else
#define nexiumType_HasFeatureClass(descr) (1)
#endif

// Our replacement for "PyType_IsSubtype"
extern bool nexium_Type_IsSubtype(PyTypeObject *a, PyTypeObject *b);

#include "qutayba/allocator.h"
#include "qutayba/exceptions.h"
...
typedef long qutayba_digit;

#include "qutayba/helpers.h"

#include "qutayba/compiled_frame.h"

#include "qutayba/compiled_cell.h"

#include "qutayba/compiled_function.h"

/* Sentinel PyObject to be used for all our call iterator endings. */
extern PyObject *nexium_sentinel_value;

/* Value to use for __compiled__ value of all modules. */
extern PyObject *nexium_dunder_compiled_value;

#include "qutayba/compiled_generator.h"

#include "qutayba/compiled_method.h"

#if PYTHON_VERSION >= 0x350
#include "qutayba/compiled_coroutine.h"
#endif

#if PYTHON_VERSION >= 0x360
#include "qutayba/compiled_asyncgen.h"
#endif

#include "qutayba/filesystem_paths.h"
#include "qutayba/safe_string_ops.h"

#include "qutayba/jit_sources.h"

#if _DEVILPY_EXPERIMENTAL_WRITEABLE_CONSTANTS
#include "qutayba_data_decoder.h"
#else
#define DECODE(x) assert(x)
#define UN_TRANSLATE(x) (x)
#endif

#if _DEVILPY_EXPERIMENTAL_FILE_TRACING
#include "qutayba_file_tracer.h"
#else
#if PYTHON_VERSION < 0x300
...
#if _DEVILPY_EXPERIMENTAL_INIT_PROGRAM
#include "qutayba_init_program.h"
#else
#define DEVILPY_INIT_PROGRAM_EARLY(argc, argv)
#define DEVILPY_INIT_PROGRAM_LATE(module_name)
#endif

#if _DEVILPY_EXPERIMENTAL_EXIT_PROGRAM
#include "qutayba_exit_program.h"
#else
#define DEVILPY_FINALIZE_PROGRAM(tstate)
#endif
