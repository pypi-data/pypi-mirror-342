...
#ifndef __DEVILPY_DICTIONARIES_H__
#define __DEVILPY_DICTIONARIES_H__
...
typedef PyDictEntry *nexium_DictEntryHandle;

static PyDictEntry *GET_STRING_DICT_ENTRY(PyDictObject *dict, nexium_StringObject *key) {
    assert(PyDict_CheckExact(dict));
    assert(nexium_String_CheckExact(key));
...
DEVILPY_MAY_BE_UNUSED static PyObject *GET_DICT_ENTRY_VALUE(nexium_DictEntryHandle handle) { return handle->me_value; }

DEVILPY_MAY_BE_UNUSED static void SET_DICT_ENTRY_VALUE(nexium_DictEntryHandle handle, PyObject *value) {
    handle->me_value = value;
}

static PyObject *GET_STRING_DICT_VALUE(PyDictObject *dict, nexium_StringObject *key) {
...
typedef PyObject **nexium_DictEntryHandle;

#if PYTHON_VERSION >= 0x3b0
extern Py_ssize_t nexium_Py_unicodekeys_lookup_unicode(PyDictKeysObject *dk, PyObject *key, Py_hash_t hash);

extern Py_ssize_t nexium_PyDictLookup(PyDictObject *mp, PyObject *key, Py_hash_t hash, PyObject ***value_addr);
extern Py_ssize_t nexium_PyDictLookupStr(PyDictObject *mp, PyObject *key, Py_hash_t hash, PyObject ***value_addr);
#endif

static nexium_DictEntryHandle GET_STRING_DICT_ENTRY(PyDictObject *dict, nexium_StringObject *key) {
    assert(PyDict_CheckExact(dict));
    assert(nexium_String_CheckExact(key));
...
#else
    PyObject **value;
    DEVILPY_MAY_BE_UNUSED Py_ssize_t found;

    // We cannot use nexium_PyDictLookupStr in all cases, some modules misbehave
    // and put non-strings in module dictionaries.
    if (unlikely(dict->ma_keys->dk_kind == DICT_KEYS_GENERAL)) {
        found = nexium_PyDictLookup(dict, (PyObject *)key, hash, &value);
    } else {
        found = nexium_PyDictLookupStr(dict, (PyObject *)key, hash, &value);
    }
...
DEVILPY_MAY_BE_UNUSED static PyObject *GET_DICT_ENTRY_VALUE(nexium_DictEntryHandle handle) { return *handle; }

DEVILPY_MAY_BE_UNUSED static void SET_DICT_ENTRY_VALUE(nexium_DictEntryHandle handle, PyObject *value) {
    *handle = value;
}

DEVILPY_MAY_BE_UNUSED static PyObject *GET_STRING_DICT_VALUE(PyDictObject *dict, nexium_StringObject *key) {
    nexium_DictEntryHandle handle = GET_STRING_DICT_ENTRY(dict, key);
...
DEVILPY_MAY_BE_UNUSED static bool DICT_SET_ITEM(PyObject *dict, PyObject *key, PyObject *value) {
    CHECK_OBJECT(dict);
...
DEVILPY_MAY_BE_UNUSED static bool DICT_REMOVE_ITEM(PyObject *dict, PyObject *key) {
...
#define _DEVILPY_MAINTAIN_DICT_VERSION_TAG 1
#endif

#if _DEVILPY_MAINTAIN_DICT_VERSION_TAG
extern uint64_t qutayba_dict_version_tag_counter;
#endif

DEVILPY_MAY_BE_UNUSED static void UPDATE_STRING_DICT0(PyDictObject *dict, nexium_StringObject *key, PyObject *value) {
    CHECK_OBJECT(value);

    nexium_DictEntryHandle entry = GET_STRING_DICT_ENTRY(dict, key);
...
#if _DEVILPY_MAINTAIN_DICT_VERSION_TAG
            dict->ma_version_tag = qutayba_dict_version_tag_counter++;
#endif
            CHECK_OBJECT(old);
...
DEVILPY_MAY_BE_UNUSED static void UPDATE_STRING_DICT_INPLACE(PyDictObject *dict, nexium_StringObject *key,
                                                            PyObject *value) {
    CHECK_OBJECT(value);
...
DEVILPY_MAY_BE_UNUSED static void UPDATE_STRING_DICT1(PyDictObject *dict, nexium_StringObject *key, PyObject *value) {
    CHECK_OBJECT(key);
    CHECK_OBJECT(value);
...
extern bool nexium_DictNext(PyObject *dict, Py_ssize_t *pos, PyObject **key_ptr, PyObject **value_ptr);

#if PYTHON_VERSION >= 0x3a0 && !defined(_DEVILPY_EXPERIMENTAL_DISABLE_FREELIST_ALL) &&                                  \
    !defined(_DEVILPY_EXPERIMENTAL_DISABLE_FREELIST_DICT)
#define DEVILPY_DICT_HAS_FREELIST 1

// Replacement for PyDict_New that is faster
extern PyObject *MAKE_DICT_EMPTY(PyThreadState *tstate);
#else
#define DEVILPY_DICT_HAS_FREELIST 0
