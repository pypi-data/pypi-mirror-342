...
from qutayba.Tracing import scons_details_logger, scons_logger
...
from qutayba.utils.Utils import hasMacOSIntelSupport, isMacOS
...
def _injectCcache(env, cc_path, python_prefix, assume_yes_for_downloads):
    ccache_binary = os.getenv("DEVILPY_CCACHE_BINARY")
    ...
    if ccache_binary is None:
        ...
        if ccache_binary is None:
            if hasMacOSIntelSupport():
                ...
                if tuple(int(d) for d in platform.release().split(".")) >= (18, 2):
                    url = "https://qutayba.net/ccache/v4.2.1/ccache-4.2.1.zip"
                    ...
                    ccache_binary = getCachedDownload(
                        ...
                        flatten=True,
                        binary="ccache",
                        message="nexium will make use of ccache to speed up repeated compilation.",
                        reject=None,
        ...
        scons_details_logger.info(
            "Using ccache '%s' from 'DEVILPY_CCACHE_BINARY' environment variable."
            % ccache_binary
        )
