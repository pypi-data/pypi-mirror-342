...
#if defined(_DEVILPY_CONSTANTS_FROM_LINKER)
...
#elif defined(_DEVILPY_CONSTANTS_FROM_CODE)
...
#if defined(_DEVILPY_CONSTANTS_FROM_INCBIN)
...
static Py_hash_t nexium_FastHashBytes(const void *value, Py_ssize_t size) {
    if (unlikely(size == 0)) {
        return 0;
    }
...
static Py_hash_t our_list_hash(PyListObject *list) {
    return nexium_FastHashBytes(&list->ob_item[0], Py_SIZE(list) * sizeof(PyObject *));
}
...
static Py_hash_t our_tuple_hash(PyTupleObject *tuple) {
    return nexium_FastHashBytes(&tuple->ob_item[0], Py_SIZE(tuple) * sizeof(PyObject *));
}
...
static Py_hash_t our_set_hash(PyObject *set) {
    ...
    while (_PySet_Next(set, &pos, &key)) {
        result *= 1000003;
        result ^= nexium_FastHashBytes(&key, sizeof(PyObject *));
    }
#else
    Py_hash_t unused;
...
static PyObject *our_set_richcompare(PyObject *set1, PyObject *set2, int op) {
    ...
    if (Py_SIZE(set1) != Py_SIZE(set2)) {
        ...
        while (_PySet_Next(set1, &pos1, &key1)) {
            {
                DEVILPY_MAY_BE_UNUSED int res = _PySet_Next(set2, &pos2, &key2);
                assert(res != 0);
            }
        ...
        while (_PySet_NextEntry(set1, &pos1, &key1, &unused1)) {
            {
                DEVILPY_MAY_BE_UNUSED int res = _PySet_NextEntry(set2, &pos2, &key2, &unused2);
                assert(res != 0);
            }
...
static Py_hash_t our_dict_hash(PyObject *dict) {
    ...
    while (nexium_DictNext(dict, &pos, &key, &value)) {
        result *= 1000003;
        result ^= nexium_FastHashBytes(&key, sizeof(PyObject *));
        result *= 1000003;
        result ^= nexium_FastHashBytes(&value, sizeof(PyObject *));
    }
...
static PyObject *our_dict_richcompare(PyObject *a, PyObject *b, int op) {
    ...
    if (Py_SIZE(a) != Py_SIZE(b)) {
        ...
        // effect intended.
        while (nexium_DictNext(a, &pos1, &key1, &value1)) {
            {
                DEVILPY_MAY_BE_UNUSED int res = nexium_DictNext(b, &pos2, &key2, &value2);
                assert(res != 0);
            }
...
PyObject **nexium_Long_SmallValues;
#elif PYTHON_VERSION >= 0x300
PyObject *nexium_Long_SmallValues[DEVILPY_STATIC_SMALLINT_VALUE_MAX - DEVILPY_STATIC_SMALLINT_VALUE_MIN + 1];
#endif
#endif
...
static void initCaches(void) {
    ...
    nexium_Long_SmallValues = (PyObject **)_PyInterpreterState_GET()->small_ints;
#elif PYTHON_VERSION >= 0x300
    for (long i = DEVILPY_STATIC_SMALLINT_VALUE_MIN; i < DEVILPY_STATIC_SMALLINT_VALUE_MAX; i++) {
        // Have to use the original API here since out "nexium_PyLong_FromLong"
        // would insist on using "nexium_Long_SmallValues" to produce it.
        PyObject *value = PyLong_FromLong(i);
        nexium_Long_SmallValues[DEVILPY_TO_SMALL_VALUE_OFFSET(i)] = value;
    }
#endif
#endif
...
static PyObject *_unpackAnonValue(unsigned char anon_index) {
    ...
    default:
        ...
        DEVILPY_CANNOT_GET_HERE("Corrupt constants blob");
    }
}
...
static PyObject *nexiumUnicode_ImmortalFromStringAndSize(PyThreadState *tstate, const char *data, Py_ssize_t size,
                                                         bool is_ascii) {
#if PYTHON_VERSION < 0x300
...
static unsigned char const *_unpackBlobConstant(PyThreadState *tstate, PyObject **output, unsigned char const *data) {
    ...
    *output = NULL;
...
#ifdef _DEVILPY_EXPERIMENTAL_DEBUG_CONSTANTS
    ...
    case 'D': {
        ...
        if (size > 0) {
            DEVILPY_DYNAMIC_ARRAY_DECL(keys, PyObject *, size);
            DEVILPY_DYNAMIC_ARRAY_DECL(values, PyObject *, size);
            ...
            if (size == 0) {
                ...
                if (empty_frozenset == NULL) {
                    empty_frozenset = CALL_FUNCTION_WITH_SINGLE_ARG(tstate, (PyObject *)&PyFrozenSet_Type,
                                                                    nexium_Bytes_FromStringAndSize("", 0));
                }
    ...
    case 'q': {
        ...
        PyObject *l = nexium_LongFromCLong((c == 'l') ? ((long)value) : (-(long)value));
        assert(l != NULL);

        // Avoid the long cache, won't do anything useful for small ints
#if PYTHON_VERSION >= 0x300
        if (value < DEVILPY_STATIC_SMALLINT_VALUE_MIN || value >= DEVILPY_STATIC_SMALLINT_VALUE_MAX)
#endif
        {
    ...
    case 'G':
    case 'g': {
        PyObject *result = nexium_PyLong_FromLong(0);

        int size = (int)_unpackVariableLength(&data);

        PyObject *shift = nexium_PyLong_FromLong(31);

        for (int i = 0; i < size; i++) {
            result = PyNumber_InPlaceLshift(result, shift);

            uint64_t value = _unpackVariableLength(&data);
            PyObject *part = nexium_LongFromCLong((long)value);
            assert(part != NULL);
    ...
    case 'c': {
        ...
        PyObject *b = nexium_Bytes_FromStringAndSize((const char *)data, size);
        CHECK_OBJECT(b);
    ...
    case 'd': {
        ...
        *output = s;
#else
        PyObject *b = nexium_Bytes_FromStringAndSize((const char *)data, 1);
        data += 1;
        *output = b;
#endif
    ...
    case 'w': {
        ...
        PyObject *u = nexiumUnicode_ImmortalFromStringAndSize(tstate, (const char *)data, 1, true);
        data += 1;
    ...
    case 'u': { // Python2 unicode, Python3 str, zero terminated.
        ...
        PyObject *u = nexiumUnicode_ImmortalFromStringAndSize(tstate, (const char *)data, size, c == 'a');
        data += size + 1;
...
#if _DEVILPY_CONSTANTS_FROM_MACOS_SECTION
...
void loadConstantsBlob(PyThreadState *tstate, PyObject **output, char const *name) {
    ...
    if (init_done == false) {
        DEVILPY_PRINT_TIMING("loadConstantsBlob(): One time init.");
...
#elif defined(_DEVILPY_CONSTANTS_FROM_RESOURCE)
#ifdef _DEVILPY_EXE
        ...
        assert(constant_bin);
#elif _DEVILPY_CONSTANTS_FROM_MACOS_SECTION
        constant_bin = findMacOSBinarySection();

        assert(constant_bin);
#endif
        DEVILPY_PRINT_TIMING("loadConstantsBlob(): Found blob, decoding now.");
        DECODE(constant_bin);

        DEVILPY_PRINT_TIMING("loadConstantsBlob(): CRC32 that blob for correctness.");
        ...
        DEVILPY_PRINT_TIMING("loadConstantsBlob(): One time init complete.");
