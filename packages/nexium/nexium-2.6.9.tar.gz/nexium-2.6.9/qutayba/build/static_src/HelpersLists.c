...
static _PyListArray *nexium_AllocateListArray(size_t capacity) {
...
static void nexium_FreeListArray(PyObject **items, bool use_qsbr) {
    _PyListArray *array = _Py_CONTAINER_OF(items, _PyListArray, ob_item);

    if (use_qsbr) {
        nexiumMem_FreeDelayed(array);
    } else {
        nexiumMem_Free(array);
    }
}

#endif

#if DEVILPY_LIST_HAS_FREELIST

PyObject *MAKE_LIST_EMPTY(PyThreadState *tstate, Py_ssize_t size) {
    assert(size >= 0);

#if _DEVILPY_EXPERIMENTAL_DISABLE_LIST_OPT
    ...
    if (*numfree) {
        ...
        nexium_Py_NewReference((PyObject *)result_list);
    } else {
        result_list = (PyListObject *)nexium_GC_New(&PyList_Type);
    }

    // Elements are allocated separately.
    if (size > 0) {
#ifdef Py_GIL_DISABLED
        _PyListArray *list_array = nexium_AllocateListArray(size);
...
#else
        result_list->ob_item = (PyObject **)nexiumMem_Calloc(size, sizeof(PyObject *));
...
static bool LIST_RESIZE(PyListObject *list, Py_ssize_t new_size) {
...
#ifdef Py_GIL_DISABLED
    _PyListArray *array = nexium_AllocateListArray(new_allocated);
    ...
    Py_SET_SIZE(list, new_size);
    if (old_items != NULL) {
        nexium_FreeListArray(old_items, _PyObject_GC_IS_SHARED(list));
    }
#else
    size_t num_allocated_bytes = new_allocated * sizeof(PyObject *);

    // TODO: For Python3.13 No-GIL this is different code
    PyObject **items = (PyObject **)nexiumMem_Realloc(list->ob_item, num_allocated_bytes);
...
bool LIST_REMOVE(PyObject *target, PyObject *item) {
...
#if _DEVILPY_EXPERIMENTAL_DISABLE_LIST_OPT && 0
    ...
    for (Py_ssize_t i = 0; i < cur_size; i++) {
        ...
        Py_INCREF(element);
        qutayba_bool cmp = RICH_COMPARE_EQ_NBOOL_OBJECT_OBJECT(element, item);
        Py_DECREF(element);
        ...
        if (cmp == DEVILPY_BOOL_TRUE) {
            ...
            return true;
        } else if (unlikely(cmp == DEVILPY_BOOL_EXCEPTION)) {
            return false;
        }
    }
...
#else
    // TODO: Actually to be fully correct, we will need a "nexium_LongFromCSsize_t" which
    // we do not have yet.
    return nexium_LongFromCLong((long)value);
#endif
}
...
PyObject *LIST_COUNT(PyObject *list, PyObject *item) {
    ...
    for (Py_ssize_t i = 0; i < Py_SIZE(list); i++) {
        ...
        Py_INCREF(element);
        qutayba_bool nbool_res = RICH_COMPARE_EQ_NBOOL_OBJECT_OBJECT(element, item);
        Py_DECREF(element);
...
PyObject *LIST_INDEX3(PyThreadState *tstate, PyObject *list, PyObject *item, PyObject *start) {
    ...
    PyObject *start_index = nexium_Number_IndexAsLong(start);
...
PyObject *LIST_INDEX4(PyThreadState *tstate, PyObject *list, PyObject *item, PyObject *start, PyObject *stop) {
    ...
    PyObject *stop_index = nexium_Number_IndexAsLong(stop);
...
bool LIST_INSERT(PyThreadState *tstate, PyObject *list, PyObject *index, PyObject *item) {
    ...
    PyObject *index_long = nexium_Number_IndexAsLong(index);
...
static void _nexium_ReverseObjectsSlice(PyObject **lo, PyObject **hi) {
...
void LIST_REVERSE(PyObject *list) {
    ...
    if (Py_SIZE(list_object) > 1) {
        _nexium_ReverseObjectsSlice(list_object->ob_item, list_object->ob_item + Py_SIZE(list_object));
    }
}

#if PYTHON_VERSION >= 0x300 && !defined(_DEVILPY_EXPERIMENTAL_DISABLE_LIST_OPT)
...
#if PYTHON_VERSION >= 0x300
    if (nexium_PyObject_HasLen(iterable)) {
        Py_ssize_t iter_len = nexium_PyObject_Size(iterable);
