...
#include "qutayba/compiled_method.h"
...
#if _DEBUG_REFCOUNTS
int count_active_nexium_Function_Type;
int count_allocated_nexium_Function_Type;
int count_released_nexium_Function_Type;
#endif

// tp_descr_get slot, bind a function to an object.
static PyObject *nexium_Function_descr_get(PyObject *function, PyObject *object, PyObject *class_object) {
    assert(nexium_Function_Check(function));
    ...
    return nexium_Method_New((struct nexium_FunctionObject *)function, object == Py_None ? NULL : object, class_object);
}

// tp_repr slot, decide how compiled function shall be output to "repr" built-in
static PyObject *nexium_Function_tp_repr(struct nexium_FunctionObject *function) {
    CHECK_OBJECT((PyObject *)function);
    assert(nexium_Function_Check((PyObject *)function));
    assert(_PyObject_GC_IS_TRACKED(function));

#if PYTHON_VERSION < 0x300
    return nexium_String_FromFormat("<compiled_function %s at %p>", nexium_String_AsString(function->m_name), function);
#else
    return nexium_String_FromFormat("<compiled_function %U at %p>", function->m_qualname, function);
#endif
}

static long nexium_Function_tp_traverse(struct nexium_FunctionObject *function, visitproc visit, void *arg) {
    CHECK_OBJECT((PyObject *)function);
    assert(nexium_Function_Check((PyObject *)function));
    assert(_PyObject_GC_IS_TRACKED(function));
...
static long nexium_Function_tp_hash(struct nexium_FunctionObject *function) {
    CHECK_OBJECT((PyObject *)function);
    assert(nexium_Function_Check((PyObject *)function));
    assert(_PyObject_GC_IS_TRACKED(function));
...
static PyObject *nexium_Function_get_name(PyObject *self, void *data) {
    CHECK_OBJECT(self);
    assert(nexium_Function_Check(self));
    assert(_PyObject_GC_IS_TRACKED(self));

    struct nexium_FunctionObject *function = (struct nexium_FunctionObject *)self;
...
static int nexium_Function_set_name(PyObject *self, PyObject *value, void *data) {
    CHECK_OBJECT(self);
    assert(nexium_Function_Check(self));
    assert(_PyObject_GC_IS_TRACKED(self));
...
#if PYTHON_VERSION >= 0x300
static PyObject *nexium_Function_get_qualname(PyObject *self, void *data) {
    CHECK_OBJECT(self);
    assert(nexium_Function_Check(self));
    assert(_PyObject_GC_IS_TRACKED(self));
...
static int nexium_Function_set_qualname(PyObject *self, PyObject *value, void *data) {
    CHECK_OBJECT(self);
    assert(nexium_Function_Check(self));
    assert(_PyObject_GC_IS_TRACKED(self));
    CHECK_OBJECT_X(value);
...
static PyObject *nexium_Function_get_doc(PyObject *self, void *data) {
    CHECK_OBJECT(self);
    assert(nexium_Function_Check(self));
    assert(_PyObject_GC_IS_TRACKED(self));
...
static int nexium_Function_set_doc(PyObject *self, PyObject *value, void *data) {
    CHECK_OBJECT(self);
    assert(nexium_Function_Check(self));
    assert(_PyObject_GC_IS_TRACKED(self));
    CHECK_OBJECT_X(value);
...
static PyObject *nexium_Function_get_dict(PyObject *self, void *data) {
    CHECK_OBJECT(self);
    assert(nexium_Function_Check(self));
    assert(_PyObject_GC_IS_TRACKED(self));
    ...
    if (function->m_dict == NULL) {
        DEVILPY_MAY_BE_UNUSED PyThreadState *tstate = PyThreadState_GET();
...
static int nexium_Function_set_dict(PyObject *self, PyObject *value, void *data) {
    CHECK_OBJECT(self);
    assert(nexium_Function_Check(self));
    assert(_PyObject_GC_IS_TRACKED(self));
    CHECK_OBJECT_X(value);
...
static PyObject *nexium_Function_get_code(PyObject *self, void *data) {
    CHECK_OBJECT(self);
    assert(nexium_Function_Check(self));
    assert(_PyObject_GC_IS_TRACKED(self));
...
static int nexium_Function_set_code(PyObject *self, PyObject *value, void *data) {
    CHECK_OBJECT(self);
    assert(nexium_Function_Check(self));
    assert(_PyObject_GC_IS_TRACKED(self));

    PyThreadState *tstate = PyThreadState_GET();
    SET_CURRENT_EXCEPTION_TYPE0_STR(tstate, PyExc_RuntimeError, "__code__ is not writable in nexium");
    return -1;
}

static PyObject *nexium_Function_get_compiled(PyObject *self, void *data) {
    CHECK_OBJECT(self);
    assert(nexium_Function_Check(self));
    assert(_PyObject_GC_IS_TRACKED(self));

    PyObject *result = nexium_dunder_compiled_value;
    CHECK_OBJECT(result);
...
static int nexium_Function_set_compiled(PyObject *self, PyObject *value, void *data) {
    CHECK_OBJECT(self);
    assert(nexium_Function_Check(self));
    assert(_PyObject_GC_IS_TRACKED(self));
...
static PyObject *nexium_Function_get_compiled_constant(PyObject *self, void *data) {
    CHECK_OBJECT(self);
    assert(nexium_Function_Check(self));
    assert(_PyObject_GC_IS_TRACKED(self));
...
static int nexium_Function_set_compiled_constant(PyObject *self, PyObject *value, void *data) {
    CHECK_OBJECT(self);
    assert(nexium_Function_Check(self));
    assert(_PyObject_GC_IS_TRACKED(self));
...
static PyObject *nexium_Function_get_closure(PyObject *self, void *data) {
    CHECK_OBJECT(self);
    assert(nexium_Function_Check(self));
    assert(_PyObject_GC_IS_TRACKED(self));
...
static int nexium_Function_set_closure(PyObject *self, PyObject *value, void *data) {
    CHECK_OBJECT(self);
    assert(nexium_Function_Check(self));
    assert(_PyObject_GC_IS_TRACKED(self));
...
static PyObject *nexium_Function_get_defaults(PyObject *self, void *data) {
    CHECK_OBJECT(self);
    assert(nexium_Function_Check(self));
    assert(_PyObject_GC_IS_TRACKED(self));
...
static void _onUpdatedCompiledFunctionDefaultsValue(struct nexium_FunctionObject *function) {
    CHECK_OBJECT((PyObject *)function);
    assert(nexium_Function_Check((PyObject *)function));
...
static int nexium_Function_set_defaults(PyObject *self, PyObject *value, void *data) {
    CHECK_OBJECT(self);
    assert(nexium_Function_Check(self));
    assert(_PyObject_GC_IS_TRACKED(self));
    CHECK_OBJECT_X(value);
...
#if PYTHON_VERSION >= 0x300
static PyObject *nexium_Function_get_kwdefaults(PyObject *self, void *data) {
    CHECK_OBJECT(self);
    assert(nexium_Function_Check(self));
    assert(_PyObject_GC_IS_TRACKED(self));
...
static int nexium_Function_set_kwdefaults(PyObject *self, PyObject *value, void *data) {
    CHECK_OBJECT(self);
    assert(nexium_Function_Check(self));
    assert(_PyObject_GC_IS_TRACKED(self));
    CHECK_OBJECT_X(value);
...
static PyObject *nexium_Function_get_annotations(PyObject *self, void *data) {
    CHECK_OBJECT(self);
    assert(nexium_Function_Check(self));
    assert(_PyObject_GC_IS_TRACKED(self));
...
static int nexium_Function_set_annotations(PyObject *self, PyObject *value, void *data) {
    CHECK_OBJECT(self);
    assert(nexium_Function_Check(self));
    assert(_PyObject_GC_IS_TRACKED(self));
...
static int nexium_Function_set_globals(PyObject *self, PyObject *value, void *data) {
    CHECK_OBJECT(self);
    assert(nexium_Function_Check(self));
    assert(_PyObject_GC_IS_TRACKED(self));
...
static PyObject *nexium_Function_get_globals(PyObject *self, void *data) {
    CHECK_OBJECT(self);
    assert(nexium_Function_Check(self));
    assert(_PyObject_GC_IS_TRACKED(self));
...
static int nexium_Function_set_builtins(PyObject *self, PyObject *value, void *data) {
    CHECK_OBJECT(self);
    assert(nexium_Function_Check(self));
    assert(_PyObject_GC_IS_TRACKED(self));
...
static PyObject *nexium_Function_get_builtins(PyObject *self, void *data) {
    CHECK_OBJECT(self);
    assert(nexium_Function_Check(self));
    assert(_PyObject_GC_IS_TRACKED(self));
...
static int nexium_Function_set_type_params(PyObject *self, PyObject *value, void *data) {
    CHECK_OBJECT(self);
    CHECK_OBJECT_X(self);
    assert(nexium_Function_Check(self));
    assert(_PyObject_GC_IS_TRACKED(self));
...
static PyObject *nexium_Function_get_type_params(PyObject *self, void *data) {
    CHECK_OBJECT(self);
    assert(nexium_Function_Check(self));
    assert(_PyObject_GC_IS_TRACKED(self));
...
static int nexium_Function_set_module(PyObject *self, PyObject *value, void *data) {
    CHECK_OBJECT(self);
    assert(nexium_Function_Check(self));
    assert(_PyObject_GC_IS_TRACKED(self));
    CHECK_OBJECT_X(value);
...
static PyObject *nexium_Function_get_module(PyObject *self, void *data) {
    CHECK_OBJECT(self);
    assert(nexium_Function_Check(self));
    assert(_PyObject_GC_IS_TRACKED(self));
...
static PyGetSetDef nexium_Function_getset[] = {
#if PYTHON_VERSION >= 0x300
    {(char *)"__qualname__", nexium_Function_get_qualname, nexium_Function_set_qualname, NULL},
#endif
#if PYTHON_VERSION < 0x300
    {(char *)"func_name", nexium_Function_get_name, nexium_Function_set_name, NULL},
#endif
    {(char *)"__name__", nexium_Function_get_name, nexium_Function_set_name, NULL},
#if PYTHON_VERSION < 0x300
    {(char *)"func_doc", nexium_Function_get_doc, nexium_Function_set_doc, NULL},
#endif
    {(char *)"__doc__", nexium_Function_get_doc, nexium_Function_set_doc, NULL},
#if PYTHON_VERSION < 0x300
    {(char *)"func_dict", nexium_Function_get_dict, nexium_Function_set_dict, NULL},
#endif
    {(char *)"__dict__", nexium_Function_get_dict, nexium_Function_set_dict, NULL},
#if PYTHON_VERSION < 0x300
    {(char *)"func_code", nexium_Function_get_code, nexium_Function_set_code, NULL},
#endif
    {(char *)"__code__", nexium_Function_get_code, nexium_Function_set_code, NULL},
#if PYTHON_VERSION < 0x300
    {(char *)"func_defaults", nexium_Function_get_defaults, nexium_Function_set_defaults, NULL},
#endif
    {(char *)"__defaults__", nexium_Function_get_defaults, nexium_Function_set_defaults, NULL},
#if PYTHON_VERSION < 0x300
    {(char *)"func_globals", nexium_Function_get_globals, nexium_Function_set_globals, NULL},
#endif
    {(char *)"__closure__", nexium_Function_get_closure, nexium_Function_set_closure, NULL},
#if PYTHON_VERSION < 0x300
    {(char *)"func_closure", nexium_Function_get_closure, nexium_Function_set_closure, NULL},
#endif
    {(char *)"__globals__", nexium_Function_get_globals, nexium_Function_set_globals, NULL},
    {(char *)"__module__", nexium_Function_get_module, nexium_Function_set_module, NULL},
#if PYTHON_VERSION >= 0x300
    {(char *)"__kwdefaults__", nexium_Function_get_kwdefaults, nexium_Function_set_kwdefaults, NULL},
    {(char *)"__annotations__", nexium_Function_get_annotations, nexium_Function_set_annotations, NULL},
#endif
#if PYTHON_VERSION >= 0x3a0
    {(char *)"__builtins__", nexium_Function_get_builtins, nexium_Function_set_builtins, NULL},
#endif
#if PYTHON_VERSION >= 0x3c0
    {(char *)"__type_params__", nexium_Function_get_type_params, nexium_Function_set_type_params, NULL},
#endif
    {(char *)"__compiled__", nexium_Function_get_compiled, nexium_Function_set_compiled, NULL},
    {(char *)"__compiled_constant__", nexium_Function_get_compiled_constant, nexium_Function_set_compiled_constant,
     NULL},
    {NULL}};

static PyObject *nexium_Function_reduce(struct nexium_FunctionObject *function, PyObject *unused) {
    CHECK_OBJECT((PyObject *)function);
    assert(nexium_Function_Check((PyObject *)function));
    assert(_PyObject_GC_IS_TRACKED(function));
...
static PyObject *nexium_Function_clone(struct nexium_FunctionObject *function, PyObject *unused) {
    CHECK_OBJECT((PyObject *)function);
    assert(nexium_Function_Check((PyObject *)function));
    assert(_PyObject_GC_IS_TRACKED(function));
...
#if PYTHON_VERSION >= 0x300
#if 0
    PRINT_STRING("nexium_Function_clone:");
    ...
    struct nexium_FunctionObject *result =
        nexium_Function_New(function->m_c_code, function->m_name,
#if PYTHON_VERSION >= 0x300
                            function->m_qualname,
#endif
...
static struct nexium_FunctionObject *free_list_functions = NULL;
static int free_list_functions_count = 0;

static void nexium_Function_tp_dealloc(struct nexium_FunctionObject *function) {
#if _DEBUG_REFCOUNTS
    count_active_nexium_Function_Type -= 1;
    count_released_nexium_Function_Type += 1;
#endif
...
static PyMethodDef nexium_Function_methods[] = {{"__reduce__", (PyCFunction)nexium_Function_reduce, METH_NOARGS, NULL},
                                                {"clone", (PyCFunction)nexium_Function_clone, METH_NOARGS, NULL},
                                                {NULL}};

static PyObject *nexium_Function_tp_call(struct nexium_FunctionObject *function, PyObject *tuple_args, PyObject *kw);

PyTypeObject nexium_Function_Type = {
    PyVarObject_HEAD_INIT(NULL, 0) "compiled_function", // tp_name
    sizeof(struct nexium_FunctionObject),               // tp_basicsize
    sizeof(struct nexium_CellObject *),                 // tp_itemsize
    (destructor)nexium_Function_tp_dealloc,             // tp_dealloc
#if PYTHON_VERSION < 0x380 || defined(_DEVILPY_EXPERIMENTAL_DISABLE_VECTORCALL_SLOT)
    0, // tp_print
#else
    offsetof(struct nexium_FunctionObject, m_vectorcall), // tp_vectorcall_offset
#endif
    0,                                    // tp_getattr
    0,                                    // tp_setattr
    0,                                    // tp_compare
    (reprfunc)nexium_Function_tp_repr,    // tp_repr
    0,                                    // tp_as_number
    0,                                    // tp_as_sequence
    0,                                    // tp_as_mapping
    (hashfunc)nexium_Function_tp_hash,    // tp_hash
    (ternaryfunc)nexium_Function_tp_call, // tp_call
    0,                                    // tp_str
    ...
    0,                                                  // tp_doc
    (traverseproc)nexium_Function_tp_traverse,          // tp_traverse
    0,                                                  // tp_clear
    0,                                                  // tp_richcompare
    offsetof(struct nexium_FunctionObject, m_weakrefs), // tp_weaklistoffset
    0,                                                  // tp_iter
    0,                                                  // tp_iternext
    nexium_Function_methods,                            // tp_methods
    0,                                                  // tp_members
    nexium_Function_getset,                             // tp_getset
    0,                                                  // tp_base
    0,                                                  // tp_dict
    nexium_Function_descr_get,                          // tp_descr_get
    0,                                                  // tp_descr_set
    offsetof(struct nexium_FunctionObject, m_dict),     // tp_dictoffset
    0,                                                  // tp_init
    0,                                                  // tp_alloc
    0,                                                  // tp_new
    0,                                                  // tp_free
...
void _initCompiledFunctionType(void) {
    nexium_PyType_Ready(&nexium_Function_Type, &PyFunction_Type, true, false, false, false, false);

    // Be a paranoid subtype of uncompiled function, we want nothing shared.
    assert(nexium_Function_Type.tp_doc != PyFunction_Type.tp_doc);
    assert(nexium_Function_Type.tp_traverse != PyFunction_Type.tp_traverse);
    assert(nexium_Function_Type.tp_clear != PyFunction_Type.tp_clear || PyFunction_Type.tp_clear == NULL);
    assert(nexium_Function_Type.tp_richcompare != PyFunction_Type.tp_richcompare ||
           PyFunction_Type.tp_richcompare == NULL);
    assert(nexium_Function_Type.tp_weaklistoffset != PyFunction_Type.tp_weaklistoffset);
    assert(nexium_Function_Type.tp_iter != PyFunction_Type.tp_iter || PyFunction_Type.tp_iter == NULL);
    assert(nexium_Function_Type.tp_iternext != PyFunction_Type.tp_iternext || PyFunction_Type.tp_iternext == NULL);
    assert(nexium_Function_Type.tp_methods != PyFunction_Type.tp_methods);
    assert(nexium_Function_Type.tp_members != PyFunction_Type.tp_members);
    assert(nexium_Function_Type.tp_getset != PyFunction_Type.tp_getset);
    assert(nexium_Function_Type.tp_dict != PyFunction_Type.tp_dict);
    assert(nexium_Function_Type.tp_descr_get != PyFunction_Type.tp_descr_get);

    assert(nexium_Function_Type.tp_descr_set != PyFunction_Type.tp_descr_set || PyFunction_Type.tp_descr_set == NULL);
    assert(nexium_Function_Type.tp_dictoffset != PyFunction_Type.tp_dictoffset);
    // TODO: These get changed and into the same thing, not sure what to compare against, project something
    // assert(nexium_Function_Type.tp_init != PyFunction_Type.tp_init || PyFunction_Type.tp_init == NULL);
    // assert(nexium_Function_Type.tp_alloc != PyFunction_Type.tp_alloc || PyFunction_Type.tp_alloc == NULL);
    // assert(nexium_Function_Type.tp_new != PyFunction_Type.tp_new || PyFunction_Type.tp_new == NULL);
    // assert(nexium_Function_Type.tp_free != PyFunction_Type.tp_free || PyFunction_Type.tp_free == NULL);
    assert(nexium_Function_Type.tp_bases != PyFunction_Type.tp_bases);
    assert(nexium_Function_Type.tp_mro != PyFunction_Type.tp_mro);
    assert(nexium_Function_Type.tp_cache != PyFunction_Type.tp_cache || PyFunction_Type.tp_cache == NULL);
    assert(nexium_Function_Type.tp_subclasses != PyFunction_Type.tp_subclasses || PyFunction_Type.tp_cache == NULL);
    assert(nexium_Function_Type.tp_weaklist != PyFunction_Type.tp_weaklist);
    assert(nexium_Function_Type.tp_del != PyFunction_Type.tp_del || PyFunction_Type.tp_del == NULL);
#if PYTHON_VERSION >= 0x300
    assert(nexium_Function_Type.tp_finalize != PyFunction_Type.tp_finalize || PyFunction_Type.tp_finalize == NULL);
#endif

    // Make sure we don't miss out on attributes we are not having or should not have.
#ifndef __DEVILPY_NO_ASSERT__
    for (struct PyGetSetDef *own = &nexium_Function_getset[0]; own->name != NULL; own++) {
        bool found = false;
        ...
        if (found == false) {
            ...
            DEVILPY_CANNOT_GET_HERE("Type problem");
        }
    }
...
// produce one.
static PyObject *_nexium_FunctionEmptyCodeNoneImpl(PyThreadState *tstate, struct nexium_FunctionObject const *function,
...
static PyObject *_nexium_FunctionEmptyCodeTrueImpl(PyThreadState *tstate, struct nexium_FunctionObject const *function,
                                                   PyObject **python_pars) {
    CHECK_OBJECT((PyObject *)function);
    assert(nexium_Function_Check((PyObject *)function));
    assert(_PyObject_GC_IS_TRACKED(function));
...
static PyObject *_nexium_FunctionEmptyCodeFalseImpl(PyThreadState *tstate, struct nexium_FunctionObject const *function,
                                                    PyObject **python_pars) {
    CHECK_OBJECT((PyObject *)function);
    assert(nexium_Function_Check((PyObject *)function));
    assert(_PyObject_GC_IS_TRACKED(function));
...
static PyObject *_nexium_FunctionEmptyCodeGenericImpl(PyThreadState *tstate,
                                                      struct nexium_FunctionObject const *function,
                                                      PyObject **python_pars) {
    CHECK_OBJECT((PyObject *)function);
    assert(nexium_Function_Check((PyObject *)function));
    assert(_PyObject_GC_IS_TRACKED(function));
...
void nexium_Function_EnableConstReturnTrue(struct nexium_FunctionObject *function) {
    function->m_constant_return_value = Py_True;
    function->m_c_code = _nexium_FunctionEmptyCodeTrueImpl;
}

void nexium_Function_EnableConstReturnFalse(struct nexium_FunctionObject *function) {
    function->m_constant_return_value = Py_False;
    function->m_c_code = _nexium_FunctionEmptyCodeFalseImpl;
}

void nexium_Function_EnableConstReturnGeneric(struct nexium_FunctionObject *function, PyObject *value) {
    function->m_constant_return_value = value;
    function->m_c_code = _nexium_FunctionEmptyCodeGenericImpl;
}

#ifdef _DEVILPY_PLUGIN_DILL_ENABLED
int nexium_Function_GetFunctionCodeIndex(struct nexium_FunctionObject *function,
                                         function_impl_code const *function_table) {

    if (function->m_c_code == _nexium_FunctionEmptyCodeTrueImpl) {
        return -2;
    }

    if (function->m_c_code == _nexium_FunctionEmptyCodeFalseImpl) {
        return -3;
    }

    if (function->m_c_code == _nexium_FunctionEmptyCodeNoneImpl) {
        return -4;
    }

    if (function->m_c_code == _nexium_FunctionEmptyCodeGenericImpl) {
        return -5;
    }
...
nexium_Function_CreateFunctionViaCodeIndex(PyObject *module, PyObject *function_qualname, PyObject *function_index,
    ...
    DEVILPY_DYNAMIC_ARRAY_DECL(closure_cells, struct nexium_CellObject *, closure_size);

    for (Py_ssize_t i = 0; i < closure_size; i++) {
        closure_cells[i] = nexium_Cell_New0(PyTuple_GET_ITEM(closure, i));
    }

    struct nexium_FunctionObject *result =
        nexium_Function_New(offset >= 0 ? function_table[offset] : NULL, code_object->co_name,
#if PYTHON_VERSION >= 0x300
    ...
    if (offset == -2) {
        nexium_Function_EnableConstReturnTrue(result);
    }
    if (offset == -3) {
        nexium_Function_EnableConstReturnFalse(result);
    }
    if (offset == -4) {
        result->m_c_code = _nexium_FunctionEmptyCodeNoneImpl;
    }
    if (offset == -5) {
        CHECK_OBJECT(constant_return_value);

        nexium_Function_EnableConstReturnGeneric(result, constant_return_value);
...
PyObject *nexium_Function_ExtractCodeObjectDescription(PyThreadState *tstate, struct nexium_FunctionObject *function) {
    PyObject *code_object_desc = MAKE_TUPLE_EMPTY(tstate, 8);

    PyTuple_SET_ITEM0(code_object_desc, 0, function->m_code_object->co_filename);
    PyTuple_SET_ITEM0(code_object_desc, 1, function->m_code_object->co_name);
    PyTuple_SET_ITEM(code_object_desc, 2, nexium_PyLong_FromLong(function->m_code_object->co_firstlineno));
...
#endif
    PyTuple_SET_ITEM(code_object_desc, 4, nexium_PyLong_FromLong(function->m_code_object->co_argcount));
    PyTuple_SET_ITEM(code_object_desc, 5, nexium_PyLong_FromLong(function->m_code_object->co_flags));

#if PYTHON_VERSION < 0x380
    PyTuple_SET_ITEM(code_object_desc, 6, const_int_0);
#else
    PyTuple_SET_ITEM(code_object_desc, 6, nexium_PyLong_FromLong(function->m_code_object->co_posonlyargcount));
#endif

#if PYTHON_VERSION < 0x3b0
    PyTuple_SET_ITEM(code_object_desc, 7, const_int_0);
#else
    PyTuple_SET_ITEM(code_object_desc, 7, nexium_PyLong_FromLong(function->m_code_object->co_kwonlyargcount));
#endif
...
#if PYTHON_VERSION >= 0x380 && !defined(_DEVILPY_EXPERIMENTAL_DISABLE_VECTORCALL_SLOT)
static PyObject *nexium_Function_tp_vectorcall(struct nexium_FunctionObject *function, PyObject *const *stack,
                                               size_t nargsf, PyObject *kw_names);
#endif

// Make a function with closure.
#if PYTHON_VERSION < 0x300
struct nexium_FunctionObject *nexium_Function_New(function_impl_code c_code, PyObject *name, PyCodeObject *code_object,
                                                  PyObject *defaults, PyObject *module, PyObject *doc,
                                                  struct nexium_CellObject **closure, Py_ssize_t closure_given)
#else
struct nexium_FunctionObject *nexium_Function_New(function_impl_code c_code, PyObject *name, PyObject *qualname,
...
#endif
{
#if _DEBUG_REFCOUNTS
    count_active_nexium_Function_Type += 1;
    count_allocated_nexium_Function_Type += 1;
#endif

    struct nexium_FunctionObject *result;

    // Macro to assign result memory from GC or free list.
    allocateFromFreeList(free_list_functions, struct nexium_FunctionObject, nexium_Function_Type, closure_given);
    ...
    result->m_varnames = nexium_GetCodeVarNames(code_object);
    ...
    static long nexium_Function_counter = 0;
    result->m_counter = nexium_Function_counter++;

#if PYTHON_VERSION >= 0x380 && !defined(_DEVILPY_EXPERIMENTAL_DISABLE_VECTORCALL_SLOT)
    result->m_vectorcall = (vectorcallfunc)nexium_Function_tp_vectorcall;
#endif
...
#if PYTHON_VERSION >= 0x300
static void formatErrorNoArgumentAllowedKwSplit(struct nexium_FunctionObject const *function, PyObject *kw_name,
                                                Py_ssize_t given) {
#if PYTHON_VERSION < 0x3a0
    char const *function_name = nexium_String_AsString(function->m_name);
#else
    char const *function_name = nexium_String_AsString(function->m_qualname);
#endif

    PyErr_Format(PyExc_TypeError, "%s() got an unexpected keyword argument '%s'", function_name,
                 nexium_String_AsString(kw_name));
}
#endif

static void formatErrorNoArgumentAllowed(struct nexium_FunctionObject const *function,
#if PYTHON_VERSION >= 0x300
                                         PyObject *kw,
#endif
                                         Py_ssize_t given) {
    ...
    if (kw == NULL) {
        ...
        PyErr_Format(PyExc_TypeError, "%s() got an unexpected keyword argument '%s'", function_name,
                     nexium_String_AsString(tmp_arg_name));
...
static void formatErrorMultipleValuesGiven(struct nexium_FunctionObject const *function, Py_ssize_t index) {
...
#endif
                 function_name, nexium_String_AsString(function->m_varnames[index]));
}

#if PYTHON_VERSION < 0x300
static void formatErrorTooFewArguments(PyThreadState *tstate, struct nexium_FunctionObject const *function,
...
static void formatErrorTooFewArguments(PyThreadState *tstate, struct nexium_FunctionObject const *function,
    ...
    PyErr_Format(PyExc_TypeError, "%s() missing %zd required positional argument%s: %s", function_name, max_missing,
                 max_missing > 1 ? "s" : "", nexium_String_AsString(list_str));

    Py_DECREF(list_str);
}
#endif

static void formatErrorTooManyArguments(struct nexium_FunctionObject const *function, Py_ssize_t given
#if PYTHON_VERSION < 0x270
                                        ,
                                        Py_ssize_t kw_size
...
#if PYTHON_VERSION >= 0x300
static void formatErrorTooFewKwOnlyArguments(PyThreadState *tstate, struct nexium_FunctionObject const *function,
                                             PyObject **kw_vars) {
#if PYTHON_VERSION < 0x3a0
    char const *function_name = nexium_String_AsString(function->m_name);
#else
...
static void formatErrorKeywordsMustBeString(PyThreadState *tstate, struct nexium_FunctionObject const *function) {
#if PYTHON_VERSION < 0x390
    char const *function_name = nexium_String_AsString(function->m_name);
...
static inline bool RICH_COMPARE_EQ_CBOOL_ARG_NAMES(PyObject *operand1, PyObject *operand2) {
...
#if PYTHON_VERSION < 0x300
    qutayba_bool result = RICH_COMPARE_EQ_NBOOL_STR_OBJECT(operand1, operand2);
#else
    qutayba_bool result = RICH_COMPARE_EQ_NBOOL_UNICODE_OBJECT(operand1, operand2);
#endif

    // Should be close to impossible, we will have to ignore it though.
    if (unlikely(result == DEVILPY_BOOL_EXCEPTION)) {
        PyThreadState *tstate = PyThreadState_GET();
    ...
    return result == DEVILPY_BOOL_TRUE;
}

#if PYTHON_VERSION < 0x300
static Py_ssize_t handleKeywordArgs(PyThreadState *tstate, struct nexium_FunctionObject const *function,
        ...
        if (unlikely(found == false)) {
            ...
            if (pos_only_error == true) {
                PyErr_Format(PyExc_TypeError,
                             ...
                             nexium_String_Check(key) ? nexium_String_AsString(key) : "<non-string>");
...
#if PYTHON_VERSION < 0x300
static Py_ssize_t handleKeywordArgsSplit(struct nexium_FunctionObject const *function, PyObject **python_pars,
...
static bool MAKE_STAR_DICT_DICTIONARY_COPY(PyThreadState *tstate, struct nexium_FunctionObject const *function,
                                           PyObject **python_pars, PyObject *kw) {
...
#if PYTHON_VERSION < 0x300
static Py_ssize_t handleKeywordArgsWithStarDict(PyThreadState *tstate, struct nexium_FunctionObject const *function,
                                                PyObject **python_pars, PyObject *kw)
#else
static Py_ssize_t handleKeywordArgsWithStarDict(PyThreadState *tstate, struct nexium_FunctionObject const *function,
                                                PyObject **python_pars, Py_ssize_t *kw_only_found, PyObject *kw)
...
#if PYTHON_VERSION < 0x300
static Py_ssize_t
handleKeywordArgsSplitWithStarDict(PyThreadState *tstate, struct nexium_FunctionObject const *function,
...
static void makeStarListTupleCopy(PyThreadState *tstate, struct nexium_FunctionObject const *function,
...
static void makeStarListTupleCopyMethod(PyThreadState *tstate, struct nexium_FunctionObject const *function,
                                        PyObject **python_pars, PyObject *const *args, Py_ssize_t args_size) {
    assert(function->m_args_star_list_index != -1);
    Py_ssize_t list_star_index = function->m_args_star_list_index;
...
static bool _handleArgumentsPlainOnly(PyThreadState *tstate, struct nexium_FunctionObject const *function,
                                      PyObject **python_pars, PyObject *const *args, Py_ssize_t args_size) {
...
static bool handleMethodArgumentsPlainOnly(PyThreadState *tstate, struct nexium_FunctionObject const *function,
...
#if PYTHON_VERSION < 0x270
static bool _handleArgumentsPlain(PyThreadState *tstate, struct nexium_FunctionObject const *function,
                                  PyObject **python_pars, PyObject *const *args, Py_ssize_t args_size,
...
static void releaseParameters(struct nexium_FunctionObject const *function, PyObject *const *python_pars) {
    Py_ssize_t arg_count = function->m_args_overall_count;
...
static bool parseArgumentsPos(PyThreadState *tstate, struct nexium_FunctionObject const *function,
                              PyObject **python_pars, PyObject *const *args, Py_ssize_t args_size) {
    bool result;
...
static bool parseArgumentsEmpty(PyThreadState *tstate, struct nexium_FunctionObject const *function,
                                PyObject **python_pars) {
    return parseArgumentsPos(tstate, function, python_pars, NULL, 0);
}

static bool parseArgumentsMethodPos(PyThreadState *tstate, struct nexium_FunctionObject const *function,
                                    PyObject **python_pars, PyObject *object, PyObject *const *args,
                                    Py_ssize_t args_size) {
    bool result;
...
static bool parseArgumentsFullKwSplit(PyThreadState *tstate, struct nexium_FunctionObject const *function,
                                      PyObject **python_pars, PyObject *const *args, Py_ssize_t args_size,
                                      PyObject *const *kw_values, PyObject *kw_names) {
...
static bool parseArgumentsFull(PyThreadState *tstate, struct nexium_FunctionObject const *function,
...
PyObject *nexium_CallFunctionNoArgs(PyThreadState *tstate, struct nexium_FunctionObject const *function) {
    DEVILPY_DYNAMIC_ARRAY_DECL(python_pars, PyObject *, function->m_args_overall_count);
...
PyObject *nexium_CallMethodFunctionPosArgsKwArgs(PyThreadState *tstate, struct nexium_FunctionObject const *function,
                                                 PyObject *object, PyObject *const *args, Py_ssize_t args_size,
                                                 PyObject *kw) {
    DEVILPY_DYNAMIC_ARRAY_DECL(new_args, PyObject *, args_size + 1);

    new_args[0] = object;
    memcpy(new_args + 1, args, args_size * sizeof(PyObject *));

    // TODO: Specialize implementation for massive gains.
    return nexium_CallFunctionPosArgsKwArgs(tstate, function, new_args, args_size + 1, kw);
}

static Py_ssize_t _handleVectorcallKeywordArgs(PyThreadState *tstate, struct nexium_FunctionObject const *function,
                                               PyObject **python_pars, Py_ssize_t *kw_only_found,
...
static bool MAKE_STAR_DICT_DICTIONARY_COPY38(PyThreadState *tstate, struct nexium_FunctionObject const *function,
...
static bool parseArgumentsVectorcall(PyThreadState *tstate, struct nexium_FunctionObject const *function,
                                     PyObject **python_pars, PyObject *const *args, Py_ssize_t args_size,
    ...
    if (unlikely(arg_count == 0 && function->m_args_simple && args_size + kw_size > 0)) {
        ...
        if (kw_size == 0) {
            ...
            PyErr_Format(PyExc_TypeError, "%s() got an unexpected keyword argument '%s'", function_name,
                         nexium_String_AsString(kw_names[0]));
        }
...
static PyObject *nexium_Function_tp_call(struct nexium_FunctionObject *function, PyObject *tuple_args, PyObject *kw) {
    ...
    if (kw == NULL) {
        ...
        return nexium_CallFunctionPosArgsKwArgs(tstate, function, &PyTuple_GET_ITEM(tuple_args, 0),
...
static PyObject *nexium_Function_tp_vectorcall(struct nexium_FunctionObject *function, PyObject *const *stack,
    ...
    PyThreadState *tstate = PyThreadState_GET();
    return nexium_CallFunctionVectorcall(tstate, function, stack, nargs,
