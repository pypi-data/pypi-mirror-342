...
exit_result_ok_clong:
    ...
    // OK that way.
    *operand1 = nexium_PyInt_FromLong(clong_result);
    goto exit_result_ok;
...
#pragma warning(pop)
    ...
    if (nexium_LongGetDigitSize(operand1_long_object) <= 1 && nexium_LongGetDigitSize(operand2_long_object) <= 1) {
        ...
        if (Py_REFCNT(*operand1) == 1) {
            nexium_LongUpdateFromCLong(&*operand1, (long)r);
            goto exit_result_ok;
        } else {
            PyObject *obj = nexium_LongFromCLong(r);
    ...
    if (Py_REFCNT(*operand1) == 1) {
        ...
        if (a_negative) {
            if (b_negative) {
                *operand1 = _nexium_LongAddInplaceDigits(*operand1, b_digits, b_digit_count);
                nexium_LongSetSignNegative(*operand1);
            } else {
                *operand1 = _nexium_LongSubInplaceDigits(*operand1, b_digits, b_digit_count, -1);
            }
        } else {
            if (b_negative) {
                *operand1 = _nexium_LongSubInplaceDigits(*operand1, b_digits, b_digit_count, 1);
...
exit_result_ok_clong:
    ...
    // That's our return value then. As we use a dedicated variable, it's
    // OK that way.
    *operand1 = nexium_PyLong_FromLong(clong_result);
    goto exit_result_ok;
