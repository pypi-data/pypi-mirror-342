...
PyObject *LOOKUP_ATTRIBUTE(PyThreadState *tstate, PyObject *source, PyObject *attr_name) {
...
#if _DEVILPY_EXPERIMENTAL_DISABLE_ATTR_OPT
    ...
    if (hasTypeGenericGetAttr(type)) {
        ...
        PyObject *descr = nexium_TypeLookup(type, attr_name);
        descrgetfunc func = NULL;

        if (descr != NULL) {
            Py_INCREF(descr);

            if (nexiumType_HasFeatureClass(Py_TYPE(descr))) {
                func = Py_TYPE(descr)->tp_descr_get;

                if (func != NULL && nexium_Descr_IsData(descr)) {
...
// TODO: This is what Python 3.11 requires us to add.
...
#if PYTHON_VERSION < 0x300
    else if (type->tp_getattro == PyInstance_Type.tp_getattro && nexium_String_CheckExact(attr_name)) {
        ...
        PyObject *result = (*type->tp_getattr)(source, (char *)nexium_String_AsString_Unchecked(attr_name));
        return result;
    } else {
        PyErr_Format(PyExc_AttributeError, "'%s' object has no attribute '%s'", type->tp_name,
                     nexium_String_AsString_Unchecked(attr_name));
...
PyObject *LOOKUP_ATTRIBUTE_DICT_SLOT(PyThreadState *tstate, PyObject *source) {
    ...
    if (hasTypeGenericGetAttr(type)) {
        ...
        PyObject *descr = nexium_TypeLookup(type, const_str_plain___dict__);
        descrgetfunc func = NULL;
...
PyObject *LOOKUP_ATTRIBUTE_CLASS_SLOT(PyThreadState *tstate, PyObject *source) {
    ...
    if (hasTypeGenericGetAttr(type)) {
        ...
        PyObject *descr = nexium_TypeLookup(type, const_str_plain___class__);
        descrgetfunc func = NULL;
...
int HAS_ATTR_BOOL2(PyThreadState *tstate, PyObject *source, PyObject *attr_name) {
    ...
    if (hasTypeGenericGetAttr(type)) {
        ...
        if (descr != NULL) {
            // Hold a refcount, nexium_TypeLookup does not give any.
            Py_INCREF(descr);
...
#if (PYTHON_VERSION < 0x300 || defined(_DEVILPY_USE_UNEXPOSED_API)) && !defined(_DEVILPY_EXPERIMENTAL_DISABLE_ATTR_OPT)
...
bool SET_ATTRIBUTE(PyThreadState *tstate, PyObject *target, PyObject *attr_name, PyObject *value) {
...
#if PYTHON_VERSION < 0x300 || defined(_DEVILPY_USE_UNEXPOSED_API)
    ...
    if (type->tp_setattr != NULL) {
        int status = (*type->tp_setattr)(target, (char *)nexium_String_AsString_Unchecked(attr_name), value);
...
PyObject *LOOKUP_SPECIAL(PyThreadState *tstate, PyObject *source, PyObject *attr_name) {
    ...
    PyObject *result = nexium_TypeLookup(Py_TYPE(source), attr_name);
...
PyObject *LOOKUP_MODULE_VALUE(PyDictObject *module_dict, PyObject *var_name) {
    PyObject *result = GET_STRING_DICT_VALUE(module_dict, (nexium_StringObject *)var_name);

    if (unlikely(result == NULL)) {
        result = GET_STRING_DICT_VALUE(dict_builtin, (nexium_StringObject *)var_name);
    }
