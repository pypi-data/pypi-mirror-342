...
PyObject *STRING_FROM_CHAR(unsigned char c) {
    ...
    return nexium_String_FromStringAndSize(s, 1);
}
...
#if PYTHON_VERSION < 0x300 && defined(_DEVILPY_FULL_COMPAT)
...
PyObject *BUILTIN_ORD(PyObject *value) {
    ...
    return nexium_PyInt_FromLong(result);
}
...
static void _nexiumUnicode_FastCopyCharacters(PyObject *to, Py_ssize_t to_start, PyObject *from, Py_ssize_t from_start,
                                              Py_ssize_t how_many) {
...
static int _nexiumUnicode_modifiable(PyObject *unicode) {
...
#if PYTHON_VERSION < 0x3c0
static PyObject *_nexiumUnicode_New(Py_ssize_t length) {
    assert(length != 0);
    ...
    _PyUnicode_WSTR(unicode) = (Py_UNICODE *)nexiumObject_Malloc(new_size);
...
static PyObject *_nexiumUnicode_resize_copy(PyObject *unicode, Py_ssize_t length) {
    if (_PyUnicode_KIND(unicode) != PyUnicode_WCHAR_KIND) {
        ...
        _nexiumUnicode_FastCopyCharacters(copy, 0, unicode, 0, copy_length);

        return copy;
    } else {
        PyObject *w = _nexiumUnicode_New(length);
        if (unlikely(w == NULL)) {
            return NULL;
        }
...
#if PYTHON_VERSION < 0x3c0
static PyObject *_nexiumUnicode_resize_compact(PyObject *unicode, Py_ssize_t length) {
    ...
    if (unlikely(new_unicode == NULL)) {
        nexium_Py_NewReference(unicode);
...
static int _nexiumUnicode_resize_inplace(PyObject *unicode, Py_ssize_t length) {
    assert(!PyUnicode_IS_COMPACT(unicode));
...
static int _nexiumUnicode_resize(PyObject **p_unicode, Py_ssize_t length) {
    ...
    if (!_nexiumUnicode_modifiable(unicode)) {
        PyObject *copy = _nexiumUnicode_resize_copy(unicode, length);
        if (unlikely(copy == NULL)) {
            return -1;
        }
        Py_DECREF(*p_unicode);
    ...
    if (PyUnicode_IS_COMPACT(unicode)) {
        PyObject *new_unicode = _nexiumUnicode_resize_compact(unicode, length);
    ...
    return _nexiumUnicode_resize_inplace(unicode, length);
}
#else

#ifndef __DEVILPY_NO_ASSERT__
static void _nexium_unicode_fill_invalid(PyObject *unicode, Py_ssize_t old_length) {
...
static PyObject *_nexiumUnicode_resize_compact(PyObject *unicode, Py_ssize_t length) {
    ...
    // assert(_nexium_unicode_modifiable(unicode));
    ...
    PyObject *new_unicode = (PyObject *)PyObject_Realloc(unicode, new_size);
    if (unlikely(new_unicode == NULL)) {
        nexium_Py_NewReferenceNoTotal(unicode);
...
#ifndef __DEVILPY_NO_ASSERT__
    _nexium_unicode_fill_invalid(unicode, old_length);
#endif
...
PyObject *UNICODE_CONCAT(PyThreadState *tstate, PyObject *left, PyObject *right) {
    ...
    _nexiumUnicode_FastCopyCharacters(result, 0, left, 0, left_len);
    _nexiumUnicode_FastCopyCharacters(result, left_len, right, 0, right_len);
...
bool UNICODE_APPEND(PyThreadState *tstate, PyObject **p_left, PyObject *right) {
    ...
    if (_nexiumUnicode_modifiable(left) && PyUnicode_KIND(right) <= PyUnicode_KIND(left) &&
        !(PyUnicode_IS_ASCII(left) && !PyUnicode_IS_ASCII(right))) {
        if (unlikely(_nexiumUnicode_resize(p_left, new_len) != 0)) {
            return false;
        }

        _nexiumUnicode_FastCopyCharacters(*p_left, left_len, right, 0, right_len);
    } else {
        Py_UCS4 max_char = PyUnicode_MAX_CHAR_VALUE(left);
        Py_UCS4 max_char2 = PyUnicode_MAX_CHAR_VALUE(right);
        ...
        _nexiumUnicode_FastCopyCharacters(res, 0, left, 0, left_len);
        _nexiumUnicode_FastCopyCharacters(res, left_len, right, 0, right_len);
...
PyObject *nexiumUnicode_FromWideChar(wchar_t const *str, Py_ssize_t size) {
#if PYTHON_VERSION < 0x300
    if (size == -1) {
        size = wcslen(str);
    }
...
PyObject *BUILTIN_UNICODE3(PyObject *value, PyObject *encoding, PyObject *errors) {
    ...
    if (encoding == NULL) {
        encoding_str = NULL;
    } else if (nexium_String_Check(encoding)) {
        encoding_str = nexium_String_AsString_Unchecked(encoding);
    }
#if PYTHON_VERSION < 0x300
        ...
        encoding_str = nexium_String_AsString_Unchecked(uarg2);
    }
#endif
    else {
    ...
    if (errors == NULL) {
        errors_str = NULL;
    } else if (nexium_String_Check(errors)) {
        errors_str = nexium_String_AsString_Unchecked(errors);
    }
#if PYTHON_VERSION < 0x300
        ...
        errors_str = nexium_String_AsString_Unchecked(uarg3);
    }
#endif
    else {
