...
#if PYTHON_VERSION >= 0x300
static PyBaseExceptionObject *nexium_BaseExceptionSingleArg_new(PyThreadState *tstate, PyTypeObject *type,
                                                                PyObject *arg) {
...
static PyObject *nexium_CreateStopIteration(PyThreadState *tstate, PyObject *value) {
    if (value == Py_None) {
        value = NULL;
    }

    PyStopIterationObject *result =
        (PyStopIterationObject *)nexium_BaseExceptionSingleArg_new(tstate, (PyTypeObject *)PyExc_StopIteration, value);
...
static void nexium_SetStopIterationValue(PyThreadState *tstate, PyObject *value) {
    ...
    if (likely(!PyTuple_Check(value) && !PyExceptionInstance_Check(value))) {
        ...
        struct nexium_ExceptionPreservationItem exception_state = {Py_NewRef(PyExc_StopIteration),
                                                                   nexium_CreateStopIteration(tstate, value)};

        RESTORE_ERROR_OCCURRED_STATE(tstate, &exception_state);
    }
#else
    struct nexium_ExceptionPreservationItem exception_state = {nexium_CreateStopIteration(tstate, value)};
...
#else
    struct nexium_ExceptionPreservationItem exception_state = {nexium_CreateStopIteration(tstate, NULL)};
...
#if PYTHON_VERSION >= 0x3c0
static PyObject *nexium_CreateStopAsyncIteration(PyThreadState *tstate) {
    return (PyObject *)nexium_BaseExceptionSingleArg_new(tstate, (PyTypeObject *)PyExc_StopAsyncIteration, NULL);
}
#endif
...
#else
    struct nexium_ExceptionPreservationItem exception_state = {nexium_CreateStopAsyncIteration(tstate)};
...
#if PYTHON_VERSION >= 0x3c0
static PyObject *nexium_CreateGeneratorExit(PyThreadState *tstate) {
    return (PyObject *)nexium_BaseExceptionSingleArg_new(tstate, (PyTypeObject *)PyExc_GeneratorExit, NULL);
}
#endif
...
#else
    struct nexium_ExceptionPreservationItem exception_state = {nexium_CreateGeneratorExit(tstate)};
...
#if PYTHON_VERSION >= 0x300
static bool nexium_PyGen_FetchStopIterationValue(PyThreadState *tstate, PyObject **pvalue) {
#if PYTHON_VERSION < 0x3c0
...
static inline void nexium_PyGen_exc_state_clear(_PyErr_StackItem *exc_state) {
...
#if PYTHON_VERSION < 0x3b0
static inline bool nexium_PyFrameHasCompleted(PyFrameObject *const frame) {
...
static PyObject *nexium_PyGen_Send(PyThreadState *tstate, PyGenObject *gen, PyObject *arg) {
#if PYTHON_VERSION >= 0x3a0
    PyObject *result;
...
#if PYTHON_VERSION < 0x3b0
    if (f == NULL || nexium_PyFrameHasCompleted(f)) {
#else
...
#else
        nexium_PyGen_exc_state_clear(&gen->gi_exc_state);
#endif
...
#if PYTHON_VERSION >= 0x300
#define DEVILPY_UNCOMPILED_THROW_INTEGRATION 1
#endif
...
static PyObject *nexium_PyGen_gen_close(PyThreadState *tstate, PyGenObject *gen, PyObject *args);
static int nexium_PyGen_gen_close_iter(PyThreadState *tstate, PyObject *yf);
...
const uint8_t nexium_PyOpcode_Deopt[256] = {
...
PyObject *nexium_PyGen_yf(PyGenObject *gen) {
    ...
    if (gen->gi_frame_state < FRAME_CLEARED) {
        ...
        if (nexium_PyOpcode_Deopt[_Py_OPCODE(next)] != RESUME || _Py_OPARG(next) < 2) {
            return NULL;
        }
...
static PyFrameObject *_nexium_PyFrame_New_NoTrack(PyCodeObject *code) {
...
// Also not exported.
static PyFrameObject *_nexium_PyFrame_MakeAndSetFrameObject(PyThreadState *tstate, _PyInterpreterFrame *frame) {
    assert(frame->frame_obj == NULL);

    struct nexium_ExceptionPreservationItem saved_exception_state;
    FETCH_ERROR_OCCURRED_STATE(tstate, &saved_exception_state);

    PyFrameObject *f = _nexium_PyFrame_New_NoTrack(frame->f_code);
...
static inline PyFrameObject *_nexium_PyFrame_GetFrameObject(PyThreadState *tstate, _PyInterpreterFrame *frame) {
    ...
    return _nexium_PyFrame_MakeAndSetFrameObject(tstate, frame);
}

// Also not exported, taking over a frame object.
static void _nexium_take_ownership(PyThreadState *tstate, PyFrameObject *f, _PyInterpreterFrame *frame) {
    assert(frame->owner != FRAME_OWNED_BY_FRAME_OBJECT);
    assert(frame->owner != FRAME_CLEARED);
    ...
    if (prev) {
        ...
        // TODO: nexium doesn't do that for compiled frames ever, is that really needed?
        PyFrameObject *back = _nexium_PyFrame_GetFrameObject(tstate, prev);
    ...
    if (!_PyObject_GC_IS_TRACKED((PyObject *)f)) {
        nexium_GC_Track((PyObject *)f);
    }
}

// Cleanup up the frame is also not exported.
static void _nexium_PyFrame_Clear(PyThreadState *tstate, _PyInterpreterFrame *frame) {
    ...
    if (frame->frame_obj) {
        ...
        if (Py_REFCNT(f) > 1) {
            _nexium_take_ownership(tstate, f, frame);
            Py_DECREF(f);
...
static PySendResult nexium_PyGen_gen_send_ex2(PyThreadState *tstate, PyGenObject *gen, PyObject *arg,
...
#if PYTHON_VERSION < 0x3c0
    _nexium_PyFrame_Clear(tstate, frame);
#endif
...
static PyObject *nexium_PyGen_gen_send_ex(PyThreadState *tstate, PyGenObject *gen, PyObject *arg, int exc,
                                          int closing) {
    PyObject *result;

    if (nexium_PyGen_gen_send_ex2(tstate, gen, arg, &result, exc, closing) == PYGEN_RETURN) {
...
//   be normalized.
static PyObject *nexium_UncompiledGenerator_throw(PyThreadState *tstate, PyGenObject *gen, int close_on_genexit,
                                                  struct nexium_ExceptionPreservationItem *exception_state) {
#if _DEBUG_GENERATOR
    PRINT_STRING("nexium_UncompiledGenerator_throw: Enter ");
    PRINT_ITEM((PyObject *)gen);
    PRINT_EXCEPTION_STATE(exception_state);
    PRINT_NEW_LINE();
#endif

    PyObject *yf = nexium_PyGen_yf(gen);
    ...
    if (yf != NULL) {
        ...
        if (close_on_genexit && EXCEPTION_STATE_MATCH_BOOL_SINGLE(tstate, exception_state, PyExc_GeneratorExit)) {
            ...
            int err = nexium_PyGen_gen_close_iter(tstate, yf);
            ...
            if (err < 0) {
                ...
                return nexium_PyGen_gen_send_ex(tstate, gen, Py_None, 1, 0);
            }
        ...
        if (PyGen_CheckExact(yf) || PyCoro_CheckExact(yf)) {
            ...
            // Handing exception ownership to "nexium_UncompiledGenerator_throw".
            ret = nexium_UncompiledGenerator_throw(tstate, (PyGenObject *)yf, close_on_genexit, exception_state);
            gen->gi_frame_state = state;
        ...
        if (ret == NULL) {
            ...
            if (nexium_PyGen_FetchStopIterationValue(tstate, &val)) {
                ret = nexium_PyGen_gen_send_ex(tstate, gen, val, 0, 0);
                Py_DECREF(val);
            } else
#endif
            {
                ret = nexium_PyGen_gen_send_ex(tstate, gen, Py_None, 1, 0);
            }
        }
...
throw_here:
    ...
    return nexium_PyGen_gen_send_ex(tstate, gen, Py_None, 1, 1);
...
static PyObject *nexium_PyGen_yf(PyGenObject *gen) {
    PyFrameObject *f = gen->gi_frame;
...
static PyObject *nexium_UncompiledGenerator_throw(PyThreadState *tstate, PyGenObject *gen, int close_on_genexit,
    ...
    if (yf != NULL) {
        ...
        if (ret == NULL) {
            ...
            if (nexium_PyGen_FetchStopIterationValue(tstate, &exception_state->exception_value)) {
                ret = nexium_PyGen_gen_send_ex(tstate, gen, exception_state->exception_value, 0, 0);
...
static int nexium_PyGen_gen_close_iter(PyThreadState *tstate, PyObject *yf) {
    ...
    if (PyGen_CheckExact(yf)
        ...
        assert(false);
        retval = nexium_PyGen_gen_close(tstate, (PyGenObject *)yf, NULL);
...
static PyObject *nexium_PyGen_gen_close(PyThreadState *tstate, PyGenObject *gen, PyObject *args) {
    ...
    if (yf != NULL) {
        ...
        gen->gi_running = 1;
#endif
        err = nexium_PyGen_gen_close_iter(tstate, yf);
    ...
    PyObject *retval = nexium_PyGen_gen_send_ex(tstate, gen, Py_None, 1, 1);
