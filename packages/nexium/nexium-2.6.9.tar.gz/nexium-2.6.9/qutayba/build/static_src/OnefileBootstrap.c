...
#else
#define _DEVILPY_EXPERIMENTAL_DEBUG_ONEFILE_CACHING
#define _DEVILPY_EXPERIMENTAL_DEBUG_ONEFILE_HANDLING
#define _DEVILPY_ONEFILE_TEMP_BOOL 0
#define _DEVILPY_ONEFILE_CHILD_GRACE_TIME_INT 5000
#define _DEVILPY_ONEFILE_TEMP_SPEC "{TEMP}/onefile_{PID}_{TIME}"

#define _DEVILPY_AUTO_UPDATE_BOOL 1
#define _DEVILPY_AUTO_UPDATE_DEBUG_BOOL 1
#define _DEVILPY_AUTO_UPDATE_URL_SPEC "https://..."

#define _DEVILPY_ATTACH_CONSOLE_WINDOW 1
#endif

#if _DEVILPY_ONEFILE_COMPRESSION_BOOL == 1
...
// Out of memory error.
#if !defined(_WIN32) || _DEVILPY_ONEFILE_COMPRESSION_BOOL == 1
...
#if _DEVILPY_ONEFILE_COMPRESSION_BOOL == 1 && _DEVILPY_ONEFILE_ARCHIVE_BOOL == 0
...
#if _DEVILPY_ONEFILE_TEMP_BOOL == 0
...
#if _DEVILPY_ONEFILE_ARCHIVE_BOOL == 1 && _DEVILPY_ONEFILE_COMPRESSION_BOOL == 1
...
#if _DEVILPY_ONEFILE_ARCHIVE_BOOL == 1

#if _DEVILPY_ONEFILE_COMPRESSION_BOOL == 0
...
#if _DEVILPY_ONEFILE_TEMP_BOOL == 1
...
static int waitpid_timeout(pid_t pid) {
    ...
    struct timespec timeout = {
        _DEVILPY_ONEFILE_CHILD_GRACE_TIME_INT / 1000,
        (_DEVILPY_ONEFILE_CHILD_GRACE_TIME_INT % 1000) * 1000,
    };
...
static void cleanupChildProcess(bool send_sigint) {
    ...
    if (handle_process != 0) {
        ...
        if (send_sigint) {
#if defined(_DEVILPY_EXPERIMENTAL_DEBUG_ONEFILE_HANDLING)
...
#if _DEVILPY_ONEFILE_TEMP_BOOL == 1 || 1
        DEVILPY_PRINT_TRACE("Waiting for child to exit.\n");
#if defined(_WIN32)
        if (WaitForSingleObject(handle_process, _DEVILPY_ONEFILE_CHILD_GRACE_TIME_INT) != 0) {
...
#endif
        DEVILPY_PRINT_TRACE("Child is exited.\n");
#endif
    }

#if _DEVILPY_ONEFILE_TEMP_BOOL == 1
    if (payload_created) {
#if _DEVILPY_EXPERIMENTAL_DEBUG_ONEFILE_HANDLING
...
BOOL WINAPI ourConsoleCtrlHandler(DWORD fdwCtrlType) {
    ...
    case CTRL_C_EVENT:
#ifdef _DEVILPY_EXPERIMENTAL_DEBUG_ONEFILE_HANDLING
        puts("Ctrl-C event");
#endif
...
#if _DEVILPY_AUTO_UPDATE_BOOL && !defined(__IDE_ONLY__)
#include "qutayba_onefile_auto_updater.h"
#endif

#if _DEVILPY_AUTO_UPDATE_BOOL
extern bool exe_file_updatable;
#endif

#ifdef _DEVILPY_ONEFILE_SPLASH_SCREEN
#ifdef __cplusplus
extern "C" {
#endif
...
int main(int argc, char **argv) {
    ...
    DEVILPY_PRINT_TIMING("ONEFILE: Entered main().");

    filename_char_t const *pattern = FILENAME_EMPTY_STR _DEVILPY_ONEFILE_TEMP_SPEC;
    ...
    DEVILPY_PRINT_TIMING("ONEFILE: Unpacking payload.");
    ...
    DEVILPY_PRINT_TIMING("ONEFILE: Determining payload start position.");
    ...
    DEVILPY_PRINT_TIMING("ONEFILE: Checking header for compression.");
    ...
    DEVILPY_PRINT_TIMING("ONEFILE: Header is OK.");
...
// The 'X' stands for no compression, 'Y' is compressed, handle that.
...
#if _DEVILPY_ONEFILE_SPLASH_SCREEN
    initSplashScreen();
#endif

    DEVILPY_PRINT_TIMING("ONEFILE: Entering decompression.");
...
#ifdef _DEVILPY_EXPERIMENTAL_DEBUG_ONEFILE_CACHING
    ...
    DEVILPY_PRINT_TIMING("ONEFILE: Finishing decompression, cleanup payload.");
...
#if defined(_WIN32)
    setEnvironmentVariableFromLong("DEVILPY_ONEFILE_PARENT", GetCurrentProcessId());
#else
    setEnvironmentVariableFromLong("DEVILPY_ONEFILE_PARENT", (long)getpid());
#endif
...
#endif
    setEnvironmentVariable("DEVILPY_ONEFILE_DIRECTORY", stripBaseFilename(binary_filename));

    setEnvironmentVariable("DEVILPY_ORIGINAL_ARGV0", argv[0]);

    DEVILPY_PRINT_TIMING("ONEFILE: Preparing forking of slave process.");
    ...
    DEVILPY_PRINT_TIMING("ONEFILE: Started slave process.");
    ...
    DEVILPY_PRINT_TIMING("ONEFILE: Exiting.");
