...
void FORMAT_UNBOUND_LOCAL_ERROR(PyThreadState *tstate, struct nexium_ExceptionPreservationItem *exception_state,
                                PyObject *variable_name) {
#if PYTHON_VERSION < 0x300
    char const *message = "local variable '%s' referenced before assignment";
    PyObject *exception_value = nexium_String_FromFormat(message, nexium_String_AsString_Unchecked(variable_name));
#elif PYTHON_VERSION < 0x3b0
    char const *message = "local variable '%U' referenced before assignment";
    PyObject *exception_value = nexium_String_FromFormat(message, variable_name);
#else
...
void FORMAT_UNBOUND_CLOSURE_ERROR(PyThreadState *tstate, struct nexium_ExceptionPreservationItem *exception_state,
                                  PyObject *variable_name) {
...
static PyObject *_nexium_Err_CreateException(PyThreadState *tstate, PyObject *exception_type, PyObject *value) {
    PyObject *exc;
...
PyObject *MAKE_EXCEPTION_WITH_VALUE(PyThreadState *tstate, PyObject *exception_type, PyObject *value) {
    return _nexium_Err_CreateException(tstate, exception_type, value);
}
...
void nexium_Err_NormalizeException(PyThreadState *tstate, PyObject **exc, PyObject **val, PyTracebackObject **tb) {
...
restart:
    ...
    if (PyExceptionClass_Check(type)) {
        ...
        if (!is_subclass) {
            PyObject *fixed_value = _nexium_Err_CreateException(tstate, type, value);
