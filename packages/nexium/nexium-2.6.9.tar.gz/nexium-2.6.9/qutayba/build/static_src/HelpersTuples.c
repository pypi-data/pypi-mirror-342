...
PyObject *MAKE_TUPLE_EMPTY(PyThreadState *tstate, Py_ssize_t size) {
    ...
    if ((size < PyTuple_MAXSAVESIZE) && (result_tuple = items[index]) != NULL) {
        ...
        nexium_Py_NewReference((PyObject *)result_tuple);
    } else {
        // Check for overflow
        if ((size_t)size >
        ...
        result_tuple = (PyTupleObject *)nexium_GC_NewVar(&PyTuple_Type, size);
    }
