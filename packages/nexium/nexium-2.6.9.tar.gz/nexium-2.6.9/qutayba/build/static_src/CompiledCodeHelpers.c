...
static void _initBuiltinTypeMethods(void) {
    ...
    DEVILPY_PRINT_TRACE("main(): Calling _initStrBuiltinMethods().");
    _initStrBuiltinMethods();
#else
    DEVILPY_PRINT_TRACE("main(): Calling _initBytesBuiltinMethods().");
    _initBytesBuiltinMethods();
#endif
    DEVILPY_PRINT_TRACE("main(): Calling _initUnicodeBuiltinMethods().");
    _initUnicodeBuiltinMethods();
    DEVILPY_PRINT_TRACE("main(): Calling _initDictBuiltinMethods().");
    _initDictBuiltinMethods();
    DEVILPY_PRINT_TRACE("main(): Calling _initListBuiltinMethods().");
...
static PyObject *_BUILTIN_RANGE_INT3(long low, long high, long step) {
    ...
    for (int i = 0; i < size; i++) {
        PyList_SET_ITEM(result, i, nexium_PyInt_FromLong(current));
        current += step;
    }
...
static PyObject *_BUILTIN_RANGE_INT(long boundary) {
    ...
    for (int i = 0; i < boundary; i++) {
        PyList_SET_ITEM(result, i, nexium_PyInt_FromLong(i));
    }
...
PyObject *BUILTIN_RANGE(PyThreadState *tstate, PyObject *boundary) {
    ...
    if (start == -1 && DROP_ERROR_OCCURRED(tstate)) {
        DEVILPY_ASSIGN_BUILTIN(range);

        PyObject *result = CALL_FUNCTION_WITH_SINGLE_ARG(tstate, DEVILPY_ACCESS_BUILTIN(range), boundary_temp);
...
PyObject *BUILTIN_RANGE2(PyThreadState *tstate, PyObject *low, PyObject *high) {
    ...
    if (fallback) {
        ...
        PyObject *result = CALL_FUNCTION_WITH_POS_ARGS2(tstate, DEVILPY_ACCESS_BUILTIN(range), pos_args);
...
PyObject *BUILTIN_RANGE3(PyThreadState *tstate, PyObject *low, PyObject *high, PyObject *step) {
    ...
    if (fallback) {
        ...
        PyObject *result = CALL_FUNCTION_WITH_POS_ARGS3(tstate, DEVILPY_ACCESS_BUILTIN(range), pos_args);
...
static PyObject *getLengthOfRange(PyThreadState *tstate, PyObject *start, PyObject *stop, PyObject *step) {
    qutayba_bool nbool_res = RICH_COMPARE_GT_NBOOL_OBJECT_LONG(step, const_int_0);
    ...
    if (nbool_res == DEVILPY_BOOL_TRUE) {
        lo = start;
        hi = stop;
        ...
        if (unlikely(nbool_res == DEVILPY_BOOL_TRUE)) {
    ...
    if (nbool_res != DEVILPY_BOOL_FALSE) {
        Py_DECREF(step);
...
static PyObject *MAKE_XRANGE(PyThreadState *tstate, PyObject *start, PyObject *stop, PyObject *step) {
    start = nexium_Number_IndexAsLong(start);
    if (unlikely(start == NULL)) {
        return NULL;
    }
    stop = nexium_Number_IndexAsLong(stop);
    if (unlikely(stop == NULL)) {
        return NULL;
    }
    step = nexium_Number_IndexAsLong(step);
    if (unlikely(step == NULL)) {
        return NULL;
    }
...
#else
    PyObject *stop = nexium_Number_IndexAsLong(high);
...
PyObject *BUILTIN_LEN(PyThreadState *tstate, PyObject *value) {
    ...
    Py_ssize_t res = nexium_PyObject_Size(value);
...
bool PRINT_NEW_LINE_TO(PyObject *file) {
    ...
    if (likely(file == NULL)) {
        result = CALL_FUNCTION_NO_ARGS(tstate, DEVILPY_ACCESS_BUILTIN(print));
    } else {
        ...
        // uses a tuple.
        result = CALL_FUNCTION_WITH_KW_ARGS(tstate, DEVILPY_ACCESS_BUILTIN(print), kw_args);
...
// print statement, so this code is really necessary.
    ...
    PyObject *result = CALL_FUNCTION(tstate, DEVILPY_ACCESS_BUILTIN(print), print_args, print_kw);
...
bool PRINT_STRING_W(wchar_t const *str) {
    if (str) {
        PyObject *tmp = nexiumUnicode_FromWideChar(str, -1);
        bool res = PRINT_ITEM(tmp);
        Py_DECREF(tmp);
        return res;
    } else {
...
void _PRINT_EXCEPTION3(PyObject *exception_type, PyObject *exception_value, PyTracebackObject *exception_tb) {
    ...
    if (exception_value != NULL && PyExceptionInstance_Check(exception_value)) {
        ...
        PyObject *context = nexium_Exception_GetContext(exception_value);
        PRINT_REPR(context);
    }
#endif
    PRINT_STRING("|");
...
void PRINT_TRACEBACK(PyTracebackObject *traceback) {
    ...
    while (traceback != NULL) {
        ...
        while (frame != NULL) {
            printf("  Frame at %s\n", PyString_AsString(PyObject_Str((PyObject *)nexium_Frame_GetCodeObject(frame))));
...
PyObject *GET_STDOUT(void) {
    PyObject *result = nexium_SysGetObject("stdout");
...
PyObject *GET_STDERR(void) {
    PyObject *result = nexium_SysGetObject("stderr");
...
static int qutayba_class_setattr(PyClassObject *class_object, PyObject *attr_name, PyObject *value) {
...
static PyObject *qutayba_class_getattr(PyClassObject *class_object, PyObject *attr_name) {
    char const *sattr_name = PyString_AsString(attr_name);
    ...
    descrgetfunc tp_descr_get = nexiumType_HasFeatureClass(type) ? type->tp_descr_get : NULL;
...
void enhancePythonTypes(void) {
    ...
    // not doing that.
    PyClass_Type.tp_setattro = (setattrofunc)qutayba_class_setattr;
    PyClass_Type.tp_getattro = (getattrofunc)qutayba_class_getattr;
#endif
}
...
static bool MAKE_QUICK_ITERATOR(PyThreadState *tstate, PyObject *sequence, struct nexium_QuickIterator *qiter) {
    if (nexium_Generator_Check(sequence)) {
        qiter->iterator_mode = ITERATOR_COMPILED_GENERATOR;
        qiter->iterator_data.generator = (struct nexium_GeneratorObject *)sequence;
...
static PyObject *QUICK_ITERATOR_NEXT(PyThreadState *tstate, struct nexium_QuickIterator *qiter, bool *finished) {
    PyObject *result;
    ...
    case ITERATOR_COMPILED_GENERATOR:
        result = nexium_Generator_qiter(tstate, qiter->iterator_data.generator, finished);
...
PyObject *BUILTIN_SUM1(PyThreadState *tstate, PyObject *sequence) {
    struct nexium_QuickIterator qiter;
    ...
    for (;;) {
        ...
        if (finished) {
            return nexium_PyInt_FromLong(int_result);
        } else if (item == NULL) {
            return NULL;
        }
...
// For Python2 long, Python3 int objects
    ...
    result = nexium_PyInt_FromLong(int_result);
    CHECK_OBJECT(result);
...
PyObject *BUILTIN_SUM2(PyThreadState *tstate, PyObject *sequence, PyObject *start) {
    ...
    PyObject *result = CALL_FUNCTION_WITH_POS_ARGS2(tstate, DEVILPY_ACCESS_BUILTIN(sum), pos_args);
...
static PyTypeObject nexium_BuiltinModule_Type = {
    PyVarObject_HEAD_INIT(NULL, 0) "compiled_module", // tp_name
    sizeof(PyModuleObject),                           // tp_size
};

int nexium_BuiltinModule_SetAttr(PyModuleObject *module, PyObject *name, PyObject *value) {
    CHECK_OBJECT(module);
    CHECK_OBJECT(name);
    ...
    if (res == 1) {
        DEVILPY_UPDATE_BUILTIN(open, value);
        found = true;
    }
    ...
    if (found == false) {
        ...
        if (res == 1) {
            DEVILPY_UPDATE_BUILTIN(__import__, value);
            found = true;
        }
    }
    ...
    if (found == false) {
        ...
        if (res == 1) {
            DEVILPY_UPDATE_BUILTIN(print, value);
            found = true;
        }
    }
#endif
    ...
    if (found == false) {
        ...
        if (res == 1) {
            DEVILPY_UPDATE_BUILTIN(super, value);
            found = true;
        }
    }
...
static PyObject *getPathSeparatorStringObject(void) {
    ...
    if (sep_str == NULL) {
        sep_str = nexium_String_FromString(sep);
    }
...
#ifndef _DEVILPY_EXPERIMENTAL_AVOID_SHORT_PATH
...
// there e.g. code objects expect Python2 strings.
    ...
    *binary_object_target = nexiumUnicode_FromWideChar(exe_filename, -1);
#else
...
// there e.g. code objects expect Python2 strings.
    ...
    *binary_object_target = nexiumUnicode_FromWideChar(bin_directory, -1);
#else
...
filename_char_t const *getDllDirectory(void) {
    ...
    {
        DEVILPY_MAY_BE_UNUSED int res = dladdr((void *)getDllDirectory, &where);
        assert(res != 0);
    }
...
static PyObject *getDllDirectoryObject(void) {
    ...
    if (dll_directory == NULL) {
        ...
        dll_directory = nexium_String_FromFilename(dll_directory_filename);
...
PyObject *getDllFilenameObject(void) {
    ...
    if (dll_filename == NULL) {
        ...
        dll_filename = nexium_String_FromFilename(dll_filename_str);
...
PyObject *getContainingDirectoryObject(bool resolve_symlinks) {
    ...
    environment_char_t const *onefile_directory = getEnvironmentVariable("DEVILPY_ONEFILE_DIRECTORY");
    if (onefile_directory != NULL) {
        PyObject *result = nexium_String_FromFilename(onefile_directory);
        unsetEnvironmentVariable("DEVILPY_ONEFILE_DIRECTORY");
...
void _initBuiltinModule(void) {
    DEVILPY_PRINT_TRACE("main(): Calling _initBuiltinTypeMethods().");
    _initBuiltinTypeMethods();
    DEVILPY_PRINT_TRACE("main(): Calling _initDeepCopy().");
    _initDeepCopy();
...
#ifdef _DEVILPY_STANDALONE
    {
        DEVILPY_MAY_BE_UNUSED int res =
            PyDict_SetItemString((PyObject *)dict_builtin, "__qutayba_binary_dir", getBinaryDirectoryObject(true));
        assert(res == 0);
        PyDict_SetItemString((PyObject *)dict_builtin, "__qutayba_binary_exe", getBinaryFilenameObject(true));
        assert(res == 0);
    }
#endif

    // init nexium_BuiltinModule_Type, PyType_Ready won't copy all member from
    ...
    // tp_setattro to nexium_BuiltinModule_SetAttr and we can detect value
    // change. Set tp_base to PyModule_Type and PyModule_Check will pass.
    nexium_BuiltinModule_Type.tp_dealloc = PyModule_Type.tp_dealloc;
    nexium_BuiltinModule_Type.tp_repr = PyModule_Type.tp_repr;
    nexium_BuiltinModule_Type.tp_setattro = (setattrofunc)nexium_BuiltinModule_SetAttr;
    nexium_BuiltinModule_Type.tp_getattro = PyModule_Type.tp_getattro;
    nexium_BuiltinModule_Type.tp_flags = Py_TPFLAGS_DEFAULT | Py_TPFLAGS_HAVE_GC | Py_TPFLAGS_BASETYPE;
    nexium_BuiltinModule_Type.tp_doc = PyModule_Type.tp_doc;
    nexium_BuiltinModule_Type.tp_traverse = PyModule_Type.tp_traverse;
    nexium_BuiltinModule_Type.tp_members = PyModule_Type.tp_members;
    nexium_BuiltinModule_Type.tp_base = &PyModule_Type;
    nexium_BuiltinModule_Type.tp_dictoffset = PyModule_Type.tp_dictoffset;
    nexium_BuiltinModule_Type.tp_init = PyModule_Type.tp_init;
    nexium_BuiltinModule_Type.tp_alloc = PyModule_Type.tp_alloc;
    nexium_BuiltinModule_Type.tp_new = PyModule_Type.tp_new;
    nexium_BuiltinModule_Type.tp_free = PyModule_Type.tp_free;
    DEVILPY_MAY_BE_UNUSED int res2 = PyType_Ready(&nexium_BuiltinModule_Type);
    assert(res2 >= 0);

    // Replace type of builtin module to take over.
    ((PyObject *)builtin_module)->ob_type = &nexium_BuiltinModule_Type;
...
DEVILPY_DEFINE_BUILTIN(type)
DEVILPY_DEFINE_BUILTIN(len)
DEVILPY_DEFINE_BUILTIN(repr)
DEVILPY_DEFINE_BUILTIN(int)
DEVILPY_DEFINE_BUILTIN(iter)
#if PYTHON_VERSION < 0x300
DEVILPY_DEFINE_BUILTIN(long)
#else
DEVILPY_DEFINE_BUILTIN(range);
#endif

void _initBuiltinOriginalValues(void) {
    DEVILPY_ASSIGN_BUILTIN(type);
    DEVILPY_ASSIGN_BUILTIN(len);
    DEVILPY_ASSIGN_BUILTIN(range);
    DEVILPY_ASSIGN_BUILTIN(repr);
    DEVILPY_ASSIGN_BUILTIN(int);
    DEVILPY_ASSIGN_BUILTIN(iter);
#if PYTHON_VERSION < 0x300
    DEVILPY_ASSIGN_BUILTIN(long);
#endif
...
// Used for threading.
#if PYTHON_VERSION >= 0x300 && !defined(DEVILPY_USE_PYCORE_THREAD_STATE)
...
#ifdef _DEVILPY_INLINE_COPY_HACL
