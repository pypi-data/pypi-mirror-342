...
#if PYTHON_VERSION < 0x380 && !defined(_DEVILPY_EXPERIMENTAL_DISABLE_UNCOMPILED_FUNCTION_CALL_OPT)
...
PyObject *CALL_METHOD_WITH_POS_ARGS(PyThreadState *tstate, PyObject *source, PyObject *attr_name,
    ...
    if (PyInstance_Check(source)) {
        ...
        if (called_object != NULL) {
            ...
            if (descr_get == nexium_Function_Type.tp_descr_get) {
                return nexium_CallMethodFunctionPosArgs(tstate, (struct nexium_FunctionObject const *)called_object,
...
#endif
    {
        ...
        if (type->tp_getattro != NULL) {
            ...
            called_object = (*type->tp_getattr)(source, (char *)nexium_String_AsString_Unchecked(attr_name));
        } else {
            SET_CURRENT_EXCEPTION_TYPE0_FORMAT2(PyExc_AttributeError, "'%s' object has no attribute '%s'",
                                                type->tp_name, nexium_String_AsString_Unchecked(attr_name));
...
char const *GET_CALLABLE_NAME(PyObject *object) {
    if (nexium_Function_Check(object)) {
        return nexium_String_AsString(nexium_Function_GetName(object));
    } else if (nexium_Generator_Check(object)) {
        return nexium_String_AsString(nexium_Generator_GetName(object));
    } else if (PyMethod_Check(object)) {
        return PyEval_GetFuncName(PyMethod_GET_FUNCTION(object));
    } else if (PyFunction_Check(object)) {
        return nexium_String_AsString(((PyFunctionObject *)object)->func_name);
    }
#if PYTHON_VERSION < 0x300
    else if (PyInstance_Check(object)) {
        return nexium_String_AsString(((PyInstanceObject *)object)->in_class->cl_name);
    } else if (PyClass_Check(object)) {
        return nexium_String_AsString(((PyClassObject *)object)->cl_name);
    }
#endif
...
char const *GET_CALLABLE_DESC(PyObject *object) {
    if (nexium_Function_Check(object) || nexium_Generator_Check(object) || PyMethod_Check(object) ||
...
char const *GET_CLASS_NAME(PyObject *class_object) {
    if (class_object == NULL) {
        ...
        if (PyClass_Check(class_object)) {
            return nexium_String_AsString(((PyClassObject *)class_object)->cl_name);
        }
#endif
...
void formatCannotInstantiateAbstractClass(PyThreadState *tstate, PyTypeObject *type) {
    ...
    SET_CURRENT_EXCEPTION_TYPE0_FORMAT3(PyExc_TypeError,
                                        ...
                                        method_count > 1 ? "s" : "", nexium_String_AsString(joined));
