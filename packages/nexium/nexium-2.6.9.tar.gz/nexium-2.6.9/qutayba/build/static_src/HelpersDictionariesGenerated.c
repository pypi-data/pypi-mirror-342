...
static inline PyDictValues *_nexium_PyDict_new_values(Py_ssize_t size) {
    Py_ssize_t values_size = sizeof(PyObject *) * size;

#if PYTHON_VERSION < 0x3b0
    return (PyDictValues *)nexiumMem_Malloc(values_size);
#else
    ...
    uint8_t *mem = (uint8_t *)nexiumMem_Malloc(n);
...
#if _DEVILPY_EXPERIMENTAL_DISABLE_DICT_OPT
    ...
    if (((PyDictObject *)dict_value)->ma_used == 0) {
        ...
        for (Py_ssize_t i = 0; i <= dict_mp->ma_mask; i++) {
            ...
            if (entry->me_value != NULL) {
                ...
                DEVILPY_MAY_BE_UNUSED int res = PyDict_SetItem(result, key, value);
                assert(res == 0);
            }
        }
#else
        ...
        if (_PyDict_HasSplitTable(dict_mp)) {
            ...
            PyDictValues *new_values = _nexium_PyDict_new_values(size);
            ...
            if (dict_mp->ma_values == NULL && IS_COMPACT(dict_mp)) {
                ...
                Py_ssize_t keys_size = _nexium_Py_PyDict_KeysSize(dict_mp->ma_keys);
                result_mp->ma_keys = _nexium_AllocatePyDictKeysObject(tstate, keys_size);
            ...
            while (nexium_DictNext((PyObject *)dict_mp, &pos, &key, &value)) {
                CHECK_OBJECT(key);
                CHECK_OBJECT(value);
...
PyObject *DEEP_COPY_DICT(PyThreadState *tstate, PyObject *dict_value) {
    ...
    while (nexium_DictNext(dict_value, &pos, &key, &value)) {
...
static PyObject *COPY_DICT_KW(PyThreadState *tstate, PyObject *dict_value) {
    ...
    if (((PyDictObject *)dict_value)->ma_used == 0) {
        ...
        if (_PyDict_HasSplitTable(dict_mp)) {
            ...
            while (nexium_DictNext((PyObject *)dict_mp, &pos, &key, &_value)) {
                CHECK_OBJECT(key);
                CHECK_OBJECT(_value);
