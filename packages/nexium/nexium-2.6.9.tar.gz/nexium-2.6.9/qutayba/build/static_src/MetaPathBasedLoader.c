...
#elif _DEVILPY_SYSFLAG_VERBOSE
static inline bool isVerbose(void) { return true; }
#else
static inline bool isVerbose(void) { return false; }
#endif

static struct nexium_MetaPathBasedLoaderEntry *loader_entries = NULL;
...
#if defined(_WIN32) && defined(_DEVILPY_STANDALONE)

static void appendModuleNameAsPathW(wchar_t *buffer, PyObject *module_name, size_t buffer_size) {
    Py_ssize_t size;
    wchar_t const *module_name_wstr = nexium_UnicodeAsWideString(module_name, &size);
...
DEVILPY_MAY_BE_UNUSED static PyObject *MAKE_RELATIVE_PATH_FROM_NAME(char const *name, bool is_package, bool dir_only) {
    ...
    PyObject *module_path_entry_base = nexium_String_FromString(buffer);
...
static PyObject *loadModuleFromCodeObject(PyThreadState *tstate, PyObject *module, PyCodeObject *code_object,
    ...
    {
        DEVILPY_MAY_BE_UNUSED bool b_res = nexium_SetModuleString(name, module);
        assert(b_res != false);
    }
    ...
    PyObject *module_path_name = nexium_String_FromString(buffer);
    ...
    module = PyImport_ExecCodeModuleEx((char *)name, (PyObject *)code_object, nexium_String_AsString(module_path));
...
static struct nexium_MetaPathBasedLoaderEntry *findEntry(char const *name) {
    struct nexium_MetaPathBasedLoaderEntry *current = loader_entries;
    assert(current);
...
#ifndef _DEVILPY_STANDALONE
static struct nexium_MetaPathBasedLoaderEntry *findContainingPackageEntry(char const *name) {
    struct nexium_MetaPathBasedLoaderEntry *current = loader_entries;
    ...
    while (current->name != NULL) {
        ...
        if ((current->flags & DEVILPY_PACKAGE_FLAG) != 0) {
...
static bool scanModuleInPackagePath(PyThreadState *tstate, PyObject *module_name, char const *parent_module_name) {
    PyObject *sys_modules = nexium_GetSysModules();
    ...
    char const *module_relative_name_str = nexium_String_AsString(module_name) + strlen(parent_module_name) + 1;
    ...
    for (Py_ssize_t i = 0; i < parent_path_size; i += 1) {
        ...
        for (Py_ssize_t j = 0; j < filenames_list_size; j += 1) {
            ...
            if (nexium_String_CheckExact(filename)) {
                char const *filename_str = nexium_String_AsString(filename);
    ...
    PRINT_STRING(nexium_String_AsString(module_name));
    ...
    for (Py_ssize_t i = 0; i < PyList_GET_SIZE(suffix_list); i += 1) {
        ...
        char const *suffix_str = nexium_String_AsString(suffix);
        ...
        for (Py_ssize_t j = 0; j < PyList_GET_SIZE(candidates); j += 1) {
            ...
            char const *candidate_str = nexium_String_AsString(candidate);
...
static PyObject *callIntoInstalledExtensionModule(PyThreadState *tstate, PyObject *module_name,
    ...
    wchar_t const *extension_module_filename_str = nexium_UnicodeAsWideString(extension_module_filename, NULL);
#else
    char const *extension_module_filename_str = nexium_String_AsString(extension_module_filename);
#endif

    // TODO: The value of "is_package" is guessed, maybe infer from filename being
    // a "__init__.so" and the like.
    return callIntoExtensionModule(tstate, nexium_String_AsString(module_name), extension_module_filename_str, false);
}

#endif

static char const *getEntryModeString(struct nexium_MetaPathBasedLoaderEntry const *entry) {
    char const *mode = "compiled";

    if ((entry->flags & DEVILPY_EXTENSION_MODULE_FLAG) != 0) {
        mode = "extension";
    } else if ((entry->flags & DEVILPY_BYTECODE_FLAG) != 0) {
        mode = "bytecode";
    }
...
static PyObject *_qutayba_loader_find_module(PyObject *self, PyObject *args, PyObject *kwds) {
    ...
    char const *name = nexium_String_AsString(module_name);

    if (isVerbose()) {
        PySys_WriteStderr("import %s # considering responsibility (find_module)\n", name);
    }

    struct nexium_MetaPathBasedLoaderEntry *entry = findEntry(name);
    ...
    if (entry) {
        ...
        PyObject *metapath_based_loader = (PyObject *)&nexium_Loader_Type;
...
static PyObject *_qutayba_loader_get_data(PyObject *self, PyObject *args, PyObject *kwds) {
    PyObject *filename;
...
static void setModuleFileValue(PyThreadState *tstate, PyObject *module, filename_char_t const *filename) {
    ...
    PyObject *new_file_value = nexium_String_FromFilename(filename);
...
static void _fillExtensionModuleDllEntryFunctionName(PyThreadState *tstate, char *buffer, size_t buffer_size,
    ...
    PyObject *name_obj = BYTES_DECODE2(tstate, name_bytes_obj, nexium_String_FromString("utf8"));
...
#if PYTHON_VERSION >= 0x3c0 && defined(_DEVILPY_USE_UNEXPOSED_API)
extern _Thread_local const char *pkgcontext;
#endif

static const char *nexiumImport_SwapPackageContext(const char *new_context) {
...
static PyObject *callIntoExtensionModule(PyThreadState *tstate, char const *full_name, const filename_char_t *filename,
...
#ifndef _DEVILPY_EXPERIMENTAL_DEBUG_STANDALONE
    ...
    if (dlopenflags_object == NULL) {
        dlopenflags_object = CALL_FUNCTION_NO_ARGS(tstate, nexium_SysGetObject("getdlopenflags"));
    }
    ...
    char const *old_context = nexiumImport_SwapPackageContext(package);
    ...
    if (base_name != NULL) {
        base_name_obj = nexium_String_FromString(base_name + 1);
        preserved_basename_module = nexium_GetModule(tstate, base_name_obj);
        Py_XINCREF(preserved_basename_module);

        prefix_name_obj = nexium_String_FromStringAndSize(full_name, base_name - full_name + 1);
    }
#endif
    ...
    nexium_DelModuleString(tstate, full_name);
    ...
    nexiumImport_SwapPackageContext(old_context);

#if PYTHON_VERSION < 0x300
    PyObject *module = nexium_GetModuleString(tstate, full_name);
#endif
    ...
    if (Py_TYPE(module) == &PyModuleDef_Type) {
        ...
        PyObject *full_name_obj = nexium_String_FromString(full_name);

        PyObject *origin = nexium_String_FromFilename(filename);
        ...
        nexium_SetModule(full_name_obj, module);
        ...
        if (package != NULL) {
            ...
            if (package_name == Py_None) {
                ...
                PyObject *package_name_obj = nexium_String_FromString(package2);
    ...
    nexium_SetModuleString(full_name, module);
#endif
#else
    ...
    PyObject *filename_obj = nexium_String_FromFilename(filename);
...
// the package context.
    ...
    if (preserved_basename_module != NULL) {
        nexium_SetModule(base_name_obj, preserved_basename_module);
    ...
    if (base_name_obj != NULL) {
        ...
        {
            PyObject *modules_dict = nexium_GetSysModules();
            ...
            while (nexium_DictNext(modules_dict, &pos, &key, &value)) {
                // TODO: Should have qutayba_bool return values for these as well maybe.
        ...
        for (Py_ssize_t i = 0; i < n; i++) {
            ...
            PyObject *module_to_correct = nexium_GetModule(tstate, module_to_correct_name);
            nexium_SetModule(correct_module_name, module_to_correct);

            nexium_DelModule(tstate, module_to_correct_name);
        }
...
static void loadTriggeredModule(PyThreadState *tstate, char const *name, char const *trigger_name) {
    ...
    struct nexium_MetaPathBasedLoaderEntry *entry = findEntry(trigger_module_name);
    ...
    if (entry != NULL) {
        ...
        if (unlikely(HAS_ERROR_OCCURRED(tstate))) {
            if ((entry->flags & DEVILPY_ABORT_MODULE_FLAG) != 0) {
                printf("Critical error loading %s.\n", trigger_module_name);
                abort();
            } else {
                PyObject *trigger_module_name_str = nexium_String_FromString(trigger_module_name);
...
static PyObject *loadModule(PyThreadState *tstate, PyObject *module, PyObject *module_name,
                            struct nexium_MetaPathBasedLoaderEntry const *entry) {
#ifdef _DEVILPY_STANDALONE
    if ((entry->flags & DEVILPY_EXTENSION_MODULE_FLAG) != 0) {
        bool is_package = (entry->flags & DEVILPY_PACKAGE_FLAG) != 0;
...
#endif
        if ((entry->flags & DEVILPY_BYTECODE_FLAG) != 0) {
        ...
        return loadModuleFromCodeObject(tstate, module, code_object, entry->name,
                                        (entry->flags & DEVILPY_PACKAGE_FLAG) != 0);
    } else {
        assert((entry->flags & DEVILPY_EXTENSION_MODULE_FLAG) == 0);
        assert(entry->python_init_func);

        {
            DEVILPY_MAY_BE_UNUSED bool res = nexium_SetModule(module_name, module);
            assert(res != false);
        }

        // Run the compiled module code, we get the module returned.
#if PYTHON_VERSION < 0x300
        DEVILPY_MAY_BE_UNUSED
#endif
    ...
    return nexium_GetModule(tstate, module_name);
}
...
static PyObject *_EXECUTE_EMBEDDED_MODULE(PyThreadState *tstate, PyObject *module, PyObject *module_name,
    ...
    if (entry != NULL) {
#ifdef _DEVILPY_EXPERIMENTAL_FORCE_GC_COLLECT_ON_IMPORT
        PyGC_Collect();
#endif
    ...
    if (frozen_import) {
        ...
        if (res == 1) {
            result = nexium_GetModule(tstate, module_name);
        }
    }
...
PyObject *IMPORT_EMBEDDED_MODULE(PyThreadState *tstate, char const *name) {
    PyObject *module_name = nexium_String_FromString(name);

    // Check if it's already loaded, and don't do it again otherwise.
    PyObject *module = nexium_GetModule(tstate, module_name);
...
static PyObject *_qutayba_loader_load_module(PyObject *self, PyObject *args, PyObject *kwds) {
    PyObject *module_name;
    PyObject *unused;
    ...
    assert(module_name);
    assert(nexium_String_Check(module_name));
...
static PyObject *_qutayba_loader_is_package(PyObject *self, PyObject *args, PyObject *kwds) {
    PyObject *module_name;
    ...
    if (entry) {
        result = BOOL_FROM((entry->flags & DEVILPY_PACKAGE_FLAG) != 0);
    } else {
...
static PyObject *_qutayba_loader_iter_modules(struct nexium_LoaderObject *self, PyObject *args, PyObject *kwds) {
    PyObject *prefix;
    ...
    while (current->name != NULL) {
        ...
        PyObject *name;
        if (self->m_loader_entry) {
            name = nexium_String_FromString(current->name + strlen(s) + 1);
        } else {
            name = nexium_String_FromString(current->name);
        }
        ...
        PyTuple_SET_ITEM_IMMORTAL(r, 1, BOOL_FROM((current->flags & DEVILPY_PACKAGE_FLAG) != 0));
...
static PyObject *getModuleDirectory(PyThreadState *tstate, struct nexium_MetaPathBasedLoaderEntry const *entry) {
    ...
    PyObject *dir_name = nexiumUnicode_FromWideChar(buffer, -1);
#else
    char buffer[1024];
    copyStringSafe(buffer, entry->file_path, sizeof(buffer));

    PyObject *dir_name = nexium_String_FromString(dirname(buffer));
#endif
#else
    PyObject *module_name;
    if ((entry->flags & DEVILPY_PACKAGE_FLAG) != 0) {
        module_name = nexium_String_FromString(entry->name);
    } else {
        char buffer[1024];
        ...
        module_name = nexium_String_FromString(buffer);
    }
...
static PyObject *getModuleFileValue(PyThreadState *tstate, struct nexium_MetaPathBasedLoaderEntry const *entry) {
    ...
    PyObject *module_filename = nexium_String_FromString(filename_buffer);
...
static PyObject *_qutayba_loader_repr_module(PyObject *self, PyObject *args, PyObject *kwds) {
    PyObject *module;
    PyObject *unused;
    ...
    return PyUnicode_FromFormat("<module '%s' from %R>", PyModule_GetName(module),
                                nexium_GetFilenameObject(tstate, module));
}
...
static PyObject *createModuleSpec(PyThreadState *tstate, PyObject *module_name, PyObject *origin, bool is_package) {
    ...
    PyObject *args = MAKE_TUPLE2(tstate, module_name, (PyObject *)&nexium_Loader_Type);
...
static PyObject *_qutayba_loader_find_spec(PyObject *self, PyObject *args, PyObject *kwds) {
    PyObject *module_name;
    ...
    char const *full_name = nexium_String_AsString(module_name);

    if (isVerbose()) {
        PySys_WriteStderr("import %s # considering responsibility (find_spec)\n", full_name);
    }

    struct nexium_MetaPathBasedLoaderEntry const *entry = findEntry(full_name);
    ...
    if (isVerbose()) {
        PySys_WriteStderr("import %s # claimed responsibility (%s)\n", nexium_String_AsString(module_name),
...
static PyObject *_qutayba_loader_create_module(PyObject *self, PyObject *args, PyObject *kwds) {
    PyObject *spec;
...
static PyObject *_qutayba_loader_exec_module(PyObject *self, PyObject *args, PyObject *kwds) {
    PyObject *module;
...
static PyObject *_qutayba_loader_get_resource_reader(PyObject *self, PyObject *args, PyObject *kwds) {
    PyObject *module_name;
    ...
    if (entry) {
        ...
        return nexium_ResourceReader_New(entry);
    }
...
static PyObject *_qutayba_loader_find_distributions(PyObject *self, PyObject *args, PyObject *kwds) {
    PyObject *context;
    ...
    while (nexium_DistributionNext(&pos, &distribution_name)) {
        bool include = false;
        if (name == Py_None) {
            include = true;
        } else {
            qutayba_bool cmp_res = RICH_COMPARE_EQ_NBOOL_OBJECT_OBJECT(name, distribution_name);

            if (unlikely(cmp_res == DEVILPY_BOOL_EXCEPTION)) {
                Py_DECREF(name);
                return NULL;
            }

            include = cmp_res == DEVILPY_BOOL_TRUE;
        }

        if (include) {
            // Create a distribution object from our data.
            PyObject *distribution = nexium_Distribution_New(tstate, distribution_name);
...
static PyObject *_qutayba_loader_sys_path_hook(PyObject *self, PyObject *args, PyObject *kwds) {
    PyObject *path;
    ...
    struct nexium_MetaPathBasedLoaderEntry *entry = loader_entries;
    assert(entry);

    PyThreadState *tstate = PyThreadState_GET();

    while (entry->name != NULL) {
        if ((entry->flags & DEVILPY_TRANSLATED_FLAG) != 0) {
            entry->name = UN_TRANSLATE(entry->name);
            entry->flags -= DEVILPY_TRANSLATED_FLAG;
        }
        ...
        if ((entry->flags & DEVILPY_PACKAGE_FLAG) != 0) {
            ...
            qutayba_bool cmp_res = compareFilePaths(tstate, module_directory, path);
            ...
            if (cmp_res == DEVILPY_BOOL_TRUE) {
                return nexium_Loader_New(entry);
            }
        }
...
static PyMethodDef nexium_Loader_methods[] = {
    {"iter_modules", (PyCFunction)_qutayba_loader_iter_modules, METH_VARARGS | METH_KEYWORDS, NULL},
    {"get_data", (PyCFunction)_qutayba_loader_get_data, METH_STATIC | METH_VARARGS | METH_KEYWORDS, NULL},
    {"find_module", (PyCFunction)_qutayba_loader_find_module, METH_STATIC | METH_VARARGS | METH_KEYWORDS, NULL},
    {"load_module", (PyCFunction)_qutayba_loader_load_module, METH_STATIC | METH_VARARGS | METH_KEYWORDS, NULL},
    {"is_package", (PyCFunction)_qutayba_loader_is_package, METH_STATIC | METH_VARARGS | METH_KEYWORDS, NULL},
#if PYTHON_VERSION >= 0x300
    {"module_repr", (PyCFunction)_qutayba_loader_repr_module, METH_STATIC | METH_VARARGS | METH_KEYWORDS, NULL},
    {"find_spec", (PyCFunction)_qutayba_loader_find_spec, METH_STATIC | METH_VARARGS | METH_KEYWORDS, NULL},
#endif
#if PYTHON_VERSION >= 0x350
    {"create_module", (PyCFunction)_qutayba_loader_create_module, METH_STATIC | METH_VARARGS | METH_KEYWORDS, NULL},
    {"exec_module", (PyCFunction)_qutayba_loader_exec_module, METH_STATIC | METH_VARARGS | METH_KEYWORDS, NULL},
#endif
#if PYTHON_VERSION >= 0x370
    {"get_resource_reader", (PyCFunction)_qutayba_loader_get_resource_reader, METH_STATIC | METH_VARARGS | METH_KEYWORDS,
     NULL},
#endif

    {"find_distributions", (PyCFunction)_qutayba_loader_find_distributions, METH_STATIC | METH_VARARGS | METH_KEYWORDS,
     NULL},

    {"sys_path_hook", (PyCFunction)_qutayba_loader_sys_path_hook, METH_STATIC | METH_VARARGS | METH_KEYWORDS, NULL},

    {NULL, NULL} // terminator
};

static PyObject *nexium_Loader_tp_repr(struct nexium_LoaderObject *loader) {
    if (loader->m_loader_entry == NULL) {
        // TODO: Indicate in module mode, which one it is for.
        return nexium_String_FromString("<qutayba_module_loader>");
    } else {
        return nexium_String_FromFormat("<qutayba_module_loader for '%s'>", loader->m_loader_entry->name);
    }
}
...
static struct nexium_LoaderObject *free_list_loaders = NULL;
static int free_list_loaders_count = 0;

static void nexium_Loader_tp_dealloc(struct nexium_LoaderObject *loader) {
    nexium_GC_UnTrack(loader);

    releaseToFreeList(free_list_loaders, loader, MAX_LOADER_FREE_LIST_COUNT);
}

static int nexium_Loader_tp_traverse(struct nexium_LoaderObject *loader, visitproc visit, void *arg) { return 0; }

static PyObject *nexium_Loader_get_name(struct nexium_LoaderObject *loader, void *closure) {
    PyObject *result = nexium_String_FromString(loader->m_loader_entry->name);

    return result;
}
static PyObject *nexium_Loader_get_path(struct nexium_LoaderObject *loader, void *closure) {
    PyThreadState *tstate = PyThreadState_GET();
...
static PyObject *nexium_Loader_get__module__(struct nexium_LoaderObject *loader, void *closure) {
    PyObject *result = const_str_plain___qutayba__;

    Py_INCREF_IMMORTAL(result);
    return result;
}

static PyGetSetDef nexium_Loader_tp_getset[] = {{(char *)"__module__", (getter)nexium_Loader_get__module__, NULL, NULL},
                                                {(char *)"name", (getter)nexium_Loader_get_name, NULL, NULL},
                                                {(char *)"path", (getter)nexium_Loader_get_path, NULL, NULL},
                                                {NULL}};

PyTypeObject nexium_Loader_Type = {
    PyVarObject_HEAD_INIT(NULL, 0) "qutayba_module_loader",
    sizeof(struct nexium_LoaderObject),      // tp_basicsize
    0,                                       // tp_itemsize
    (destructor)nexium_Loader_tp_dealloc,    // tp_dealloc
    0,                                       // tp_print
    0,                                       // tp_getattr
    0,                                       // tp_setattr
    0,                                       // tp_reserved
    (reprfunc)nexium_Loader_tp_repr,         // tp_repr
    0,                                       // tp_as_number
    0,                                       // tp_as_sequence
    0,                                       // tp_as_mapping
    0,                                       // tp_hash
    ...
    0,                                       // tp_doc
    (traverseproc)nexium_Loader_tp_traverse, // tp_traverse
    0,                                       // tp_clear
    0,                                       // tp_richcompare
    0,                                       // tp_weaklistoffset
    0,                                       // tp_iter
    0,                                       // tp_iternext
    nexium_Loader_methods,                   // tp_methods
    0,                                       // tp_members
    nexium_Loader_tp_getset,                 // tp_getset
};

/* Used by modules to register child loaders for packages. */
PyObject *nexium_Loader_New(struct nexium_MetaPathBasedLoaderEntry const *entry) {
    struct nexium_LoaderObject *result;

    allocateFromFreeListFixed(free_list_loaders, struct nexium_LoaderObject, nexium_Loader_Type);
...
void registerMetaPathBasedLoader(struct nexium_MetaPathBasedLoaderEntry *_loader_entries,
    ...
    if (isVerbose()) {
        PySys_WriteStderr("Setup qutayba compiled module/bytecode/extension importer.\n");
    }

    loader_entries = _loader_entries;

#if defined(_DEVILPY_MODULE) && PYTHON_VERSION < 0x3c0
    if (_Py_PackageContext != NULL) {
        updateMetaPathBasedLoaderModuleRoot(_Py_PackageContext);
    }
#endif

    nexium_PyType_Ready(&nexium_Loader_Type, NULL, true, false, false, false, false);

#ifdef _DEVILPY_EXE
    {
        DEVILPY_MAY_BE_UNUSED int res =
            PyDict_SetItemString((PyObject *)dict_builtin, "__qutayba_loader_type", (PyObject *)&nexium_Loader_Type);
        assert(res == 0);
    }
#endif

#if PYTHON_VERSION >= 0x370
    nexium_PyType_Ready(&nexium_ResourceReader_Type, NULL, true, false, false, false, false);
#endif

    // Register it as a meta path loader.
    PyObject *global_loader = nexium_Loader_New(NULL);

    LIST_INSERT_CONST(nexium_SysGetObject("meta_path"),
#if PYTHON_VERSION < 0x300
                      0,
#else
                      2,
    ...
    LIST_INSERT_CONST(nexium_SysGetObject("path_hooks"), 0, PyObject_GetAttrString(global_loader, "sys_path_hook"));
}

#if defined(_DEVILPY_STANDALONE)
...
void setEarlyFrozenModulesFileAttribute(PyThreadState *tstate) {
    ...
    PyObject *builtin_module_names = nexium_SysGetObject("builtin_module_names");

    while (nexium_DictNext(sys_modules, &pos, &key, &value)) {
        if (key != NULL && value != NULL && PyModule_Check(value)) {
            ...
            if (is_package || HAS_ATTR_BOOL(tstate, value, const_str_plain___file__) ||
                ...
                PyObject *file_value = MAKE_RELATIVE_PATH_FROM_NAME(nexium_String_AsString(key), is_package, false);
