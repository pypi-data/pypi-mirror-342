...
#include "qutayba/checksum_tools.h"
...
static DWORD nexium_GetFinalPathNameByHandleW(HANDLE hFile, LPWSTR FilePath, DWORD cchFilePath, DWORD dwFlags) {
...
static void resolveFileSymbolicLink(wchar_t *resolved_filename, wchar_t const *filename, DWORD resolved_filename_size,
    ...
    if (resolve_symlinks) {
        ...
        DWORD len = nexium_GetFinalPathNameByHandleW(file_handle, resolved_filename, resolved_filename_size,
        ...
        if (*source == L'{') {
            ...
            *w = 0;
            ...
            if (wcsicmp(var_name, L"TEMP") == 0) {
                ...
                is_path = true;
#ifdef DEVILPY_COMPANY_NAME
            } else if (wcsicmp(var_name, L"COMPANY") == 0) {
                appendWStringSafeW(target, L"" DEVILPY_COMPANY_NAME, buffer_size);
#endif
#ifdef DEVILPY_PRODUCT_NAME
            } else if (wcsicmp(var_name, L"PRODUCT") == 0) {
                appendWStringSafeW(target, L"" DEVILPY_PRODUCT_NAME, buffer_size);
#endif
#ifdef DEVILPY_VERSION_COMBINED
            } else if (wcsicmp(var_name, L"VERSION") == 0) {
                appendWStringSafeW(target, L"" DEVILPY_VERSION_COMBINED, buffer_size);
#endif
...
bool expandTemplatePath(char *target, char const *source, size_t buffer_size) {
    ...
    DEVILPY_MAY_BE_UNUSED bool var_started = false;
        ...
        if (*source == '{') {
            ...
            *w = 0;
            ...
            if (strcasecmp(var_name, "TEMP") == 0) {
                ...
                appendStringSafe(target, DEVILPY_COMPANY_NAME, buffer_size);
#endif
#ifdef DEVILPY_PRODUCT_NAME
            } else if (strcasecmp(var_name, "PRODUCT") == 0) {
                appendStringSafe(target, DEVILPY_PRODUCT_NAME, buffer_size);
#endif
#ifdef DEVILPY_VERSION_COMBINED
            } else if (strcasecmp(var_name, "VERSION") == 0) {
                appendStringSafe(target, DEVILPY_VERSION_COMBINED, buffer_size);
#endif
