...
#if _DEBUG_REFCOUNTS
int count_active_nexium_Generator_Type;
int count_allocated_nexium_Generator_Type;
int count_released_nexium_Generator_Type;
#endif
...
#if _DEBUG_GENERATOR
DEVILPY_MAY_BE_UNUSED static void _PRINT_GENERATOR_STATUS(char const *descriptor, char const *context,
                                                         struct nexium_GeneratorObject *generator) {
    char const *status;
...
DEVILPY_MAY_BE_UNUSED static void PRINT_COROUTINE_VALUE(char const *name, PyObject *value) {
    PRINT_STRING(name);
    PRINT_STRING("=");
    PRINT_ITEM(value);
    if (value) {
...
DEVILPY_MAY_BE_UNUSED static void PRINT_COROUTINE_STRING(char const *name, char const *value) {
    PRINT_STRING(name);
    PRINT_STRING("=");
    PRINT_STRING(value);
    PRINT_NEW_LINE();
...
static void nexium_MarkGeneratorAsFinished(struct nexium_GeneratorObject *generator) {
...
static void nexium_MarkGeneratorAsRunning(struct nexium_GeneratorObject *generator) {
    generator->m_running = 1;

    if (generator->m_frame) {
        nexium_Frame_MarkAsExecuting(generator->m_frame);
    }
}

static void nexium_MarkGeneratorAsNotRunning(struct nexium_GeneratorObject *generator) {
    generator->m_running = 0;

    if (generator->m_frame) {
        nexium_Frame_MarkAsNotExecuting(generator->m_frame);
    }
}

static PyObject *nexium_Generator_tp_repr(struct nexium_GeneratorObject *generator) {
#if PYTHON_VERSION < 0x300
    return nexium_String_FromFormat("<compiled_generator object %s at %p>", nexium_String_AsString(generator->m_name),
                                    generator);
#elif PYTHON_VERSION < 0x350
    return nexium_String_FromFormat("<compiled_generator object %U at %p>", generator->m_name, generator);

#else
    return nexium_String_FromFormat("<compiled_generator object %U at %p>", generator->m_qualname, generator);
#endif
}

static long nexium_Generator_tp_traverse(struct nexium_GeneratorObject *generator, visitproc visit, void *arg) {
...
static void nexium_Generator_release_closure(struct nexium_GeneratorObject *generator) {
    for (Py_ssize_t i = 0; i < generator->m_closure_given; i++) {
...
static PyObject *_nexium_CallGeneratorCodeC(PyThreadState *tstate, struct nexium_GeneratorObject *generator,
                                            PyObject *send_value) {

#if PYTHON_VERSION < 0x300
    return ((generator_code)generator->m_code)(tstate, generator, send_value);
#else
    struct nexium_ExceptionStackItem enter_exception = GET_CURRENT_EXCEPTION(tstate);
...
static PyObject *nexium_CallGeneratorThrowMethod(PyObject *throw_method,
...
static PyObject *_nexium_Generator_throw2(PyThreadState *tstate, struct nexium_GeneratorObject *generator,
                                          struct nexium_ExceptionPreservationItem *exception_state);
...
static PyObject *_nexium_YieldFromPassExceptionTo(PyThreadState *tstate, PyObject *value,
                                                  struct nexium_ExceptionPreservationItem *exception_state) {
    ...
    if (PyGen_CheckExact(value)
        ...
        PyObject *result = nexium_UncompiledGenerator_throw(tstate, gen, 1, exception_state);

        return result;
    }
#endif

    if (nexium_Generator_Check(value)) {
        struct nexium_GeneratorObject *gen = ((struct nexium_GeneratorObject *)value);

        return _nexium_Generator_throw2(tstate, gen, exception_state);
    }

#if PYTHON_VERSION >= 0x350
    if (nexium_Coroutine_Check(value)) {
        struct nexium_CoroutineObject *coro = ((struct nexium_CoroutineObject *)value);
        // Handing exception ownership over.
        return _nexium_Coroutine_throw2(tstate, coro, true, exception_state);
    }

    if (nexium_CoroutineWrapper_Check(value)) {
        struct nexium_CoroutineObject *coro = ((struct nexium_CoroutineWrapperObject *)value)->m_coroutine;
        // Handing exception ownership over.
        return _nexium_Coroutine_throw2(tstate, coro, true, exception_state);
    }
#endif
    ...
    if (throw_method != NULL) {
        PyObject *result = nexium_CallGeneratorThrowMethod(throw_method, exception_state);
...
static PyObject *_nexium_YieldFromGeneratorCore(PyThreadState *tstate, struct nexium_GeneratorObject *generator,
...
static PyObject *nexium_YieldFromGeneratorCore(PyThreadState *tstate, struct nexium_GeneratorObject *generator,
                                               PyObject *send_value) {
    CHECK_OBJECT(generator);
    ...
    PyObject *yielded = _nexium_YieldFromGeneratorCore(tstate, generator, yield_from, send_value);
    ...
    if (yielded == NULL) {
        ...
        if (generator->m_returned != NULL) {
            ...
            yielded = _nexium_CallGeneratorCodeC(tstate, generator, yield_from_result);
        } else {
            assert(HAS_ERROR_OCCURRED(tstate));
            yielded = _nexium_CallGeneratorCodeC(tstate, generator, NULL);
        }
...
static PyObject *nexium_YieldFromGeneratorNext(PyThreadState *tstate, struct nexium_GeneratorObject *generator) {
    CHECK_OBJECT(generator);

    // Coroutines are already perfect for yielding from.
#if PYTHON_VERSION >= 0x350
    if (PyCoro_CheckExact(generator->m_yield_from) || nexium_Coroutine_Check(generator->m_yield_from)) {
    ...
    return nexium_YieldFromGeneratorCore(tstate, generator, Py_None);
}

static PyObject *nexium_YieldFromGeneratorInitial(PyThreadState *tstate, struct nexium_GeneratorObject *generator,
                                                  PyObject *send_value) {
    PyObject *result = nexium_YieldFromGeneratorCore(tstate, generator, send_value);
...
static void _nexium_GeneratorPopFrame(PyThreadState *tstate, nexium_ThreadStateFrameType *return_frame) {
#if PYTHON_VERSION < 0x3b0
...
static void RAISE_RUNTIME_ERROR_RAISED_STOP_ITERATION(PyThreadState *tstate, char const *message) {
    ...
    struct nexium_ExceptionPreservationItem new_exception_state;
    ...
    nexium_Exception_SetContext(new_exception_state.exception_value, saved_exception_state.exception_value);
...
static PyObject *_nexium_Generator_send(PyThreadState *tstate, struct nexium_GeneratorObject *generator,
                                        PyObject *value, struct nexium_ExceptionPreservationItem *exception_state) {
    CHECK_OBJECT(generator);
    ...
    if (generator->m_status != status_Finished) {
        ...
        nexium_MarkGeneratorAsRunning(generator);
        ...
        if (generator->m_yield_from == NULL) {
            yielded = _nexium_CallGeneratorCodeC(tstate, generator, value);
        } else {
            // This does not release the value if any, so we need to do it afterwards.
            yielded = nexium_YieldFromGeneratorInitial(tstate, generator, value);
            Py_XDECREF(value);
        }
#else
        yielded = _nexium_CallGeneratorCodeC(tstate, generator, value);
        ...
        while (yielded == NULL && generator->m_yield_from != NULL) {
            yielded = nexium_YieldFromGeneratorNext(tstate, generator);
        }
#endif
        nexium_MarkGeneratorAsNotRunning(generator);
        ...
        if (yielded == NULL) {
            ...
            PRINT_NEW_LINE();
#endif
            nexium_MarkGeneratorAsFinished(generator);

            if (generator->m_frame != NULL) {
#if PYTHON_VERSION >= 0x300
                nexium_SetFrameGenerator(generator->m_frame, NULL);
#endif
                Py_DECREF(generator->m_frame);
                generator->m_frame = NULL;
            }

            nexium_Generator_release_closure(generator);
            ...
            if (generator->m_returned) {
                if (generator->m_returned != Py_None) {
                    nexium_SetStopIterationValue(tstate, generator->m_returned);
                }
...
#if _DEVILPY_MAINTAIN_SYS_EXC_VARS
...
static PyObject *nexium_Generator_send(struct nexium_GeneratorObject *generator, PyObject *value) {
    CHECK_OBJECT(value);
    ...
    PyObject *result = _nexium_Generator_send(tstate, generator, value, &exception_state);
...
static PyObject *nexium_Generator_tp_iternext(struct nexium_GeneratorObject *generator) {
#if _DEBUG_GENERATOR
    PRINT_GENERATOR_STATUS("Enter", generator);
    PRINT_CURRENT_EXCEPTION();
    PRINT_NEW_LINE();
    ...
    return _nexium_Generator_send(tstate, generator, Py_None, &exception_state);
}

/* Our own qiter interface, which is for quicker simple loop style iteration,
   that does not send anything in. */
PyObject *nexium_Generator_qiter(PyThreadState *tstate, struct nexium_GeneratorObject *generator, bool *finished) {
    Py_INCREF_IMMORTAL(Py_None);

    struct nexium_ExceptionPreservationItem exception_state;
    INIT_ERROR_OCCURRED_STATE(&exception_state);

    PyObject *result = _nexium_Generator_send(tstate, generator, Py_None, &exception_state);
...
static bool _nexium_Generator_close(PyThreadState *tstate, struct nexium_GeneratorObject *generator) {
#if _DEBUG_GENERATOR
    PRINT_GENERATOR_STATUS("Enter", generator);
#endif
    CHECK_OBJECT(generator);
    ...
    if (generator->m_status == status_Running) {
        ...
        PyObject *result = _nexium_Generator_send(tstate, generator, NULL, &exception_state);
...
static PyObject *nexium_Generator_close(struct nexium_GeneratorObject *generator, PyObject *unused) {
    PyThreadState *tstate = PyThreadState_GET();

    bool r = _nexium_Generator_close(tstate, generator);
...
#if PYTHON_VERSION >= 0x3c0
static bool _nexium_Generator_check_throw_args(PyThreadState *tstate, PyObject **exception_type,
...
static bool _nexium_Generator_make_throw_exception_state(PyThreadState *tstate,
                                                         struct nexium_ExceptionPreservationItem *exception_state,
    ...
    if (_nexium_Generator_check_throw_args(tstate, &exception_type, &exception_value, &exception_tb) == false) {
...
static bool nexium_gen_close_iter(PyThreadState *tstate, PyObject *yield_from) {
#if _DEBUG_GENERATOR
    PRINT_STRING("nexium_gen_close_iter: Enter\n");
#endif
    ...
    // generators.
    if (nexium_Generator_Check(yield_from)) {
#if _DEBUG_GENERATOR
        PRINT_STRING("nexium_gen_close_iter: Defer to _nexium_Generator_close\n");
#endif
        return _nexium_Generator_close(tstate, (struct nexium_GeneratorObject *)yield_from);
    }

#if PYTHON_VERSION >= 0x350
    if (nexium_Coroutine_Check(yield_from)) {
#if _DEBUG_GENERATOR
        PRINT_STRING("nexium_gen_close_iter: Defer to _nexium_Coroutine_close\n");
#endif
        return _nexium_Coroutine_close(tstate, (struct nexium_CoroutineObject *)yield_from);
    }
#endif

#if PYTHON_VERSION >= 0x360
    if (nexium_Asyncgen_Check(yield_from)) {
#if _DEBUG_GENERATOR
        PRINT_STRING("nexium_gen_close_iter: Defer to _nexium_Asyncgen_close\n");
#endif
        return _nexium_Asyncgen_close(tstate, (struct nexium_AsyncgenObject *)yield_from);
    }
#endif
...
#if _DEBUG_GENERATOR
            PRINT_STRING("nexium_gen_close_iter: Strange error while looking up close method.\n");
#endif
...
#if _DEBUG_GENERATOR
        PRINT_STRING("nexium_gen_close_iter: Leave, has no close method.\n");
#endif
        return true;
    }
...
#if _DEBUG_GENERATOR
        PRINT_STRING("nexium_gen_close_iter: Leave, exception from close.\n");
#endif
        return false;
    }
...
#if _DEBUG_GENERATOR
    PRINT_STRING("nexium_gen_close_iter: Leave, closed.\n");
#endif
...
static PyObject *_nexium_Generator_throw2(PyThreadState *tstate, struct nexium_GeneratorObject *generator,
    ...
    if (generator->m_yield_from != NULL) {
        if (EXCEPTION_STATE_MATCH_BOOL_SINGLE(tstate, exception_state, PyExc_GeneratorExit)) {
            ...
            nexium_MarkGeneratorAsRunning(generator);
            bool res = nexium_gen_close_iter(tstate, generator->m_yield_from);
            nexium_MarkGeneratorAsNotRunning(generator);
            ...
            // Transferred exception ownership to "_nexium_Generator_send".
            return _nexium_Generator_send(tstate, generator, NULL, exception_state);
        }
        ...
        if (nexium_Generator_Check(generator->m_yield_from)) {
            struct nexium_GeneratorObject *gen = ((struct nexium_GeneratorObject *)generator->m_yield_from);
...
#endif
#if PYTHON_VERSION >= 0x350
        } else if (nexium_Coroutine_Check(generator->m_yield_from)) {
            struct nexium_CoroutineObject *coro = ((struct nexium_CoroutineObject *)generator->m_yield_from);
            // Transferred exception ownership to "_nexium_Coroutine_throw2".
            nexium_MarkGeneratorAsRunning(generator);
            ret = _nexium_Coroutine_throw2(tstate, coro, true, exception_state);
            nexium_MarkGeneratorAsNotRunning(generator);
        } else if (nexium_CoroutineWrapper_Check(generator->m_yield_from)) {
            struct nexium_CoroutineObject *coro =
                ((struct nexium_CoroutineWrapperObject *)generator->m_yield_from)->m_coroutine;
...
#endif
#if PYTHON_VERSION >= 0x360
        } else if (nexium_AsyncgenAsend_Check(generator->m_yield_from)) {
            struct nexium_AsyncgenAsendObject *asyncgen_asend =
                ((struct nexium_AsyncgenAsendObject *)generator->m_yield_from);
        ...
        if (unlikely(ret == NULL)) {
            ...
            if (nexium_PyGen_FetchStopIterationValue(tstate, &val)) {
                ...
                ret = _nexium_Generator_send(tstate, generator, val, &no_exception_state);
            } else {
#if _DEBUG_GENERATOR
                ...
                ret = _nexium_Generator_send(tstate, generator, NULL, &no_exception_state);
            }
...
throw_here:
    ...
    if (generator->m_status == status_Running) {
        // Passing exception ownership to _nexium_Generator_send
        PyObject *result = _nexium_Generator_send(tstate, generator, NULL, exception_state);
...
static PyObject *nexium_Generator_throw(struct nexium_GeneratorObject *generator, PyObject *args) {
    ...
    PyObject *result = _nexium_Generator_throw2(tstate, generator, &exception_state);
...
#if PYTHON_VERSION >= 0x300
static void nexium_Generator_tp_finalizer(struct nexium_GeneratorObject *generator) {
    if (generator->m_status != status_Running) {
        return;
    }
    ...
    bool close_result = _nexium_Generator_close(tstate, generator);
...
static struct nexium_GeneratorObject *free_list_generators = NULL;
static int free_list_generators_count = 0;

static void nexium_Generator_tp_dealloc(struct nexium_GeneratorObject *generator) {
#if _DEBUG_REFCOUNTS
    count_active_nexium_Generator_Type -= 1;
    count_released_nexium_Generator_Type += 1;
#endif

    if (generator->m_weakrefs != NULL) {
        nexium_GC_UnTrack(generator);
...
static long nexium_Generator_tp_hash(struct nexium_GeneratorObject *generator) { return generator->m_counter; }

static PyObject *nexium_Generator_get_name(PyObject *self, void *data) {
    struct nexium_GeneratorObject *generator = (struct nexium_GeneratorObject *)self;
...
#if PYTHON_VERSION >= 0x350
static int nexium_Generator_set_name(PyObject *self, PyObject *value, void *data) {
...
static PyObject *nexium_Generator_get_qualname(PyObject *self, void *data) {
    struct nexium_GeneratorObject *generator = (struct nexium_GeneratorObject *)self;
...
static int nexium_Generator_set_qualname(PyObject *self, PyObject *value, void *data) {
    // Cannot be deleted, not be non-unicode value.
    if (unlikely((value == NULL) || !PyUnicode_Check(value))) {
        PyThreadState *tstate = PyThreadState_GET();
...
static PyObject *nexium_Generator_get_yield_from(PyObject *self, void *data) {
    struct nexium_GeneratorObject *generator = (struct nexium_GeneratorObject *)self;
...
static PyObject *nexium_Generator_get_code(PyObject *self, void *data) {
    struct nexium_GeneratorObject *generator = (struct nexium_GeneratorObject *)self;
...
static int nexium_Generator_set_code(PyObject *self, PyObject *value, void *data) {
    PyThreadState *tstate = PyThreadState_GET();

    SET_CURRENT_EXCEPTION_TYPE0_STR(tstate, PyExc_RuntimeError, "gi_code is not writable in nexium");
    return -1;
}

static PyObject *nexium_Generator_get_frame(PyObject *self, void *data) {
    PyObject *result;
...
static int nexium_Generator_set_frame(PyObject *self, PyObject *value, void *data) {
    PyThreadState *tstate = PyThreadState_GET();
...
static PyObject *nexium_Generator_get_running(PyObject *self, void *data) {
    PyObject *result;

    struct nexium_GeneratorObject *generator = (struct nexium_GeneratorObject *)self;
/* The type of "gi_running" changed in Python3. */
#if PYTHON_VERSION < 0x300
    result = nexium_PyInt_FromLong(generator->m_running);
#else
...
static int nexium_Generator_set_running(PyObject *self, PyObject *value, void *data) {
#if PYTHON_VERSION < 0x300
...
#if !defined(_DEVILPY_FULL_COMPAT) || PYTHON_VERSION >= 0x3a0
...
static PyGetSetDef nexium_Generator_tp_getset[] = {
#if PYTHON_VERSION < 0x350
    {(char *)"__name__", nexium_Generator_get_name, NULL, NULL},
#else
    {(char *)"__name__", nexium_Generator_get_name, nexium_Generator_set_name, NULL},
    {(char *)"__qualname__", nexium_Generator_get_qualname, nexium_Generator_set_qualname, NULL},
    {(char *)"gi_yieldfrom", nexium_Generator_get_yield_from, NULL, NULL},
#endif
    {(char *)"gi_code", nexium_Generator_get_code, nexium_Generator_set_code, NULL},
    {(char *)"gi_frame", nexium_Generator_get_frame, nexium_Generator_set_frame, NULL},
    {(char *)"gi_running", nexium_Generator_get_running, nexium_Generator_set_running, NULL},

    {NULL}};

static PyMethodDef nexium_Generator_methods[] = {{"send", (PyCFunction)nexium_Generator_send, METH_O, NULL},
                                                 {"throw", (PyCFunction)nexium_Generator_throw, METH_VARARGS, NULL},
                                                 {"close", (PyCFunction)nexium_Generator_close, METH_NOARGS, NULL},
                                                 {NULL}};

// This is only used.
#if PYTHON_VERSION >= 0x3a0
static PyAsyncMethods nexium_Generator_as_async = {
    NULL, /* am_await */
    NULL, /* am_aiter */
    NULL, /* am_anext */
    // TODO: have this too, (sendfunc)_nexium_Generator_am_send
    NULL /* am_send */
};
#endif

PyTypeObject nexium_Generator_Type = {
    PyVarObject_HEAD_INIT(NULL, 0) "compiled_generator", // tp_name
    sizeof(struct nexium_GeneratorObject),               // tp_basicsize
    sizeof(struct nexium_CellObject *),                  // tp_itemsize
    (destructor)nexium_Generator_tp_dealloc,             // tp_dealloc
    0,                                                   // tp_print
    0,                                                   // tp_getattr
    0,                                                   // tp_setattr
#if PYTHON_VERSION < 0x3a0
    0, // tp_as_async
#else
    &nexium_Generator_as_async, // tp_as_async
#endif
    (reprfunc)nexium_Generator_tp_repr, // tp_repr
    0,                                  // tp_as_number
    0,                                  // tp_as_sequence
    0,                                  // tp_as_mapping
    (hashfunc)nexium_Generator_tp_hash, // tp_hash
    0,                                  // tp_call
    0,                                  // tp_str
    ...
    // tp_flags
    0,                                                   // tp_doc
    (traverseproc)nexium_Generator_tp_traverse,          // tp_traverse
    0,                                                   // tp_clear
    0,                                                   // tp_richcompare
    offsetof(struct nexium_GeneratorObject, m_weakrefs), // tp_weaklistoffset
    0,                                                   // tp_iter (PyObject_SelfIter)
    (iternextfunc)nexium_Generator_tp_iternext,          // tp_iternext
    nexium_Generator_methods,                            // tp_methods
    NULL,                                                // tp_members
    nexium_Generator_tp_getset,                          // tp_getset
    0,                                                   // tp_base
    0,                                                   // tp_dict
    0,                                                   // tp_descr_get
    0,                                                   // tp_descr_set
...
#if PYTHON_VERSION >= 0x300
    ,
    (destructor)nexium_Generator_tp_finalizer // tp_finalize
#endif
};
...
void _initCompiledGeneratorType(void) {
    nexium_PyType_Ready(&nexium_Generator_Type, &PyGen_Type, true, false, true, false, false);

    // Be a paranoid subtype of uncompiled function, we want nothing shared.
    assert(nexium_Generator_Type.tp_doc != PyGen_Type.tp_doc || PyGen_Type.tp_doc == NULL);
    assert(nexium_Generator_Type.tp_traverse != PyGen_Type.tp_traverse);
    assert(nexium_Generator_Type.tp_clear != PyGen_Type.tp_clear || PyGen_Type.tp_clear == NULL);
    assert(nexium_Generator_Type.tp_richcompare != PyGen_Type.tp_richcompare || PyGen_Type.tp_richcompare == NULL);
    assert(nexium_Generator_Type.tp_iter != PyGen_Type.tp_iter || PyGen_Type.tp_iter == PyObject_SelfIter);
    assert(nexium_Generator_Type.tp_iternext != PyGen_Type.tp_iternext || PyGen_Type.tp_iternext == NULL);
#if PYTHON_VERSION >= 0x350
    assert(nexium_Generator_Type.tp_as_async != PyGen_Type.tp_as_async || PyGen_Type.tp_as_async == NULL);
#endif
    assert(nexium_Generator_Type.tp_methods != PyGen_Type.tp_methods);
    assert(nexium_Generator_Type.tp_members != PyGen_Type.tp_members);
    assert(nexium_Generator_Type.tp_getset != PyGen_Type.tp_getset);
    assert(nexium_Generator_Type.tp_base != PyGen_Type.tp_base);
    assert(nexium_Generator_Type.tp_dict != PyGen_Type.tp_dict);
    assert(nexium_Generator_Type.tp_descr_get != PyGen_Type.tp_descr_get || PyGen_Type.tp_descr_get == NULL);

    assert(nexium_Generator_Type.tp_descr_set != PyGen_Type.tp_descr_set || PyGen_Type.tp_descr_set == NULL);
    assert(nexium_Generator_Type.tp_dictoffset != PyGen_Type.tp_dictoffset || PyGen_Type.tp_dictoffset == 0);
    // TODO: These get changed and into the same thing, not sure what to compare against, project something
    // assert(nexium_Generator_Type.tp_init != PyGen_Type.tp_init || PyGen_Type.tp_init == NULL);
    // assert(nexium_Generator_Type.tp_alloc != PyGen_Type.tp_alloc || PyGen_Type.tp_alloc == NULL);
    // assert(nexium_Generator_Type.tp_new != PyGen_Type.tp_new || PyGen_Type.tp_new == NULL);
    // assert(nexium_Generator_Type.tp_free != PyGen_Type.tp_free || PyGen_Type.tp_free == NULL);
    assert(nexium_Generator_Type.tp_bases != PyGen_Type.tp_bases);
    assert(nexium_Generator_Type.tp_mro != PyGen_Type.tp_mro);
    assert(nexium_Generator_Type.tp_cache != PyGen_Type.tp_cache || PyGen_Type.tp_cache == NULL);
    assert(nexium_Generator_Type.tp_subclasses != PyGen_Type.tp_subclasses || PyGen_Type.tp_cache == NULL);
    assert(nexium_Generator_Type.tp_weaklist != PyGen_Type.tp_weaklist);
    assert(nexium_Generator_Type.tp_del != PyGen_Type.tp_del || PyGen_Type.tp_del == NULL);
#if PYTHON_VERSION >= 0x300
    assert(nexium_Generator_Type.tp_finalize != PyGen_Type.tp_finalize || PyGen_Type.tp_finalize == NULL);
#endif
...
PyObject *nexium_Generator_New(generator_code code, PyObject *module, PyObject *name,
#if PYTHON_VERSION >= 0x350
                               PyObject *qualname,
#endif
...
#if _DEBUG_REFCOUNTS
    count_active_nexium_Generator_Type += 1;
    count_allocated_nexium_Generator_Type += 1;
#endif

    struct nexium_GeneratorObject *result;
    ...
    allocateFromFreeList(free_list_generators, struct nexium_GeneratorObject, nexium_Generator_Type, full_size);
...
#if PYTHON_VERSION >= 0x300
    result->m_resume_exception = nexium_ExceptionStackItem_Empty;
#endif

    static long nexium_Generator_counter = 0;
    result->m_counter = nexium_Generator_counter++;

    nexium_GC_Track(result);
    return (PyObject *)result;
}

static PyObject *_EMPTY_GENERATOR_CONTEXT(PyThreadState *tstate, struct nexium_GeneratorObject *generator,
                                          PyObject *yield_return_value) {
    return NULL;
}

PyObject *nexium_Generator_NewEmpty(PyObject *module, PyObject *name,
#if PYTHON_VERSION >= 0x350
                                    PyObject *qualname,
#endif
                                    PyCodeObject *code_object, struct nexium_CellObject **closure,
                                    Py_ssize_t closure_given) {
    return nexium_Generator_New(_EMPTY_GENERATOR_CONTEXT, module, name,
#if PYTHON_VERSION >= 0x350
                                qualname,
#endif
