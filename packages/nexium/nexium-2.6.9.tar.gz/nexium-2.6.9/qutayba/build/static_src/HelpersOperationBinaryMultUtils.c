...
static Py_ssize_t CONVERT_LONG_TO_REPEAT_FACTOR(PyObject *value) {
    ...
    Py_ssize_t digits_count = nexium_LongGetDigitSize(value);

    if (digits_count == 0) {
        return 0;
    }

    bool is_negative = nexium_LongIsNegative(value);

    PyLongObject *long_value = (PyLongObject *)value;

    digit *digits = nexium_LongGetDigitPointer(long_value);
