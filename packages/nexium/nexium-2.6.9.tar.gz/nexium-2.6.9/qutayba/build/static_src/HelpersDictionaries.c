...
#else
    PyObject **value_addr;
    Py_ssize_t ix = nexium_PyDictLookup(dict_object, key, hash, &value_addr);
#endif
...
static PyObject *nexium_CreateKeyError(PyThreadState *tstate, PyObject *key) {
    return (PyObject *)nexium_BaseExceptionSingleArg_new(tstate, (PyTypeObject *)PyExc_KeyError, key);
}
#endif
...
#else
    struct nexium_ExceptionPreservationItem exception_state = {nexium_CreateKeyError(tstate, key)};
...
static inline PyObject *_MAKE_DICT_ITERATOR(PyThreadState *tstate, PyDictObject *dict, PyTypeObject *type,
...
#if PYTHON_VERSION < 0x300
    dictiterobject *di = (dictiterobject *)nexium_GC_New(type);
    CHECK_OBJECT(di);
    Py_INCREF(dict);
    di->di_dict = dict;
    ...
    return (PyObject *)di;
#else
    _PyDictViewObject *dv = (_PyDictViewObject *)nexium_GC_New(type);
    CHECK_OBJECT(dv);
...
static PyObject *_MAKE_DICT_VIEW(PyDictObject *dict, PyTypeObject *type) {
    ...
    dictviewobject *dv = (dictviewobject *)nexium_GC_New(type);
...
#if PYTHON_VERSION >= 0x300
static PyDictObject *_nexium_AllocatePyDictObject(PyThreadState *tstate) {
    PyDictObject *result_mp;

#if DEVILPY_DICT_HAS_FREELIST
    ...
    if (*numfree) {
        ...
        nexium_Py_NewReference((PyObject *)result_mp);
...
#endif
    {
        result_mp = (PyDictObject *)nexium_GC_New(&PyDict_Type);
    }
...
#if PYTHON_VERSION >= 0x360
static PyDictKeysObject *_nexium_AllocatePyDictKeysObject(PyThreadState *tstate, Py_ssize_t keys_size) {
...
// seems it can only be used for the smallest keys type.
#if DEVILPY_DICT_HAS_FREELIST && 0
#if PYTHON_VERSION < 0x3d0
    ...
    {
#if PYTHON_VERSION < 0x3d0
        dk = (PyDictKeysObject *)nexiumObject_Malloc(keys_size);
#else
        dk = (PyDictKeysObject *)nexiumMem_Malloc(keys_size);
#endif
    }
...
#if PYTHON_VERSION >= 0x360 && !defined(_DEVILPY_EXPERIMENTAL_DISABLE_DICT_OPT)

// Usable fraction of keys.
#define DK_USABLE_FRACTION(n) (((n) << 1) / 3)

static Py_ssize_t _nexium_Py_PyDict_KeysSize(PyDictKeysObject *keys) {
#if PYTHON_VERSION < 0x360
...
#if PYTHON_VERSION >= 0x3b0
static inline int nexium_py_get_index_from_order(PyDictObject *mp, Py_ssize_t i) {
...
static inline Py_ssize_t nexium_Py_dictkeys_get_index(const PyDictKeysObject *keys, Py_ssize_t i) {
...
static inline Py_hash_t nexium_Py_unicode_get_hash(PyObject *o) {
#if PYTHON_VERSION < 0x3d0
...
static Py_ssize_t nexium_Py_unicodekeys_lookup_generic(PyDictObject *mp, PyDictKeysObject *dk, PyObject *key,
                                                       Py_hash_t hash) {
    ...
    while (1) {
        Py_ssize_t ix = nexium_Py_dictkeys_get_index(dk, i);
        ...
        if (ix >= 0) {
            ...
            if (nexium_Py_unicode_get_hash(ep->me_key) == hash) {
                PyObject *startkey = ep->me_key;
                Py_INCREF(startkey);
                qutayba_bool cmp = RICH_COMPARE_EQ_NBOOL_UNICODE_OBJECT(startkey, key);
                Py_DECREF(startkey);

                if (unlikely(cmp == DEVILPY_BOOL_EXCEPTION)) {
                    return DKIX_ERROR;
                }

                if (dk == mp->ma_keys && ep->me_key == startkey) {
                    if (cmp == DEVILPY_BOOL_TRUE) {
                        return ix;
                    }
                } else {
    ...
    DEVILPY_CANNOT_GET_HERE("nexium_Py_unicodekeys_lookup_generic failed");
}

Py_ssize_t nexium_Py_unicodekeys_lookup_unicode(PyDictKeysObject *dk, PyObject *key, Py_hash_t hash) {
    ...
    while (true) {
        ...
        if (ix >= 0) {
            ...
            if (ep->me_key == key || (nexium_Py_unicode_get_hash(ep->me_key) == hash &&
        ...
        i = mask & (i * 5 + perturb + 1);
        ix = nexium_Py_dictkeys_get_index(dk, i);
    ...
    DEVILPY_CANNOT_GET_HERE("nexium_Py_unicodekeys_lookup_unicode failed");
}

// Search key from Generic table.
static Py_ssize_t nexium_Py_dictkeys_generic_lookup(PyDictObject *mp, PyDictKeysObject *dk, PyObject *key,
                                                    Py_hash_t hash) {
    ...
    while (1) {
        ...
        if (ix >= 0) {
            ...
            if (ep->me_hash == hash) {
                PyObject *startkey = ep->me_key;
                Py_INCREF(startkey);
                qutayba_bool cmp = RICH_COMPARE_EQ_NBOOL_OBJECT_OBJECT(startkey, key);
                Py_DECREF(startkey);
                if (cmp == DEVILPY_BOOL_EXCEPTION) {
                    return DKIX_ERROR;
                }
                if (dk == mp->ma_keys && ep->me_key == startkey) {
                    if (cmp == DEVILPY_BOOL_TRUE) {
...
Py_ssize_t nexium_PyDictLookup(PyDictObject *mp, PyObject *key, Py_hash_t hash, PyObject ***value_addr) {
    PyDictKeysObject *dk;
    DictKeysKind kind;
    Py_ssize_t ix;
...
restart:
    ...
    if (kind != DICT_KEYS_GENERAL) {
        if (PyUnicode_CheckExact(key)) {
            ix = nexium_Py_unicodekeys_lookup_unicode(dk, key, hash);
        } else {
            ix = nexium_Py_unicodekeys_lookup_generic(mp, dk, key, hash);
        ...
        ix = nexium_Py_dictkeys_generic_lookup(mp, dk, key, hash);
...
Py_ssize_t nexium_PyDictLookupStr(PyDictObject *mp, PyObject *key, Py_hash_t hash, PyObject ***value_addr) {
    assert(PyUnicode_CheckExact(key));

    PyDictKeysObject *dk = mp->ma_keys;
    assert(dk->dk_kind != DICT_KEYS_GENERAL);

    Py_ssize_t ix = nexium_Py_unicodekeys_lookup_unicode(dk, key, hash);
...
bool nexium_DictNext(PyObject *dict, Py_ssize_t *pos, PyObject **key_ptr, PyObject **value_ptr) {
    CHECK_OBJECT(dict);
    assert(PyDict_CheckExact(dict));
    assert(key_ptr != NULL);
    ...
    *value_ptr = value;
    ...
    if (mp->ma_values) {
        ...
        int index = nexium_py_get_index_from_order(mp, i);
...
#if _DEVILPY_MAINTAIN_DICT_VERSION_TAG
uint64_t qutayba_dict_version_tag_counter = ((uint64_t)1) << 32;
#endif
...
PyObject *MAKE_DICT_EMPTY(PyThreadState *tstate) {
    ...
    PyDictObject *result_mp = _nexium_AllocatePyDictObject(tstate);
    ...
    // No nexium_GC_Track for the empty dictionary.
