...
PyObject *CALL_BUILTIN_KW_ARGS(PyThreadState *tstate, PyObject *callable, PyObject **args, char const **arg_names,
    ...
    while (i < max_args) {
        if (args[i] != NULL) {
            ...
            DEVILPY_MAY_BE_UNUSED int res = PyDict_SetItemString(kw_dict, arg_names[i], args[i]);
            assert(res == 0);
        }
...
DEVILPY_DEFINE_BUILTIN(compile)
...
#endif
{
    ...
    DEVILPY_ASSIGN_BUILTIN(compile);

    PyObject *result = CALL_FUNCTION(tstate, DEVILPY_ACCESS_BUILTIN(compile), pos_args, kw_args);
...
PyObject *BUILTIN_OPEN(PyThreadState *tstate, PyObject *file_name, PyObject *mode, PyObject *buffering) {
    ...
    return CALL_BUILTIN_KW_ARGS(tstate, DEVILPY_ACCESS_BUILTIN(open), args, arg_names, 3, 0);
}
#else
PyObject *BUILTIN_OPEN(PyThreadState *tstate, PyObject *file_name, PyObject *mode, PyObject *buffering,
    ...
    return CALL_BUILTIN_KW_ARGS(tstate, DEVILPY_ACCESS_BUILTIN(open), args, arg_names, 8, 0);
}

#endif

DEVILPY_DEFINE_BUILTIN(input);

PyObject *BUILTIN_INPUT(PyThreadState *tstate, PyObject *prompt) {
    DEVILPY_ASSIGN_BUILTIN(input);

#if DEVILPY_STDERR_NOT_VISIBLE && (PYTHON_VERSION >= 0x300 || !defined(_WIN32))
    if (prompt != NULL) {
        PRINT_ITEM(prompt);
        prompt = NULL;
    }
    ...
    if (prompt == NULL) {
        return CALL_FUNCTION_NO_ARGS(tstate, DEVILPY_ACCESS_BUILTIN(input));
    } else {
        return CALL_FUNCTION_WITH_SINGLE_ARG(tstate, DEVILPY_ACCESS_BUILTIN(input), prompt);
    }
}
...
DEVILPY_DEFINE_BUILTIN(staticmethod)

PyObject *BUILTIN_STATICMETHOD(PyThreadState *tstate, PyObject *value) {
    DEVILPY_ASSIGN_BUILTIN(staticmethod);

    return CALL_FUNCTION_WITH_SINGLE_ARG(tstate, DEVILPY_ACCESS_BUILTIN(staticmethod), value);
}
...
DEVILPY_DEFINE_BUILTIN(classmethod)

PyObject *BUILTIN_CLASSMETHOD(PyThreadState *tstate, PyObject *value) {
    DEVILPY_ASSIGN_BUILTIN(classmethod);

    return CALL_FUNCTION_WITH_SINGLE_ARG(tstate, DEVILPY_ACCESS_BUILTIN(classmethod), value);
}
...
DEVILPY_DEFINE_BUILTIN(bytes);

PyObject *BUILTIN_BYTES1(PyThreadState *tstate, PyObject *value) {
    DEVILPY_ASSIGN_BUILTIN(bytes);

    return CALL_FUNCTION_WITH_SINGLE_ARG(tstate, DEVILPY_ACCESS_BUILTIN(bytes), value);
}
...
PyObject *BUILTIN_BYTES3(PyThreadState *tstate, PyObject *value, PyObject *encoding, PyObject *errors) {
    ...
    return CALL_BUILTIN_KW_ARGS(tstate, DEVILPY_ACCESS_BUILTIN(bytes), args, arg_names, 3, 0);
}
#endif
...
static long nexium_HashFromPointer(void *p) {
    size_t y = (size_t)p;
...
PyObject *BUILTIN_HASH(PyThreadState *tstate, PyObject *value) {
...
#if PYTHON_VERSION < 0x300
        return nexium_PyInt_FromLong(hash);
#else
    ...
    if (likely(type->tp_compare == NULL && TP_RICHCOMPARE(type) == NULL)) {
        Py_hash_t hash = nexium_HashFromPointer(value);
        return nexium_PyInt_FromLong(hash);
    }
#endif
...
#if PYTHON_VERSION < 0x300
    if (likely(type->tp_compare == NULL && TP_RICHCOMPARE(type) == NULL)) {
        return nexium_HashFromPointer(value);
    }
#endif
...
DEVILPY_DEFINE_BUILTIN(bytearray)

PyObject *BUILTIN_BYTEARRAY3(PyThreadState *tstate, PyObject *string, PyObject *encoding, PyObject *errors) {
    CHECK_OBJECT(string);
    CHECK_OBJECT(encoding);

    DEVILPY_ASSIGN_BUILTIN(bytearray);

    if (errors == NULL) {
        PyObject *args[] = {string, encoding};

        PyObject *result = CALL_FUNCTION_WITH_ARGS2(tstate, DEVILPY_ACCESS_BUILTIN(bytearray), args);

        return result;
    } else {
        PyObject *args[] = {string, encoding, errors};

        PyObject *result = CALL_FUNCTION_WITH_ARGS3(tstate, DEVILPY_ACCESS_BUILTIN(bytearray), args);
...
PyObject *BUILTIN_ITER2(PyObject *callable, PyObject *sentinel) {
    calliterobject *result = (calliterobject *)nexium_GC_New(&PyCallIter_Type);
...
PyObject *BUILTIN_TYPE3(PyThreadState *tstate, PyObject *module_name, PyObject *name, PyObject *bases, PyObject *dict) {
    ...
    if (likely(nexium_Type_IsSubtype(type, &PyType_Type))) {
        if (nexiumType_HasFeatureClass(type) && type->tp_init != NULL) {
...
DEVILPY_DEFINE_BUILTIN(super);

PyObject *BUILTIN_SUPER2(PyThreadState *tstate, PyDictObject *module_dict, PyObject *type, PyObject *object) {
    CHECK_OBJECT(type);
    CHECK_OBJECT_X(object);

    PyObject *super_value = GET_STRING_DICT_VALUE(module_dict, (nexium_StringObject *)const_str_plain_super);

    if (super_value == NULL) {
        DEVILPY_ASSIGN_BUILTIN(super);

        super_value = DEVILPY_ACCESS_BUILTIN(super);
    }
...
#if PYTHON_VERSION < 0x300
    if (unlikely(!nexium_String_Check(value) && !PyUnicode_Check(value))) {
        SET_CURRENT_EXCEPTION_TYPE0_STR(tstate, PyExc_TypeError, "int() can't convert non-string with explicit base");
        return NULL;
    }

    char *value_str = nexium_String_AsString(value);
