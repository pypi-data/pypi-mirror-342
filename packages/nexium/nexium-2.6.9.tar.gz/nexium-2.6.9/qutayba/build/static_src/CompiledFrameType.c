...
#if _DEBUG_REFCOUNTS
int count_active_nexium_Frame_Type = 0;
int count_allocated_nexium_Frame_Type = 0;
int count_released_nexium_Frame_Type = 0;
#endif
...
static PyMemberDef nexium_Frame_members[] = {
...
#else
#define nexium_Frame_members 0
#endif

#if PYTHON_VERSION < 0x300

static PyObject *_nexium_Frame_get_exc_traceback(PyObject *self, void *data) {
    assert(nexium_Frame_CheckExact(self));
    CHECK_OBJECT(self);
    assert(_PyObject_GC_IS_TRACKED(self));

    struct nexium_FrameObject *frame = (struct nexium_FrameObject *)self;
...
static int _nexium_Frame_set_exc_traceback(PyObject *self, PyObject *traceback, void *data) {
    assert(nexium_Frame_CheckExact(self));
    CHECK_OBJECT(self);
    assert(_PyObject_GC_IS_TRACKED(self));
...
static PyObject *_nexium_Frame_get_exc_type(PyObject *self, void *data) {
    assert(nexium_Frame_CheckExact(self));
    CHECK_OBJECT(self);
    assert(_PyObject_GC_IS_TRACKED(self));
...
static int _nexium_Frame_set_exc_type(PyObject *self, PyObject *exception_type, void *data) {
    assert(nexium_Frame_CheckExact(self));
    CHECK_OBJECT(self);
    assert(_PyObject_GC_IS_TRACKED(self));
...
static PyObject *_nexium_Frame_get_exc_value(PyObject *self, void *data) {
    assert(nexium_Frame_CheckExact(self));
    CHECK_OBJECT(self);
    assert(_PyObject_GC_IS_TRACKED(self));
...
static int _nexium_Frame_set_exc_value(PyObject *self, PyObject *exception_value, void *data) {
    assert(nexium_Frame_CheckExact(self));
    CHECK_OBJECT(self);
    assert(_PyObject_GC_IS_TRACKED(self));
...
static PyObject *_nexium_Frame_get_restricted(PyObject *self, void *data) {
    assert(nexium_Frame_CheckExact(self));
    CHECK_OBJECT(self);
    assert(_PyObject_GC_IS_TRACKED(self));
...
static PyObject *_nexium_Frame_get_locals(PyObject *self, void *data) {
    assert(nexium_Frame_CheckExact(self));
    CHECK_OBJECT(self);
    assert(_PyObject_GC_IS_TRACKED(self));
    ...
    struct nexium_FrameObject *qutayba_frame = (struct nexium_FrameObject *)self;
    if (qutayba_frame->m_type_description == NULL) {
#if PYTHON_VERSION < 0x3b0
        PyFrameObject *locals_owner = &qutayba_frame->m_frame;
#else
        _PyInterpreterFrame *locals_owner = &qutayba_frame->m_interpreter_frame;
#endif
        ...
        PyObject **var_names = nexium_GetCodeVarNames(nexium_GetFrameCodeObject(qutayba_frame));

        char const *w = qutayba_frame->m_type_description;
        char const *t = qutayba_frame->m_locals_storage;

        while (*w != 0) {
            switch (*w) {
            case DEVILPY_TYPE_DESCRIPTION_OBJECT:
            case DEVILPY_TYPE_DESCRIPTION_OBJECT_PTR: {
            ...
            case DEVILPY_TYPE_DESCRIPTION_CELL: {
                struct nexium_CellObject *value = *(struct nexium_CellObject **)t;
                assert(nexium_Cell_Check((PyObject *)value));
                CHECK_OBJECT(value);
                ...
                t += sizeof(struct nexium_CellObject *);

                break;
            }
            case DEVILPY_TYPE_DESCRIPTION_NULL: {
                break;
            }
            case DEVILPY_TYPE_DESCRIPTION_BOOL: {
                int value = *(int *)t;
                t += sizeof(int);
                switch ((qutayba_bool)value) {
                case DEVILPY_BOOL_TRUE: {
                    DICT_SET_ITEM(result, *var_names, Py_True);
                    break;
                }
                case DEVILPY_BOOL_FALSE: {
...
static PyObject *_nexium_Frame_get_lineno(PyObject *self, void *data) {
    assert(nexium_Frame_CheckExact(self));
    CHECK_OBJECT(self);
    assert(_PyObject_GC_IS_TRACKED(self));

    struct nexium_FrameObject *frame = (struct nexium_FrameObject *)self;
    return nexium_PyInt_FromLong(frame->m_frame.f_lineno);
}

static PyObject *_nexium_Frame_get_trace(PyObject *self, void *data) {
    assert(nexium_Frame_CheckExact(self));
    CHECK_OBJECT(self);
    assert(_PyObject_GC_IS_TRACKED(self));
...
static int _nexium_Frame_set_trace(PyObject *self, PyObject *value, void *data) {
    assert(nexium_Frame_CheckExact(self));
    CHECK_OBJECT(self);
    assert(_PyObject_GC_IS_TRACKED(self));
#if !defined(_DEVILPY_DEPLOYMENT_MODE) && !defined(_DEVILPY_NO_DEPLOYMENT_FRAME_USELESS_SET_TRACE)
    if (value == Py_None) {
        return 0;
    } else {
        ...
        SET_CURRENT_EXCEPTION_TYPE0_STR(
            ...
            "f_trace is not writable in nexium, ignore with '--no-deployment-flag=frame-useless-set-trace'");
        return -1;
    }
#else
    return 0;
...
static PyObject *_nexium_Frame_get_trace_lines(PyObject *self, void *data) {
    assert(nexium_Frame_CheckExact(self));
    CHECK_OBJECT(self);
    assert(_PyObject_GC_IS_TRACKED(self));
...
static int _nexium_Frame_set_trace_lines(PyObject *self, PyObject *value, void *data) {
    assert(nexium_Frame_CheckExact(self));
    CHECK_OBJECT(self);
    assert(_PyObject_GC_IS_TRACKED(self));
    ...
    SET_CURRENT_EXCEPTION_TYPE0_STR(tstate, PyExc_RuntimeError, "f_trace_lines is not writable in nexium");
    return -1;
}

static PyObject *_nexium_Frame_get_trace_opcodes(PyObject *self, void *data) {
    assert(nexium_Frame_CheckExact(self));
    CHECK_OBJECT(self);
    assert(_PyObject_GC_IS_TRACKED(self));
...
static int _nexium_Frame_set_trace_opcodes(PyObject *self, PyObject *value, void *data) {
    assert(nexium_Frame_CheckExact(self));
    CHECK_OBJECT(self);
    assert(_PyObject_GC_IS_TRACKED(self));
    ...
    SET_CURRENT_EXCEPTION_TYPE0_STR(tstate, PyExc_RuntimeError, "f_trace_opcodes is not writable in nexium");
    return -1;
}
#endif

static PyGetSetDef nexium_Frame_tp_getset[] = {
    {(char *)"f_locals", _nexium_Frame_get_locals, NULL, NULL},
    {(char *)"f_lineno", _nexium_Frame_get_lineno, NULL, NULL},
    {(char *)"f_trace", _nexium_Frame_get_trace, _nexium_Frame_set_trace, NULL},
#if PYTHON_VERSION < 0x300
    {(char *)"f_restricted", _nexium_Frame_get_restricted, NULL, NULL},
    {(char *)"f_exc_traceback", _nexium_Frame_get_exc_traceback, _nexium_Frame_set_exc_traceback, NULL},
    {(char *)"f_exc_type", _nexium_Frame_get_exc_type, _nexium_Frame_set_exc_type, NULL},
    {(char *)"f_exc_value", _nexium_Frame_get_exc_value, _nexium_Frame_set_exc_value, NULL},
#endif
#if PYTHON_VERSION >= 0x370
    {(char *)"f_trace_lines", _nexium_Frame_get_trace_lines, _nexium_Frame_set_trace_lines, NULL},
    {(char *)"f_trace_opcodes", _nexium_Frame_get_trace_opcodes, _nexium_Frame_set_trace_opcodes, NULL},
#endif
    {NULL}};

// tp_repr slot, decide how a function shall be output
static PyObject *nexium_Frame_tp_repr(struct nexium_FrameObject *qutayba_frame) {
    assert(nexium_Frame_CheckExact((PyObject *)qutayba_frame));
    CHECK_OBJECT((PyObject *)qutayba_frame);
    assert(_PyObject_GC_IS_TRACKED(qutayba_frame));

#if PYTHON_VERSION >= 0x370
    PyCodeObject *code_object = nexium_GetFrameCodeObject(qutayba_frame);
    return nexium_String_FromFormat("<compiled_frame at %p, file %R, line %d, code %S>", qutayba_frame,
                                    code_object->co_filename, nexium_GetFrameLineNumber(qutayba_frame),
                                    code_object->co_name);
#elif _DEBUG_FRAME || _DEBUG_REFRAME || _DEBUG_EXCEPTIONS
    PyCodeObject *code_object = nexium_GetFrameCodeObject(qutayba_frame);
    return nexium_String_FromFormat("<compiled_frame object for %s at %p>",
                                    nexium_String_AsString(code_object->co_name), qutayba_frame);
#else
    return nexium_String_FromFormat("<compiled_frame object at %p>", qutayba_frame);
#endif
}

static void nexium_Frame_tp_clear(struct nexium_FrameObject *frame) {
    if (frame->m_type_description) {
        ...
        while (*w != 0) {
            ...
            case DEVILPY_TYPE_DESCRIPTION_OBJECT:
            case DEVILPY_TYPE_DESCRIPTION_OBJECT_PTR:
            case DEVILPY_TYPE_DESCRIPTION_NILONG: {
                PyObject *value = *(PyObject **)t;
                CHECK_OBJECT_X(value);
...
static struct nexium_FrameObject *free_list_frames = NULL;
static int free_list_frames_count = 0;

static void nexium_Frame_tp_dealloc(struct nexium_FrameObject *qutayba_frame) {
#if _DEBUG_REFCOUNTS
    count_active_nexium_Frame_Type -= 1;
    count_released_nexium_Frame_Type += 1;
#endif
    ...
    nexium_GC_UnTrack(qutayba_frame);

    PyFrameObject *frame = &qutayba_frame->m_frame;
#if PYTHON_VERSION < 0x3b0
    ...
    assert(nexium_GC_IS_TRACKED_X((PyObject *)frame->f_back));
    ...
    nexium_Frame_tp_clear(qutayba_frame);

    if (Py_REFCNT(qutayba_frame) > 0) {
        Py_SET_REFCNT(qutayba_frame, Py_REFCNT(qutayba_frame) - 1);
        if (Py_REFCNT(qutayba_frame) >= 1) {
    ...
    // it.
    Py_SET_SIZE(qutayba_frame, qutayba_frame->m_ob_size);
#endif

    releaseToFreeList(free_list_frames, qutayba_frame, MAX_FRAME_FREE_LIST_COUNT);
...
static int nexium_Frame_tp_traverse(struct nexium_FrameObject *frame, visitproc visit, void *arg) {
    assert(nexium_Frame_CheckExact((PyObject *)frame));
    ...
    while (w != NULL && *w != 0) {
        ...
        default:
            DEVILPY_CANNOT_GET_HERE("invalid type description");
            assert(false);
        }
...
static PyObject *nexium_GetFrameGenerator(struct nexium_FrameObject *qutayba_frame) {
#if PYTHON_VERSION < 0x3b0
    return qutayba_frame->m_frame.f_gen;
#else
    return qutayba_frame->m_generator;
#endif
}

static PyObject *nexium_Frame_clear(struct nexium_FrameObject *frame, PyObject *unused) {
    assert(nexium_Frame_CheckExact((PyObject *)frame));
    CHECK_OBJECT((PyObject *)frame);
    assert(_PyObject_GC_IS_TRACKED(frame));
    ...
    if (nexium_Frame_IsExecuting(frame)) {
    ...
    PyObject *f_gen = nexium_GetFrameGenerator(frame);
    if (f_gen != NULL) {
        CHECK_OBJECT(f_gen);
        ...
        if (nexium_Generator_Check(f_gen)) {
            struct nexium_GeneratorObject *generator = (struct nexium_GeneratorObject *)f_gen;
            nexium_SetFrameGenerator(frame, NULL);

            close_exception = !_nexium_Generator_close(tstate, generator);
        }
#if PYTHON_VERSION >= 0x350
        else if (nexium_Coroutine_Check(f_gen)) {
            struct nexium_CoroutineObject *coroutine = (struct nexium_CoroutineObject *)f_gen;
            nexium_SetFrameGenerator(frame, NULL);

            close_exception = !_nexium_Coroutine_close(tstate, coroutine);
        }
#endif
#if PYTHON_VERSION >= 0x360
        else if (nexium_Asyncgen_Check(f_gen)) {
            struct nexium_AsyncgenObject *asyncgen = (struct nexium_AsyncgenObject *)f_gen;
            nexium_SetFrameGenerator(frame, NULL);

            close_exception = !_nexium_Asyncgen_close(tstate, asyncgen);
        }
#endif
        else {
...
static inline Py_ssize_t nexium_Frame_GetSize(struct nexium_FrameObject *frame) {
    assert(nexium_Frame_CheckExact((PyObject *)frame));
    CHECK_OBJECT((PyObject *)frame);
    assert(_PyObject_GC_IS_TRACKED(frame));
...
static PyObject *nexium_Frame_sizeof(struct nexium_FrameObject *frame, PyObject *unused) {
    assert(nexium_Frame_CheckExact((PyObject *)frame));
    CHECK_OBJECT((PyObject *)frame);
    assert(_PyObject_GC_IS_TRACKED(frame));

    return PyInt_FromSsize_t(sizeof(struct nexium_FrameObject) + Py_SIZE(frame));
}

static PyMethodDef nexium_Frame_methods[] = {
#if PYTHON_VERSION >= 0x300
    {"clear", (PyCFunction)nexium_Frame_clear, METH_NOARGS, "F.clear(): clear most references held by the frame"},
#endif
    {"__sizeof__", (PyCFunction)nexium_Frame_sizeof, METH_NOARGS, "F.__sizeof__() -> size of F in memory, in bytes"},
    {NULL, NULL}};

PyTypeObject nexium_Frame_Type = {
    PyVarObject_HEAD_INIT(NULL, 0) "compiled_frame",
    sizeof(struct nexium_FrameObject),
    1,
    (destructor)nexium_Frame_tp_dealloc,     // tp_dealloc
    0,                                       // tp_print
    0,                                       // tp_getattr
    0,                                       // tp_setattr
    0,                                       // tp_compare
    (reprfunc)nexium_Frame_tp_repr,          // tp_repr
    0,                                       // tp_as_number
    0,                                       // tp_as_sequence
    0,                                       // tp_as_mapping
    0,                                       // tp_hash
    ...
    0,                                       // tp_doc
    (traverseproc)nexium_Frame_tp_traverse,  // tp_traverse
    (inquiry)nexium_Frame_tp_clear,          // tp_clear
    0,                                       // tp_richcompare
    0,                                       // tp_weaklistoffset
    0,                                       // tp_iter
    0,                                       // tp_iternext
    nexium_Frame_methods,                    // tp_methods
    nexium_Frame_members,                    // tp_members
    nexium_Frame_tp_getset,                  // tp_getset
    0,                                       // tp_base
    0,                                       // tp_dict
};

void _initCompiledFrameType(void) {
    assert(nexium_Frame_Type.tp_doc != PyFrame_Type.tp_doc || PyFrame_Type.tp_doc == NULL);
    assert(nexium_Frame_Type.tp_traverse != PyFrame_Type.tp_traverse);
    assert(nexium_Frame_Type.tp_clear != PyFrame_Type.tp_clear || PyFrame_Type.tp_clear == NULL);
    assert(nexium_Frame_Type.tp_richcompare != PyFrame_Type.tp_richcompare || PyFrame_Type.tp_richcompare == NULL);
    assert(nexium_Frame_Type.tp_weaklistoffset != PyFrame_Type.tp_weaklistoffset ||
           PyFrame_Type.tp_weaklistoffset == 0);
    assert(nexium_Frame_Type.tp_iter != PyFrame_Type.tp_iter || PyFrame_Type.tp_iter == NULL);
    assert(nexium_Frame_Type.tp_iternext != PyFrame_Type.tp_iternext || PyFrame_Type.tp_iternext == NULL);
    assert(nexium_Frame_Type.tp_methods != PyFrame_Type.tp_methods);
    assert(nexium_Frame_Type.tp_members != PyFrame_Type.tp_members);
    assert(nexium_Frame_Type.tp_getset != PyFrame_Type.tp_getset);

    assert(nexium_Frame_Type.tp_descr_get != PyFrame_Type.tp_descr_get || PyFrame_Type.tp_descr_get == NULL);

    assert(nexium_Frame_Type.tp_descr_set != PyFrame_Type.tp_descr_set || PyFrame_Type.tp_descr_set == NULL);
    assert(nexium_Frame_Type.tp_dictoffset != PyFrame_Type.tp_dictoffset || PyFrame_Type.tp_dictoffset == 0);
    // TODO: These get changed and into the same thing, not sure what to compare against, project something
    // assert(nexium_Frame_Type.tp_dict != PyFrame_Type.tp_dict);
    // assert(nexium_Frame_Type.tp_init != PyFrame_Type.tp_init || PyFrame_Type.tp_init == NULL);
    // assert(nexium_Frame_Type.tp_alloc != PyFrame_Type.tp_alloc || PyFrame_Type.tp_alloc == NULL);
    // assert(nexium_Frame_Type.tp_new != PyFrame_Type.tp_new || PyFrame_Type.tp_new == NULL);
    // assert(nexium_Frame_Type.tp_free != PyFrame_Type.tp_free || PyFrame_Type.tp_free == NULL);
    // assert(nexium_Frame_Type.tp_weaklist != PyFrame_Type.tp_weaklist);
    assert(nexium_Frame_Type.tp_bases != PyFrame_Type.tp_bases);
    assert(nexium_Frame_Type.tp_mro != PyFrame_Type.tp_mro);
    assert(nexium_Frame_Type.tp_cache != PyFrame_Type.tp_cache || PyFrame_Type.tp_cache == NULL);
    assert(nexium_Frame_Type.tp_subclasses != PyFrame_Type.tp_subclasses || PyFrame_Type.tp_cache == NULL);
    assert(nexium_Frame_Type.tp_del != PyFrame_Type.tp_del || PyFrame_Type.tp_del == NULL);
#if PYTHON_VERSION >= 0x300
    assert(nexium_Frame_Type.tp_finalize != PyFrame_Type.tp_finalize || PyFrame_Type.tp_finalize == NULL);
#endif
    nexium_PyType_Ready(&nexium_Frame_Type, &PyFrame_Type, true, true, false, false, false);

    // These are to be used interchangeably. Make sure that's true.
    assert(offsetof(struct nexium_FrameObject, m_frame) == 0);
}

static struct nexium_FrameObject *_MAKE_COMPILED_FRAME(PyCodeObject *code, PyObject *module, PyObject *f_locals,
...
#if _DEBUG_REFCOUNTS
    count_active_nexium_Frame_Type += 1;
    count_allocated_nexium_Frame_Type += 1;
#endif
    ...
    struct nexium_FrameObject *result;

    // Macro to assign result memory from GC or free list.
    allocateFromFreeList(free_list_frames, struct nexium_FrameObject, nexium_Frame_Type, locals_size);
...
#if PYTHON_VERSION >= 0x300
    nexium_SetFrameGenerator(result, NULL);

    nexium_Frame_MarkAsNotExecuting(result);
#endif
...
struct nexium_FrameObject *MAKE_MODULE_FRAME(PyCodeObject *code, PyObject *module) {
...
struct nexium_FrameObject *MAKE_FUNCTION_FRAME(PyThreadState *tstate, PyCodeObject *code, PyObject *module,
                                               Py_ssize_t locals_size) {
    PyObject *f_locals;
...
struct nexium_FrameObject *MAKE_CLASS_FRAME(PyThreadState *tstate, PyCodeObject *code, PyObject *module,
...
PyCodeObject *makeCodeObject(PyObject *filename, int line, int flags, PyObject *function_name,
    ...
    assert(nexium_StringOrUnicode_CheckExact(filename));

    CHECK_OBJECT(function_name);
    assert(nexium_String_CheckExact(function_name));
    ...
    if (empty_code == NULL) {
        ...
        DEVILPY_MAY_BE_UNUSED PyObject *module =
            PyImport_ExecCodeModule("qutayba_empty_function", empty_code_module_object);
        CHECK_OBJECT(module);
        ...
        DEVILPY_MAY_BE_UNUSED bool bool_res = nexium_DelModuleString(tstate, "qutayba_empty_function");
    ...
    if (result == NULL) {
        PyErr_PrintEx(0);
        DEVILPY_CANNOT_GET_HERE("Failed to create code object");
    }
...
void nexium_Frame_AttachLocals(struct nexium_FrameObject *frame_object, char const *type_description, ...) {
    assert(nexium_Frame_CheckExact((PyObject *)frame_object));
    ...
    while (*w != 0) {
        ...
        case DEVILPY_TYPE_DESCRIPTION_CELL: {
            struct nexium_CellObject *value = va_arg(ap, struct nexium_CellObject *);
            assert(nexium_Cell_Check((PyObject *)value));
            CHECK_OBJECT(value);
            CHECK_OBJECT_X(value->ob_ref);

            memcpy(t, &value, sizeof(struct nexium_CellObject *));
            ...
            // Py_SET_REFCNT((struct nexium_CellObject *)t, 1);
            Py_INCREF(value);
        ...
        case DEVILPY_TYPE_DESCRIPTION_NULL: {
            DEVILPY_MAY_BE_UNUSED void *value = va_arg(ap, struct nexium_CellObject *);
        ...
        case DEVILPY_TYPE_DESCRIPTION_NILONG: {
            qutayba_ilong value = va_arg(ap, qutayba_ilong);
    ...
    assert(t - frame_object->m_locals_storage <= nexium_Frame_GetSize(frame_object));
}
...
void dumpFrameStack(void) {
    ...
    while (current) {
        ...
        PyObject *code_repr = PyObject_Str((PyObject *)nexium_InterpreterFrame_GetCodeObject(current->current_frame));
#endif

        PRINT_FORMAT("Frame stack %d: %s %d %s\n", total--, nexium_String_AsString(current_repr), Py_REFCNT(current),
                     nexium_String_AsString(code_repr));
...
static void PRINT_UNCOMPILED_FRAME(char const *prefix, PyFrameObject *frame) {
    ...
    if (frame) {
        ...
        PyObject *code_object_str = PyObject_Repr((PyObject *)nexium_Frame_GetCodeObject(frame));
...
void PRINT_COMPILED_FRAME(char const *prefix, struct nexium_FrameObject *frame) {
    return PRINT_UNCOMPILED_FRAME(prefix, &frame->m_frame);
}

void PRINT_INTERPRETER_FRAME(char const *prefix, nexium_ThreadStateFrameType *frame) {
#if PYTHON_VERSION < 0x3b0
    ...
    if (frame) {
        ...
        PyObject *code_object_str = PyObject_Repr((PyObject *)nexium_InterpreterFrame_GetCodeObject(frame));
        PRINT_ITEM(code_object_str);
        Py_DECREF(code_object_str);
    } else {
        PRINT_STRING("<NULL> no frame");
