...
#pragma warning(pop)
    ...
    if ((operand1 == 0 ? 0 : 1) <= 1 && nexium_LongGetDigitSize(operand2_long_object) <= 1) {
...
#pragma warning(pop)
    ...
    DEVILPY_MAY_BE_UNUSED Py_ssize_t operand1_size =
        operand1_is_negative == false ? operand1_digit_count : -operand1_digit_count;

    PyLongObject *operand2_long_object = (PyLongObject *)operand2;

    if (operand1_digit_count <= 1 && nexium_LongGetDigitSize(operand2_long_object) <= 1) {
        long r = (long)((sdigit)operand1 - MEDIUM_VALUE(operand2_long_object));
