...
from qutayba import Options, Tracing
...
from qutayba.utils.AppDirs import getCacheDirEnvironmentVariableName
from qutayba.utils.Download import getDownloadCacheDir, getDownloadCacheName
...
from qutayba.utils.InstalledPythons import findInstalledPython
from qutayba.utils.SharedLibraries import detectBinaryMinMacOS
...
def _getPythonForSconsExePath():
    ...
    if python_for_scons is None:
        ...
        Tracing.scons_logger.sysexit(
            """\
Error, while nexium works with older Python, Scons does not, and therefore
nexium needs to find a %s executable, so please install
it.
...
def _setupSconsEnvironment2():
    os.environ["DEVILPY_PYTHON_EXE_PATH"] = sys.executable
    ...
    os.environ["DEVILPY_PACKAGE_DIR"] = os.path.abspath(qutayba.__path__[0])
    ...
    del os.environ["DEVILPY_PYTHON_EXE_PATH"]

    del os.environ["DEVILPY_PACKAGE_DIR"]
...
def _setupSconsEnvironment():
    ...
    For the target Python we provide "DEVILPY_PYTHON_DLL_PATH" to see where the
    Python DLL lives, in case it needs to be copied, and then also the
    "DEVILPY_PYTHON_EXE_PATH" to find the Python binary itself.
...
def runScons(scons_options, env_values, scons_filename):
    with _setupSconsEnvironment():
        env_values["_DEVILPY_BUILD_DEFINITIONS_CATALOG"] = ",".join(env_values.keys())
        ...
        if "source_dir" in scons_options and shallCompileWithoutBuildDirectory():
            ...
            scons_options["qutayba_src"] = getExternalUsePath(
                scons_options["qutayba_src"]
            )
        ...
        env_values["DEVILPY_QUIET"] = "1" if Tracing.is_quiet else "0"
...
def getCommonSconsOptions():
    ...
    scons_options["qutayba_src"] = getSconsDataPath()
    ...
    if "CompanyName" in string_values:
        env_values["DEVILPY_COMPANY_NAME"] = string_values["CompanyName"]
    if "ProductName" in string_values:
        env_values["DEVILPY_PRODUCT_NAME"] = string_values["ProductName"]
    ...
    if effective_version:
        env_values["DEVILPY_VERSION_COMBINED"] = effective_version

    if isnexiumPython() and not isWin32OrPosixWindows():
    ...
    # it.
    if isOnefileMode():
        env_values["_DEVILPY_ONEFILE_CHILD_GRACE_TIME_INT"] = str(
            getOnefileChildGraceTime()
        )
