...
class SetLinkStateOperation(BaseModel):
    ...
    __properties: ClassVar[List[str]] = ["controllers", "link"]

    @field_validator('link')
    def link_validate_regular_expression(cls, value):
        """Validates the regular expression"""
        if value is None:
            return value

        if not re.match(r"^(UP|DOWN)$", value):
            raise ValueError(r"must validate the regular expression /^(UP|DOWN)$/")
        return value
