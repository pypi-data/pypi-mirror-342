...
from commonLib import validatePlaylistExclusions, validatePlaybook, getRunList, envFileHelper, isKeystackUIAlive, getHttpIpAndPort
...
class Playbook:
    ...
    reconfigs:                     Optional[str] = None
    # if reconfigs != '', reconfigData = readYaml(reconfigs)
    ...
    wireshark:                     bool = Field(default=False)
    startLsu:                      bool = Field(default=False)
    ...
    def __post_init__(self):  
        try: 
            load_dotenv()
            self.devMode = os.getenv('DJANGO_DEBUG', False)
            ...
            self.removePastFoldersBasedOnFolderTimestamp()
    ...
    def getDomain(self):
        regexMatch = re.search('.*DOMAIN=(.+?)/.*', self.playbook)
        if regexMatch:
            self.domain = regexMatch.group(1)
        else:
            if self.domain is not None:
                domainData = readYaml(GlobalVars.domainsFile)
                if self.domain not in list(domainData):
                    raise Exception(f'validatePlaybook(): No such domain found. Verify typo or create the domain first: {self.domain}\n')
            else:
                # If the playbook value doesn't include DOMAIN and did not include -domain
                raise Exception(f'validatePlaybook(): Must include a domain in either using -domain <domain> or -playbook DOMAIN=<domain>/<playbook>\n')
    ...
    def verifyUserCliApiKey(self): 
        ...
        self.keystackLogger.debug(f'verifyUserCliApiKey: isFromKeystackUI:{self.isFromKeystackUI} isCliInsideContainer:{self.isCliInsideContainer} cliSecuredMode:{self.cliSecuredMode} cliUserApiKey:{self.cliUserApiKey}')
    ...
    def removePastFoldersBasedOnFolderTimestamp(self): 
        """
        Remove pipeline results that are not in the 'Runnning' status if left for over removeDaysOlderThan
        ...
        """
        if keystackSettings.get('removeResultsFolder', GlobalVars.removeResultFoldersAfterDays) == 'never':
            return

        folder = f'{GlobalVars.resultsFolder}/DOMAIN={self.domain}/PLAYBOOK/{self.playbook}'
            ...
            if int(daysRecorded) >= int(removeDaysOlderThan):
                try:
                    if os.path.exists(f'{root}/overallSummary.json'):
                        ...
                        if len(isPidExists) > 0 or overallSummary['status'] == 'Running':
                            continue
                                                        
                        print('removePastFoldersBasedOnFolderTimestamp:', root)
                    ...
                    #print(f'removePastFoldersBasedOnFolderTimestamp error: {root}\n{errMsg}')
                    pass
    ...
    def readReconfigData(self):
        if self.reconfigs:
            regexMatch = re.search(f'.*({self.reconfigs})', self.reconfigs)
            
            if regexMatch:
                reconfigPath = regexMatch.group(1)
            else:
                reconfigPath = self.reconfigs
            
            if reconfigPath.endswith('.yml') is False:
                reconfigPath = f'{reconfigPath}.yml'
                    
            self.reconfigReferenceFile = f'{GlobalVars.reconfigsFolder}/{reconfigPath}'
            if os.path.exists(self.reconfigReferenceFile) is False:
                sys.exit(f'No such reconfig reference file: {self.reconfigReferenceFile}')

            self.reconfigData = readYaml(self.reconfigReferenceFile)
            self.keystackLogger.debug(f'Reconfigs: {self.reconfigReferenceFile }: {self.reconfigData}') 
