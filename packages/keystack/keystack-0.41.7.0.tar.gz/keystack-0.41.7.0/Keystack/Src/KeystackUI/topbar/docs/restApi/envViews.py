...
def getTableData(request, domain, envGroup, user, indexStart, indexEnd) -> str:
    ...
    try:
            ...
            if bool(search(f'^{GlobalVars.keystackTestRootPath}/{envGroup}$', rootPath)) and files:
                ...
                for index, envYmlFile in enumerate(files[indexStart:indexEnd]):
                    if bool(search('.+\.(yml|ymal)$', envYmlFile)):
                        ...
                        if portGroups and domain is not None:
                            for portGroup in portGroups:
                                ...
                                if len(ports) > 0:
                                    ...
                                    portGroupDropdown += f"<a class='dropdown-toggle textBlack' data-toggle='dropdown' role='button' aria-haspopup=true aria-expanded=false>Ports</a>"        
                                    portGroupDropdown += f'<ul id="envPortGroupDropdownId" class="dropdown-menu dropdownSizeMedium" aria-labelledby="">'
                                ...
                                for deviceName, value in ports.items():
                                    ...
                                    portGroupDropdown += f'<li class="mainFontSize textBlack paddingLeft20px">Domain: {deviceDomain}&emsp;&emsp;On-Device: {deviceName}:</li><br>'
                        ...
                        tableData += f'<td class="textAlignCenter">{portGroupDropdown}</td>'
                        ...
                        if isParallelUsage is False:
                            tableData += f'<td class="textAlignLeft"><a href="#" onclick="activeUsersList(this)" env={envName} data-bs-toggle="modal" data-bs-target="#activeUsersModal">Active: {totalActiveUsers}&emsp;&ensp;Waiting: {totalWaiting}</a></td>'
                        # else:
                        #     tableData += f'<td class="textAlignLeft"><a href="#" onclick="activeUsersList(this)" env={envName} data-bs-toggle="modal" data-bs-target="#activeUsersModal">Active: {totalActiveUsers}</a></td>'
                        ...
                        if isParallelUsage is False:
                            ...
                            #tableData += f'<td class="textAlignCenter"><button onclick="reserveEnv(this)" env={envName} class="btn btn-sm btn-outline-primary">Use</button></td>'
                            tableData += '<td></td>'
        ...
        tableData += '<tr></tr>'
        #print('\nenvViews error:', traceback.format_exc(None, errMsg))
        pass
        ...
        if remoteControllerIp and remoteControllerIp != mainControllerIp:
            ...
            for retry in range(1, 4):
                try:
                    ...
                    SystemLogsAssistant().log(user=user, webPage=Vars.webpage, action='ReleaseEnvOnFailure', msgType='Info',
            ...
            try:
                ...
                for env in envs:
                    ...
                    if releaseJob:
                        ...
                        #JobSchedulerAssistant().addToScheduler(DB.name, 'env', data2)
                        JobSchedulerAssistant().createCronJob(releaseJob)

                    SystemLogsAssistant().log(user=user, webPage=Vars.webpage, action='AddEnvSchedule', msgType='Info', msg=newJob.replace('&webhook=true', ''))            
            ...
            try:
                ...
                for cron in removeScheduledEnvs:
                    ...
                    if RedisMgr.redis:
                        ...
                        SystemLogsAssistant().log(user=user, webPage=Vars.webpage, action='DeleteScheduledEnv', msgType='Info',
                                                  msg=cron, forDetailLogs='')
                    else: 
                        SystemLogsAssistant().log(user=user, webPage=Vars.webpage, action='DeleteScheduledEnv', msgType='Failed',
