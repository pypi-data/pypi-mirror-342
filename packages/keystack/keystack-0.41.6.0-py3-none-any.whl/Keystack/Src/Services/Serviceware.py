...
class vars:    
    ...
    keystackLogsService = f'{keystackRootPath}/Services/keystackLogs.py'
...
class KeystackServices:
        ...
        Two ways of using this class:
            ...
            1> Each service .py module such as keystacAwsS3.py will set typeOfService accordingly to the type of service.
               Ex: serviceObj = Serviceware.KeystackServices(typeOfService='keystackAwsS3', isFromKeystackUI=True)
                   ...
                   serviceObj.restartService('keystackAws')
    ...
    def writeToServiceLogFile(self, msgType, msg, playbookName=None, sessionId=None, logFile=None, fileLock=None, showOutput=False):
        """ 
        ...
        if os.path.exists(logFile) == False:
            ...
            execSubprocessInShellMode(f'chown :Keystack {logFile}', showStdout=False) 
            execSubprocessInShellMode(f'chmod 770 {logFile}', showStdout=False) 
    ...
    def isServiceRunning(self, service, showStdout=False):
        """
        service: keystackAwsS3, keystackJira, keystackLogs, keystackScheduler, keystackWatchdog
        """
        if showStdout:
        ...
        if process:
            match = re.search(f'.*/{service}.py.*', process)
    ...
    def startService(self, service):
        ...
        if service == 'keystackLogs':
            serviceScript = vars.keystackLogsService
        ...
        if self.isFromKeystackUI:
            ...
            # Below line is the original 
            #cmd = f'{vars.pythonPath} {serviceScript} > /dev/null &'
        else:
    ...
    def stopService(self, service, showStdout=False):
        """ 
        Parameters
            service: <str>: keystackLogs|keystackAwsS3
        """ 
        ...
        for line in buffer[1].split('\n'):
            ...
            if match:
                ...
                execSubprocessInShellMode(f'sudo kill -9 {match.group(1)}', showStdout=False)
    ...
    def startLogsService(self):
        self.startService('keystackLogs')
        
    def stopLogsService(self):
        self.setService('keystackLogs')
        self.stopService('keystackLogs')
