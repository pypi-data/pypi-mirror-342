...
from commonLib import KeystackException, execCliCommand, ConnectTelnet
...
class EnvAssistant:
    ...
    def envHandler(self):
        ...
        try:
            ...
            if self.runTaskObj.envFile:
                ...
                envDomainSearch = re.search(f'.*DOMAIN=(.+?)/(.*)(\.yml)?', self.runTaskObj.envFile)
                envDomain = envDomainSearch.group(1)
                ...
                if self.runTaskObj.playbookObj.isKeystackAlive and self.runTaskObj.env and self.runTaskObj.env != 'not-required':
                    ...
                    # PORT-GROUP HANDLINGS: Get Port-Group ports and port etails to include in keystackEnv resources
    ...
    def configLayer1Interconnects(self):
        ...
        TODO: Release the testbed if failed
        """
        print('\n---- configLayer1Interconnects: setenv:', self.runTaskObj.env)
        ...
        if self.runTaskObj.envPortGroups:
            for portGroup in self.runTaskObj.envPortGroups:
                self.portGroupObj = ManagePortGroup(domain=self.runTaskObj.playbookObj.domain, portGroup=portGroup)
                # {'device_1': {'domain': 'Communal', 'ports': ['1/1', '1/2']}}
                self.ports = self.portGroupObj.getAllPorts()
                #print(f'--- portGroup:{portGroup}  ports: {self.ports}')
                
                for device, properties in self.ports.items():
                    labObj = InventoryMgmt(domain=properties['domain'], device=device)
                    ...
                    {'device_1': {'additionalKeys': 'None',
                                    ...
                                    'vendor': None}}
                    """
                    deviceDetails = labObj.getDeviceDetails()
                    self.deviceType = deviceDetails[device]['deviceType']
                    self.deviceVendor = deviceDetails[device]['vendor']
                    self.deviceModel = deviceDetails[device]['model']
                    print(f'\n--- deviceType:{self.deviceType} deviceVendor:{self.deviceVendor} deviceModel:{self.deviceModel}')

                    portConnections = labObj.getPortsInPortMgmt(ports=properties['ports'])
                    #print('\n--- portConnections ---')
                    #pprint(portConnections)
                    """ 
                    {'1/1': {'additionalKeyValues': {},
                            'autoNeg': True,
    ...
    def reserveEnv(self, overallSummaryFile, env):
        ...
        if self.runTaskObj.execRestApiObj:
            ...
            if response.status_code != 200:
                self.runTaskObj.keystackLogger.error(f'EnvAssist.reserveEnv: {response.json["errorMsg"]}')
                self.runTaskObj.playbookobj.overallSummaryData['exceptionErrors'].append(response.json["errorMsg"])
                self.runTaskObj.playbookObj.updateOverallSummaryDataOnRedis()
                raise KeystackException(f'EnvAssistants: reserveEnv failed: {response.json["errorMsg"]}')
        ...
        try:
            ...
            for portGroup in self.runTaskObj.envPortGroups:
                try:
                    ...
                    if response.status_code != 200:
                        self.runTaskObj.keystackLogger.error(f'EnvAssist:reserveAndWaitForPortGroup:  stage:{stage}  task:{task} portGroup:{portGroup} /api/v1/portGroup/reserveUI response failed status code: {response.status_code}. ErrorMsg: {response.json()["errorMsg"]}')
                    ...
                    self.runTaskObj.logTaskExceptionError(f'reserveAndWaitForPortGroup: Failed. Called /api/v1/portGroup/reserveUI. Status code {response.status_code}')
                    return
            ...
            for portGroup in self.runTaskObj.envPortGroups:
                ...
                self.runTaskObj.keystackLogger.debug(f'EnvAssist:reserveAndWaitForPortGroup: Entering while loop calling /portGroup/amINext -> portGroup:{portGroup} stage:{stage} task:{task}')
                ...
                while True:
                    ...
                    if response.json()['status'] == 'failed':
                        errorMsg = f'reserveAndWaitForPortGroup: Failed. Called /api/v1/portGroup/amINext. Status code {response.status_code}'
                        self.runTaskObj.keystackLogger.error(errorMsg)
                        self.runTaskObj.logTaskExceptionError(errorMsg)
                        return
                    ...
                    if response.json()['result']:
                        # I am next to run
                        self.runTaskObj.taskSummaryData.update({'status': 'Running'})
                        self.runTaskObj.keystackLogger.debug(f'/api/v1/portGroup/amINext: Yes. Status changed to Running -> portGroup:{portGroup} stage:{stage} task:{task}')
                        break
                    else:
                        if doOnce is False:
                            self.runTaskObj.taskSummaryData.update({'status': 'Waiting-For-PortGroup'})
                            writeToJson(self.runTaskObj.taskSummaryFile, self.runTaskObj.taskSummaryData, mode='w', threadLock=self.runTaskObj.statusFileLock)
                            doOnce = True
    ...
    def setup(self, envFile: str, keystackLogger: object=None, label: str='Env-Auto-Setup'):
        """ 
        This function is used by 2 different places:
           - Keystack:RunTask() -> EnvAssistant:envHandler()
           - envViews:reserveEnv
