    ...
    def post(self, request, data=None):
        ...
        try:
            ...
            if request.GET:
                ...
                self.reconfigs             = request.GET.get('reconfigs', None)
        ...
        if request.data:
            ...
            try:
                ...
                self.reconfigs                     = request.data.get('reconfigs', None)
        ...
        if self.remoteControllerIp and self.remoteControllerIp != self.mainControllerIp:                
            ...
            params = {'remoteController': self.remoteControllerIp, 'scheduledJob': self.scheduledJob, 
                      ...
                      'holdEnvsIfFailed': self.holdEnvsIfFailed, 'abortTestOnFailure': self.abortTestOnFailure, 'reconfigs': self.reconfigs,
            ...
            if runOnLocalController:
                ...
                if os.environ.get('DJANGO_DEBUG', False):
                    # command = f'{sys.executable} /opt/Keystack/Src/keystack.py run -playbook {self.playbookPath} -isFromKeystackUI -domain {self.domain} -resultsFolder {resultTimestampFolder} -user "{user}"'
                    command = f'{sys.executable} /opt/Keystack/Src/keystack.py run -playbook {self.playbookPath} -is_from_keystack_ui -domain {self.domain} -results_folder {resultTimestampFolder} -user "{user}"'
                else:
                    # command = f'keystack run -playbook {self.playbookPath} -isFromKeystackUI -resultsFolder {resultTimestampFolder} -domain {self.domain} -user "{user}"'
                ...
                if self.reconfigs:                    command += f' -reconfigs {self.reconfigs}'
                ...
                try:
                    ...
                    if self.removeJobAfterRunning:
                        if RedisMgr.redis:
                            keyName = f'scheduler-remove-{self.playbook}'
        ...
        if remoteControllerIp and remoteControllerIp != mainControllerIp:
            ...
            try:
                ...
                for playbook in playbooks:
                    ...
                    SystemLogsAssistant().log(user=user, webPage=Vars.webpage, action='AddPlaybookSchedule', msgType='Info', msg=newJob.replace('&webhook=true', ''))            
            ...
            try:
                ...
                for cron in removeScheduledPlaybooks:
                    ...
                    if RedisMgr.redis:
                        ...
                        SystemLogsAssistant().log(user=user, webPage=Vars.webpage, action='DeleteScheduledPlaybook', msgType='Info',
