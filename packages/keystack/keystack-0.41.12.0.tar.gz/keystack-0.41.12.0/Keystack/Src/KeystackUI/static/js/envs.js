...
const getSetupTableData = async (getPageNumberButton=null, previousNextPage=null) => {
    ...
    if (isDropdownOpen(portGroupDropdownId)) {
        return
    }

    let isEnvSharedDropdownId = document.querySelector("#isEnvSharedDropdownId");
    if (isDropdownOpen(isEnvSharedDropdownId)) {
        return
    }
...
const createEnv = async () => {
    try {
        const domain = document.querySelector('#pageAttributes').getAttribute('topbarDisplayDomain');
        ...
        if (envGroup === '') {
            let resourceGroup = document.querySelector('#pageAttributes').getAttribute('topbarDisplayGroup');
...
const getEnvCronScheduler = async (object) => {
    ...
    if (envCheckboxes.length == 0) {
        alert('Please select 1 or more envs to schedule a reservation')
        return
    }

    for (let x=0; x < envCheckboxes.length; x++) {
        if (envCheckboxes[x].getAttribute('parallelUsage') == 'Yes') {
            let envName = envCheckboxes[x].getAttribute('envName');
            alert(`No need to schedule the env. The env parallelUsage=True. Please deselect the env: ${envName}`);
            return
        }
    }
    */
