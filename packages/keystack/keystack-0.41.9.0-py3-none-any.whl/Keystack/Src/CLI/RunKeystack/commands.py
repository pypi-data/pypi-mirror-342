...
@click.option('-reconfigs',   
              required=False, default='', type=str,  
              help='Which reconfiguration reference file to use for changing script configurations')
...
class RunPlaybook:        
    def __init__(self, kwargs):
        ...
        self.playbookParams = kwargs
    
    def checkParams(self):
        if self.playbookParams['pipeline']:
            if '.yml' not in self.playbookParams['pipeline']:
                self.playbookParams['pipeline'] = f'{self.playbookParams["pipeline"]}.yml'
            
            pipelineFile = f'{GlobalVars.pipelineFolder}/{self.playbookParams["pipeline"]}'
            if os.path.exists(pipelineFile) == False:
                sys.exit(f'\nError: No such pipeline: {self.playbookParams["pipeline"]}\n')
            ...
            if self.playbookParams['api_key']:
                # TODO: Use rest-api for AccountMgr().getApiKeyUser and  DomainMgr().getUserAllowedDomains
                apiKeyUser = AccountMgr().getApiKeyUser(self.playbookParams['api_key'])
            ...
            for key, value in pipelineArgs.items():
                ...
                if value:
                    ...
                    self.playbookParams.update({key: value})
        else:
            if self.playbookParams['playbook'] is None:
                sys.exit('\nKeystack.py error: Must include a playbook to run test\n')
        
            if self.playbookParams['domain'] is None and 'DOMAIN' not in self.playbookParams['playbook']:
                sys.exit('\nMissing domain.  Include param -domain or include DOMAIN=<domain> in param value -playbook\n')
                
        if len(self.playbookParams['app_args']) == 0:
            self.appArgs = ''
        else:
            self.appArgs = self.playbookParams['app_args'][0]

    def instantiatePlaybook(self):
        try:
            self.playbookObj = Playbook(domain = self.playbookParams['domain'],
                                        user = self.playbookParams['user'],
                                        cliUserApiKey = self.playbookParams['api_key'],
                                        playbook = self.playbookParams['playbook'],
                                        appArgs = self.appArgs,
                                        abortTestOnFailure = self.playbookParams['abort_test_on_failure'],
                                        emailResults = self.playbookParams['email_results'],
                                        debug = self.playbookParams['debug'],
                                        continuousIntegration = self.playbookParams['ci'],
                                        timestampFolder = self.playbookParams['results_folder'],
                                        sessionId = self.playbookParams['session_id'],
                                        pauseOnFailure = self.playbookParams['pause_on_failure'], 
                                        holdEnvsIfFailed = self.playbookParams['hold_envs_if_failed'], 
                                        reconfigs = self.playbookParams['reconfigs'],
                                        includeLoopTestPassedResults = self.playbookParams['include_loop_test_passed_results'],
                                        isFromKeystackUI = self.playbookParams['is_from_keystack_ui'],
                                        trackResults = self.playbookParams['track_results'],
                                        awsS3Upload = self.playbookParams['aws_s3'],
                                        jira = self.playbookParams['jira'],
                                        wireshark = self.playbookParams['wireshark'], 
                                        startLsu = self.playbookParams['start_lsu'])
        except Exception as errMsg:
            if self.playbookParams['verbose']:
