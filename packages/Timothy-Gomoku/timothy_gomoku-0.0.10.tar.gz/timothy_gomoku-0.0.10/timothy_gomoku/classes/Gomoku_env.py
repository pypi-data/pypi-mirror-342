...
class Gomoku_env:
    ...
    __win_combos = __win_cols + __win_rows + __win_diags_1 + __win_diags_2

    def win(self):
        for win_combo in Gomoku_env.__win_combos:
            ok = True
    ...
    def load(self, move):
        """
        Load a move to the board.
        :param move: (x, y) tuple
        :return: None
        """
        for m in move:
            x, y = m
    ...
    def state(self):
        """
        Encode board as 2x15x15 tensor: [my_stones, opponent_stones]
        ...
        state = np.stack([my_stones, opp_stones])
