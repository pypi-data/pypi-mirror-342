...
@LastEditTime: 2025-04-17 12:00:02
...
class UserBaseModel(FrameBaseModel):
    ...
    def save_user_by_openid(self, app_id, act_id, open_id, user_nick, avatar, union_id="", login_ip="", is_update_user_nick=True, mix_nick='', plat_type=0, continue_request_expire=1, is_acquire_lock=False, is_refresh_token=True, is_authenticat_app_id=False):
        """
        ...
        :param is_authenticat_app_id：是否验证app_id,默认不验证
        :return: 返回用户信息
        :last_editors: HuangJianYi
        """
        ...
        try:
            ...
            if not user_info:
                if is_authenticat_app_id == True:
                    from seven_cloudapp_frame.models.act_base_model import ActBaseModel
                    act_base_model = ActBaseModel(context=self.context)
                    act_info_dict = act_base_model.get_act_info_dict(act_id, is_cache=True, field='app_id')
