...
class FreeGpt:
    ...
    def chat(self, messages: dict = None, model: str = "deepseek-ai/DeepSeek-R1-Turbo", temperature: float = 0.7, max_tokens: int = 4000) -> dict:
    ...
    def speech(self, model: str = "tts-1", text: str = None, voice: str = "nova", filename: str = "ozv") -> bool:
        '''Озвучивание текста'''
        try:
            if text is None:
    ...
    def image(self, prompt: str = None, width: int = 1024, height: int = 1024, seed: int = None, enhance: bool = True, private: bool = True, nologo: bool = True, model: str = 'flux', save: bool = False, filename: str = 'image.jpg') -> Union[str, bool]:
    ...
    def pollai_chat(self, messages: list = None, model: str = 'openai-large', seed: int = None, jsonMode: bool = False, private: bool = True, reasoning_effort: str = 'high') -> str:
        '''Генерация ответа с помощью PollinationsAi'''
        try:
            if messages is None:
...
class ChatGPT:
    ...
    def generate_chat(self, messages: list = None, model: str = None) -> Union[str, list]:
        try:
            if messages is None:
                ...
                set_url = f"{self.url}/chat/completions"
                response = requests.post(url=set_url, headers=self.headers, json=data)
                if response.status_code == 200:
                    return response.json()
                elif response.status_code == 429:
                    return "Слишком много запросов"
    ...
    def get_models(self, headers: list = {}) -> Union[str, bool]:
        try:
            set_url = f"{self.url}/models"
            response = requests.get(url=set_url, headers=headers)
            if response.status_code == 200:
                return response.json()
            else:
                return False
