...
class AxiStreamHandler():
    ...
    def __cast_float_to_binary_to_unsigned(self, num, remote=False):
        ...
        if remote == False:
            cmd = "fp2bin "+str(exponent_bits)+" "+str(mantissa_bits)+" "+str(num)
            xReq = os.popen(cmd)
            convertedNumber = xReq.read().replace("\n","")
            return int(convertedNumber, 2)
    ...
    def prepare_data(self):
        ...
        def cast_batch(batch):
            """
            Flatten the batch, convert each element according to data_type,
            and rebuild a numpy array of the correct HW dtype.
            """
            flat = batch.ravel()
            if self.model_specs["data_type"].startswith("fps"):
                ...
                conv = [self.__cast_float_to_binary_to_unsigned(x) for x in flat]
            else:
                return batch.astype(self.datatype)

            arr = np.array(conv, dtype=hw_dtype)
            return arr.reshape(batch.shape)
                ...
                if self.model_specs["data_type"][:3] == "fps":
                    ...
                    for i in range(0, self.n_output):
                        if self.benchcore == True and i == self.n_output - 1:
                            ...
                            prob_float = self.__convert_binary_to_float(binary_str, remote=False)
