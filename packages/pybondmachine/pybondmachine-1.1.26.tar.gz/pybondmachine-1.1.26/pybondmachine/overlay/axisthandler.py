...
class AxiStreamHandler():
    ...
    def __init__(self, overlay, model_specs):
        ...
        if self._use_local_flp:
            self._flp_pool = ThreadPoolExecutor(max_workers=4096)
    ...
    def __parse_prediction(self, outputs):
        ...
        # FLP branch: floating‑point with custom exponent/mantissa
            ...
            with ThreadPoolExecutor(max_workers=4096) as pool:
                preds = list(pool.map(parse_row, data))

        # IEEE float16 / float32 branch: reinterpret bits as real floats
        else:
            reg = self.model_specs['register_size']
            if reg == 16:
                # data is uint16 bit‑patterns; ensure correct endianness
                be = data.astype(np.uint16).byteswap().newbyteorder()
                probs = be.view(np.float16)
            else:  # reg == 32
                be = data.astype(np.uint32).byteswap().newbyteorder()
                probs = be.view(np.float32)
