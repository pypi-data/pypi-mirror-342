...
class AxiStreamHandler():
    ...
    def __init__(self, overlay, model_specs):
        ...
        if self._use_local_flp:
            self._flp_pool = ProcessPoolExecutor(max_workers=os.cpu_count())
    ...
    def prepare_data(self):
        ...
        def cast_batch(batch):
            ...
            if self.model_specs["data_type"].startswith("fps"):
                ...
                # fully parallelize the flp conversion
                futures = [self._flp_pool.submit(self.__cast_float_to_binary_to_unsigned, x)
                           for x in flat]
                conv = [f.result() for f in as_completed(futures)]

            else:
                return batch.astype(self.datatype)

            arr = np.frombuffer(np.array(conv, dtype=hw_dtype), 
                                dtype=hw_dtype).reshape(batch.shape)
            return arr
