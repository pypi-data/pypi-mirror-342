...
from .monai_rag.query_rag_db import MONAI_RAG
...
class Agent:
    def __init__(
        ...
        self.knowledge_base = MONAI_RAG(model=self.acfg.knowledge_base.model)
    ...
    def query_knowledge_base(self, query: str, topk: int=3) -> str: 
        ...
        if self.acfg.enable_monai_knowledge_base:
            try:
                ...
                print("Retrieve Failed! Plz check the knowledge base status")
        return result
    ...
    def _prompt_impl_guideline(self):
        ...
        if self.acfg.force_monai_with_prompt:
            impl_guideline.append(
                "You are recommanded to use MONAI instead of PyTorch to implement these parts:"
                "1. **Data Pipeline**:"
                ...
                "     * Remember to use appropate spatial_size with Resized when the image sizes are different"
                "2. **Network Design**:"
