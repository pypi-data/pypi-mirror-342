...
class WebUI:
    """
    WebUI encapsulates the Streamlit application logic for the MedDLE Machine Learning Engineer Agent.
    """
    ...
    def setup_page():
        ...
        """
        st.set_page_config(
            page_title="MedDLE: Machine Learning Engineer Agent",
            layout="wide",
        )
        WebUI.load_css()
    ...
    def render_input_section(self, results_col):
        ...
        if st.button("Run MedDLE", type="primary", use_container_width=True):
            with st.spinner("MedDLE is running..."):
                results = self.run_meddle(
    ...
    def handle_file_upload(self):
        ...
        if st.session_state.get("example_files"):
            st.info("Example files loaded! Click 'Run MedDLE' to proceed.")
    ...
    def run_meddle(self, files, goal_text, eval_text, num_steps, results_col):
        """
        Run the MedDLE experiment with the provided inputs.
    ...
    def initialize_experiment(input_dir, goal_text, eval_text):
        """
        Initialize the MedDLE Experiment.
