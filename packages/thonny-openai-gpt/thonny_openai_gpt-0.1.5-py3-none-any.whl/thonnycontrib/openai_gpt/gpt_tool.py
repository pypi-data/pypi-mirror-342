...
# 修改導入方式，兼容 Thonny 4.1.7
try:
    from thonny import get_shell, get_editor_notebook
except ImportError:
    # 如果找不到 get_editor_notebook，則使用替代方法
        ...
        try:
            ...
            if editor_notebook:
                ...
                if editor:
                    # 嘗試獲取編輯器的文本
                    try:
                        code = editor.get_text_widget().get("1.0", tk.END)
                    except AttributeError:
                        # 如果結構改變，嘗試替代方法
                        try:
                            code = editor.get_text_content()
                        except:
                            # 最後嘗試直接獲取
                ...
                messagebox.showinfo("提示", "無法獲取編輯器，請先打開一個程式碼檔案")
        except Exception as e:
