...
from PyQt6.QtWidgets import QVBoxLayout, QDialog, QHBoxLayout, QGroupBox, QLineEdit, QPushButton, QCheckBox, \
    QGridLayout, QLabel, QComboBox, QDialogButtonBox, QFileDialog, QMessageBox
...
class OutputOptionsDialog(QDialog):
    def __init__(self, parent=None):
        ...
        # Set default destination folder based on saved preference or file location
    ...
    def set_default_output_folder(self):
        # Try to get last output directory from parent (AudiobooksApp)
        default_folder = ""

        # First priority: Use last saved directory if available
        if hasattr(self.parent_app, "settings") and self.parent_app.settings.contains("last_output_dir"):
            default_folder = self.parent_app.settings.value("last_output_dir")

        # Second priority: Use the directory containing the epub file
        elif hasattr(self, 'file_path') and self.file_path:
            default_folder = os.path.dirname(os.path.abspath(self.file_path))

        # Third priority: Use user's home directory
        else:
    ...
    def select_folder(self):
        ...
        start_dir = self.folder_path.text() if self.folder_path.text() else os.path.expanduser("~")
    ...
    def accept(self):
        ...
        # Check if output folder is the application directory
        app_dir_normalized = os.path.normpath(self.app_dir)
        output_folder_normalized = os.path.normpath(output_folder)

        if output_folder_normalized == app_dir_normalized:
            QMessageBox.warning(
                self,
                "Invalid Output Location",
                "You cannot use the application directory as the output folder. Please select a different location."
            )
            return

        # Save the selected directory for future use
        if hasattr(self.parent_app, "settings"):
            self.parent_app.settings.setValue("last_output_dir", output_folder)

        # Continue with standard accept
        super().accept()
