...
from PyQt6.QtCore import Qt, QThread, pyqtSignal, QSize, QEvent, QTimer, QSettings
...
class AudiobooksApp(QMainWindow):
    def __init__(self):
        super().__init__()
        self.settings = QSettings("AutiobooksQTa", "AutiobooksQTa")
    ...
    # Add this to the AudiobooksApp class after initializing settings

    def load_saved_settings(self):
        """Load saved voice and speed settings"""
        # Load voice setting (already have self.settings initialized)
        saved_voice = self.settings.value("voice_preference", "")
        if saved_voice and saved_voice in voices_emojified:
            self.voice_combo.setCurrentText(saved_voice)

        # Load speed setting
        saved_speed = self.settings.value("speed_preference", 1.0, type=float)
        if 0.5 <= saved_speed <= 2.0:
            # Update both slider and text field
            self.speed_slider.setValue(int(saved_speed * 100))
            self.speed_entry.setText(f"{saved_speed:.1f}")

    def save_current_settings(self):
        """Save current voice and speed settings"""
        # Save voice
        self.settings.setValue("voice_preference", self.voice_combo.currentText())

        # Save speed
        try:
            speed = float(self.speed_entry.text())
            if 0.5 <= speed <= 2.0:
                self.settings.setValue("speed_preference", speed)
        except ValueError:
            pass  # If the speed is invalid, don't save it
