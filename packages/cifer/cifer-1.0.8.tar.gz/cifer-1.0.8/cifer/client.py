...
class CiferClient:
    def __init__(self, encoded_project_id, encoded_company_id, encoded_client_id, base_api=None, dataset_path=None, model_path=None, use_encryption=False):
        # รองรับโค้ดเก่าที่ยังไม่มี use_encryption
    ...
    def upload_model(self, model, accuracy):
        if self.use_encryption:
            ...
            response = requests.post(f"{self.api_url}/upload_encrypted_model", json=payload)
        else:
            model.save(self.model_path)
