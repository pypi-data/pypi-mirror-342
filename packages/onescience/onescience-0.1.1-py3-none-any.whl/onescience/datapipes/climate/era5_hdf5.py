...
class ERA5HDF5Datapipe(Datapipe):
    ...
    def __init__(
        ...
        if self.use_cos_zenith:
            ...
            self.latlon_torch = torch.tensor(self.data_latlon, dtype=torch.float32, 
                                             requires_grad=False)
    ...
    def train_dataloader(self):
        train_data = ERA5Dataset(
            ...
            latlon_resolution=self.latlon_resolution,
            stats_dir=self.stats_dir,
            latlon_torch=self.latlon_torch,
...
class ERA5Dataset(Dataset):
    ...
    def __init__(
        ...
        # 从train_dataloader传入参数
        img_shape: Iterable[int],
        mu: List[float],
        sd: List[float],
        interpolation_type: Union[str, None],
        latlon_resolution: Union[Tuple[int, int], None],
        ...
        self.latlon_torch = latlon_torch
    ...
    def __getitem__(self, idx):
        ...
        if self.shuffle and self.epoch_idx != self.last_epoch:
            # All workers use the same rng seed so the resulting
            # indices are the same across workers.
            np.random.default_rng(seed=self.epoch_idx).shuffle(self.indices)
            self.last_epoch = self.epoch_idx
        ...
        if self.stats_dir is not None:
            if self.num_history == 0:
                invar = (invar - self.mu[0]) / self.sd[0]
            else:
        ...
        # Resize.
        if self.interpolation_type is not None:  
            target_size = (self.latlon_resolution[0], self.latlon_resolution[1])
            if self.num_history == 0:
                invar = torch.unsqueeze(invar, 0)
            invar = F.interpolate(invar, size=target_size, 
                                          mode=self.interpolation_type)
            if self.num_history == 0:
                invar = torch.squeeze(invar, dim=0)
            outvar = F.interpolate(outvar, size=target_size, 
                                           mode=self.interpolation_type)
        ...
        if self.use_cos_zenith:
            ...
            data = [{"invar":invar,
                    ...
                    "time_of_year_idx":np.array(time_of_year_idx)}]
            return data
        else:
            data = [{"invar":invar,
                    "outvar":outvar,
