...
from wechat_draft.utils.logger import set_log_level
...
class WechatDraft:
    ...
    def __init__(self, app_id: str, app_secret: str, access_token_file: str = None, log_level: str = 'INFO'):
        """
        初始化微信公众号文章草稿和永久素材管理
        ...
        :param log_level: 日志记录器日志级别：DEBUG, INFO, WARNING, ERROR, CRITICAL。默认INFO
        """
    ...
    def get_access_token(self) -> Optional[str]:
        ...
        if self._load_access_token():
            ...
            if self.access_token and current_time < self.access_token_expire_time:
                self.log.info("正在使用本地缓存的 access_token")
        ...
        try:
            ...
            if data.get("errcode"):
                self.log.error(f"获取access_token失败: 错误代码 {data['errcode']}, 错误信息 {data['errmsg']}")
                return None
            ...
            self.log.error(f"获取access_token网络请求失败: {e}")
            return None
    ...
    def _load_access_token(self) -> bool:
        ...
        if os.path.exists(self.access_token_file):
            try:
                ...
                self.log.debug("成功加载本地缓存的 access_token")
            except json.JSONDecodeError:
                self.log.warning(f"access_token 文件 {self.access_token_file} JSON解码失败，将重新获取")
                return False
            except Exception as e:
                self.log.error(f"加载 access_token 文件失败: {e}")
                return False
            return True
        return False
    ...
    def _save_access_token(self, expires_in: int = 7200):
        ...
        try:
            ...
            self.log.info(f"access_token 已保存到文件 {self.access_token_file}")
        except Exception as e:
            self.log.error(f"保存 access_token 失败: {e}")
    ...
    def _parse_response(self, response: requests.Response,
        ...
        try:
            ...
            self.log.debug(f"API 响应内容: {json.dumps(result, ensure_ascii=False)}")  # 记录完整的响应内容

            if error_key in result and result[error_key] != 0:  # 显式检查 errcode 是否为 0 表示成功
                handle_error(result, self.log)
                return None
            else:
                return result
        except Exception as e:
            self.log.error(f"API 响应内容解析失败: {e}")
            return None
    ...
    def add_permanent_material(self, material_type: str,
        ...
        if material_type == 'video':
            if not title:
                self.log.error("错误：新增视频素材需要提供标题 (title)")
                return None
            if not introduction:
                self.log.error("错误：新增视频素材需要提供描述 (introduction)")
                return None
    ...
    def get_permanent_material(self, media_id: str):
        ...
        """
        if not media_id:
            self.log.error("参数 media_id 不能为空")
            return None
    ...
    def get_material_total(self, material_type: str = None) -> Optional[int]:
        ...
        if material_type and material_type not in ["image", "video", "voice", "news"]:
            self.log.error("素材类型错误，支持image/video/voice/news")
            return None
        access_token = self.get_access_token()
        if not access_token:
            return None
        ...
        try:
            ...
            self.log.error(f"获取素材总数失败: {str(e)}")
        return None
    ...
    def get_material_list(self, material_type: str = 'news', offset: int = 0, count: int = 20) -> Optional[Dict]:
        ...
        if material_type not in valid_types:
            self.log.error(f"无效的素材类型，支持{valid_types}")
            return None
        if offset < 0:
            self.log.error("offset不能为负数")
            return None
        if not (1 <= count <= 20):
            self.log.error("count必须在1-20之间")
            return None
        ...
        try:
            ...
            if result:
                ...
                self.log.info(f"获取{material_type}素材列表成功，共{total_count}条，本次获取{len(items)}条")
                return result

        except Exception as e:
            self.log.error(f"获取{material_type}素材列表失败: {str(e)}")
        return None
    ...
    def check_draft_switch_state(self) -> Optional[bool]:
        ...
        try:
            ...
            self.log.error(f"检测草稿箱开关状态请求失败: {e}")
            return None
    ...
    def open_draft(self) -> Optional[bool]:
        ...
        self.log.warning("警告：开启草稿箱功能不可逆！操作后服务器后台生效需要一些时间，请耐心等待。")
        ...
        try:
            ...
            self.log.error(f"开启草稿箱功能请求失败: {e}")
            return None
    ...
    def upload_news_image(self, image_path: str) -> Optional[str]:
        ...
        if not os.path.exists(image_path):
            self.log.error(f"错误：图片文件不存在 - {image_path}")
            return None

        file_ext = os.path.splitext(image_path)[1].lower()
        if file_ext not in ('.jpg', '.jpeg', '.png'):
            self.log.error(f"错误：仅支持JPEG/PNG格式图片，当前文件格式为{file_ext}")
            return None

        file_size = os.path.getsize(image_path)
        if file_size > 1 * 1024 * 1024:  # 1MB限制
            self.log.error(f"错误：图片大小超过限制（1MB），当前大小为{file_size / 1024 / 1024:.2f}MB")
            return None
        ...
        try:
            ...
            if result.get("url"):
                url = result.get("url")
                self.log.info(f"上传图文图片成功，URL：{url}")
                return url
        except Exception as e:
            self.log.error(f"上传图文图片请求失败：{str(e)}")
        return None
    ...
    def get_crop_params(self, image_file_path: str,
        ...
        try:
            ...
            self.log.debug(f"图片原始尺寸：宽度={original_width}px, 高度={original_height}px")

            start_x_px, start_y_px = start_point
            self.log.debug(f"截图起点（像素坐标）：x={start_x_px}px, y={start_y_px}px")
            self.log.debug(f"横轴剪切宽度（像素）：{crop_width_px}px")
            self.log.debug(f"自动调整超出边界参数：{'开启' if auto_adjust_if_exceed else '关闭'}")
            ...
            for ratio_name, ratio_hw in ratios.items():
                ...
                if end_x_px > original_width or end_y_px > original_height:
                    if auto_adjust_if_exceed:
                        self.log.warning(f"比例 {ratio_name} 裁剪尺寸超出图片边界，尝试自动调整...")
                        # 优先调整宽度，确保宽度不超过边界
                        if end_x_px > original_width:
                            ...
                            end_x_px = start_x_px + current_crop_width_px
                            end_y_px = start_y_px + crop_height_px
                            self.log.warning(f"宽度超出边界，已调整 crop_width_px 为 {current_crop_width_px}px")
                        ...
                        self.log.debug(f"比例 {ratio_name} 自动调整后裁剪参数:")
                        self.log.debug(
                ...
                self.log.debug(f"比例 {ratio_name} 裁剪参数:")
                self.log.debug(
                    f"  裁剪区域（像素坐标）：左上角=({start_x_px}px, {start_y_px}px), 右下角=({end_x_px}px, {end_y_px}px)")
                self.log.debug(f"  归一化坐标：x1={x1_str}, y1={y1_str}, x2={x2_str}, y2={y2_str}")
            ...
            self.log.debug(f"裁剪参数字典：{crop_params_dict}")
            return crop_params_dict

        except FileNotFoundError:
            self.log.error(f"错误：图片文件未找到：{image_file_path}")
            return None
        except ValueError as ve:
            if not auto_adjust_if_exceed:
                self.log.error(f"参数错误: {ve}")  # 只有在不自动调整时才记录 ValueError，否则自动调整已经warning了
                return None
            else:  # 如果开启了自动调整，但还是有其他ValueError，例如 Pillow 库的错误，也需要捕获并返回空字典
                self.log.error(f"处理图片时发生错误: {ve}")
                return None
        except Exception as e:
            self.log.error(f"处理图片时发生未知错误: {e}")
            return None
    ...
    def create_draft(self,
        ...
        # 基础必填参数校验
        if not title:
            self.log.error("错误：标题（title）为必填参数")
            return None
        if len(title) > 64:
            self.log.error(f"错误：标题长度{len(title)}超过限制（最多64字，当前 {len(title)} 字）")
            return None
        if not content:
            self.log.error("错误：内容（content）为必填参数")
            return None

        # 类型相关参数校验
        if article_type not in ["news", "newspic"]:
            self.log.error("错误：article_type仅支持 news（图文） 或 newspic（图片消息）")
            return None
        ...
        if article_type == "news":
            if not thumb_media_id:
                self.log.error("错误：图文消息（news）必须提供封面素材ID（thumb_media_id）")
                return None
        ...
        # 处理图片消息特有参数（newspic类型）
            ...
            if not image_info or not image_info.get("image_list"):
                self.log.error("错误：图片消息（newspic）必须提供 image_info.image_list")
                return None
            if len(image_info["image_list"]) > 20:
                self.log.error("错误：图片消息最多支持20张图片，当前 {len(image_info['image_list'])} 张")
                return None
        ...
        try:
            ...
            if result:
                ...
                self.log.info(f"新增草稿成功，media_id: {media_id}")
                return media_id

        except Exception as e:
            self.log.error(f"新增草稿请求失败: {str(e)}")
        return None
    ...
    def get_draft_list(self, offset: int = 0, count: int = 20, no_content: bool = True) -> Optional[dict]:
        ...
        """
        if offset < 0:
            self.log.error("offset 必须大于等于 0")
            return None
        if not (1 <= count <= 20):
            self.log.error("count 必须在 1 到 20 之间")
            return None
    ...
    def count_drafts(self) -> Optional[int]:
        ...
        try:
            ...
            if result:
                ...
                self.log.info(f"草稿总数: {total_count}")
                return total_count

        except Exception as e:
            self.log.error(f"获取草稿总数失败: {str(e)}")
        return None
    ...
    def publish_article(self, media_id: str) -> Optional[str]:
        ...
        try:
            ...
            if result:
                ...
                self.log.info(f"发布文章成功，publish_id: {publish_id}")
                return publish_id
        except Exception as e:
            self.log.error(f"发布文章请求失败: {str(e)}")
        return None

    def get_publish_status(self, publish_id: str) -> Optional[int]:
        """
        获取发布状态
        ...
        try:
            ...
            if result:
                ...
                self.log.info(f"发布状态: {all_publish_status.get(publish_status)}")
                return result
        except Exception as e:
            self.log.error(f"获取发布状态请求失败: {str(e)}")
        return None
    ...
    def get_publish_list(self, offset: int = 0, count: int = 20, no_content: bool = True) -> Optional[dict]:
        ...
        """
        if offset < 0:
            self.log.error("offset 不能小于0")
            return None
        if count < 1 or count > 20:
            self.log.error("count 取值在1到20之间，默认为20")
            return None
        ...
        try:
            ...
            self.log.error(f"获取已发布的文章列表请求失败: {str(e)}")
        return None
    ...
    def get_publish_article(self, article_id: str) -> Optional[dict]:
        ...
        try:
            ...
            self.log.error(f"获取已发布的文章请求失败: {str(e)}")
        return None
    ...
    def delete_publish_article(self, article_id: str, index: int = 0) -> Optional[bool]:
        ...
        try:
            ...
            self.log.error(f"删除已发布的文章请求失败: {str(e)}")
            return None
    ...
    def get_all_permanent_news(self, save_path: str = None, simple_data: bool = True) -> list:
            ...
            if simple_data:
                for item in items:
                    all_permanent_news.append({
                        "title": item.get("content", {}).get("news_item", [{}])[0].get("title", ""),  # 标题
                        "url": item.get("content", {}).get("news_item", [{}])[0].get("url", ""),  # url
                        "create_time": item.get("content", {}).get("create_time", 0)  # 时间戳
                    })
            else:
        ...
        if save_path:
            with open(save_path, "w", encoding="utf-8") as f:
                ...
                self.log.info(f"保存永久图文素材成功，保存路径: {save_path}")

        self.log.info(f"获取永久图文素材成功，共 {len(all_permanent_news)} 条")
