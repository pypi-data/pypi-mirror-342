...
class WechatDraft:
    ...
    def _handle_error(self, errcode: int, errmsg: str):
        """
        错误码说明
        ...
        self.log.error(f"微信API错误码 {errcode}：{error_desc}")
    ...
    def _parse_response(self, response: requests.Response, success_key: str, error_key: str = "errcode"):
        ...
        try:
            ...
            self.log.error(f"API 响应 JSON 解码失败，响应原始内容: {response.text}")
            return None
    ...
    def check_draft_switch_state(self):
        ...
        try:
            ...
            self.log.debug(f"检测开关状态响应: {json.dumps(result, ensure_ascii=False)}")
    ...
    def open_draft(self):
        ...
        try:
            ...
            response.raise_for_status()  # 检查HTTP状态码
            result = response.json()  # 无需 _parse_response，结构简单
            self.log.debug(f"切换开关响应: {json.dumps(result, ensure_ascii=False)}")
            if result.get("errcode") == 0:
                self.log.info("已发送开启草稿箱功能请求，后台将尽快生效。")
                return True
            else:
                self._handle_error(result["errcode"], result.get("errmsg"))
                return False
    ...
    def upload_news_image(self, image_path: str) -> Optional[str]:
        ...
        if file_ext not in ('.jpg', '.jpeg'):
            self.log.error(f"错误：仅支持JPEG格式图片，当前文件格式为{file_ext}")
            return None
        ...
        try:
            ...
            response.raise_for_status()
            result = response.json()
            self.log.debug(f"上传图片响应：{json.dumps(result, ensure_ascii=False)}")
            ...
            self.log.error(f"响应解析失败，原始内容：{response.text}")
            return None
    ...
    def create_draft(self,
        ...
        try:
            ...
            if result:
                self.log.info(f"新增草稿成功，media_id: {result}")
                return result
            else:
                self.log.error("新增草稿失败，请检查参数")
