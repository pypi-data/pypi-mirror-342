...
class BursaryRefereeAdmin(admin.ModelAdmin):
    ...
    def get_fieldsets(self, request, obj=None):
        fieldsets = (
            ...
            ('Bursary application', {
                'fields': (
                    'bursary_request', 'bursary_budget', 'bursary_need',
    ...
    def get_list_display(self, request):
        if self.is_admin(request):
            return (
                ...
                'bursary_budget', 'contrib_score', 'outreach_score', 'notes',
            )
        return (
            'full_name', 'email', 'final', 'bursary_request', 'bursary_budget',
    ...
    def bursary_budget(self, instance):
        budget_label = instance.bursary.budget
        default = f'Unknown budget ({budget_label})'
        return settings.DEBCONF_BURSARY_BUDGETS.get(budget_label, {})\
                                               .get('description', default)
    bursary_budget.short_description = 'Budget within which the bursary has been requested'
