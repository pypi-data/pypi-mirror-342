...
__all__ = ['THS', 'FuTHS', 'ZxTHS', 'BkTHS', 'Adjust', 'Interval', 'Response', 'ResponseData']
...
master_addr = 'default_zhu_addr'
fu_addr = 'default_fu_addr'
zx_addr = 'default_info_addr'
bk_addr = 'default_block_addr'
...
class THS:
    def __init__(self, ops: dict = None):
        ops = ops or {}
        ops.setdefault('addr', master_addr)
            ...
            if start is None or end is None:
                ...
                if isinstance(start, int):
                    if len(str(start)) == 8:  # Check if the integer is 8 digits
                        ...
                        china_tz = pytz.timezone("Asia/Shanghai")  # Define China Standard Time timezone
                        start = china_tz.localize(start)  # Localize the datetime to CST
                        start = int(start.timestamp())  # Convert to timestamp
                if isinstance(end, int):
                    if len(str(end)) == 8:  # Check if the integer is 8 digits
                        ...
                        china_tz = pytz.timezone("Asia/Shanghai")  # Define China Standard Time timezone
                        end = china_tz.localize(end)  # Localize the datetime to CST
                        end = int(end.timestamp())  # Convert to timestamp
