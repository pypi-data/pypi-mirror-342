...
async def main(main_args):
    ...
    username = main_args.user if main_args.user else getpass.getuser()

    gar_client = GARClient(endpoint, username, allow_self_signed_certificate=main_args.allow_self_signed_certificate)
    ...
    def on_heartbeat():
        logging.info("First heartbeat received")
    ...
    try:
        if main_args.publish:
            ...
            gar_client.subscribe("S1", mode=subscription_mode,
            ...
            if subscription_mode == "Snapshot":
                ...
                def on_snapshot_complete(name):
                    logging.info("Snapshot received for subscription: %s", name)
                    snapshot_received.set()
                    gar_client.clear_snapshot_complete_handler()

                gar_client.register_snapshot_complete_handler(on_snapshot_complete)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--url", type=str, default="ws://127.0.0.1:6502",
                        help="GAR WebSocket URL (default: ws://127.0.0.1:6502)")
    parser.add_argument("--user", type=str, default=None,
                        help="Username (default: OS environment username)")
    parser.add_argument("--allow-self-signed-certificate", action="store_true",
    ...
    parser.add_argument("--publish", nargs=3, metavar=('key', 'topic', 'value'),
