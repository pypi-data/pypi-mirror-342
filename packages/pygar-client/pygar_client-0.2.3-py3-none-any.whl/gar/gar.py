...
class GARClient:
    ...
    def __init__(self, ws_endpoint: str, user: str, heartbeat_interval: int = 4000, allow_self_signed_certificate: bool = False):
        """
    ...
    async def connect(self):
        ...
        while self.running and not self.connected:
            try:
                ...
                async with websockets.connect(self.ws_endpoint, **connect_kwargs) as websocket:
    ...
    async def _receive_messages(self):
        ...
        while self.connected and self.running:
            try:
                ...
                break
        self.stop()
        asyncio.run_coroutine_threadsafe(self.message_queue.put(None), self.loop) # Put None into the queue to stop the send loop
    ...
    def clear_introduction_handler(self):
    ...
    def clear_logoff_handler(self):
        """Remove the registered logoff handler."""
        self.message_handlers.pop("Logoff", None)

    def register_error_handler(self, handler: Callable[[str], None]):
    ...
    def clear_topic_introduction_handler(self):
    ...
    def clear_key_introduction_handler(self):
    ...
    def clear_delete_key_handler(self):
    ...
    def clear_snapshot_complete_handler(self):
        """Remove the registered snapshot complete handler."""
        self.message_handlers.pop("SnapshotComplete", None)
    ...
    def clear_unsubscribe_handler(self):
        """Remove the registered unsubscribe handler."""
        self.message_handlers.pop("Unsubscribe", None)
    ...
    def clear_delete_record_handler(self):
    ...
    def clear_record_update_handler(self):
    ...
    def clear_shutdown_handler(self):
    ...
    def clear_heartbeat_timeout_handler(self):
    ...
    def clear_stopped_handler(self):
    ...
    def check_heartbeat(self):
        ...
        if time.time() > cutoff:
            self.logger.warning("Heartbeat failured; previous heartbeat: %.3fs", self.last_heartbeat_time)
            self.exit_code = 1
            self.halt()
