...
def resolve_payload(node, **kwargs):
    ...
    # Convert paths to strings and normalize for case-insensitive deduplication
    current_assets = []
    seen = set()
...
# In /Users/alaaibrahim/Documents/coding/ciohoudini/ciohoudini/assets.py

# ... (other imports and functions) ...

def check_path(file_path):
    """
    Checks if a given file path is acceptable for asset scanning.

    Filters out:
    - Paths containing wildcards (*).
    - Paths starting with /tmp/.
    - Paths containing 'houdini_temp'.
    - Paths that resolve to the directory containing the current .hip file.

    Args:
        file_path (str): The file path to check.

    Returns:
        bool: True if the path is okay, False otherwise.
    """
    if not file_path: # Handle empty or None paths
        return False

    file_path = file_path.replace("\\", "/")  # Normalize slashes

    # Check 1: Skip wildcard paths
    if "*" in file_path:
        # logger.debug(f"Skipping wildcard path: {file_path}")
        return False

    # Check 2: Skip temporary files/folders
    if file_path.startswith("/tmp/"):
        # logger.debug(f"Skipping temporary file path: {file_path}")
        return False

    # Check 3: Skip Houdini temp folder paths
    if "houdini_temp" in file_path:
        # logger.debug(f"Skipping Houdini temp file path: {file_path}")
        return False

    # Check 4: Skip if the path is the HIP folder itself
    try:
        ...
        if hip_folder:
            ...
            if normalized_file_path == normalized_hip_folder:
                logger.debug(f"Skipping HIP folder path: {file_path}")
                return False
    except Exception as e:
        # Log error if HIP folder check fails, but don't necessarily block the path
        logger.warning(f"Could not check if path is HIP folder due to error: {e}")


    # If none of the above conditions were met, the path is okay
    return True
