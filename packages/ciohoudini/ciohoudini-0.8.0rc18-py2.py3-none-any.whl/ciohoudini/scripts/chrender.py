...
def prep_arnold(node, **kwargs):
    ...
    camera_output_path = set_output_path(**kwargs)
...
def get_camera_number(camera_path):
    """
    ...
    Args:
        camera_path (str): The name of the camera (e.g., "/camera_rig_sphere/Instance11", "/path/to/cam_05").

    Returns:
        str or None: The formatted camera number string (e.g., "00012") or None if extraction fails.
    """
    if not camera_path:
        print("get_camera_number received an empty camera name.")
        return None
    ...
    if match:
        try:
            ...
            print(f"An unexpected error occurred in get_camera_number for '{camera_name}': {e}")
            return "00000"
    else:
...
def set_output_path(**kwargs):
    """
    Constructs a formatted output path string based on provided parameters.

    Uses 5-digit padding for the frame number token ($F5).
    Desired format: CameraOutputPath/FrameNumber/SubjectName_FrameNumber_CameraNumber_VersionNumber.exr
    Example result: $HIP/render/koi/$F5/koi_$F5_00012_v01.exr

    Args:
        kwargs: Dictionary potentially containing:
            camera_output_folder (str): The base directory path (e.g., "$HIP/render/koi").
            subject_name (str): The subject identifier (e.g., "koi").
            camera_number (str): The formatted camera number (e.g., "00012").
            version_number (str): The formatted version number (e.g., "v01").

    Returns:
        str or None: The constructed output path string, or None if essential components are missing.
    """
    # Retrieve components from kwargs, providing defaults
    output_folder = kwargs.get("camera_output_path", None)
    subject_name = kwargs.get("subject_name", "default_subject")
    # Use the camera_number directly as it should already be formatted (e.g., "00012")
    camera_path = kwargs.get("camera", None)
    if camera_path:
        camera_number = get_camera_number(camera_path)
    else:
        camera_number = "00000"

    # Use the version_number directly as it should already be formatted (e.g., "v01")
    version_number = kwargs.get("version_number", "v00")

    # Houdini token for 5-digit padded frame number
    frame_folder = "$F5"
    frame_token = "<F5>"
    print(f"Using frame token: {frame_token}")
    #frame_token = hou.expandString("$F5")

    # --- Input Validation ---
    if not output_folder:
        print("Error: 'camera_output_path' not provided in kwargs for set_output_path.")
        return None
    # --- Construct the filename part ---
    # Desired format: SubjectName_FrameNumber_CameraNumber_VersionNumber.exr
    # Example: koi_$F5_00012_v01.exr

    filename = f"{subject_name}_{frame_token}_{camera_number}_{version_number}.exr"
    print(f"Using filename: {filename}")

    # --- Construct the full path ---
    # Desired structure: CameraOutputPath/FrameNumber/Filename
    # Example: $HIP/render/koi/$F5/koi_$F5_00012_v01.exr
    # Using os.path.join for cross-platform compatibility.
    # full_path = os.path.join(output_folder, frame_folder, filename)
    full_path = os.path.join(output_folder, frame_token, filename)

    # Return the pattern with $HIP intact, as Houdini will resolve it later
    print(f"Constructed output path pattern: {full_path}")
...
def ensure_directory_exists(path):
    ...
    try:
        ...
        if directory:
            # Convert to POSIX path if it contains a drive letter
            if DRIVE_LETTER_RX.match(directory):
                directory = DRIVE_LETTER_RX.sub("", directory).replace("\\", "/")
            print(f"Ensuring directory exists: {directory}")
            os.makedirs(directory, exist_ok=True)
            print(f"Directory {directory} created or already exists")
        else:
            print(f"No directory to create for path: {path}")
    except Exception as e:
...
def parse_args():
    ...
    parser.add_argument("-c", dest="camera")
    parser.add_argument("-f", dest="frames", nargs=3, type=int)
    parser.add_argument("-s", dest="subject_name")
    parser.add_argument("-n", dest="camera_number")
    parser.add_argument("-v", dest="version_number")
    parser.add_argument("-o", dest="camera_output_path")
...
def render(args):
    ...
    try:
        ...
        # print("camera_number: '{}'".format(camera_number))
        print("version_number: '{}'".format(version_number))
