...
def get_metrics():
    ...
    # Make three attempts with bbk-cli
    for i in range(1, 4):
        result = get_bbk_metrics()
        try:
            if result["bbkcli_latency"]["successful"]:
                print("\t Data obtained", result)
                for metric_name, metric_values in result.items():
                    metric_results[metric_name] = metric_values
                break
            else:
                sleep(0.5)
                print(f"\tINFO: bbk-cli: Retry {i} of 3")
        except Exception as e:
            print(f" ERROR: bbk-cli metric could not be collected: \n {e}")
