...
from fsd.util.utils import Spinner
...
class RepoMap:
    CACHE_VERSION = 3
    TAGS_CACHE_DIR = f".zinley.tags.cache.v{CACHE_VERSION}"
    ...
    def load_tags_cache(self):
        ...
        try:
            ...
            try:
                if path.exists():
                    path.unlink()  # Delete the file if it exists
                self.TAGS_CACHE = Cache(path)  # Try to create a new cache
