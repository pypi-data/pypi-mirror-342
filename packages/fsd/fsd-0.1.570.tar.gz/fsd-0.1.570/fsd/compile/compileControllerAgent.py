...
class CompileControllerAgent:
    ...
    async def start_CLI_compile_process(self, instruction, code_files, original_prompt_language, file_attachments, focused_files):
        ...
        logger.info(" #### `SnowX` is creating plan for clarification")
        ...
        while True:
            ...
            if user_prompt == 's':
                logger.info(" #### `SnowX` has detected that the run/compile process has been skipped.")
                return

            if user_prompt == "a":
                break
            else:
                logger.info(" #### `SnowX` is updating based on user feedback.")
