...
class FileManagerAgent:
    ...
    async def get_file_planning(self, idea):
        ...
        try:
            logger.debug("\n #### The `SnowX` is initiating a request to the AI for file planning")
            response = await self.ai.prompt(messages, 4096, 0.2, 0.1)
            logger.debug("\n #### The `SnowX` has successfully received a response from the AI")
            return json.loads(response.choices[0].message.content)
        except json.JSONDecodeError:
            logger.debug("\n #### The `SnowX` encountered a JSON decode error and is attempting to repair it")
            good_json_string = repair_json(response.choices[0].message.content)
            plan_json = json.loads(good_json_string)
            return plan_json
        except Exception as e:
            logger.debug(f" #### The `SnowX` encountered an unexpected error: `{e}`")
            return {
                "reason": str(e)
            }
    ...
    async def get_adding_file_planning(self, idea, tree, file_attachments):
        ...
        try:
            logger.debug("\n #### The `SnowX` is commencing the process of adding file planning")
            response = await self.ai.prompt(messages, 4096, 0.2, 0.1)
            logger.debug("\n #### The `SnowX` has successfully completed the adding file planning process")
            return json.loads(response.choices[0].message.content)
        except json.JSONDecodeError:
            logger.debug("\n #### The `SnowX` encountered a JSON decode error and is attempting to repair it")
            good_json_string = repair_json(response.choices[0].message.content)
            ...
            logger.debug(f" #### The `SnowX` encountered an unexpected error during adding file planning: `{e}`")
            return {
                "reason": str(e)
            }
    ...
    async def get_moving_file_planning(self, idea, tree, file_attachments):
        ...
        try:
            logger.debug("\n #### The `SnowX` is initiating the process of moving file planning")
            response = await self.ai.prompt(messages, 4096, 0.2, 0.1)
            logger.debug("\n #### The `SnowX` has successfully completed the moving file planning process")
            return json.loads(response.choices[0].message.content)
        except json.JSONDecodeError:
            logger.debug("\n #### The `SnowX` encountered a JSON decode error and is attempting to repair it")
            good_json_string = repair_json(response.choices[0].message.content)
            ...
            logger.debug(f" #### The `SnowX` encountered an unexpected error during moving file planning: `{e}`")
            return {
                "reason": str(e)
            }
    ...
    async def get_moving_file_plannings(self, idea, tree, file_attachments):
        ...
        """
        logger.debug("\n #### The `SnowX` is beginning the process of retrieving moving file plannings")
        all_file_contents = self.repo.print_tree()

        plan = await self.get_moving_file_planning(idea, all_file_contents, file_attachments)
        logger.debug("\n #### The `SnowX` has successfully retrieved the moving file plannings")
        return plan
    ...
    async def get_adding_file_plannings(self, idea, tree, file_attachments):
        ...
        Returns:
            dict: JSON response with the plan.
        """
        logger.debug("\n #### The `SnowX` is commencing the process of retrieving adding file plannings")
        all_file_contents = self.repo.print_tree()

        plan = await self.get_adding_file_planning(idea, all_file_contents, file_attachments)
        logger.debug("\n #### The `SnowX` has successfully retrieved the adding file plannings")
        return plan
    ...
    async def get_file_plannings(self, idea):
        ...
        Returns:
            dict: JSON response with the plan.
        """
        logger.debug("\n #### The `SnowX` is initiating the process of retrieving file plannings")
        plan = await self.get_file_planning(idea)
        logger.debug("\n #### The `SnowX` has successfully retrieved the file plannings")
        return plan