...
class ImageAnalysAgent:
    ...
    async def get_idea_plan(self, user_prompt, original_prompt_language, file_attachments, focused_files, assets_link):
        ...
        try:
            logger.debug("\n #### The `SnowX` is initiating the AI prompt for idea generation")
            response = await self.ai.arch_stream_prompt(self.conversation_history, 4096, 0.2, 0.1)
            logger.debug("\n #### The `SnowX` has successfully received the AI response")
            return response
        except Exception as e:
            logger.error(f"  The `SnowX` encountered an error during idea generation\n Error: {e}")
            return {
                "reason": str(e)
            }
