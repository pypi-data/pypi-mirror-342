...
class ControllerAgent:
    ...
    async def get_pro_fixing_requests(
        ...
        logger.info("-------------------------------------------------")
        logger.info("#### The `SnowX` is starting the fixing phase in Snow mode.")
        ...
        async def process_task(task, coding_agent):
            ...
            if self.is_coding_file(full_file):
                ...
                if main_path:
                    ...
                    logger.info(
                        f"#### The `SnowX` is processing file: `{os.path.relpath(full_file, self.directory_path)}`."
                    )
                    ...
                    try:
                        ...
                        expert_prompt = (
                            "You are a world-class software engineer with exceptional attention to detail. "
                            "Your code must be flawless in both logic and implementation. Remember:\n\n"
                            "IMPLEMENTATION EXCELLENCE:\n"
                            "- Deliver complete, production-ready solutions with no TODOs or placeholders\n"
                            "- Take full ownership of your code - never defer implementation details\n"
                            "- Write robust error handling with appropriate recovery mechanisms\n"
                            "- Ensure proper resource management (memory, connections, file handles)\n\n"
                            "UI/UX PERFECTION:\n"
                            "- Create pixel-perfect layouts that work across all screen sizes\n"
                            "- Ensure smooth animations and transitions (60fps target)\n"
                            "- Implement accessible interfaces (WCAG AA compliance minimum)\n"
                            "- Design intuitive user flows with appropriate feedback mechanisms\n\n"
                            "TECHNICAL RIGOR:\n"
                            "- Write performant code that scales with increasing data/users\n"
                            ...
                            "Incomplete implementations are unacceptable. Your code must be ready for immediate deployment."
                        )
                        ...
                        if is_svg:
                            user_prompt += (
                                ...
                                "CRITICAL: You MUST implement ALL fixes completely - no placeholders or TODOs allowed.\n"
                            )
                        else:
                            user_prompt += (
    ...
    async def get_pro_coding_requests(
        ...
        if file_count <= 3:
            ...
            logger.info(f"#### Starting with {file_count} files. Grab a coffee, we got this! ☕")
        else:
        ...
        commits = plan.get('commits', "")
        logger.info("-------------------------------------------------")
        logger.info("#### The `SnowX` is starting the coding phase.")
                    ...
                    try:
                        ...
                        if is_svg:
                            ...
                            user_prompt += (
                                ...
                                "- Coding Rule: Only define variables, functions, and imports that are actually used in the code.\n"
                                "- File Structure: Always check the project structure to ensure correct import paths and file references.\n"
                                "- Tech Stack: Use the correct syntax and patterns for the specific technology stack being used.\n"
                                "- CSS Integration: Match and use CSS files correctly, maintaining consistent class naming conventions.\n"
                                "- UI Components: Ensure perfect alignment, consistent padding, and responsive design.\n"
                                "- Logic Implementation: Write robust code with proper error handling and edge case coverage.\n"
                                "- Data Flow: Implement clear data flow patterns with appropriate state management.\n"
                                "- API Integration: Handle API requests with proper loading states and error handling.\n"
                                "- Configuration: Pay attention to configuration files and environment settings.\n"
                                "\n"
                                "Examples of context-aware implementation:\n"
                                "- When modifying components, check related files and ensure proper integration.\n"
                                "- For backend code, ensure consistency with existing patterns and authentication.\n"
                                "- When updating models, verify all dependent services and queries.\n"
                                "- For configuration changes, check all affected systems and requirements.\n"
                            )
                            user_prompt += "IMPORTANT: Before coding, analyze ALL relevant files including configuration files, routes, models, and related components. Always refer back to the High level development instruction to ensure alignment and completeness.\n"
    ...
    async def add_files_folders_pipeline(self, finalPrompt, role, file_attachments):
        ...
        logger.info(" #### The `SnowX` is processing files.")
        ...
        try:
            ...
            if crawl_plan:
                for step in crawl_plan.get('crawl_tasks', []):
                    ...
                    if crawl_url:
                        logger.info(f" #### The `SnowX` is reading: [{crawl_url}]({crawl_url})")
                        result = self.crawler.process(crawl_url)
                        logger.info(f" #### The `SnowX` has finished reading: [{crawl_url}]({crawl_url})")
                        crawl_logs.append({
                            'url': crawl_url,
                            'result': result
                        })

            logger.debug(" #### The `SnowX` is performing dependency checks.")
            await self.dependency.get_started_coding_pipeline(solution, original_prompt_language, [], [])
            logger.debug(" #### The `SnowX` has finished with dependency checks.")
            logger.debug("-------------------------------------------------")
            ...
            if add or move:
                ...
                if commits:
                    self.repo.add_all_files(f"Zinley - {commits}")
            logger.debug("\n #### The `SnowX` has completed processing files.")
            logger.debug("-------------------------------------------------")
            ...
            logger.debug(" #### The `SnowX` has encountered some bugs. We apologize for the inconvenience. Please try again!")
    ...
    async def regular_code_task_pipeline(self, tier, finalPrompt, role, original_prompt_language, file_attachments, focused_files, snow_mode, auto_mode, imageModel, codingModel):
        try:
            ...
            logger.info(" #### The `SnowX` has completed the coding phase.")
            logger.info("-------------------------------------------------")
        except Exception as e:
            logger.debug(f" #### The `SnowX` has encountered an error: {str(e)}. We apologize for the inconvenience. Please try again!")
    ...
    async def codingProgress(self, tier, finalPrompt, role, original_prompt_language, file_attachments, focused_files, snow_mode, auto_mode, imageModel, codingModel):
        ...
        while True:
            ...
            if idea_plan and not auto_mode:
                logger_message = " #### The `SnowX` is asking for your approval. Click `Approve` if you feel satisfied, click `Skip` to end this process, or type your feedback below."
            ...
            logger.info(" #### The `SnowX` is gathering context and preparing relevant data...")
            crawl_plan = await self.crawlerPlaner.get_crawl_plans(finalPrompt)
            if crawl_plan:
                for step in crawl_plan.get('crawl_tasks', []):
                    crawl_url = step.get('crawl_url')
                    if crawl_url and crawl_url not in crawled_urls:
                        logger.info(f" #### The `SnowX` is reading: `{crawl_url}`")
                        result = self.crawler.process(crawl_url)
                        logger.info(f" #### The `SnowX` has finished reading: `{crawl_url}`")
                        crawl_logs.append({
                            'url': crawl_url,
                            'result': result
                        })
                        ...
                        logger.info(f" #### The `SnowX` is reusing previously crawled data for: `{crawl_url}`")
            ...
            if all_files:
                ...
                logger.info(f" #### The `SnowX` is reading these files:\n{files_list}")
            logger.info("#### Thinking completed.")

            self.idea.initial_setup(role, crawl_logs, context, file_attachments, assets_link)

            if idea_plan:
                logger.info(" #### The `SnowX` is updating implementation plan.")
            else:
                logger.info(" #### The `SnowX` is creating implementation plan.")
    ...
    async def get_started(self, user_prompt, tier, file_attachments, focused_files, snow_mode, auto_mode, imageModel, codingModel):
        ...
        logger.info("-------------------------------------------------")
        logger.info(" #### The `SnowX` will now begin processing your request.")
