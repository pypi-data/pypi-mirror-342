...
class InitNewProjectAgent:
    ...
    async def get_initNewProject_plan(self, user_prompt):
        ...
        try:
            ...
            logger.error(f"The `SnowX` encountered an error during plan generation: {e}")
            return {
                "reason": str(e)
            }
    ...
    async def clone_template_repository(self, pipeline, folder_name):
        ...
        try:
            ...
            logger.info(f"#### The `SnowX` is starting clone of `{framework_name}` project into folder `{folder_name}`")
            logger.info(f"#### Project repository URL: `{repo_url}`")
            logger.info(f"#### Target path: `{target_path}`")
            
            # Create the folder if it doesn't exist
            if not os.path.exists(target_path):
                logger.info(f"#### The `SnowX` is creating directory `{folder_name}`")
                os.makedirs(target_path)
            else:
                logger.info(f"#### Directory `{folder_name}` already exists")
                
            # Clone the repository
            logger.info(f"#### The `SnowX` is cloning repository... this may take a moment")
            ...
            logger.info(f"#### The `SnowX` has successfully cloned `{framework_name}` project")
                
            # Remove the .git directory to start fresh
            git_dir = os.path.join(target_path, ".git")
            if os.path.exists(git_dir):
                logger.info(f"#### The `SnowX` is removing .git directory to start fresh")
    ...
    async def get_initNewProject_plans(self, user_prompt):
        ...
        logger.debug(f"The `SnowX` has successfully completed preparing for the user prompt: {user_prompt}")
        ...
        if 'pipeline' in plan and plan['pipeline'] != 0 and plan['pipeline'] != '0' and 'folder_name' in plan and plan['folder_name']:
            ...
            if result:
                logger.info("Template repository successfully cloned")
                plan['template_cloned'] = True
            else:
                logger.info("Failed to clone template repository")
                plan['template_cloned'] = False
                plan['error'] = "Failed to clone template repository"
