...
class CommandRunner:
    ...
    async def get_config_requests(self, instructions, file_name):
        ...
        main_path = file_name
        logger.debug(f" #### The `SnowX` is initiating the processing of file: {file_name} in {main_path}")
        logger.info(f" #### The `SnowX` has been assigned the following task: {instructions}")
        ...
        if main_path:
            ...
            logger.info(f" #### The `SnowX` has successfully completed its work on {file_name}")
        else:
            logger.debug(f" #### The `SnowX` encountered an issue: Unable to locate the file: {file_name}")
