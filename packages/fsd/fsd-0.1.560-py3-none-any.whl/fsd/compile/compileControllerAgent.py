...
class CompileControllerAgent:
    ...
    async def start_CLI_compile_process(self, instruction, code_files, original_prompt_language, file_attachments, focused_files):
        ...
        logger.info(" #### The `SnowX` is creating plan for clarification")
        ...
        while True:
            ...
            if user_prompt == 's':
                logger.info(" #### The `SnowX` has detected that the run/compile process has been skipped.")
                return

            if user_prompt == "a":
                break
            else:
                logger.info(" #### The `SnowX` is updating based on user feedback.")
        ...
        logger.info(" #### The `SnowX` is now organizing and preparing the task for execution.")
        ...
        logger.info(" #### The `SnowX` has successfully completed the compilation process.")
        logger.info("-------------------------------------------------")
    ...
    async def get_started(self, user_prompt, original_prompt_language, file_attachments, focused_files):
        ...
        logger.info("-------------------------------------------------")
        logger.info(" #### The `SnowX` is initiating the compilation request process.")

        await self.start_CLI_compile_process(user_prompt, [], original_prompt_language, file_attachments, focused_files)

        logger.info(f" #### The `SnowX` has completed processing the user prompt: {user_prompt}.")
        logger.info("-------------------------------------------------")
