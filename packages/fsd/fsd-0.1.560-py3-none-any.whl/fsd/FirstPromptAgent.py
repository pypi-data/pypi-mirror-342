...
class FirstPromptAgent:
    ...
    async def get_prePrompt_plans(self, user_prompt):
        ...
        """
        try:
            logger.info("#### Hi there! The `SnowX` is processing your request.")
            messages = self._create_messages(user_prompt)
            response = await self.ai.prompt(messages, 4096, 0.2, 0.1)
            return self._parse_response(response)
        except Exception as e:
            logger.error(f" The `SnowX` encountered an error during pre-prompt planning:\n{str(e)}")
            return {"reason": str(e)}

    def _create_messages(self, user_prompt):
        logger.debug("#### The `SnowX` is constructing messages for the AI gateway")
        system_content = (
    ...
    def _parse_response(self, response):
        logger.debug("#### The `SnowX` is parsing the AI gateway response")
        try:
            return json.loads(response.choices[0].message.content)
        except json.JSONDecodeError:
            logger.error(f" The `SnowX` encountered an error and is attempting to repai itself.")
