...
class MainBuilderAgent:
    ...
    async def get_pipeline_plan(self, files: str, tree: str) -> Dict:
        ...
        logger.debug("\n #### The `SnowX` is initiating the process to obtain a pipeline plan")
        ...
        try:
            logger.debug("\n #### The `SnowX` is sending a request to OpenAI for completion")
            ...
            if "choices" in plan and len(plan["choices"]) > 0:
                ...
                logger.debug("\n #### The `SnowX` has successfully received and parsed the pipeline plan")
                return plan_json
        except json.JSONDecodeError:
            logger.debug("\n #### The `SnowX` encountered a JSON decoding error, attempting to repair")
            good_json_string = repair_json(message_content)
            plan_json = json.loads(good_json_string)
            logger.debug("\n #### The `SnowX` has successfully repaired and parsed the JSON")
            return plan_json 
        except Exception as e:
            error_message = plan.get("error", {}).get(
                "message", "Unknown error"
            )
            logger.error(f"  The `SnowX` encountered an error:\n{error_message}")
    ...
    async def get_pipeline_plans(self, files: List[str]) -> Dict:
        ...
        logger.debug("\n #### The `SnowX` is preparing to retrieve pipeline plans for multiple files")
        all_file_contents = self.repo.print_tree()
        logger.debug("\n #### The `SnowX` is calling get_pipeline_plan with the prepared data")
