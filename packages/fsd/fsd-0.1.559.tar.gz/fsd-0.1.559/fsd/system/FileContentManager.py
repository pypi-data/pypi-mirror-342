...
class FileContentManager:
    ...
    async def write_file(file_path: str, content: str):
        ...
        try:
            ...
            if not os.path.exists(directory):
                ...
                logger.debug(f"#### The `SnowX` has created a new directory: `{directory}` for the file: `{file_path}`")

            async with aiofiles.open(file_path, 'w', encoding='utf-8', newline='') as file:
                await file.write(content)

            logger.debug(f"#### The `SnowX` has successfully written to the file: `{file_path}`")
        except Exception as e:
            error_message = f"#### The `SnowX` encountered an error while writing to file `{file_path}`. Error details: `{e}`"
    ...
    async def apply_changes(cls, file_path: str, blocks: List[Tuple[str, str]]) -> str:
            ...
            try:
                if search:
                    ...
                    if new_content is None:
                        logger.info(f"#### The `SnowX` couldn't find a match for search block in file: `{file_path}`")
                        ...
                        if similar_lines and cls.AUTOMATIC_REPLACE_SIMILAR:
                            ...
                            logger.info(f"#### The `SnowX` has replaced similar lines in file: `{file_path}`")
                            successful_blocks += 1
                        elif similar_lines:
        ...
        if content != original_content:
            logger.debug(f"#### The `SnowX` has successfully applied changes to the content of file: `{file_path}`")
        else:
            logger.info(f"#### The `SnowX` did not make any changes to the file: `{file_path}`")
        return content
    ...
    async def process_coding_agent_response(cls, file_path: str, coding_agent_response: str):
        ...
        try:
            ...
            try:
                ...
                error_message = f"#### The `SnowX` found no valid SEARCH/REPLACE blocks in the coding agent response for file: `{file_path}`"
                logger.error(error_message)
        ...
        logger.debug(f"#### The `SnowX` has automatically applied changes to file: `{file_path}`")
    ...
    async def handle_coding_agent_response(cls, file_path: str, coding_agent_response: str):
        ...
        try:
            ...
            logger.debug(f"#### The `SnowX` has successfully processed the coding agent response for file: `{file_path}`")
        except Exception as e:
            error_message = f"#### The `SnowX` encountered an error while processing the coding agent response for file `{file_path}`. Error details: `{e}`"
            logger.error(error_message)
