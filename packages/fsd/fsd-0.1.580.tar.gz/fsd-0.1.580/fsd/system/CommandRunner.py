...
class CommandRunner:
    ...
    def run_command(self, command: str, is_localhost_command: str, method: str = 'bash', inactivity_timeout: int = 7, use_timeout: bool = True) -> Tuple[int, List[str]]:
        ...
        try:
            ...
            logger.info("`CommandRunner` has completed the current step and is proceeding to the next one.")
    ...
    def update_file(self, file_name, content):
        ...
        try:
            with open(file_name, 'a') as file:
                file.write(content + '\n')
            logger.info(f" #### `CommandRunner` has successfully updated the file: {file_name}")
            return f"Successfully updated {file_name}"
        except Exception as e:
            logger.error(f"  `CommandRunner` encountered an error while attempting to update {file_name}: {str(e)}")
    ...
    async def execute_steps(self, steps_json, compile_files, original_prompt_language):
        ...
        for step in steps:
            ...
            if step['method'] == 'bash' and ("sudo" in step['command'] or "runas" in step['command'].lower()):
                logger.info(
                ...
                if step['method'] == 'bash' and ("sudo" in step['command'] or "runas" in step['command'].lower()):
                    ...
                    if user_prompt == 'exit':
                        logger.info(" #### The user has chosen to exit. `CommandRunner` is halting execution.")
                        return "Execution stopped by user"
                    elif user_prompt == 's':
                        logger.info(" #### The user has chosen to skip this step.")
                        continue

                    logger.info(f" #### `CommandRunner` is now executing the following step: {step['prompt']}")
                    ...
                    while retry_count < self.max_retry_attempts:
                        if step['method'] == 'bash':
                            ...
                            if return_code != 0 and return_code != None:
                                ...
                                logger.error(f"  `CommandRunner` reports: Command execution failed with return code {return_code}: {error_message}")
                                ...
                                fixing_steps = await self.get_error_planner_requests(error_message, step['prompt'], self.repo.return_os(), compile_files, original_prompt_language)
                                ...
                                if fixing_result == "Execution stopped by user":
                                    logger.info(" #### The user has chosen to exit during the fixing steps. `CommandRunner` is skipping the current step.")
                                    break
                                elif fixing_result == "All fixing steps completed successfully":
                                    logger.info(" #### `CommandRunner` has fixed the issues. Moving to the next step.")
                            ...
                            if file_name != 'N/A':
                                ...
                                logger.debug("\n #### `CommandRunner` reports: Update method specified but no file name provided.")
                            break
                        else:
                            logger.error(f"  `CommandRunner` encountered an unknown method: {step['method']}")
                            break

                    if retry_count == self.max_retry_attempts:
                        logger.error(f"  `CommandRunner` reports: Step failed after {self.max_retry_attempts} attempts: {step['prompt']}")
        ...
        logger.info(" #### `CommandRunner` has successfully completed all steps")
    ...
    async def execute_fixing_steps(self, steps_json, compile_files, original_prompt_language):
        ...
        for step in steps:
            ...
            logger.info(f" #### `CommandRunner` is now executing the following fixing step: {step['error_resolution']}")
            ...
            if retry_count == self.max_retry_attempts:
                logger.error(f"  `CommandRunner` reports: Step failed after {self.max_retry_attempts} attempts: {step['error_resolution']}")
            ...
            logger.info(" #### `CommandRunner` has completed the current fixing step and is proceeding to the next one.")

        logger.info(" #### `CommandRunner` has successfully completed all fixing steps")
