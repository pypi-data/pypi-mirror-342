...
class CrawlerAgent:
    ...
    def crawl_website(self, website_url, format='markdown'):
        ...
        for _ in range(len(self.api_keys)):
            try:
                ...
                if not crawl_id:
                    logger.debug("\n #### The `SnowX` failed to initiate the crawl: No crawl ID was returned.")
                    raise Exception("Failed to initiate crawl")
                logger.info(" #### The `SnowX` has commenced the reading process!")

                while True:
                    status = self.app.check_crawl_status(crawl_id)
                    logger.info(f" #### The `SnowX` reports current reading status: `{status.get('status')}`")
                ...
                logger.debug(f" #### The `SnowX` encountered an error during the reading process: `{e}`")
    ...
    def process(self, website_url):
        ...
        if results is None:
            logger.debug("\n #### The `SnowX` reports that the crawling process has failed.")
            return None
