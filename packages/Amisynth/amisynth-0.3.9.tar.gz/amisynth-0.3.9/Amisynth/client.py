...
class AmiClient(commands.Bot):
    ...
    async def ejecutar_eventos(self, tipo, 
                               ...
                               ctx_guild_join_env=None,
                               ctx_guild_leave_env=None
                               ):
        ...
        if tipo in self.eventos_personalizados:
            for codigo in self.eventos_personalizados[tipo]:
                kwargs = {
                    ...
                    "ctx_guild_join_env": ctx_guild_join_env,
                    "ctx_guild_leave_env": ctx_guild_leave_env,
    ...
    async def on_guild_join(self, guild):
        utils.ContextAmisynth(guild)
        await self.ejecutar_eventos("$onBotJoinServer", ctx_guild_join_env=guild)


    async def on_guild_remove(self, guild):
        utils.ContextAmisynth(guild)
        await self.ejecutar_eventos("$onBotLeaveServer", ctx_guild_leave_env=guild)
