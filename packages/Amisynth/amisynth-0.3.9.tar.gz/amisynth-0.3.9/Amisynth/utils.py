...
class ContextAmisynth:
    ...
    def set_context(self, ctx):
        ...
        if isinstance(ctx, commands.Context):
            ...
            self.member_count = len(ctx.channel.members)
            ...
            self.guild_id = ctx.id
            self.guild_name = ctx.name
            self.obj_guild = ctx
            ...
            self.user_roles = [role.id for role in ctx.member.roles if role.name != "@everyone"]
    ...
    def get_user_id_by_username(self, username: str):
        ...
        if not self.guild_id:
            return None  # No hay un servidor asociado

        guild = self.ctx.guild  # Obtener la guild desde el contexto
    ...
    def get_username_by_id(self, user_id: int):
        ...
        if guild:
            ...
            if member:
                return member.name  # Retorna el nombre de usuario
    ...
    async def get_channel_cache(self, channel_id:int):
       channel = bot_inst.fetch_channel(channel_id)
       return channel
    ...
    async def modificar_rol(self, user_id: int, role_id: int, action: str):
        ...
        :param action: 'agregar' para añadir el rol, 'remover' para quitarlo.
        """
        
        if not self.ctx:
            return
        
        guild = self.obj_guild
