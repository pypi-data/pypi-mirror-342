...
class TaskHandler(BufferingHandler):
    ...
    def __init__(self, task: Any, capacity: int = None, use_subprocess: bool = False) -> None:
        ...
        self._background_log = BackgroundLogService(
            worker=task.session.worker,
            task=task,
            session=task.session,
            wait_period=float(DevWorker.report_period),
    ...
    def shouldFlush(self, record: LogRecord) -> bool:
        ...
        return self.counter >= self.capacity and self._background_log and self._background_log_size >= self.capacity
    ...
    def close(self, wait: bool = False) -> None:
        ...
        if _background_log:
            if not _background_log.is_subprocess_mode() or _background_log.is_alive():
                ...
                if wait and (
                    not _background_log.is_subprocess_mode() or _background_log.is_subprocess_mode_and_parent_process()
                ):
                    # noinspection PyBroadException
                    try:
                        timeout = 1.0 if _background_log.empty() else self.__wait_for_flush_timeout
    ...
    @classmethod
    def report_offline_session(cls, task: Any, folder: str, iteration_offset: int = 0) -> bool:
    ...
    @staticmethod
    def _log_stderr(msg: str, level: int = INFO) -> None:
