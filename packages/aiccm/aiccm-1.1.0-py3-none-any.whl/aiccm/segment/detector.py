...
class Segmenter:
    def __init__(self):
        self.segmenter = None
        self.model_path = os.path.join(here, './nerve.onnx')
        try:
            self.set_segmenter(self.model_path)
        except:
            pass

    def set_segmenter(self, model_path):
        self.segmenter = NerveSegmenter(model_path)
        self.model_path = model_path

    def __call__(self, *args, **kwargs):
        return self.segmenter(*args, **kwargs)


segmenter = Segmenter()


def set_segmenter(model_path):
    global segmenter
    segmenter.set_segmenter(model_path)
