...
class NovelAIGenerator:
    def __init__(self, persistent_token: str):
        ...
        self.endpoint = "https://image.novelai.net/ai/generate-image"
...
class NovelAIInference(BaseImageInference):
    ...
    def __init__(
        ...
        batch_size: int = 1,  # NovelAI API generates one image at a time
        ...
        if not persistent_token:
            raise ValueError(
                "NovelAI persistent token must be provided either in initialization or as NOVELAI_TOKEN environment variable",
            )
