...
BORON_DIR = os.path.dirname(__file__)
C_OUT = os.path.join(BORON_DIR, "c_lib.dll")
INIT_FILE = os.path.join(BORON_DIR, "__init__.py")
...
def link_python(py_sources):
    ...
    for path, func in py_sources:
        ...
        # Optional: try import just to be sure
        try:
            __import__(module_path)
            lines.append(f"from {module_path} import {func}")
        except Exception as e:
            print(f"⚠️ Skipping {func} from {module_path}: Import failed -> {e}")

    return lines

def generate_init(c_sources, py_lines):
    print("🧩 Generating boron/__init__.py...")
    lines = []

    if c_sources:
        lines += [
            "import ctypes",
            f'c_lib = ctypes.CDLL(r"{os.path.abspath(C_OUT)}")',
...
def main():
    ...
    safe_remove(os.path.join(os.path.dirname(__file__), "c_lib.dll"))
    safe_remove(os.path.join(os.path.dirname(__file__), "__init__.py"))
    ...
    generate_init(sections["C"], py_lines)
...
# Use current working directory instead of script's dir
WORKDIR = os.getcwd()
...
def link_python(py_sources):
    ...
    for path, func in py_sources:
        ...
        try:
            ...
            lines.append(f"from {path.replace('/', '.').replace('\\\\', '.').removesuffix('.py')} import {func}")
        except Exception as e:
