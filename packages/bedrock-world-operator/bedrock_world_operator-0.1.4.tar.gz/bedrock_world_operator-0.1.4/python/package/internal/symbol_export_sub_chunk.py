...
def sub_chunk_blocks(id: int, layer: int) -> list[int]:
    raw = as_python_bytes(LIB.SubChunk_Blocks(CInt(id), CInt(layer)))
    result = []
...
def sub_chunk_set_blocks(id: int, layer: int, blocks: list[int]) -> str:
    writer = BytesIO()
    for i in blocks:
        writer.write(struct.pack("<I", i))
    return as_python_string(
        LIB.SubChunk_SetBlocks(CInt(id), CInt(layer), as_c_bytes(writer.getvalue()))
    )
