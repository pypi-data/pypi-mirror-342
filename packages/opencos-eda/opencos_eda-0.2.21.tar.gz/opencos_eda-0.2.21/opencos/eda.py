...
# glbl_config is still used by interactive() mode, it is set in process_tokens() (type dict)
glbl_config = None
...
class CommandDesign(Command):
    ...
    def resolve_target(self, target, no_recursion=False, caller_info=''):
        ...
        if os.path.exists(target):
            ...
            if file_ext != '':
                self.add_file(target, caller_info=caller_info)
                return True
    ...
    def resolve_target_core(self, target, no_recursion, caller_info=''):
        ...
        if self.config['deps_markup_supported']:
            deps = deps_helpers.DepsFile(command_design_ref=self, target_path=target_path, cache=self.cached_deps)
        ...
        if found_deps_file and found_target:
            ...
            for x in deps_targets_to_resolve:
                ...
                if x and type(x) is tuple:
                    ...
                    if os.path.exists(x):
                        self.add_file(filename=x, caller_info=caller_info)
                    else:
        ...
        if self.config['deps_legacy_supported'] and not found_deps_file and os.path.isfile(deps_file):
            ...
            for line in f:
                ...
                self.append_work_dir_add_srcs( add_srcs=work_dir_add_srcs_list,
                                               caller_info=str(deps_file + '::' + target + '::line=' + line_number) )
                ...
                for dep in line.split():
                    ...
                    m = re.match(r'^\+\w+', dep) # +define+ +incdir+ +<name> +<name>=value
                    if m:
                    ...
                    if os.path.exists(dep_path):
                        ...
                        self.add_file(dep_path, caller_info=str(deps_file + '::' + target + '::line=' + line_number))
                    else:
    ...
    def add_file(self, filename, use_abspath=True, add_to_non_sources=False, caller_info:str=''):
        ...
        v_file_ext_list = self.config.get('file_extensions', {}).get('verilog', [])
        sv_file_ext_list = self.config.get('file_extensions', {}).get('systemverilog', [])
        vhdl_file_ext_list = self.config.get('file_extensions', {}).get('vhdl', [])
        cpp_file_ext_list = self.config.get('file_extensions', {}).get('cpp', [])
    ...
    def process_tokens(self, tokens, process_all=True, pwd=None):
        ...
        for token in unparsed:
            if os.path.exists(token):
                file_abspath = os.path.abspath(token)
                ...
                if file_ext == '':
                    ...
                    self.add_file(filename=token, caller_info=caller_info)

                    if self.args['top'] == "" and \
                       file_ext in self.config.get('file_extensions', {}).get('inferred_top', []):
...
class CommandSim(CommandDesign):
    ...
    def __init__(self, config:dict):
        ...
        self.args_help.update({
            ...
            'sim-plusargs':  '"simulation" step run-time args passed to tool',
...
class CommandMulti(CommandParallel):
    ...
    def resolve_target(self, base_path, target, command, level=0):
        ...
        if len(target_path_parts) == 1:
            ...
            deps_file = util.get_deps_markup_file(base_path)
            if self.config['deps_markup_supported'] and deps_file:
                data = util.deps_markup_safe_load(deps_file)
                ...
                for e in os.listdir(base_path):
                    ...
                    if os.path.isdir(e):
                        ...
                        if (self.config['deps_legacy_supported'] and \
                            ...
                            (self.config['deps_markup_supported'] and deps_yaml_file):
...
class ToolVerilator(Tool):
    ...
    def get_versions(self) -> str:
        ...
        if path is None:
            self.error(f'"{self._EXE}" not in path, need to get it (sudo apt-get install {self._EXE})')
        coverage_path = shutil.which('verilator_coverage')
        if coverage_path is None:
            self.error(f'"verilator_coverage" not in path, need from same path as "{self._EXE}"')

        version_ret = subprocess.run([self._EXE, '--version'], capture_output=True)
        ...
        if len(words) < 1:
            self.error(f'{self._EXE} --version: returned unexpected string {version_ret=}')
        version = words[1]
        ver_list = version.split('.')
        if len(ver_list) != 2:
            self.error(f'{self._EXE} --version: returned unexpected string {version_ret=} {version=}')
...
class CommandSimVerilator(CommandSim, ToolVerilator):
    ...
    def get_post_simulate_command_lists(self):
        ...
        verilated_post_exec_coverage_command_list = ['verilator_coverage']
...
class ToolSlangYosys(ToolYosys):
    ...
    _TOOL = 'slang_yosys'
    _URL = 'https://github.com/povik/yosys-slang'
...
class ToolVivado(Tool):
    ...
    def __init__(self):
        ...
        super().__init__() # calls self.get_versions()
    ...
    def get_versions(self) -> str:
        ...
        if not xilinx_vivado or \
           os.path.abspath(os.path.join(xilinx_vivado, 'bin', 'vivado')) != os.path.abspath(vivado_path):
        ...
        if version:
            ...
            self.error(f"Vivado version not found, {vivado_path=}")
        return self._VERSION
    ...
    def get_compile_command_lists(self) -> list():
        ...
        if len(self.files_v) or self.args['xilinx']:
            ...
            if self.args['xilinx']:
                ...
                glbl_v = shutil.which('vivado').replace('bin/vivado', 'data/verilog/src/glbl.v')
...
class ToolQuesta(Tool):
    ...
    def get_versions(self) -> str:
        ...
        if not path:
            ...
            if self._EXE == 'qrun' and 'modelsim_ase' in path:
...
def init_config(config=None, quiet=False, tool=None) -> dict:
    '''Sets or clears entries in config (dict) so tools can be re-loaded.'''
    global glbl_config

    if not config:
        if not glbl_config:
            # Get a default config and print a warning, someone called
            # init config w/out main() being called.
            util.warning('Using default config on call to init_config(None, ..)')
            config, _args = eda_config.get_eda_config([])
        else:
            config = glbl_config
...
"""
Usage:
    eda [<options>] <command> <options> <targets>
...
def interactive():
    ...
    while True:
        ...
        if m: line = m.group(1)
        tokens = line.split()
        process_tokens(tokens, interactive=True)
...
def get_argparser() -> argparse.ArgumentParser:
    ...
    parser.add_argument('--tool', type=str, default=None, help='Tool to use for this command, such as: verilator')
    return parser
...
def process_tokens(tokens:list, config=None, interactive=False):
...
def main(*args):
    ...
    if len(args) == 0 or (len(args) == 1 and '--debug' in args):
        ...
        return process_tokens(list(unparsed), config=config)
