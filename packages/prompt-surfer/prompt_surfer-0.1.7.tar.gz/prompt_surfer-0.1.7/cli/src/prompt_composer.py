...
class PromptComposer:
    def __init__(self, api_key=None):
        ...
        # Initialize console early for prompting
        ...
        # If no API key, prompt the user to enter one
        if not self.api_key:
        ...
        self.models = {
            ...
            'gpt-4.1-mini-2025-04-14': {
                'name': 'GPT-4.1 mini (Apr 2025)',
                'description': 'Balanced performance and cost-effectiveness'
            },
            'gpt-4.1-nano-2025-04-14': {
                'name': 'GPT-4.1 nano (Apr 2025)',
                'description': 'Fast and affordable with good capabilities'
            }
        }
    ...
    def prompt_for_api_key(self):
        """Prompt the user to enter an OpenAI API key and save it."""
        # Display a welcome message
        self.console.print("\n" + "═" * 70, style="retro_border")
        self.console.print("[bold cyan]Welcome to Prompt Surfer![/bold cyan]")
        self.console.print("[yellow]No OpenAI API key found. You'll need to provide one to continue.[/yellow]")
        self.console.print("═" * 70 + "\n", style="retro_border")

        # Import getpass here to avoid circular imports
        from getpass import getpass

        # Keep prompting until a valid key is provided
        while True:
            self.console.print("[bold cyan]Enter your OpenAI API key:[/bold cyan]")
            self.console.print("[dim](Input will be hidden for security)[/dim]")
            new_api_key = getpass("API Key: ")

            if not new_api_key:
                self.console.print("[error]API key cannot be empty. Please try again.[/error]")
                continue

            # Basic validation - OpenAI keys typically start with 'sk-'
            if not new_api_key.startswith("sk-"):
                self.console.print("[warning]Warning: This doesn't look like a valid OpenAI API key.[/warning]")
            ...
            save_to_home = questionary.confirm(
            ...
            if save_to_home:
                self.save_api_key_to_home(new_api_key)

            self.console.print("[success]API key set successfully![/success]")
            break

    def save_api_key_to_home(self, api_key):
        """Save the API key to a .env file in the user's home directory."""
        try:
            # Create .prompt-surfer directory in user's home if it doesn't exist
            home_dir = os.path.expanduser('~')
            config_dir = os.path.join(home_dir, '.prompt-surfer')
            ...
            self.console.print(f"[success]API key saved to {env_path}[/success]")

        except Exception as e:
            self.console.print(f"[error]Failed to save API key: {str(e)}[/error]")
    ...
    def clear_screen(self):
        ...
        # Flush stdout to ensure the screen is cleared immediately
        sys.stdout.flush()

    def draw_header(self):
        """Draw the ASCII art header."""
        # Use a single string for the ASCII art to prevent any partial rendering
        ascii_art = """
  ██████╗ ██████╗  ██████╗ ███╗   ███╗██████╗ ████████╗
  ██╔══██╗██╔══██╗██╔═══██╗████╗ ████║██╔══██╗╚══██╔══╝
  ██████╔╝██████╔╝██║   ██║██╔████╔██║██████╔╝   ██║
  ...
  ╚══════╝ ╚═════╝ ╚═╝  ╚═╝╚═╝     ╚══════╝╚═╝  ╚═╝
   🤖 🏄
            """
        self.console.print(ascii_art, style="retro_text")
        self.console.print("═════════════════════════════════════════════════════════", style="retro_border")
    ...
    def run(self):
        ...
        while True:
            ...
            if selected_action == ACTION_QUIT:
                ...
                if selected_type:
                    # Store the clear_screen method in the history handler
                    self.history_handler.clear_screen_func = self.clear_screen
                ...
                try:
                    ...
                    # Save to user's home directory if user wants
                    save_to_home = questionary.confirm(
                        "Save this API key for future use?",
                        style=self.custom_style,
                        default=True
