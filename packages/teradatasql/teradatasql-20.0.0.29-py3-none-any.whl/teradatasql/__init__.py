...
def _printResultSet (wCurrent, nFormat, sTitle, cur):
    ...
    for iRow, row in enumerate (cur.fetchall ()):
        for iColumn, oValue in enumerate (row):
            ...
            sValue = str (oValue) if oValue else None
...
def main (asArgs):
    ...
    if not asArgs:
        ...
        print ("-i turns on interactive mode to prompt for commands from System.in")
                        ...
                        if sCommand == "":
                            ...
                            with con.cursor () as cur:
                                ...
                                for iResult in itertools.count ():
                                    _printResultSet (wCurrent, nFormat, "Result {}".format (iResult + 1), cur)
                                    if not cur.nextset ():
                                        break
