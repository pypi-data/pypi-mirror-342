...
class Milvus:
    ...
    def __init__(self, milvus_config: dict, model_config: dict):
        ...
        # 初始化milvus客户端
        collection_name = milvus_config["collection"]["aisearch_answer"]
        ...
        search_res = self.search(
            collection_name=collection_name, query="test", output_fields=["question", "answer"]
        )
        global_instance_logger.log_info(
            msg="milvus end initialization", biz_data={"search_res": search_res}
        )
    ...
    def query(
        ...
        global_instance_logger.log_info(msg="milvus end query", biz_data={"query_res": query_res})
        return query_res
    ...
    def insert(
        ...
        global_instance_logger.log_info(msg="milvus end insert", biz_data={"res": res})
        return res
    ...
    def upsert(
        ...
        global_instance_logger.log_info(msg="milvus end upsert", biz_data={"res": res})
        return res
...
global_instance_milvus = Milvus(
    milvus_config=global_config["milvus"], model_config=global_config["model"]["provider"]["azure_embedding"]
)
