...
from contentor_video.storage import ResumableStorage


class ResumableBaseWidget(FileInput):
    template_name = "contentor_video/admin_file_input.html"
...
class ResumableAdminWidget(ResumableBaseWidget):
    ...
    def media(self):
        js = ["resumable.js"]
        return forms.Media(js=[static("contentor_video/js/%s" % path) for path in js])


class ResumableWidget(ResumableBaseWidget):
    template_name = "contentor_video/user_file_input.html"
