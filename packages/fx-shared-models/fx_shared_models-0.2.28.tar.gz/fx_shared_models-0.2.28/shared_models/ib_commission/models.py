...
class ClientIBMapping(BaseModel):
    ...
    agreement = models.ForeignKey(IBAgreement, on_delete=models.PROTECT, null=True, blank=True,
...
class IBAgreementMember(BaseModel):
    ...
    """
    agreement = models.ForeignKey(IBAgreement, on_delete=models.PROTECT, related_name='members')
    customer = models.ForeignKey('customers.Customer', on_delete=models.PROTECT, 
...
class IBCommissionRule(BaseModel):
    ...
    agreement = models.ForeignKey(IBAgreement, on_delete=models.PROTECT, related_name='commission_rules')
...
class CommissionTracking(BaseModel):
    ...
    deal_ticket = models.BigIntegerField(primary_key=True)
    customer = models.ForeignKey('customers.Customer', on_delete=models.PROTECT, 
                               related_name='commissions_generated')
    direct_ib_customer = models.ForeignKey('customers.Customer', on_delete=models.PROTECT, 
                                        related_name='direct_commissions')
    
    # Add account reference while keeping mt5_login for backward compatibility
    client_account = models.ForeignKey('accounts.Account', on_delete=models.PROTECT,
                                   related_name='commission_trackings', null=True, blank=True,
                                   help_text='Client account that generated the deal')
    mt5_login = models.IntegerField(help_text='MT5 login that generated the deal')
    
    # Trading details
    symbol = models.CharField(max_length=50, null=True, blank=True, help_text='Trading instrument symbol')
    volume = models.DecimalField(max_digits=20, decimal_places=5, null=True, blank=True, 
                              help_text='Trade volume in lots')
    volume_closed = models.DecimalField(max_digits=20, decimal_places=5, null=True, blank=True, 
                                     help_text='Volume closed for partial closes')
    profit = models.DecimalField(max_digits=20, decimal_places=5, null=True, blank=True, 
                               help_text='Profit/loss from the trade')
    action = models.CharField(max_length=10, null=True, blank=True, 
                            help_text='Trade action (BUY/SELL)')
    entry = models.CharField(max_length=20, null=True, blank=True, 
                               help_text='Entry type (IN/OUT/INOUT/OUT_BY)')
    ...
    exit_price = models.DecimalField(max_digits=20, decimal_places=5, null=True, blank=True, 
                                  help_text='Exit price of the trade')
    position_id = models.BigIntegerField(null=True, blank=True, help_text='Position ID')
    
    commission_type = models.CharField(max_length=10, choices=COMMISSION_TYPES)
    rule = models.ForeignKey(IBCommissionRule, on_delete=models.PROTECT, related_name='commissions')
    amount = models.DecimalField(max_digits=20, decimal_places=5)
    processed_time = models.DateTimeField(null=True, blank=True)
    trade_open_time = models.DateTimeField(null=True, blank=True)
    trade_close_time = models.DateTimeField(null=True, blank=True)

    def save(self, *args, **kwargs):
        """
        Override save to keep client_account and mt5_login in sync.
        If client_account is provided, update mt5_login to match.
        """
        if self.client_account and self.client_account.login != self.mt5_login:
            self.mt5_login = self.client_account.login
        super().save(*args, **kwargs)
...
class CommissionDistribution(BaseModel):
    ...
    deal_ticket = models.ForeignKey(CommissionTracking, on_delete=models.PROTECT, 
                                  related_name='distributions')
    customer = models.ForeignKey('customers.Customer', on_delete=models.PROTECT, 
                               related_name='commission_distributions')
    
    # Add account reference while keeping mt5_login for backward compatibility
    ib_account = models.ForeignKey('accounts.Account', on_delete=models.PROTECT,
                               related_name='received_distributions', null=True, blank=True,
                               help_text="IB's account receiving commissions")
    ...
    is_pass_up = models.BooleanField(default=False, 
                                   help_text='Whether this distribution is passed up to parent IB')
    processed_time = models.DateTimeField()
    is_processed = models.BooleanField(default=False, 
                                     help_text='Whether this distribution has been processed by the financial system')
    processing_status = models.CharField(max_length=20, choices=PROCESSING_STATUS_CHOICES, default='PENDING',
                                      help_text='Current status of the distribution processing')
    processing_notes = models.TextField(blank=True, null=True,
                                     help_text='Additional notes about processing status')
    transaction = models.ForeignKey('transactions.CommissionRebateTransaction', on_delete=models.SET_NULL,
                                  related_name='distributions', null=True, blank=True,
                                  help_text='Associated financial transaction record')
    rule = models.ForeignKey(IBCommissionRule, on_delete=models.PROTECT, 
                           related_name='distributions', null=True, blank=True,
                           help_text='The commission rule that generated this distribution')
    ...
    class Meta:
        ...
        constraints = [
            models.UniqueConstraint(
                fields=['deal_ticket', 'customer', 'rule', 'is_pass_up'], 
