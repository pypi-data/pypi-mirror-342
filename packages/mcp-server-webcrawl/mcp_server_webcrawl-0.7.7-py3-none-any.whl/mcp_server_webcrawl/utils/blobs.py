...
class ThumbnailManager:
    ...
    def __is_valid_url(self, path: str) -> Tuple[bool, Optional[ParseResult]]:
        try:
    ...
    def __get_extension(self, path: str) -> Optional[str]:
    ...
    def __process_image_data(self, data: bytes, temp_file: Path) -> Optional[str]:
    ...
    async def __get_file(self, path: str, key: str) -> Optional[str]:
        try:
            file_path = Path(path)
    ...
    async def __process_path(
        ...
        result: Optional[str] = await self.__fetch_url(session, path, key) if is_valid_url else await self.__get_file(path, key)
        results[path] = result
        if result is None:
    ...
    async def __get_blobs_async(self, paths: list[str]) -> dict[str, Optional[str]]:
    ...
    def __create_webp_thumbnail(self, image_data: bytes, size: int = 512) -> Optional[bytes]:
        img = None
        try:
    ...
    def get_thumbnails(self, paths: list[str]) -> dict[str, Optional[str]]:
        """
