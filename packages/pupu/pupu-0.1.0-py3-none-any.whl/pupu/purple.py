...
from PySide6.QtWidgets import (QApplication, QMainWindow, QWidget, QVBoxLayout, 
                              ...
                              QFormLayout, QCheckBox, QFileDialog, QGridLayout, QFrame, 
                              QScrollArea, QSplitter, QMenuBar, QMenu)
...
class AddUserDialog(QDialog):
    def __init__(self, existing_username: str = None):
        ...
        card_layout.addLayout(form_layout)
        card_layout.addWidget(save_button, alignment=Qt.AlignCenter)
...
class UserManager(QWidget):
    ...
    def refresh_users(self):
        ...
        for row_idx, user in enumerate(users):
            self.user_table.setItem(row_idx, 0, QTableWidgetItem(user[1]))
            self.user_table.setItem(row_idx, 1, QTableWidgetItem(user[2]))
            self.user_table.setItem(row_idx, 2, QTableWidgetItem("Да" if user[3] else "Нет"))
        self.user_table.resizeColumnsToContents()

    def on_cell_clicked(self, row, column):
        self.selected_user_id = self.db.get_users()[row][0]
...
class TableEditor(QWidget):
    def __init__(self, db: Database, table: str, is_admin: bool):
        ...
        # Нижняя панель: поля ввода и кнопки
        bottom_panel = QFrame()
        bottom_panel.setStyleSheet("QFrame { border: 1px solid #AB47BC; border-radius: 5px; padding: 10px; }")
        ...
        main_layout.addWidget(table_container, stretch=7)
...
class MainWindow(QMainWindow):
    ...
    def __init__(self, db: Database, is_admin: bool):
        ...
        import_action = file_menu.addAction("Импортировать из Excel")
        import_action.triggered.connect(self.import_from_excel)

        logout_action = file_menu.addAction("Выйти")
        logout_action.triggered.connect(self.logout)

        if self.is_admin:
            admin_menu = menubar.addMenu("Администрирование")
            manage_users_action = admin_menu.addAction("Управление пользователями")
            manage_users_action.triggered.connect(self.manage_users)

        # Панель с выбором таблицы
        toolbar = QWidget()
        toolbar_layout = QHBoxLayout()
        toolbar_layout.setSpacing(15)
        ...
        toolbar_layout.addWidget(self.table_selector)
        toolbar_layout.addStretch()
