# Authors: Your Name <ofatteh25@ubishops.ca>
...
def _joint_probabilities(distances, perplexity, verbose=False):
    ...
    # Search for appropriate sigmas for each point
    for i in range(n_samples):
        if verbose and i % 500 == 0:
            print(f"Computing P-values for point {i}/{n_samples}")
            
        # Calculate entropy and find optimal sigma
        betamin = -np.inf
        betamax = np.inf
        beta = 1.0
        
        # Get distances from the current point to all others
        dist_i = distances[i].copy()
        
        # For numerical stability, zero out self-distances
        dist_i[i] = 0.0
        
        # Calculate entropy with current beta
        sum_Pi = 0.0
        for _ in range(50):  # Max 50 iterations for binary search
            # Calculate probabilities with current beta
            Pi = np.exp(-dist_i * beta)
            Pi[i] = 0.0  # Set self-probability to 0
            sum_Pi = np.sum(Pi)
            
            if sum_Pi == 0.0:
                prob_i = np.zeros_like(Pi)
            else:
                prob_i = Pi / sum_Pi
            ...
            # Calculate difference from target entropy (perplexity = 2^H)
        ...
        P[i] = Pi / sum_Pi if sum_Pi > 0 else np.zeros_like(Pi)
    ...
    # Ensure diagonal is still zero after symmetrization
    np.fill_diagonal(P, 0.0)
    
    # For numerical stability
    P = np.maximum(P, MACHINE_EPSILON)
    
    # Ensure diagonal is zero again
    np.fill_diagonal(P, 0.0)
