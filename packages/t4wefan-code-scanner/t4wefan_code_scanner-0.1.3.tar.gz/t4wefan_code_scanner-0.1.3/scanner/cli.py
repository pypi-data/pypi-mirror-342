...
def parse_args(args: List[str]) -> CLIArgs:
    ...
    scan_parser.add_argument(
        "dir", nargs="?", default=".", help="Directory to scan (default: .)"
    )
    scan_parser.add_argument(
    ...
    config_add_parser = config_subparsers.add_parser(
        "add", help="Add configuration options"
    )
    config_add_parser.add_argument(
        "dir",
        nargs="?",
        default=".",
        help="Directory containing the config file (default: .)",
    )
    config_add_parser.add_argument(
        "--no-file-tree", action="store_true", help="Disable file tree output"
    )
    config_add_parser.add_argument(
        "--max-size", "-m", type=float, help="Maximum file size (MB)"
    )
    ...
    config_init_parser = config_subparsers.add_parser(
        "init", help="Initialize config file"
    )
    ...
    if parsed_args.command == "scan":
        ...
        if parsed_args.subcommand == "add":
            ...
            cli_args.config_add.no_file_tree = (
                parsed_args.no_file_tree
                if hasattr(parsed_args, "no_file_tree")
                else None
            )
            cli_args.config_add.max_size = (
                parsed_args.max_size if hasattr(parsed_args, "max_size") else None
            )
            cli_args.config_add.output = (
                parsed_args.output if hasattr(parsed_args, "output") else None
            )
            cli_args.config_add.ignore = (
                parsed_args.ignore if hasattr(parsed_args, "ignore") else None
            )
