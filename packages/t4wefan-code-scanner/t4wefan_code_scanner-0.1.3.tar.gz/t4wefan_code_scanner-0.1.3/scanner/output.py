...
def generate_file_tree(dir: str, scan_result: ScanResult, config: ConfigData) -> str:
    ...
    root_dirs = [
        d for d in scan_result.dir_paths if os.path.dirname(d) == dir and d != dir
    ]
...
def is_empty_directory(
    dir_path: str, scan_result: ScanResult, config: ConfigData
) -> bool:
...
def write_markdown_to_file(dir: str, scan_result: ScanResult, config: ConfigData):
    ...
    output_file = config.output.replace(
        "{当前文件夹名}", os.path.basename(os.path.abspath(dir))
    )
