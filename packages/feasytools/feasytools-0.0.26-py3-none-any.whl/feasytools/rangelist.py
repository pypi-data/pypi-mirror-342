...
class RangeList:
    ...
    def __init__(
        self,
        data: "Union[list[tuple[int,int]], ET.Element]",
        loop_times: int = 1,
        ...
        if loop_times > 1 and len(data) >= 1:
            ...
            ), f"Loop period {loop_period} must be greater than end time {data[i][1]}."
            n = len(data)
...
class RangeListAlwaysTrue(RangeList):
    def __init__(self):
        super().__init__([])
...
def CreateRangeList(
    data: "Union[list[tuple[int,int]], ET.Element, None, RangeList]",
) -> RangeList:
    if isinstance(data, RangeList):
        return data
    if data is None:
        return RangeListAlwaysTrue()
    return RangeList(data)
