...
#     for action in action_list:
        ...
        if action["init_side"]:
            ...
            if player_list[side]["faction"] not in unit_list[(x,y)].faction.split("/"):
                flags["correct_recruit_faction"] = False
                ...
                if unit_list[attacker_coord].name==attacker and unit_list[defender_coord].name==defender:
                    ...
                    stats = {
                        attacker_coord:{
                            ...
                            ,"opp_attack":None if not retaliation else defender_attack
                            ,"distance":distance
                    ...
                    for result in results:
                        ...
                        if hit_known and re.search(r"dies=(yes|no)",content):    
                            ...
                            stats[current_coord]["hits_estimated"] += chance
                            ...
                            if hits:
                                ...
                                stats[current_coord]["hits_missed"] += 1
                                stats[stats[current_coord]["opp_coord"]]["hits_dodged"] += 1
                    ...
                    combat_string = " | ".join([f'{s["current_unit"]} ({s["position"]}) did {s["dmg"]} dmg in {s["hits"]} hit{"s" if s["hits"]>1 else ""}' for s in stats.values()])    
                    ...
                    for k,v in stats.items():
                        ...
                        combat_data[(cid,v["uid"])] = {
                            "turn": turn
                            ,"tod": tod
                            ,"coord": ",".join([str(k1 for k1 in k)])
                            ,"leader": v["leader"]
                            ,"unit_lvl": v["unit_lvl"]
                            ,"unit_cost": v["unit_cost"]
                            ,"current_unit": v["current_unit"]
                            ,"position": v["position"]
                            ,"attack": v["attack"].name if v["attack"] else None
                            ,"attack_type": v["attack"].attack_type if v["attack"] else None
                            ,"distance": v["distance"]
                            ,"oid": v["oid"]
                            ,"opponent": v["opponent"]
                            ,"opp_attack": v["opp_attack"].name if v["opp_attack"] else None
                            ,"opp_attack_type": v["opp_attack"].attack_type if v["opp_attack"] else None
                            ,"dmg": v["dmg"]
                            ,"hits": v["hits"]
                            ,"hits_estimated": v["hits_estimated"]
                            ,"hits_missed": v["hits_missed"]
                            ,"dmg_taken": v["dmg_taken"]
                            ,"hits_taken": v["hits_taken"]
                            ,"hits_dodged": v["hits_dodged"]
                            ,"kills": v["victory"]
                            ,"leveled": None
                            ,"resurrected": None
                        }
