...
from Component.ReplicaDb import ReplicaCluster,ReplicaCollection
...
def ProcesarDatos(Coleccion,args:dict,idcolecion=None,session=None,ColecionString=True,ReturnId=False):
    if ColecionString:
...
def ProcesarDatosPRG(Coleccion:Collection,args:dict,idcolecion=None,session=None):
    collection=Coleccion
    result=None
    Execcion=False
    idactual=0
...
def GetTestConexion(connection = False):
    msj="Exito"
    conectado=True
    try:
        HOST,PASSWORD,USER,DATABASE,PORT = TypeConnection()
        con = conn.connect(
            host=HOST,
            user= USER,
            password=PASSWORD,
...
def CallProced(NameProc,prm=None,dicionario=False):
    con,cur=Get()
    resp=[(0)] if dicionario==False else [{'resp':0}]
    if prm is None:
        cur.callproc(NameProc)
    else:
        cur.callproc(NameProc,prm)
...
def TypeConnection():
    try:
        # archivo=open(".servex","r")
        # cadenaconexion=archivo.read()
        ...
        #conncadena=json.loads(cadenaconexion["Cadena"])
        #for i in conncadena:
