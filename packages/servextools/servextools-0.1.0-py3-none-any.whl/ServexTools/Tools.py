...
def RegistrarAreas(nombre, archivo, visiblepara = 'T',conexion:MongoClient=None):
    try:
...
def TiempoEspera(tiempo, eventlet=False):
    """## Usamos este for para hacer un tiempo de espera manual y poder manipularlo"""
...
def FechaHora(AddDias=0.0,AddHoras=0.0,AddMinutos=0.0,AddSegundo=0.0):
    ...
    fecha=StrToDateTime(Fecha(AddDias,AddHoras,AddMinutos,AddSegundo)+" "+Hora(AddMinutos,AddSegundo))
    return fecha
...
def BorrarArchivo(nombre,tiempoEspera=60.0):
    try:
...
def BorrarArchivoHoraCreado(nombre,NumeroHoras=1):
