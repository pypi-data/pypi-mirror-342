    ...
    if result == 0:  # Port is in use
        ...
        try:
            ...
            try:
                ...
                try:
                    logger.info(f"Attempting to kill process using port {port} with fuser")
                ...
                try:
                    logger.info(f"Attempting to find and kill process using port {port} with lsof")
                ...
                try:
                    logger.info(f"Attempting to find and kill process using port {port} with netstat")
                    process = subprocess.run(
    ...
    if os.environ.get("WERKZEUG_RUN_MAIN") != "true":
        if is_port_in_use(8050):
            ...
            if not force_release_port(8050):
                logger.error("Failed to release port 8050. The application may fail to start.")
