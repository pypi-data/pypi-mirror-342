...
class import_agent(agent_graph_walker, Walker):
        ...
        if safe_to_import and (package_path := Utils.find_package_folder(package_root, daf_name)):
            ...
            if agent_node:
                if agent_node.healthcheck_status != 200:
                    self.logger.error(f'{agent_node.name} failed healthcheck. Any memory or knowledge packaged for import will be deferred.')
                    return agent_node
