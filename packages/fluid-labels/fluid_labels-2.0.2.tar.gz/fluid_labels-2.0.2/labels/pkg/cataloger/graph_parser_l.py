...
from labels.utils.env import TREE_SITTER_PARSERS
...
def lang_from_so(language: str) -> Language:
    so_library_path = os.path.join(
        TREE_SITTER_PARSERS or "",
        f"{language}.so",
    )
...
def parse_content(content: bytes, language: str) -> Tree:
