...
class ReadListingInfo:
    ...
    def __init__(self,listing_folder_path, structure):
    ...
    def load_shop_task(self):
        ...
        self.shop_id = self.listing_folder_name.split("_")[0]
        self.shop_name = self.listing_folder_name.split("_")[1]
...
def get_folder_listing_task(plat_name,yaml_path,wait_time=10):
    ...
    assert plat_name in FOLDER_RULE['structure'].keys(), f"非法平台：{plat_name},可用平台[{','.join(FOLDER_RULE['structure'].keys())}]"
        ...
        for shop in shops_dir:
            ...
            for listing in listing_dir:
                if "日志.txt" not in [x.name.__str__() for x in listing.iterdir() if x.is_file()]:
                    ...
                    # 获取上架文件夹中得所有上架文件
                    rli = ReadListingInfo(listing.__str__(),FOLDER_RULE['structure'][plat_name])
                    yield rli
        time.sleep(wait_time)
