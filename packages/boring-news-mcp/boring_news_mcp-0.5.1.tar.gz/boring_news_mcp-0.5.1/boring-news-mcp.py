...
Created at: {article.get('created_at', 'Unknown')}
Origin: {article.get('origin', 'Unknown')}
Category: {article.get('category', 'Unknown')}
Section: {article.get('section', 'Unknown')}
URL: {article.get('original_url', 'Unknown')}
Sentiment: {article.get('sentiment', 'Unknown')} (Polarity: {article.get('polarity', 'Unknown')})
Tags: {', '.join(article.get('tags', []))}
People: {', '.join(article.get('people', []))}
Main Ideas: {', '.join(article.get('main_ideas', []))}
"""
...
async def get_articles_by_person(person: str) -> str:
    ...
    """
    print(f"Getting articles mentioning: {person}")
...
async def get_article_groups(date: Optional[str] = None) -> str:
    ...
    Args:
        date: Target date in YYYY-MM-DD format (optional, defaults to today)
    """
    print(f"Getting article groups for date: {date}")
    params = {}
    if date:
        params['date'] = date
...
async def get_categories(date: Optional[str] = None) -> str:
    ...
    Args:
        date: Target date in YYYY-MM-DD format (optional, defaults to today)
    """
    print(f"Getting categories for date: {date}")
    params = {}
    if date:
        params['date'] = date
...
def daily_news_summary(target_date: str) -> str:
    """Génère un résumé journalistique des actualités du jour, regroupées par thèmes et catégories.
    Args:
...
Utilise les fonctions suivantes pour obtenir les informations nécessaires:
- get_categories() pour avoir une vue d'ensemble des catégories du jour
- get_article_groups() pour voir les groupes d'articles liés
- get_articles_by_date() avec les filtres appropriés pour obtenir les détails des articles
"""

@mcp.prompt()
def daily_news_highlights(target_date: str) -> str:
    """Génère un résumé journalistique des actualités majeures du jour.
    Args:
        target_date: Date cible au format YYYY-MM-DD
    """
    if target_date is None:
...
def daily_cultural_news(target_date: str) -> str:
    """Génère un résumé journalistique focalisé sur l'actualité culturelle du jour.
    Args:
        target_date: Date cible au format YYYY-MM-DD
    """
    if target_date is None:
...
Utilise les fonctions suivantes pour obtenir les informations nécessaires:
- get_categories() pour identifier les catégories culturelles du jour
- get_article_groups() pour voir les groupes d'articles liés à la culture
- get_articles_by_date() avec les filtres de catégories culturelles pour obtenir les détails des articles
"""
