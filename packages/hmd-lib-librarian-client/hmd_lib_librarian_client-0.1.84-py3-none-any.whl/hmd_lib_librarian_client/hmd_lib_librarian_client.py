...
class HmdLibrarianClient:
    def __init__(
        ...
        assert not (
            base_client and (base_url or (api_key or auth_token))
        ), f"Expected either 'base_client' or ('base_url' and ('api_key' or 'auth_token'))"
        if base_client:
            self.base_client = base_client
        else:
            self.base_client = RestClient(
                base_url, None, api_key, auth_token=auth_token
            )
