...
class FileManager:
    ...
    def __init__(self, env):
        ...
        # 注册事件
        self.env.event_registry.register_event(self.id, 'file_created')
        self.env.event_registry.register_event(self.id, 'file_transfer_started')
        self.env.event_registry.register_event(self.id, 'file_transferred')
        self.env.event_registry.register_event(self.id, 'file_computing_started')
        self.env.event_registry.register_event(self.id, 'file_computed')
        self.env.event_registry.register_event(self.id, 'file_location_changed')
    ...
    def create_file(self, owner_agent_id, file_name, file_size, file_type, content=None, properties: Dict[str, Any] = None) -> str:
        ...
        print(f"时间：{self.env.now}: 文件 {file_id} ({file_name}) 已创建")
        return file_id
    ...
    def register_file(self, file_id: str, properties: Dict[str, Any]) -> bool:
        ...
        """
        if file_id in self.files:
            print(f"警告: 文件 {file_id} 已存在，将更新属性")
    ...
    def mark_file_transfer_started(self, file_id: str, source_agent_id: str, target_agent_id: str) -> bool:
        ...
        print(f"时间 {self.env.now}: 文件 {file_id} 开始从代理 {source_agent_id} 传输到代理 {target_agent_id}")
        return True
    ...
    def mark_file_transferred(self, file_id: str, target_agent_id: str, location: Optional[Tuple[float, float, float]] = None) -> bool:
        ...
        """
        if file_id not in self.files:
            print(f"警告: 文件 {file_id} 不存在，无法标记为已传输")
            return False
        
        if self.file_status.get(file_id) != self.STATUS_TRANSFERRING:
            print(f"警告: 文件 {file_id} 当前状态为 {self.file_status.get(file_id)}，不能标记为已传输")
            return False
        ...
        print(f"时间 {self.env.now}: 文件 {file_id} 已传输到代理 {target_agent_id}")
        return True
    ...
    def mark_file_computing_started(self, file_id: str, agent_id: str) -> bool:
        ...
        """
        if file_id not in self.files:
            print(f"警告: 文件 {file_id} 不存在，无法标记为开始计算")
            return False
        
        if self.file_status.get(file_id) not in [self.STATUS_CREATED, self.STATUS_TRANSFERRED]:
            print(f"警告: 文件 {file_id} 当前状态为 {self.file_status.get(file_id)}，不能标记为开始计算")
            return False
        ...
        print(f"时间 {self.env.now}: 文件 {file_id} 开始在代理 {agent_id} 上计算")
        return True
    ...
    def mark_file_computed(self, file_id: str, result_file_id: str) -> bool:
        ...
        """
        if file_id not in self.files:
            print(f"警告: 文件 {file_id} 不存在，无法标记为已计算")
            return False
        
        if self.file_status.get(file_id) != self.STATUS_COMPUTING:
            print(f"警告: 文件 {file_id} 当前状态为 {self.file_status.get(file_id)}，不能标记为已计算")
            return False
        ...
        print(f"时间 {self.env.now}: 文件 {file_id} 已计算完成，生成结果文件 {result_file_id}")
        return True
