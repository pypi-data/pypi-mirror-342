...
class ComponentManager:
    ...
    def __init__(self, env):
        ...
        # 注册事件
        self.env.event_registry.register_event(self.id, 'component_recommended')
        self.env.event_registry.register_event(self.id, 'component_selected')
        ...
        try:
            ...
            for _, module_name, is_pkg in pkgutil.iter_modules([str(component_path)]):
                ...
                try:
                    ...
                    print(f"警告: 加载模块 {module_name} 时出错: {str(e)}")
        
        except ImportError:
            print("警告: 无法导入airfogsim.component包")
    ...
    def register_component_class(self, component_class: Type[Component]) -> None:
        ...
        print(f"时间 {self.env.now}: 组件管理器注册组件类 {component_name}")
    ...
    def create_component(self, component_class_name: str, agent: Agent, 
        ...
        if not component_class:
            print(f"时间 {self.env.now}: 找不到组件类 {component_class_name}")
            return None
        ...
        try:
            ...
            print(f"时间 {self.env.now}: 创建组件 {component_class_name} 实例时出错: {str(e)}")
            import traceback
            traceback.print_exc()
            return None
