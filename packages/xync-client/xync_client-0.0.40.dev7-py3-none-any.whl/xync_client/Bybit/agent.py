...
class AgentClient(BaseAgentClient):  # Bybit client
    ...
    async def order_request(self, req: OrderRequest):
        crpr = await self._post("/fiat/otc/item/simple", data={"item_id": req.model_dump()["itemId"]})
        req.curPrice = crpr["result"]["curPrice"]
