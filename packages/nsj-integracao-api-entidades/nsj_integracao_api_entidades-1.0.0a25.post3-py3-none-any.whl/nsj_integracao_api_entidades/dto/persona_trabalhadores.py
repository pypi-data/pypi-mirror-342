...
class TrabalhadoreDTO(DTOBase):
    ...
    salarioliquidoestimado: float = DTOField()
    salarioliquidodiario: float = DTOField()
    ...
    formatributacaoexterior: str = DTOField()
    condicaoambientetrabalho: uuid.UUID = DTOField(
      strip=True,
      min=36,
      max=36,
      validator=DTOFieldValidators().validate_uuid,)
    nomeemergencia: str = DTOField()
    dddtelemergencia: str = DTOField()
    telefoneemergencia: str = DTOField()
    relacaoemergencia: str = DTOField()
    matriculaesocial: str = DTOField()
    tipocontapix: int = DTOField()
    cnpjentidadequalificadora: str = DTOField()
    indicativocontratacaoaprendiz: int = DTOField()
    # Atributos de lista
