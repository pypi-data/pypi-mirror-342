...
class WSNOCLIB : 
    ...
    def kafka_listen(self,new_kafka_release=False,beautify=False) :  
        ...
        else if new_kafka_release:
            self.logger.debug(f"unset JMX_Port before starting new listener ...")
            self.channel.sendall("unset JMX_PORT\n")
            cmd=f"docker exec nspos-kafka ./opt/bitnami/kafka/bin/kafka-console-consumer.sh --bootstrap-server nspos-kafka:9193 --topic {self.kafka.get('topicId')} --consumer.config /opt/bitnami/kafka/config/consumer.properties"
        self.logger.debugf(f'Kafka Listen is using {cmd}')
        while True :
            sleep(5)
            if self.killed :
