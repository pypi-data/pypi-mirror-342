...
def test_get_by_status(queries, connection, fill_db, status, value):
    ...
    assert q.execute(connection, status=status).one_or_none() == (
        value, str(value), status
    )
    assert q.one_or_none(connection, status=status) == (
        value, str(value), status
    )
...
def test_get_by_status_none(queries, connection, fill_db):
    q = queries.from_file('tasks/joined_get_by_status.sql')
    assert q.execute(connection).many() == [
        (1, '1'), (2, '2'), (3, '3'),
    ]
...
def test_count_by_status(queries, connection, fill_db, status, value):
    ...
    if status:
        assert q.execute(connection, status=status).scalar() == value
        assert q.scalar(connection, status=status) == value
    else:
        assert q.execute(connection).scalar() == value
        assert q.scalar(connection) == value
