from typing import Optional

from jinja2 import Template

from .types import Connection, Cursor
from .templates import Renderer
from .params_styles import ParamStyleRecognizer
from .result import Result


class Query:

    def __init__(
            self, template: Template,
            renderer: Renderer,
            param_style_recognizer: ParamStyleRecognizer,
    ):
        self.template = template
        self.sql = None
        self.parameters = None
        self.renderer = renderer
        self._recognize_param_style = param_style_recognizer.get

    def __call__(
        self,
        conn,
        batch_params: Optional[list[object]] = None,
        /,
        **kwargs: object,
    ) -> Result:
        return self.execute(conn, batch_params, **kwargs)

    def execute(
        self,
        conn,
        batch_params: Optional[list[object]] = None,
        /,
        **kwargs: object,
    ) -> Result:
        param_style = self._recognize_param_style(conn)
        cursor = conn.cursor()

        sql, ordered_params = self.renderer.prepare_query(
            self.template, param_style, kwargs or {},
        )

        if batch_params is not None:
            assert not kwargs, ('Only batch_params or kwargs '
                                'allowed at the same time')
            cursor.executemany(sql, batch_params)
        else:
            cursor.execute(sql, ordered_params)

        return Result(cursor)

    def many(
        self,
        conn,
        batch_params: Optional[list[object]] = None,
        /,
        **kwargs: object,
    ) -> list[object]:
        return self.execute(conn, batch_params, **kwargs).many()

    def one(
        self,
        conn,
        batch_params: Optional[list[object]] = None,
        /,
        **kwargs: object,
    ) -> object:
        return self.execute(conn, batch_params, **kwargs).one()

    def one_or_none(
        self,
        conn,
        batch_params: Optional[list[object]] = None,
        /,
        **kwargs: object,
    ) -> object | None:
        return self.execute(conn, batch_params, **kwargs).one_or_none()

    def scalar(
        self,
        conn,
        batch_params: Optional[list[object]] = None,
        /,
        **kwargs: object,
    ) -> object:
        return self.execute(conn, batch_params, **kwargs).scalar()
