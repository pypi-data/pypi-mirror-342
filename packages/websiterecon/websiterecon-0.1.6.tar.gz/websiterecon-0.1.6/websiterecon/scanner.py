    ...
    async def run_scan(self):
        try:
            ...
            with Progress(
                ...
                transient=False  # Make progress persistent
            ) as progress:
                # If SSL scan is enabled, do it first (it's synchronous)
                if self.options["ssl_scan"]:
                    task = progress.add_task("[cyan]Checking SSL certificate...", total=1)
                    self.get_ssl_info()
                    progress.update(task, completed=1, description="[green]✓ SSL certificate checked")
                ...
                if self.options["subdomain_scan"]:
                    ...
                    progress.update(task, completed=1, description=f"[green]✓ Found {len(self.subdomains)} subdomains")
                
                # Scan ports
                if self.options["port_scan"]:
                    task = progress.add_task("[cyan]Scanning ports...", total=1)
                    await self.scan_ports()
                    progress.update(task, completed=1, description=f"[green]✓ Found {len(self.open_ports)} open ports")
                ...
                if self.options["content_scan"]:
                    ...
                    progress.update(task, completed=1, 
                        description=f"[green]✓ Crawled {len(self.visited_urls)} pages")
