...
@task_deco('fs_put_local', function_timeout=120, concurrent_mode=ConcurrentModeEnum.ASYNC, broker_kind=BrokerEnum.LOCAL_PYTHON_QUEUE, specify_async_loop=local_loop, max_retry_times=1)
...
@task_deco('fs_get_{}_local'.format(cfg['NODE_ID']), function_timeout=60, concurrent_mode=ConcurrentModeEnum.ASYNC, broker_kind=BrokerEnum.LOCAL_PYTHON_QUEUE, specify_async_loop=local_loop, max_retry_times=2)
async def file_transfer_get_local(bucket, bucket_name, refresh='no', test=False):
    global local_queue_get
...
def get_system_status():
    ...
    long_idle_warning = time.time() - last_ping > 60
