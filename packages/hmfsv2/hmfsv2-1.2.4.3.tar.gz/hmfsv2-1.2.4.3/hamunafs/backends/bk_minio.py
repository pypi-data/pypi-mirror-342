...
class MinioBackend(BackendBase):
    ...
    async def put_buffer_async(self, buffer, bucket, bucket_name):
        try:
            ...
            ret, e = await asyncio.wait_for(self.client.upload_file_by_buffer(buffer, self.default_bucket, b_name), timeout=20)
            if ret is not None:
    ...
    async def get_async(self, download_path, bucket, bucket_name, tries=0):
        try:
            if tries >= 3:
                ...
                ret, e = await asyncio.wait_for(self.client.download_file(download_path, self.default_bucket, '{}_{}'.format(bucket, bucket_name)), timeout=5)
                if ret:
                    return True, e
            ...
            if tries >= 3:
                ...
                return await self.get(download_path, bucket, bucket_name, tries+1)
