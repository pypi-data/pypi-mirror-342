...
class QemaOptimState2bit(TorchAOBaseTensor):
    ...
    def __init__(self, codes: Tensor, scale: Tensor, alpha: Tensor, signed: bool, shape, quantile):
        ...
        Given `codes` and `scale`, `block_size` is calculated as `codes.numel() * 4 // scale.numel()`.
        The extra `* 4` is because `codes` is 2-bit data packed in 8-bit storage.
        """
        ...
        self.alpha = alpha
        self._shape = shape
        self.block_size = codes.numel() * 4 // scale.numel()
    ...
    @classmethod
    def zeros(cls, shape, signed: bool = False, block_size: int = 128, quantile: float=0.25, device=None, **other_kwargs):
...
if not TORCH_VERSION_AT_LEAST_2_4:
    ...
    QemaOptimState2bit.to = _to
    del _to  # make sure to not re-use


@QemaOptimState2bit.implements(aten.copy_.default)
def _(func, types, args, kwargs):
    dst = args[0]
    src = args[1]

    if isinstance(dst, QemaOptimState2bit) and isinstance(src, QemaOptimState2bit):
        assert (
    ...
    elif isinstance(dst, QemaOptimState2bit):
        scaled_src, scale, alpha = scale_tensor(src.view(-1), dst.block_size, dst.quantile)
        dst.alpha.copy_(
            alpha.pow(1 / NUMS)
        )
        codes = quantize_with_alpha(scaled_src, dst.alpha, BITS)
        dst.codes.copy_(
            (codes[::4] << 6) | (codes[1::4] << 4) | (codes[2::4] << 2) | codes[3::4]
        )  # packing
...
@QemaOptimState2bit.implements(aten._to_copy.default)
def _(func, types, args, kwargs):
    # ignore dtype
    device = kwargs.get("device", None)
    out = QemaOptimState2bit(
...
@QemaOptimState2bit.implements(aten.lerp.Scalar)
def _(func, types, args, kwargs):
    args = [x.dequantize() if isinstance(x, QemaOptimState2bit) else x for x in args]
    return func(*args, **kwargs)


# this is needed for DTensor.from_local() and for flattening tensor
@QemaOptimState2bit.implements(aten.view.default)
def _(func, types, args, kwargs):
    x, shape = args

    if tuple(x.shape) == tuple(shape):
        return QemaOptimState2bit(x.codes, x.scale, x.alpha, x.signed, x._shape, x.quantile)

    if len(shape) == 1 and shape[0] == -1:
        return QemaOptimState2bit(x.codes, x.scale, x.alpha, x.signed, (x.numel(),), x.quantile)
...
def _(func, types, args, kwargs):
    x = args[0]
    if not isinstance(x, QemaOptimState2bit):
        raise ValueError(f"expecting a QemaOptimState2bit but found {type(x)}")
    ...
    return QemaOptimState2bit(codes, scale, alpha, x.signed, shape, x.quantile)


# required by torch.distributed.checkpoint.save
# note that we don't actually implement pin memory for this tensor subclass
# (pin_memory argument is ignored in aten._to_copy)
@QemaOptimState2bit.implements(aten.is_pinned.default)
def _(func, types, args, kwargs):
    return (
...
@QemaOptimState2bit.implements(aten.slice.Tensor)
def _(func, types, args, kwargs):
    ...
    # note that for 2-bit, we store .codes as flattened buffer
    # divide by 4 since we store 4x 2-bit in 1x uint8
    codes = x.codes[start * stride // 4 : end * stride // 4]
...
if TORCH_VERSION_AT_LEAST_2_5:
    ...
    add_safe_globals([QemaOptimState2bit])
