...
class QemaOptimState8bit(TorchAOBaseTensor):
    ...
    def __init__(self, codes: Tensor, scale: Tensor, alpha: Tensor, signed: bool, shape, quantile):
        """Create quantized 8-bit optimizer state as proposed in https://arxiv.org/abs/2309.01507
    ...
    def dequantize(self, output_dtype=None):
        float_data = dequant_with_alpha(self.codes, self.alpha, self.scale)
    ...
    def zeros(cls, shape, signed: bool = False, block_size: int = 128, quantile: float = 1.e-14, device=None, **other_kwargs):
        ...
        codes = torch.zeros(n_elems, dtype=torch.uint8, device=device)
...
if not TORCH_VERSION_AT_LEAST_2_4:
    ...
    QemaOptimState8bit.to = _to
    del _to  # make sure to not re-use


@QemaOptimState8bit.implements(aten.copy_.default)
def _(func, types, args, kwargs):
    dst = args[0]
    src = args[1]

    if isinstance(dst, QemaOptimState8bit) and isinstance(src, QemaOptimState8bit):
        assert (
    ...
    elif isinstance(dst, QemaOptimState8bit):
...
@QemaOptimState8bit.implements(aten._to_copy.default)
def _(func, types, args, kwargs):
    # ignore dtype
    device = kwargs.get("device", None)
    out = QemaOptimState8bit(
...
@QemaOptimState8bit.implements(aten.lerp.Scalar)
def _(func, types, args, kwargs):
    args = [x.dequantize() if isinstance(x, QemaOptimState8bit) else x for x in args]
    return func(*args, **kwargs)


# this is needed for DTensor.from_local() and for flattening tensor
@QemaOptimState8bit.implements(aten.view.default)
def _(func, types, args, kwargs):
    x, shape = args

    if tuple(x.shape) == tuple(shape):
        return QemaOptimState8bit(x.codes, x.scale, x.alpha, x.signed, x._shape, x.quantile)

    if len(shape) == 1 and shape[0] == -1:
        return QemaOptimState8bit(x.codes, x.scale, x.alpha, x.signed, (x.numel(),), x.quantile)
...
def _(func, types, args, kwargs):
    x = args[0]
    if not isinstance(x, QemaOptimState8bit):
        raise ValueError(f"expecting a QemaOptimState8bit but found {type(x)}")
    ...
    return QemaOptimState8bit(codes, scale, alpha, x.signed, shape, x.quantile)


# required by torch.distributed.checkpoint.save
# note that we don't actually implement pin memory for this tensor subclass
# (pin_memory argument is ignored in aten._to_copy)
@QemaOptimState8bit.implements(aten.is_pinned.default)
def _(func, types, args, kwargs):
    return (
...
@QemaOptimState8bit.implements(aten.slice.Tensor)
def _(func, types, args, kwargs):
    ...
    codes = x.codes[start * stride : end * stride]
...
if TORCH_VERSION_AT_LEAST_2_5:
    ...
    add_safe_globals([QemaOptimState8bit])