...
class QemaOptimState4bit(TorchAOBaseTensor):
...
if not TORCH_VERSION_AT_LEAST_2_4:
    ...
    QemaOptimState4bit.to = _to
    del _to  # make sure to not re-use


@QemaOptimState4bit.implements(aten.copy_.default)
def _(func, types, args, kwargs):
    dst = args[0]
    src = args[1]

    if isinstance(dst, QemaOptimState4bit) and isinstance(src, QemaOptimState4bit):
        assert (
...
@QemaOptimState4bit.implements(aten._to_copy.default)
def _(func, types, args, kwargs):
    # ignore dtype
    device = kwargs.get("device", None)
    out = QemaOptimState4bit(
...
@QemaOptimState4bit.implements(aten.lerp.Scalar)
def _(func, types, args, kwargs):
    args = [x.dequantize() if isinstance(x, QemaOptimState4bit) else x for x in args]
    return func(*args, **kwargs)


# this is needed for DTensor.from_local() and for flattening tensor
@QemaOptimState4bit.implements(aten.view.default)
def _(func, types, args, kwargs):
    x, shape = args

    if tuple(x.shape) == tuple(shape):
        return QemaOptimState4bit(x.codes, x.scale, x.alpha, x.signed, x._shape, x.quantile)

    if len(shape) == 1 and shape[0] == -1:
        return QemaOptimState4bit(x.codes, x.scale, x.alpha, x.signed, (x.numel(),), x.quantile)
...
def _(func, types, args, kwargs):
    x = args[0]
    if not isinstance(x, QemaOptimState4bit):
        raise ValueError(f"expecting a QemaOptimState4bit but found {type(x)}")
    ...
    return QemaOptimState4bit(codes, scale, alpha, x.signed, shape, x.quantile)


# required by torch.distributed.checkpoint.save
# note that we don't actually implement pin memory for this tensor subclass
# (pin_memory argument is ignored in aten._to_copy)
@QemaOptimState4bit.implements(aten.is_pinned.default)
def _(func, types, args, kwargs):
    return (
...
@QemaOptimState4bit.implements(aten.slice.Tensor)
def _(func, types, args, kwargs):
...
if TORCH_VERSION_AT_LEAST_2_5:
    ...
    add_safe_globals([QemaOptimState4bit])
