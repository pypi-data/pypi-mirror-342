...
class SignIn:
    async def sign_in(
        ...
        Returns:
            :obj:`~pyroitaly.types.User` | :obj:`~pyroitaly.types.TermsOfService` | bool: On success, in case the
            authorization completed, the user is returned. In case the phone number needs to be registered first AND the
            terms of services accepted (with :meth:`~pyroitaly.Client.accept_terms_of_service`), an object containing
