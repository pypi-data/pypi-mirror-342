...
git clone https://wulan17:"$DOCS_KEY"@github.com/ItalyMusic/pyroitaly-docs.git
cd pyroitaly-docs || exit 1
mkdir -p "$branch"
cd "$branch" || exit 1
...
git config --local user.name "ItalyMusic"
git config --local user.email "mgn.egypt@gmail.com"
git add --all
