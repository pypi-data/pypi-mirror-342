...
from pyroitaly.handlers.handler import Handler
from pyroitaly.handlers import (
...
from pyroitaly.raw.types import (
...
class Dispatcher:
    ...
    def __init__(self, client: "pyroitaly.Client"):
        self.client = client
        ...
        async def message_parser(update, users, chats):
            return (
                await pyroitaly.types.Message._parse(self.client, update.message, users, chats,
        ...
        async def bot_business_message_parser(update, users, chats):
            return (
                await pyroitaly.types.Message._parse(
                    self.client,
                    update.message,
                    users,
                    chats,
        ...
        async def callback_query_parser(update, users, chats):
            return (
                await pyroitaly.types.CallbackQuery._parse(self.client, update, users),
                CallbackQueryHandler
            )

        async def user_status_parser(update, users, chats):
            return (
                pyroitaly.types.User._parse_user_status(self.client, update),
                UserStatusHandler
            )

        async def inline_query_parser(update, users, chats):
            return (
                pyroitaly.types.InlineQuery._parse(self.client, update, users),
                InlineQueryHandler
            )

        async def poll_parser(update, users, chats):
            return (
                await pyroitaly.types.Poll._parse_update(self.client, update, users),
                PollHandler
            )

        async def chosen_inline_result_parser(update, users, chats):
            return (
                pyroitaly.types.ChosenInlineResult._parse(self.client, update, users),
                ChosenInlineResultHandler
            )

        async def chat_member_updated_parser(update, users, chats):
            return (
                pyroitaly.types.ChatMemberUpdated._parse(self.client, update, users, chats),
                ChatMemberUpdatedHandler
            )

        async def chat_join_request_parser(update, users, chats):
            return (
                pyroitaly.types.ChatJoinRequest._parse(self.client, update, users, chats),
                ChatJoinRequestHandler
            )

        async def story_parser(update, users, chats):
            return (
                await pyroitaly.types.Story._parse(self.client, update.story, update.peer),
                StoryHandler
            )

        async def shipping_query_parser(update, users, chats):
            return (
                await pyroitaly.types.ShippingQuery._parse(self.client, update, users),
                ShippingQueryHandler
            )

        async def pre_checkout_query_parser(update, users, chats):
            return (
                await pyroitaly.types.PreCheckoutQuery._parse(self.client, update, users),
                PreCheckoutQueryHandler
            )

        async def message_bot_na_reaction_parser(update, users, chats):
            return (
                pyroitaly.types.MessageReactionUpdated._parse(self.client, update, users, chats),
                MessageReactionUpdatedHandler
            )

        async def message_bot_a_reaction_parser(update, users, chats):
            return (
                pyroitaly.types.MessageReactionCountUpdated._parse(self.client, update, users, chats),
                MessageReactionCountUpdatedHandler
            )

        async def bot_business_connect_parser(update, users, chats):
            return (
                await pyroitaly.types.BotBusinessConnection._parse(self.client, update.connection),
                BotBusinessConnectHandler
            )

        async def purchased_paid_media_parser(update, users, chats):
            return (
                pyroitaly.types.PurchasedPaidMedia._parse(self.client, update, users),
                PurchasedPaidMediaHandler
            )
                    ...
                    for handler in group:
                        try:
                            if parsed_update is not None:
                                ...
                                break
                        except (pyroitaly.StopPropagation, pyroitaly.ContinuePropagation) as e:
                            if isinstance(e, pyroitaly.StopPropagation):
                                raise
                        except Exception as exception:
                            if parsed_update is not None:
                                await self._handle_exception(parsed_update, exception)
        except pyroitaly.StopPropagation:
            pass
    ...
    async def _handle_exception(self, parsed_update: types.Update, exception: Exception):
        ...
        for error_handler in self.error_handlers:
            try:
                ...
                raise
            except pyroitaly.ContinuePropagation:
                continue
