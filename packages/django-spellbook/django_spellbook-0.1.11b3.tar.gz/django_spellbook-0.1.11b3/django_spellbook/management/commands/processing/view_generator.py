...
"""
    ...
    def check_for_dangerous_content(self, context_str: str):
        dangerous_patterns = [
            r"(['\"])\s*[\}\)]",    # Top critical
            r"__import__\s*\(",     # Runner-up 1
            r"eval\s*\(",           # Runner-up 2
            r"exec\s*\("
        ]
        
        combined = re.compile(r'|'.join(dangerous_patterns), re.IGNORECASE)
        
        if combined.search(context_str):
            raise ValueError(f"Potentially unsafe content detected in context for { combined.pattern }")
