...
class TestViewGeneratorErrors(TestCase):
    ...
    def test_context_dict_with_dangerous_content(self):
        ...
        dangerous_contexts = [
        # TOP CRITICAL: String termination + code execution
        (
            {'payload': "'); os.system('rm -rf /')"},  # String escape with command execution
            "code-breaking pattern"
        ),
        
        # Runner-Up 1: Dangerous dynamic import
        (
            {'payload': "__import__('os').system('clear')"},  # Direct OS access
            "__import__ pattern"
        ),

        # Runner-Up 2: Code evaluation
        (
            {'payload': "eval('__import__(\\'os\\').system(\\'ls\\')')"},  # Nested evaluation
            "eval pattern"
        )
    ]
