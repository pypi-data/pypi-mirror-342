...
def get_mat_stats(
    ...
    with omx.open_file(mat_file.with_suffix(".OMX")) as file:
        ...
        for tab in tabs:
            ...
            stats[tab]["Total"] = matrix.sum()
            stats[tab]["Intras."] = np.sum(np.diag(matrix))
            stats[tab]["Min."] = matrix.min()
            stats[tab]["Mean"] = matrix.mean()
            stats[tab]["Max."] = matrix.max()
            stats[tab]["Std."] = matrix.std()

            # Handle Min > 0, Max > 0, and Std. when there are no positive values
            positive_values = matrix[matrix > 0]
            if positive_values.size > 0:
                stats[tab]["Min > 0"] = positive_values.min()
                stats[tab]["Mean > 0"] = positive_values.mean()
                stats[tab]["Max > 0"] = positive_values.max()
                stats[tab]["Std. > 0"] = positive_values.std()
            else:
                stats[tab]["Min > 0"] = 0
                stats[tab]["Mean > 0"] = 0
                stats[tab]["Max > 0"] = 0
    ...
    file.close()
    # Create df
    stats_df = pl.from_dict(stats)
    # Cleanup
    mat_file.with_suffix(".OMX").unlink()
...
def furness(
    ...
    for i in range(max_iterations):
        ...
        row_factors = productions[:, None] / row_sums  # Broadcasting (n, 1)
        matrix *= row_factors

        # Column adjustment
        col_sums = matrix.sum(axis=0, keepdims=True)  # Shape (1, m)
        col_factors = attractions[None, :] / col_sums  # Broadcasting (1, m)
        matrix *= col_factors
...
def expand_matrix(
    ...
    expanded_mx = expanded_mx.join(
        mx_df, on=[mx_df.columns[0], mx_df.columns[1]], how="outer"
    ).fill_null(strategy="zero")
    # remove columns containing "_right"
    expanded_mx = expanded_mx.select(
        [col for col in expanded_mx.columns if "_right" not in col]
    )
...
def wide_mx_2_long_mx(
    ...
    o_col : str, optional
        name of origin/production column, by default "origin"
    d_col : str, optional
        name of destination/attraction column, by default "destination"
    v_col : str, optional
        name of values column, by default "value"
...
def read_omx_to_polars(
    ...
    o_col : str, optional
        name of origin/production vector header, by default "origin"
    d_col : str, optional
        name of destination/attraction vector header, by default "destination"
    ...
    matrix_df = pl.DataFrame()
    # open omx
    omx_mat = omx.open_file(omx_file_path)
    # get matrices
            ...
            if stack:
                ...
                if i == 0:
                    ...
                    matrix_df = matrix_df.join(
                        mx_lvl_df, on=[o_col, d_col], how="outer"
                    )
...
def read_mat_to_array(
    ...
    omx_mat = omx.open_file(omx_path)
    # get matrices
    if limited_tabs is None or len(limited_tabs) == 0:
