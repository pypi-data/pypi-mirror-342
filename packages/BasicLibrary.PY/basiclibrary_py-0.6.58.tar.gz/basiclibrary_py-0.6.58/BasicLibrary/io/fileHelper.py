...
class FileHelper:
    ...
    @staticmethod
    def get_media_type(file_full_name: PathLike | str):
        """
        获取文件的媒体类型
        :param file_full_name:
        :return:
        """
        return MediaHelper.get_media_type(file_full_name)

    pass

    @staticmethod
    def get_media_type_name(file_full_name: PathLike | str):
        """
        获取文件的媒体类型
        :param file_full_name:
        :return:
        """
        return MediaHelper.get_media_type_name(file_full_name)

    pass

    @classmethod
    def modify[P](cls, file_full_name: PathLike | str, deal_file_content: Callable[[str, P], str], file_encoding=None,
                  *args, **kwargs_for_func):
        """
        修改文件内容
        :param file_encoding: 文件的编码格式。如果不指定的话，那么系统自动判定
        :param deal_file_content:对文件内容进行操作的函数，参数为文件原来的内容，返回值是修改后的内容
        :param file_full_name: 带全路径的文件名称
        :return:修改后的文件内容
        """
        if not file_encoding:
        ...
        with open(file_full_name, 'r', encoding=file_encoding) as r:
            content = r.read()
            # 对文件内容进行操作
            result_content = deal_file_content(content, *args, **kwargs_for_func)
        pass
