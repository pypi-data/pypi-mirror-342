...
from BasicLibrary.pattern.loopHelper import LoopHelper
...
class ListHelper:
    ...
    def get_moving_average[T:int | float](array_data: list[T], window_size) -> list[float]:
        ...
        :return: 移动平均值数组
        """
        arr_len = len(array_data)

        if window_size <= 0 or arr_len < window_size:
            return []
        pass
        ...
        for i in range(arr_len):
            if i == 0:
                prefix_sum.append(array_data[i])
                continue
            pass

            prefix_sum.append(prefix_sum[i - 1] + array_data[i])
        pass
        ...
        for i in range(arr_len):
            ...
            start_sum = prefix_sum[i - window_size] if (i - window_size >= 0) else 0
            total = prefix_sum[i] - start_sum
    ...
    def product(*args: list):
        ...
        :return:
        """
        return LoopHelper.product(*args)
