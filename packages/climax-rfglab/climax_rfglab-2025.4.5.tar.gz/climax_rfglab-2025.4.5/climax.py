...
__version__: str = '2025.4.5'  # this must be here for pyproject.toml to find it.


class climax(App):
    """a Command Line IMAge eXplorer."""

    __version__: str = __version__  # defines climax.__version__
    ...
    def display_slice(self, reset_slices: Optional[bool] = False, curslice: Optional[int] = 0) -> None:
        if reset_slices or self.view_plane != str(self.slice_radio_set.pressed_button.label):
            ...
            self.curslice = curslice if 0 <= curslice <= self.Z_slider.max else 0
            self.Z_slider.value = self.curslice

            self._setupUI()

        elif curslice:
            self.curslice = curslice if 0 <= curslice <= self.Z_slider.max else self.curslice
            self.Z_slider.value = self.curslice
    ...
    def parse_arguments(cls) -> argparse.Namespace:
        ...
        parser.add_argument('filename')  # this could be a list?
        parser.add_argument('-s', '--strings', nargs='*', default=climax.__default_channels__)
        parser.add_argument('-c', '--cmap', default=climax.__default_cmap__)
