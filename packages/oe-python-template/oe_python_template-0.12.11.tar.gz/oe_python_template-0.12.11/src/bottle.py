...
callable = lambda x: builtins.callable(x)
...
class Router:
    ...
    def _itertokens(self, rule):
        ...
        for match in self.rule_syntax.finditer(rule):
            ...
            g = match.groups()
            if g[2] is not None:
                depr(0, 13, "Use of old route syntax.", "Use <name> instead of :name in routes.", stacklevel=4)
...
class Route:
    ...
    def get_undecorated_callback(self):
        ...
        while hasattr(func, "__closure__") and func.__closure__:
            attributes = func.__closure__
    ...
    def POST(self):
        ...
        for part in parser.parse():
            if not part.filename and part.is_buffered():
                ...
                post[part.name] = FileUpload(part.file, part.name, part.filename, part.headerlist)
...
class BaseResponse:
    ...
    expires = HeaderProperty(
        "Expires", reader=lambda x: datetime.fromtimestamp(parse_date(x), UTC), writer=lambda x: http_date(x)
    )
...
class JSONPlugin:
    ...
    def setup(self, app):
        app.config._define(
            "json.enable", default=True, validate=bool, help="Enable or disable automatic dict->json filter."
        )
        app.config._define(
            "json.ascii", default=False, validate=bool, help="Use only 7-bit ASCII characters in output."
        )
        app.config._define(
            "json.indent", default=True, validate=bool, help="Add whitespace to make json more readable."
        )
        app.config._define(
            "json.dump_func",
            default=None,
...
def _lscmp(a, b):
    ...
    """
    return not sum(0 if x == y else 1 for x, y in zip(a, b, strict=False)) and len(a) == len(b)
...
class StplParser:
    ...
    def set_syntax(self, syntax):
        ...
        if syntax not in self._re_cache:
            ...
            pattern_vars = dict(zip(names.split(), etokens, strict=False))
