...
def convert_aidge_to_onnx(graph_view: aidge_core.GraphView,
                ...
                verbose: bool = False,
                enable_custom_op: bool = False,
                opset: int = None) -> onnx.ModelProto:
    """Export a :py:class:`aidge_core.GraphView` to an ONNX file.
    ...
    all_tensor_def = all([all([not o.undefined() for o in node.get_operator().get_outputs()]) for node in graph_view.get_nodes()])
    ...
    if forward_dims_required:
        for input_node in graph_view.get_input_nodes():
            for in_idx in range(len(input_node.inputs())):
                if input_node.get_operator().is_optional_input(): continue
