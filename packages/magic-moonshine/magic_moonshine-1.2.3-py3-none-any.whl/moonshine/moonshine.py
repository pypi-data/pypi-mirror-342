...
@dataclasses.dataclass
class Subindex:
    """A class to maintain a collection of video IDs."""
    
    # Default to empty list if no video_ids provided
    video_ids: List[str] = field(default_factory=list)
    
    def __post_init__(self):
        """Process the input after initialization."""
        ...
        elif isinstance(self.video_ids, list):
            self.video_ids = self.video_ids.copy()
        else:
            raise TypeError("video_ids must be a string or list of strings")
    
    def add(self, videos: Union[str, List[str]]) -> None:
        """
        Add one or more video IDs to the subindex.
        
        Args:
            videos: A single video ID or a list of video IDs
        """
        if isinstance(videos, str):
            self.video_ids.append(videos)
        elif isinstance(videos, list):
            self.video_ids.extend(videos)
        else:
            raise TypeError("videos must be a string or list of strings")
    
    def remove(self, videos: Union[str, List[str]]) -> None:
        """
        Remove one or more video IDs from the subindex.
        
        Args:
            videos: A single video ID or a list of video IDs
        """
        if isinstance(videos, str):
            if videos in self.video_ids:
                self.video_ids.remove(videos)
        elif isinstance(videos, list):
            self.video_ids = [vid for vid in self.video_ids if vid not in videos]
        else:
            raise TypeError("videos must be a string or list of strings")
    
    def clear(self) -> None:
        """Remove all video IDs from the subindex."""
        self.video_ids.clear()
    
    def list(self) -> List[str]:
        """Return a list of all video IDs in the subindex."""
        return self.video_ids.copy()
    
    def contains(self, video_id: str) -> bool:
        """
        Check if a video ID is in the subindex.
        
        Args:
            video_id: The video ID to check
            
        Returns:
            bool: True if the video ID is in the subindex, False otherwise
        """
        return video_id in self.video_ids
    
    def count(self) -> int:
        """Return the number of video IDs in the subindex."""
        return len(self.video_ids)
    
    def is_empty(self) -> bool:
        """Check if the subindex is empty."""
        return len(self.video_ids) == 0
    
    def __len__(self) -> int:
        """Allow using len() function on the subindex."""
        return len(self.video_ids)
    
    def __contains__(self, video_id: str) -> bool:
        """Allow using 'in' operator with the subindex."""
        return video_id in self.video_ids
