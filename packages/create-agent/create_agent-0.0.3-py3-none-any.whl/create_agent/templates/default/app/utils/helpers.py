...
def format_sse_chunk(model, message=None, finish_reason=None, created_at=None):
    """Format a chunk for SSE streaming in OpenAI format."""
    delta = {}
    if message and message.type:
        if message.type == "AIMessageChunk":
            delta["role"] = "assistant"
        elif message.type == "HumanMessageChunk":
            delta["role"] = "user"
        elif message.type == "SystemMessageChunk":
            delta["role"] = "system"
    if message and message.content:
        delta["content"] = message.content
    id = message and message.id or "stop"
    return {
        "id": id,
