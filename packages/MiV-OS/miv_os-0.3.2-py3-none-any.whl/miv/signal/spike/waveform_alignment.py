...
@dataclass
class WaveformAlignment(OperatorMixin):
    """
    Align waveforms based on the distribution of the waveform.
    """

    tag: str = "waveform alignment"
    ...
    def spike_alignment(spike_mat, spike_time, ss):
        ...
        for i in range(len(ind_min_neg)):
            ...
            d = abs(
                np.tile(ind_min_neg[i] + 1, (1, len(loc_peaks_neg))) - loc_peaks_neg
            )
            ...
            n_shift = matlab_round(
                (ind_min_neg[i] + 1 - loc_peaks_neg[ind_cluster]) / r_upsample
            )
        ...
        for i in range(len(ind_max_pos)):
            ...
            # ind_cluster determines index of the final selected peak as alignment point.
            d = abs(
                np.tile(ind_max_pos[i] + 1, (1, len(loc_peaks_pos))) - loc_peaks_pos
            )
            ind_cluster = d.argmin()
            min_shift = d[0][ind_cluster]
            ...
            n_shift = matlab_round(
                (ind_max_pos[i] + 1 - loc_peaks_pos[ind_cluster]) / r_upsample
            )
            ...
            if n_shift > 0:
                ...
                R = int(max_shift - n_shift)
                if R == 0:
                    newSpikeMat[ind_pos_find[i], :] = spike_mat[
                        int(ind_pos_find[i]), L:
                    ]
                else:
                    newSpikeMat[ind_pos_find[i], :] = spike_mat[
                        int(ind_pos_find[i]), L:-R
                    ]
