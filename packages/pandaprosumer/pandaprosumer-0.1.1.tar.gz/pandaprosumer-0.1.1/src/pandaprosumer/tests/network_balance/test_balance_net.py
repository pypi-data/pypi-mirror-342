...
class TestBalanceNet:
    ...
    def test_balance_net(self):
        ...
        # Create a new coupling controller in the demander prosumers
        connector_controllerids = []  # Keeps the list of coupling controller
        ...
        hc_element_indexes = [0]
        connector_prosumer = prosumer_dmd1
        ...
        ppies_balance_ctrl = PandapipesBalanceControl(net=net_pipes,
                                                      ...
                                                      connector_prosumers=[connector_prosumer],
        ...
        # Create a mapping between the pandapipes_balance_controller and the coupling controller of every prosumer
        for connector_controllerid in connector_controllerids:
        ...
        # For each demander, create a mapping between this controller and
        # the Heat Exchanger controller in the corresponding prosumer
        for prosumer_dmd, heat_consumer, connector_controllerid in zip([prosumer_dmd1],
                                                                       [heat_consumer_data],
                                                                       connector_controllerids):
            hx_index = 1  # index of the HX controller that is connected to the DHN (in this case 1 in every prosumer)
        ...
        # For each producer prosumer, create a 'FakeDemand' controller
        # and map the t_feed, t_return and mdot read from the res_circ_pump_pressure
        for prosumer_prod, pump_data, load_id in zip([prosumer_prod1],
            ...
            heat_demand_index = create_heat_demand(prosumer_prod, **hd_param_default)
            ...
            # Map the t_feed, t_return and mdot read from the res_circ_pump_pressure to the 'FakeDemand' controller
            ...
            GenericEnergySystemMapping(container=prosumer_prod,
                                       initiator_id=hp_index,
                                       initiator_column="p_comp_kw",  # Power consumption of the Heat Pump
                                       responder_net=net_power,
                                       responder_id=load_control_index,
                                       responder_column="p_in_kw",  # Power consumption of the Load
                                       order=1)
        ...
        assert_series_equal(prod_hd_res_df.mdot_kg_per_s, hc_mdot_kg_per_s_res_df.sum(axis=1), check_dtype=False, atol=.11, check_names=False, check_index=False, check_freq=False)
        ...
        print(fcc_res_df.t_return_out_c, jct_t_k_res_df["4"]-273.15)
        assert_series_equal(fcc_res_df.t_return_out_c, jct_t_k_res_df["4"]-273.15, check_dtype=False, atol=3, rtol=.01, check_names=False, check_index=False, check_freq=False)
