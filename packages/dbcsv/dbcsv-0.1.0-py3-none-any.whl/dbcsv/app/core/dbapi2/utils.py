...
import jwt
import time
from .exception import InterfaceError, OperationalError, AuthenticationError
...
def login(url: str, username: str, password: str) -> str:
    ...
    try:
        r = requests.post(f"{url}/connect", data=data, timeout=5)
...
def validate_token(url: str, token: str) -> Union[str, None]:
    ...
    refresh_threshold = int(os.getenv("ACCESS_TOKEN_DELTA_SECONDS"))
    ...
    try:
        ...
        r = requests.post(f"{url}/refresh", headers=header, timeout=5)
        r.raise_for_status()
        return r.json()["access_token"]
    except HTTPError as exc:
        err_json = exc.response.json()  # FastAPI return JSON: {"detail": "..."}
...
def query(url: str, token: str, schema: str, query: str) -> dict:
    ...
    try:
        r = requests.post(
            f"{url}/query/sql/", json=data, headers=header, timeout=5
        )
        r.raise_for_status()
        return r.json()
    except HTTPError as exc:
