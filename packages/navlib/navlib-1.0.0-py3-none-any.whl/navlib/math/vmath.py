...
def remove_offset(mat: Union[np.ndarray, List[float]], offset: float) -> np.ndarray:
    """
    Removes a constant offset from a vector or matrix.

    Args:
        mat (Union[np.ndarray, List[float]]): Vector or matrix.
            offset (float): Offset to remove.
...
def derivative(
    ...
    mat = np.array(mat) if isinstance(mat, list) else mat
    time = np.array(time) if isinstance(time, list) else time

    # Convert time to a column vector if mat has 2 dimensions
    time = time.reshape(-1, 1)

    # Check both have the same shape
    if mat.shape[0] != time.shape[0]:
        raise ValueError("Both input arrays must have the same number of rows.")

    return np.diff(mat, axis=0, n=1) / np.diff(time, axis=0, n=1)
...
def resample(
    ...
    if x[0] < xp[0] or x[-1] > xp[-1]:
        raise ValueError(
            "The new time vector must be within the range of the old time vector."
        )
...
def distance_traveled(
    ...
    the distance traveled is calculated for column.
