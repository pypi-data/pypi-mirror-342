...
class TileUnit(Tileable):
  ...
  def _setup_regularised_prototile(self) -> None:
    ...
    if self.regularised_prototile is None:
      ...
      tiles = gpd.GeoSeries([
        p.buffer(tiling_utils.RESOLUTION * 100, join_style=2, cap_style=3) \
          .simplify(tiling_utils.RESOLUTION * 100) for p in tiles])
      polygon = tiles.union_all()
      polygon = polygon.buffer(
        -tiling_utils.RESOLUTION * 100, join_style=2, cap_style=3)
      polygon = polygon.simplify(tiling_utils.RESOLUTION * 100)
