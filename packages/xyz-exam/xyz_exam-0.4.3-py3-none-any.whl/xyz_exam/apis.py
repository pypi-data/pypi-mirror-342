...
class PaperViewSet(ViewsMixin, UserApiMixin, BatchActionMixin, viewsets.ModelViewSet):
    ...
    def collect_questions(self, request):
        ...
        if owner_id:
            d['papers'] = {
                '$in':list(models.Paper.objects.filter(owner_id=owner_id[0]).values_list('id', flat=True))
            }
...
class ExamViewSet(BatchActionMixin, viewsets.ModelViewSet):
    ...
    def permission_denied(self, request, message=None, code=None):
        ...
        pk = self.kwargs['pk']
        r = helper.check_token(request, pk)
        if r == 'success':
            return
        if r is None:
            return super(ExamViewSet, self).permission_denied(request, message=message, code=code)
        raise exceptions.PermissionDenied(detail=r, code='403')
