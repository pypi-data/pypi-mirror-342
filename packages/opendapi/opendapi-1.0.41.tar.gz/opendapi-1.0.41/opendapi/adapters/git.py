...
def run_git_command(cwd: str, command_split: List[str]) -> str:
    ...
    try:
        ...
        # TODO: figure out why this is not sent to sentry but CI fails
...
def get_merge_base(cwd: str, current_ref: str, base_ref: str) -> str:
    ...
    logger.info(f"Merge base of {current_ref} and {base_ref} is {merge_base}")
    return merge_base
...
def get_upstream_commit_sha(cwd: str, ref: str, steps: int) -> str:
    ...
    logger.info(f"Upstream commit SHA of {ref}~{steps} is {upstream_sha}")
    return upstream_sha
