...
from .模块_对象 import (
    类_行情数据, 类_成交数据, 类_委托数据, 类_持仓数据,
    类_账户数据, 类_合约数据, 类_日志数据, 类_报价数据,
    类_订阅请求, 类_委托请求, 类_撤单请求, 类_历史数据请求, 类_报价请求, 类_K线数据
)
...
class 类_基础网关(ABC):
    ...
    def 推送行情(self, 行情实例: 类_行情数据) -> None:
        ...
        # 推送品种专属行情事件
        self.触发事件(f"{事件类型_行情}{行情实例.唯一标识}", 行情实例)

    def 推送成交(self, 成交实例: 类_成交数据) -> None:
        """处理成交数据推送"""
        self.触发事件(事件类型_成交, 成交实例)
        self.触发事件(f"{事件类型_成交}{成交实例.唯一标识}", 成交实例)

    def 推送订单(self, 订单实例: 类_委托数据) -> None:
        """处理订单状态更新"""
        self.触发事件(事件类型_订单, 订单实例)
        self.触发事件(f"{事件类型_订单}{订单实例.订单唯一标识}", 订单实例)

    def 推送持仓(self, 持仓实例: 类_持仓数据) -> None:
        """处理持仓数据更新"""
        self.触发事件(事件类型_持仓, 持仓实例)
        self.触发事件(f"{事件类型_持仓}{持仓实例.持仓唯一标识}", 持仓实例)
    ...
    def 推送报价(self, 报价实例: 类_报价数据) -> None:
        ...
        self.触发事件(f"{事件类型_报价}{报价实例.报价唯一标识}", 报价实例)
    ...
    @abstractmethod
    def 撤消订单(self, 撤单请求: 类_撤单请求) -> None:
        """撤销指定订单"""
        pass
    ...
    def 撤消报价(self, 撤单请求: 类_撤单请求) -> None:
        """撤销指定报价"""
        pass
