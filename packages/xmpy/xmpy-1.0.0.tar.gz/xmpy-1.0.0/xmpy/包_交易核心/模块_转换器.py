...
from .模块_对象 import 类_合约数据, 类_委托数据, 类_成交数据, 类_持仓数据, 类_委托请求
...
class 类_持仓转换器:
    ...
    def 更新成交(self, 成交实例: 类_成交数据) -> None:
        ...
        if not self.需要转换(成交实例.唯一标识):
            return
        持仓明细 = self.获取持仓明细(成交实例.唯一标识)
        持仓明细.更新成交(成交实例)

    def 更新委托(self, 订单实例: 类_委托数据) -> None:
        """更新订单状态"""
        if not self.需要转换(订单实例.唯一标识):
            return
        持仓明细 = self.获取持仓明细(订单实例.唯一标识)
        持仓明细.更新委托(订单实例)

    def 更新委托请求(self, 请求: 类_委托请求, 订单标识: str) -> None:
        """处理委托请求更新"""
        if not self.需要转换(请求.唯一标识):
            return
        持仓明细 = self.获取持仓明细(请求.唯一标识)
        持仓明细.更新委托请求(请求, 订单标识)
...
class 类_持仓明细:
    ...
    def 更新委托(self, 订单实例: 类_委托数据) -> None:
        """处理委托单更新"""
        if 订单实例.是否活跃():
            self.活跃委托字典[订单实例.订单唯一标识] = 订单实例
        else:
            self.活跃委托字典.pop(订单实例.订单唯一标识, None)
        self.计算冻结量()

    def 更新委托请求(self, 请求: 类_委托请求, 订单标识: str) -> None:
        """处理委托请求转换"""
        网关名称, 订单编号 = 订单标识.split(".")
        新订单 = 请求.生成委托数据(订单编号, 网关名称)
        self.更新委托(新订单)
    ...
    def 处理多头冻结(self, 订单: 类_委托数据, 冻结量: float) -> None:
        """处理多头方向冻结"""
        if 订单.开平 == 类_开平.平今:
            self.空头今冻 += 冻结量
        elif 订单.开平 == 类_开平.平昨:
    ...
    def 处理空头冻结(self, 订单: 类_委托数据, 冻结量: float) -> None:
        """处理空头方向冻结"""
        if 订单.开平 == 类_开平.平今:
            self.多头今冻 += 冻结量
        elif 订单.开平 == 类_开平.平昨:
    ...
    def 转换委托请求_上期所(self, 请求: 类_委托请求) -> List[类_委托请求]:
        """上期所特殊平仓规则处理"""
        if 请求.开平 == 类_开平.开仓:
            return [请求]
    ...
    def 转换委托请求_锁定(self, 请求: 类_委托请求) -> List[类_委托请求]:
        """锁仓模式转换"""
        if 请求.方向 == 类_方向.做多:
            今仓数量 = self.空头今仓
    ...
    def 转换委托请求_净仓(self, 请求: 类_委托请求) -> List[类_委托请求]:
        """净仓模式转换"""
        if 请求.方向 == 类_方向.做多:
            可用总量 = self.空头持仓 - self.空头冻结
            今仓可用 = self.空头今仓 - self.空头今冻
    ...
    def 处理上期所净仓(self, 请求: 类_委托请求, 可用总量: float, 今仓: float, 昨仓: float) -> List[类_委托请求]:
        """处理上期所净仓转换"""
        请求列表 = []
        剩余量 = 请求.数量
    ...
    def 处理普通净仓(self, 请求: 类_委托请求, 可用总量: float) -> List[类_委托请求]:
        """处理普通交易所净仓转换"""
        请求列表 = []
        剩余量 = 请求.数量
