...
class UIExplorer:
    ...
    async def _click_ui_element(
        ...
        if hierarchy_data and "hierarchy" in hierarchy_data:
            ...
            # Make minimal call to explore_ui with only required params
            # Use the same control_type as the element we're trying to click if provided
            result = await self._explore_ui(
                visible_only=True,
                min_size=5,  # Use minimum size to capture all possible elements
                depth=10     # Use deeper search to capture all nested elements
            )
        ...
        # Existing path-based logic
            ...
            def search_element(element, current_path=""):
                ...
                # Only search children if present in the element
