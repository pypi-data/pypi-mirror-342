...
from scrawlpy.commands.create import *
...
def main():
    ...
    spider.add_argument(
        "-p", "--project", help="创建项目 如 scrawlpy create -p <project_name>", metavar=""
    )
    spider.add_argument(
        "-s",
        "--spider",
        help="创建爬虫 如 scrawlpy create -s <spider_name>",
        metavar="",
    )
    spider.add_argument(
        "-i",
        "--item",
        help="创建item 如 scrawlpy create -i <table_name> 支持模糊匹配 如 scrawlpy create -i %%table_name%%",
        metavar="",
    )
    spider.add_argument(
        "-t", "--table", help="根据json创建表 如 scrawlpy create -t <table_name>", metavar=""
    )
    spider.add_argument(
        "-init", help="创建__init__.py 如 scrawlpy create -init", action="store_true"
    )
    ...
    spider.add_argument(
        "--setting", help="创建全局配置文件" "scrawlpy create --setting", action="store_true"
    )
