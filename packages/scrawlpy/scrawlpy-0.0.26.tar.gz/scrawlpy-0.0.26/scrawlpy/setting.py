...
class BaseSettings:
    ...
    ITEM_PIPELINES = [
        "scrawlpy.pipelines.mysql_pipeline.MysqlPipeline",
        # "scrawlpy.pipelines.mongo_pipeline.MongoPipeline",
        # "scrawlpy.pipelines.console_pipeline.ConsolePipeline",
    ]
    ...
    PROXY_POOL = "scrawlpy.network.proxy_pool.ProxyPool"  # 代理池
    ...
    # 下载
    DOWNLOADER = "scrawlpy.network.downloader.RequestsDownloader"  # 请求下载器
    SESSION_DOWNLOADER = "scrawlpy.network.downloader.RequestsSessionDownloader"
    RENDER_DOWNLOADER = "scrawlpy.network.downloader.SeleniumDownloader"  # 渲染下载器
    # RENDER_DOWNLOADER="scrawlpy.network.downloader.PlaywrightDownloader"
    ...
    def get_attributes(cls):
        ...
        for key, value in vars(cls).items():
            # 排除所有方法（包括普通方法、@classmethod、@staticmethod）、property、特殊方法（__xxx__）
            if not (
                    callable(value) or  # 普通方法、@classmethod、@staticmethod
                    isinstance(value, (classmethod, staticmethod, property)) or  # 类方法、静态方法、property
                     key.startswith('__')  # 排除 __xxx__ 特殊方法
            ):
    ...
    @classmethod
    def fields_map(cls):
        return inspect_all_attr(cls)

    @classmethod
    def fields_map_type(cls):
        return {k: type(v) for k, v in cls.fields_map().items()}

    @classmethod
    def convert_type(cls, type_, value):
        if type_ == str:
            return str(value)
        elif type_ == bool:
            return bool(value)
...
if __name__ == '__main__':
    ...
    print(BaseSettings.to_dict())
    # from attr import fields

    # print(Settings.get_fields_map())
    # print(Settings.get_field_list())
