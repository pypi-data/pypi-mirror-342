    ...
    def load_settings(self):
        ...
        try:
            if os.path.exists("redmailer_gui_settings.json"):
                ...
                # Load sample data if settings file doesn't exist
                samples_dir = os.path.join(os.path.dirname(__file__), "samples")
                if os.path.exists(samples_dir):
                    # Load sample accounts
                    accounts_path = os.path.join(samples_dir, "accounts.json")
                    if os.path.exists(accounts_path):
                        self.accounts_path.setText(accounts_path)
                        try:
                            self.accounts = redmail.load_accounts(accounts_path)
                            self.refresh_accounts()
                            self.populate_reader_accounts()
                            self.log("Loaded sample accounts from samples directory")
                        except Exception as e:
                            self.log(f"Error loading sample accounts: {str(e)}")
                    
                    # Load sample targets
                    targets_path = os.path.join(samples_dir, "targets.txt")
                    if os.path.exists(targets_path):
                        self.targets_path.setText(targets_path)
                        try:
                            self.targets = redmail.load_targets(targets_path)
                            self.log("Loaded sample targets from samples directory")
                        except Exception as e:
                            self.log(f"Error loading sample targets: {str(e)}")
                    
                    # Load sample template
                    template_path = os.path.join(samples_dir, "template.html")
                    if os.path.exists(template_path):
                        self.template_path.setText(template_path)
                        try:
                            with open(template_path, 'r') as f:
                                self.content_edit.setPlainText(f.read())
                            self.log("Loaded sample template from samples directory")
                        except Exception as e:
                            self.log(f"Error loading sample template: {str(e)}")
                    
                    # Set default content type to HTML for sample template
                    html_index = self.content_type.findText("html")
                    if html_index >= 0:
                        self.content_type.setCurrentIndex(html_index)
                    
                    # Show welcome message for first-time users
                    QMessageBox.information(
                        self,
                        "Welcome to EET Toolkit",
                        "Welcome! Since this is your first time running the application, "
                        "sample data has been loaded from the samples directory.\n\n"
                        "You can find these sample files in:\n"
                        f"{samples_dir}\n\n"
                        "Feel free to modify them or load your own files."
                    )
                else:
                    self.log("No settings file found and samples directory not available")
        except Exception as e:
...
def main():
    ...
    try:
        ...
        # Check for common Qt platform plugin errors
        if "Could not load the Qt platform plugin" in error_msg and "xcb" in error_msg:
            print("\nError: Qt XCB platform plugin could not be initialized.")
            print("\nThis is likely because your system is missing required XCB libraries.")
            print("\nPlease install the following packages and try again:")
            print("  sudo apt install libxcb-cursor0 libxcb-xinerama0 libxcb-randr0")
            print("  sudo apt install libxcb-xtest0 libxcb-shape0 libxcb-xkb1 libxkbcommon-x11-0")
            print("\nFor Fedora/RHEL-based systems:")
            print("  sudo dnf install libxcb libxcb-cursor libxkbcommon-x11")
            
            print("\nAfter installing these packages, try running 'eet' again.")
        else:
