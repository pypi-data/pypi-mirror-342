...
def find_marker_genes(adata):
    ...
    for i in range(len(adata.obs["leiden"].unique())):
        cluster_id = str(i)
        markers = [
            gene for gene in adata.uns["rank_genes_groups"]["names"][cluster_id][:20]
        ]
...
def main():
    ...
    adata.obs["entropy"] = (
        adata.obs["leiden"].astype(str).map(consensus_results["entropy"])
    )
    ...
    with open(result_file, "w") as f:
        ...
        for cluster in sorted(final_annotations.keys(), key=int):
            ...
            entropy = consensus_results["entropy"][cluster]
            f.write(
                f"{cluster}\t{final_annotations[cluster]}\t{cp:.2f}\t{entropy:.2f}\n"
            )
