...
from mllmcelltype.consensus import check_consensus, check_consensus_with_llm
...
Only output these 4 lines, nothing else."""
    ...
    print_results(
        "Results from find_agreement", consensus, consensus_proportion, entropy
    )
    ...
    print_results(
        "Results from check_consensus", consensus, consensus_proportion, entropy
    )
    ...
    try:
        print("\nTesting check_consensus_with_llm function:")
        consensus, consensus_proportion, entropy = check_consensus_with_llm(
            test_predictions
        )
        print_results(
            "Results from check_consensus_with_llm",
            consensus,
            consensus_proportion,
            entropy,
        )
    except Exception as e:
        print(f"\nError testing check_consensus_with_llm: {str(e)}")
        print(
