...
def analyze_examples(
    ...
    analysis = {
        ...
        "response_types": {
            ...
            "opinion": 0,
            "step_by_step": 0,
            "negative_response": 0,  # "I don't know" or similar
            "clarification": 0       # Asking for more information
        },
        "question_types": {
            "what": 0,
            "how": 0,
    ...
    try:
        ...
        for example in example_set.examples:
            # Calculate input and reference answer lengths
            input_words = example.input.split()
            input_length = len(input_words)
            
            # Get reference answer text
            if hasattr(example.reference_answer, "content"):
                reference_answer_text = example.reference_answer.content
            else:
                reference_answer_text = str(example.reference_answer)
                
            reference_answer_words = reference_answer_text.split()
            reference_answer_length = len(reference_answer_words)
            
            # Track lengths for correlation calculation
            if hasattr(example, "input_length"):
                example.input_length = input_length
            else:
                setattr(example, "input_length", input_length)
                
            if hasattr(example, "reference_answer_length"):
                example.reference_answer_length = reference_answer_length
            else:
                setattr(example, "reference_answer_length", reference_answer_length)
            
            # Update totals
            total_input_words += input_length
            total_reference_answer_words += reference_answer_length
        ...
        if example_set.examples:
            ...
            # Calculate correlation between input and reference answer lengths
            length_correlation = calculate_length_correlation(example_set)
            analysis["length_correlation"] = length_correlation

        # Merge word frequencies from inputs and reference answers
        merged_word_frequencies = {**input_word_frequencies}
        for word, count in reference_answer_word_frequencies.items():
            if word in merged_word_frequencies:
                merged_word_frequencies[word] += count
            else:
                merged_word_frequencies[word] = count
        
        analysis["word_frequencies"] = merged_word_frequencies
    ...
    if method in ["semantic", "hybrid"] and client is not None:
        try:
            ...
            if n_clusters == "auto":
                ...
                n_clusters = determine_optimal_clusters(combined_features, log_func)
...
def determine_optimal_clusters(features: np.ndarray, log_func=None) -> int:
    """
    Determine the optimal number of clusters for a dataset.
    
    This function uses multiple methods to find the optimal number of clusters:
    1. Elbow method using inertia
    2. Silhouette score
    3. Gap statistic (if available)
    ...
    # Use find_optimal_clusters from utils.py as the primary method
    try:
        n_clusters = find_optimal_clusters(features)
        log_func(f"Determined {n_clusters} clusters using elbow method")
        return n_clusters
    except Exception as e:
        log_func(f"Elbow method failed: {str(e)}, trying alternative methods")
    
    # Fallback to silhouette score if available
    try:
        from sklearn.cluster import KMeans
        from sklearn.metrics import silhouette_score
        
        # Try a range of cluster counts
        max_clusters = min(20, len(features) // 5)
        max_clusters = max(2, min(10, max_clusters))
        ...
        # Get the cluster count with the highest silhouette score
        best_n, best_score = max(silhouette_scores, key=lambda x: x[1])
        log_func(f"Determined {best_n} clusters using silhouette score (score: {best_score:.4f})")
        return best_n
    except Exception as e:
        log_func(f"Silhouette method failed: {str(e)}, using default")
    
    # Default fallback based on dataset size
    default_n = max(2, min(5, len(features) // 20))
    log_func(f"Using default of {default_n} clusters based on dataset size")
    return default_n

def calculate_length_correlation(example_set: ExampleSet) -> Dict[str, float]:
    """
    Calculate correlation between input and reference answer lengths.
    
    Args:
        example_set: The example set to analyze
        
    Returns:
        Dict[str, float]: Dictionary with correlation metrics
    """
    # Collect length data
    input_lengths = []
    reference_answer_lengths = []
    
    for example in example_set.examples:
        if not hasattr(example, "reference_answer") or not example.reference_answer:
            continue
        ...
        # Get reference answer length
        if hasattr(example, "reference_answer_length"):
            reference_answer_length = example.reference_answer_length
        else:
            reference_answer_text = example.reference_answer.content if hasattr(example.reference_answer, "content") else str(example.reference_answer)
            reference_answer_length = len(reference_answer_text.split())
            
        input_lengths.append(input_length)
        reference_answer_lengths.append(reference_answer_length)
    
    # Calculate correlation if we have enough data
    if len(input_lengths) < 2:
        return {
            "pearson_correlation": 0.0,
            "input_length_mean": float(np.mean(input_lengths)) if input_lengths else 0.0,
            "reference_answer_length_mean": float(np.mean(reference_answer_lengths)) if reference_answer_lengths else 0.0,
            "length_ratio": 0.0,
            "sample_size": len(input_lengths)
        }
    
    try:
        # Calculate Pearson correlation
        correlation = np.corrcoef(input_lengths, reference_answer_lengths)[0, 1]
        
        # Calculate means
        input_length_mean = float(np.mean(input_lengths))
        reference_answer_length_mean = float(np.mean(reference_answer_lengths))
        
        # Calculate average ratio of reference answer length to input length
        ratios = [ref_len / inp_len if inp_len > 0 else 0 for inp_len, ref_len in zip(input_lengths, reference_answer_lengths)]
        length_ratio = float(np.mean(ratios)) if ratios else 0.0
        
        return {
            "pearson_correlation": float(correlation),
            "input_length_mean": input_length_mean,
            "reference_answer_length_mean": reference_answer_length_mean,
            "length_ratio": length_ratio,
            "sample_size": len(input_lengths)
        }
    except Exception as e:
        logger.error(f"Failed to calculate length correlation: {str(e)}")
        return {
            "pearson_correlation": 0.0,
            "error": str(e),
            "sample_size": len(input_lengths)
