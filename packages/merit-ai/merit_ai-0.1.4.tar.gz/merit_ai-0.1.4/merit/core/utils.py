...
def extract_content(obj: Any) -> str:
    """
    Extract content from various types of objects.
    
    This utility function helps handle different input types consistently
    by extracting the content string from various object types.
    
    Examples:
        # Extract from string
        content = extract_content("Hello world")  # Returns "Hello world"
        
        # Extract from Input object
        from merit.core.models import Input
        input_obj = Input(content="Hello world")
        content = extract_content(input_obj)  # Returns "Hello world"
        
        # Extract from Response object
        from merit.core.models import Response
        response_obj = Response(content="Hello world")
        content = extract_content(response_obj)  # Returns "Hello world"
        
        # Extract from Document object
        from merit.core.models import Document
        doc_obj = Document(content="Hello world", metadata={}, id="1")
        content = extract_content(doc_obj)  # Returns "Hello world"
        
        # Extract from dictionary with 'content' key
        dict_obj = {"content": "Hello world"}
        content = extract_content(dict_obj)  # Returns "Hello world"
        
        # Extract from other object types
        content = extract_content(123)  # Returns "123"
    
    Args:
        obj: The object to extract content from.
