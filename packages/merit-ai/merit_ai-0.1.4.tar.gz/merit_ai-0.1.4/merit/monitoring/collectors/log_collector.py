...
from .base_collector import BaseLogCollector, CollectionResult, CollectionStatus
from ..models import LLMRequest, LLMResponse, LLMInteraction, TokenInfo


class LogDataCollector(BaseLogCollector):
    """
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        Args:
            config: Configuration dictionary with the following options:
                ...
                - format: Format of log entries ("json", "regex", or "custom")
                - regex_pattern: Regex pattern for extracting data (if format="regex")
                - json_path: JSON path to LLM data (if format="json")
                - custom_parser: Function to parse log entries (if format="custom")
                - field_mapping: Dictionary mapping log fields to LLMInteraction fields
    ...
    def _process_log_entry(self, line: str, result: CollectionResult) -> None:
        ...
        try:
            ...
            elif self.format == "custom" and self.custom_parser:
                data = self._extract_custom_data(line)
            else:
                # Try to auto-detect format
                data = self._auto_detect_format(line)
    ...
    def _extract_custom_data(self, line: str) -> Optional[Dict[str, Any]]:
        """
        Extract data using a custom parser function.
        
        Args:
            line: Log line to process using custom parser
            
        Returns:
            Extracted data dictionary or None if parsing failed
        """
        try:
            # Call the custom parser function
            data = self.custom_parser(line)
            
            # Verify this is valid data
            if isinstance(data, dict):
                return data
            ...
            print(f"Error in custom parser: {str(e)}")
            return None
    
    def _auto_detect_format(self, line: str) -> Optional[Dict[str, Any]]:
        """
        Try to auto-detect the format of a log entry.
        
        Args:
            line: Log line to process
            
        Returns:
            Extracted data dictionary or None if no format detected
        """
        # Try JSON first
        try:
            data = json.loads(line)
        ...
        # Try regex if pattern is defined
    ...
    def _create_interaction(self, data: Dict[str, Any]) -> Optional[LLMInteraction]:
        ...
        """
        try:
            # If field mapping is provided, use it to map fields
                ...
                for target_field, source_field in self.field_mapping.items():
                    ...
                    if isinstance(source_field, str) and '.' in source_field:
                        ...
                        if value is not None:
                            # Handle nested target fields with dot notation
                            if '.' in target_field:
                        ...
                        if '.' in target_field:
                            ...
                            for key in keys[:-1]:
                                if key not in current:
                                    current[key] = {}
                                current = current[key]
                            current[keys[-1]] = data[source_field]
                        else:
                            mapped_data[target_field] = data[source_field]
                    
                    # Handle function mapping
                    elif callable(source_field):
                        mapped_data[target_field] = source_field(data)
                
                # Use the mapped data for creating the interaction
                data = mapped_data
            ...
            if tokens_data:
                input_tokens = tokens_data.get('input', 0) or tokens_data.get('prompt', 0) or tokens_data.get('input_tokens', 0) or 0
                output_tokens = tokens_data.get('output', 0) or tokens_data.get('completion', 0) or tokens_data.get('output_tokens', 0) or 0
                total_tokens = tokens_data.get('total', 0) or tokens_data.get('total_tokens', 0) or 0
