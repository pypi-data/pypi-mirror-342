...
def find_optimal_clusters(features: np.ndarray, max_clusters: int = 20, random_state: int = 42) -> int:
    ...
    try:
        ...
        for n_clusters in range(2, max_clusters + 1):
            # Use ensure_all_finite parameter instead of force_all_finite
...
def compute_quality_metrics(
    ...
    try:
        ...
        if n_components >= 2:
            ...
            original_features_clean = np.nan_to_num(original_features, nan=0.0, posinf=1e10, neginf=-1e10)
            selected_features_clean = np.nan_to_num(selected_features, nan=0.0, posinf=1e10, neginf=-1e10)
            
            pca = PCA(n_components=n_components)
            original_reduced = pca.fit_transform(original_features_clean)
            subset_reduced = pca.transform(selected_features_clean)
            
            try:
                # Ensure no NaN or inf values for ConvexHull
                if not np.isnan(original_reduced).any() and not np.isinf(original_reduced).any() and \
                   not np.isnan(subset_reduced).any() and not np.isinf(subset_reduced).any():
                ...
                logger.debug(f"ConvexHull calculation failed: {str(e)}")
    except ImportError:
