...
from maleo_metadata.clients.utils.logger import MaleoMetadataClientLoggerManager
from maleo_metadata.clients.http.controllers.blood_type import MaleoMetadataBloodTypeHTTPClientController
...
class MaleoMetadataBloodTypeClientService:
    @staticmethod
    @BaseExceptions.service_exception_handler(
        operation="retrieving blood types",
        logger=MaleoMetadataClientLoggerManager.get(),
    ...
    async def get_blood_types(
        ...
        if controller_type == MaleoMetadataGeneralEnums.ClientControllerType.HTTP:
            controller_result = await MaleoMetadataBloodTypeHTTPClientController.get_blood_types(parameters=parameters)
        else:
            message = "Invalid controller type"
    ...
    async def get_blood_type(
        ...
        if controller_type == MaleoMetadataGeneralEnums.ClientControllerType.HTTP:
            controller_result = await MaleoMetadataBloodTypeHTTPClientController.get_blood_type(parameters=parameters)
        else:
            message = "Invalid controller type"
            description = "The provided controller type has not been implemented"
            return MaleoMetadataBloodTypeGeneralResultsTransfers.Fail(message=message, description=description)
