...
class Conv2dDynamicSamePadding(nn.Conv2d):
    ...
    def forward(self, x):
        ...
        pad_h = max((oh - 1) * self.stride[0] + (kh - 1) *
                    self.dilation[0] + 1 - ih, 0)
        pad_w = max((ow - 1) * self.stride[1] + (kw - 1) *
                    self.dilation[1] + 1 - iw, 0)
...
class Conv2dStaticSamePadding(nn.Conv2d):
    ...
    def __init__(self, in_channels, out_channels, kernel_size, image_size=None,
                 **kwargs):
...
class BlockDecoder(object):
    """ Block Decoder for readability, straight from
    the official TensorFlow repository """
    ...
    def decode(string_list):
        ...
        :return: a list of BlockArgs named-tuples of block args
        """
    ...
    def encode(blocks_args):
        ...
        :param blocks_args: a list of BlockArgs named-tuples of block args
...
def efficientnet(width_coefficient=None, depth_coefficient=None, dropout_rate=0.2,
    ...
    global_params = GlobalParams(
        ...
        # removed, this is always true in PyTorch
...
def get_model_params(model_name, override_params):
    ...
    if model_name.startswith('efficientnet'):
        ...
        raise NotImplementedError('model name is not pre-defined: %s' %
                                  model_name)
    if override_params:
        # ValueError will be raised here if override_params
        # has fields not included in global_params.
