...
async def private_connection(
    ...
    log.debug(f"parse_dotenv(sub_account) {parse_dotenv(sub_account)}")

    log.warning(f"sub_account {sub_account} endpoint {endpoint} params {params} connection_url {connection_url}")
