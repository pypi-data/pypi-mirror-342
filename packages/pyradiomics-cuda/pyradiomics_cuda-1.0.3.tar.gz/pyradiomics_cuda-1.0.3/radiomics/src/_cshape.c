...
static PyObject *cshape_calculate_coefficients(PyObject *self, PyObject *args) {
  ...
  if (CALCULATE_COEFFICIENTS_FUNC(mask, size, strides, spacing, &SA, &Volume,
                                  diameters)) {
...
int check_arrays(PyArrayObject *mask_arr, PyArrayObject *spacing_arr, int *size,
  ...
  if (PyArray_NDIM(mask_arr) != dimension || PyArray_NDIM(spacing_arr) != 1) {
    ...
    PyErr_Format(PyExc_ValueError,
                 "Expected a %iD array for mask, 1D for spacing.", dimension);
    return 2;
  }

  if (!PyArray_IS_C_CONTIGUOUS(mask_arr) ||
      !PyArray_IS_C_CONTIGUOUS(spacing_arr)) {
    Py_XDECREF(mask_arr);
    Py_XDECREF(spacing_arr);
    PyErr_SetString(PyExc_ValueError,
                    "Expecting input arrays to be C-contiguous.");
    return 3;
  }
  ...
  if (PyArray_DIM(spacing_arr, 0) != PyArray_NDIM(mask_arr)) {
    ...
    Py_XDECREF(spacing_arr);
    PyErr_SetString(PyExc_ValueError,
                    "Expecting spacing array to have shape (3,).");
    return 4;
  }
  ...
  for (i = 0; i < dimension; i++) {
    ...
    strides[i] =
        (int)(PyArray_STRIDE(mask_arr, i) / PyArray_ITEMSIZE(mask_arr));
  }
