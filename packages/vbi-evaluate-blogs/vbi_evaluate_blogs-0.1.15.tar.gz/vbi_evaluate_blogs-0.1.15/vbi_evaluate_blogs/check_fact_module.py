...
def check_claims_with_tool(llm: AzureChatOpenAI, text: str) -> list[dict]:
    """
    ...
    def should_draw(state: State) -> bool:
        ...
        return "No relevant results found" in last_message and is_url(extract_url(last_message))

    def route(state: State):
        if should_draw(state):
            return "draw"
        elif "tool" in state["messages"][-1].content.lower():
            return "search"
        else:
            return END
    ...
    claims = llm.invoke(f"""
Bạn là chuyên gia trong việc tách các phát biểu cần xác thực từ trong các bài phân tích. Sau đây tôi sẽ đưa cho bạn 1 bài phân tích, bạn hãy trả về danh sách các phát biểu trên từng dòng.
...
if __name__ == "__main__":
    ...
    results = check_claims_with_tool(llm,text)
    for result in results:
