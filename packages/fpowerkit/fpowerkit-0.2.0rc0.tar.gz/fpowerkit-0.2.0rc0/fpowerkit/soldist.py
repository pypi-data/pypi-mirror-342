...
class DistFlowSolver(SolverBase):
    ...
    def __solve(self, _t: int, island: _Island, timeout_s: float, relaxV: bool, relaxI: bool, clear_results: bool = False) -> 'tuple[IslandResult, float]':
        ...
        for j, bus in island.BusItems():
            ...
            cons.append(
                inflow + cp.sum(pg[j]) == outflow + pd[j] - dec
            )
        ...
        for j, bus in island.BusItems():
            bus._v = v[j].value.item() ** 0.5

        for lid, line in island.LineItems():
            line.I = l[lid].value.item() ** 0.5
            line.P = P[lid].value.item()
            line.Q = Q[lid].value.item()

        for j, gen in island.GenItems():
            p = pg0[j]
            if isinstance(p, Var) and p.value is not None: gen._p = p.value.item()
            q = qg0[j]
            if isinstance(q, Var): gen._q = q.value.item()
        
        for pID, p in island.PVWItems():
            p._pr = pvwp[pID].value.item()
            p._qr = pvwq[pID].value.item()
            pgen = p.P(_t)
        ...
        for bus, lim in self._decb.items():
            lim.Reduction = Pdec[bus].value.item()
            if lim.Reduction < 1e-8: lim.Reduction = 0

        return IslandResult.OverFlow if overflow else IslandResult.OK, goal.value