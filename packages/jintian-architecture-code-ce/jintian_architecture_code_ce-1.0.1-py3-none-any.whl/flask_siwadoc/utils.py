...
def convert_query_params(query_prams: MultiDict, model: Type[BaseModel]) -> dict:
    ...
    return {
        ...
        **{key: value for key, value in query_prams.to_dict(flat=False).items() if
           key in model.model_fields and  (hasattr(model.model_fields[key].annotation, '__bases__') and (BaseModel in model.model_fields[key].annotation.__bases__ or list in model.model_fields[key].annotation.__mro__))}
    }
