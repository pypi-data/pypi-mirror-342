...
async def publish_manual(
    ...
    try:
        ...
        # Add a notification that a job has started
        manual_service._add_notification(
            status=NotificationStatus.PROCESSING,
            title="Manual Publication Started",
            message=f"Starting publication process for {solution_value}"
...
async def stream_notifications(
    ...
    - user_id field indicating who initiated the job
    - document_title field showing the title of the Notion document being processed
    - notification_type field indicating the type of notification:
        - "job_result": For job completion/failure notifications. Frontend should refresh the
                        notification panel in the top-right corner.
        - "document_update": For document update/creation notifications. Frontend should mark
                             the specific document node in the tree as needing to be published.
                             These notifications include a 'node' field with details about the updated document.
        - "system": For other system notifications.
    - node field (for document_update notifications) containing document node information:
        - object_id: The ID of the updated Notion page
        - title: The title of the document
        - is_directory: Whether the node is a directory
        - parent_id: The ID of the parent node
        - notion_url: URL to the Notion page
        - index: Position in the parent's children list
    """
    try:
            ...
            try:
                ...
                while True:
                    ...
                    try:
                        ...
                        # Add document title to notification if job_id exists and it's not already set
                        if (
                            notification.job_id
                            and notification.job_id in manual_service.active_jobs
...
@router.post("/update-manuals")
async def update_manuals(
    selected_solution: SolutionType = Query(
        default=SolutionType.ZCP,
        description="The solution type to update manuals for (zcp, apim, amdp)",
    ),
    notion_page_id: Optional[str] = Query(
        None, description="Optional Notion page ID that was created or updated"
    ),
    manual_service: ManualService = Depends(get_manual_service),
    current_user: TokenData = Depends(get_current_user),
):
    """Update the manuals cache for the specified solution.

    This endpoint can be called manually or set up as a webhook from Notion.
    It triggers a refresh of the cached manuals data for the specified solution.

    Args:
        selected_solution: The solution type to update
        notion_page_id: Optional specific page ID that was updated
    """
    try:
        success = await manual_service.update_manuals_cache(
            selected_solution, notion_page_id
        )

        if success:
            return {
                "status": "success",
                "message": f"Successfully updated manuals cache for {selected_solution.value}"
                + (f" with focus on page {notion_page_id}" if notion_page_id else ""),
                "timestamp": datetime.now().isoformat(),
            }
        else:
            raise HTTPException(
                status_code=500,
                detail=f"Failed to update manuals cache for {selected_solution.value}",
            )
    except Exception as e:
        logger.error(f"Error updating manuals: {str(e)}")
        raise HTTPException(status_code=500, detail=str(e))


@router.post("/webhook/notion", include_in_schema=True, openapi_extra={"security": []})
async def notion_webhook(
    request: Request,
    background_tasks: BackgroundTasks,
    webhook_event: NotionWebhookEvent = Body(
        ...,
        description="The webhook event data from Notion",
        examples={
            "verification": {
                "summary": "URL Verification Request",
                "value": {
                    "id": "123e4567-e89b-12d3-a456-426614174000",
                    "timestamp": "2024-04-23T15:44:21.123Z",
                    "workspace_id": "workspace_123",
                    "type": "url_verification",
                    "authors": [{"id": "user_123", "type": "person"}],
                    "entity": {"id": "test_entity", "type": "page"},
    ...
    """Webhook endpoint for Notion to trigger when any page is updated.

    This endpoint handles both webhook verification and actual webhook events from Notion.
    For verification:
    - Notion sends a POST request with a verification_token
    - We must respond with the same token to verify the webhook

    For actual webhook events:
    - Notion sends updates about specific pages with a signature header
    - We validate the signature using HMAC-SHA256
    - The service updates both in-memory and filesystem caches
    """
    try:
        ...
        logger.info(f"Received webhook request with body: {body_text}")
        logger.info(f"Received headers: {dict(request.headers)}")

        # Handle verification request (special case)
        if webhook_event.type == "url_verification":
            logger.info("Received Notion webhook verification request")
            if not webhook_event.data.challenge:
                raise HTTPException(
                    status_code=400, detail="Missing challenge in verification request"
                )
            return {
                "type": "url_verification",
                "challenge": webhook_event.data.challenge,
            }

        # For actual webhook events, validate the signature if present
        if x_notion_signature:
            verification_token = os.environ.get("NOTION_WEBHOOK_SECRET")
            ...
            if verification_token:
                ...
                logger.info(f"Raw verification token: {verification_token}")
                ...
                logger.warning("No verification token found for signature validation")

        # Process the webhook event based on its type
        event_type = webhook_event.type
        logger.info(f"Processing webhook event of type: {event_type}")

        # Extract page ID based on event type
        notion_page_id = None
        ...
        if not notion_page_id:
            raise HTTPException(
                status_code=400, detail="Could not determine page ID from webhook event"
            )

        logger.info(f"Extracted page ID from event: {notion_page_id}")
        ...
        try:
            ...
            raise HTTPException(
                status_code=400, detail=f"Invalid Notion page ID format: {str(e)}"
            )

        # Try to determine which solution this page belongs to
        solution_found = False
        for solution_type in SolutionType:
            if solution_type in manual_service.root_page_ids:
                # Update each solution's cache in the background
                background_tasks.add_task(
                    manual_service.update_manuals_cache, solution_type, notion_page_id
                )
                solution_found = True
        ...
        if not solution_found:
            raise HTTPException(
                status_code=404,
                detail="No solutions configured with root page IDs",
            )

        return {
            "status": "accepted",
            "message": f"Cache update scheduled for page {notion_page_id}",
            "timestamp": datetime.now().isoformat(),
        }
