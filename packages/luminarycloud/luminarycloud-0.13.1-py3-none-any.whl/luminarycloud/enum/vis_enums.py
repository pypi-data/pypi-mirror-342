...
def visquantity_text(quantity: VisQuantity) -> str:
    enum_opts = (
        quantitypb.DESCRIPTOR.enum_types_by_name["QuantityType"]
        .values_by_number[quantity]
        .GetOptions()
    )
    # Should be one fields object w/ the options
    # there isn't really any documentation for this in protobuf
    # ListFields returns a list of tuples, there should just be one entry
    # and the second member of the tuple is the extensions
    extensions = enum_opts.ListFields()[0][1]
