...
bstack111l11lll1l_opy_ = 1000
bstack111l11lll11_opy_ = 2
class bstack111l1l11111_opy_:
    def __init__(self, handler, bstack111l1l111l1_opy_=bstack111l11lll1l_opy_, bstack111l11ll111_opy_=bstack111l11lll11_opy_):
        self.queue = []
    ...
    def start(self):
        if not (self.timer and self.timer.is_alive()):
            self.bstack111l1l1111l_opy_()
    def bstack111l1l1111l_opy_(self):
        self.bstack1l1l1llll1l_opy_ = threading.Event()
        def bstack111l11lllll_opy_():
            self.bstack1l1l1llll1l_opy_.wait(self.bstack111l11ll111_opy_)
            if not self.bstack1l1l1llll1l_opy_.is_set():
    ...
    def bstack111l11ll1ll_opy_(self):
        try:
            if self.bstack1l1l1llll1l_opy_ and not self.bstack1l1l1llll1l_opy_.is_set():
                self.bstack1l1l1llll1l_opy_.set()
            if self.timer and self.timer.is_alive() and self.timer != threading.current_thread():
                self.timer.join()
        except Exception as e:
            logger.debug(bstack1ll1l11_opy_ (u"ࠬࡡࡳࡵࡱࡳࡣࡹ࡯࡭ࡦࡴࡠࠤࡊࡾࡣࡦࡲࡷ࡭ࡴࡴ࠺ࠡࠩᵜ") + (str(e) or bstack1ll1l11_opy_ (u"ࠨࡅࡹࡥࡨࡴࡹ࡯࡯࡯ࠢࡦࡳࡺࡲࡤࠡࡰࡲࡸࠥࡨࡥࠡࡥࡲࡲࡻ࡫ࡲࡵࡧࡧࠤࡹࡵࠠࡴࡶࡵ࡭ࡳ࡭ࠢᵝ")))
        finally:
            self.timer = None
    ...
    def bstack111l11ll11l_opy_(self, source = bstack1ll1l11_opy_ (u"ࠧࠨᵞ")):
        with self.lock:
            if not self.queue:
        ...
        self.handler(data)
        if source != bstack1ll1l11_opy_ (u"ࠨࡵ࡫ࡹࡹࡪ࡯ࡸࡰࠪᵟ"):
            self.bstack111l11ll1l1_opy_()
    def shutdown(self):
        self.bstack111l11ll1ll_opy_()
        while self.queue:
            self.bstack111l11ll11l_opy_(source=bstack1ll1l11_opy_ (u"ࠩࡶ࡬ࡺࡺࡤࡰࡹࡱࠫᵠ"))