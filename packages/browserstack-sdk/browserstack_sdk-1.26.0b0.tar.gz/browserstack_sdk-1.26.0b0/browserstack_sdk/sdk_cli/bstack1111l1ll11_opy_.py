...
class bstack1111l1l1ll_opy_:
    timeout: int
    bstack1111l1ll1l_opy_: Union[None, Callable]
    bstack1111l1llll_opy_: Union[None, Callable]
    def __init__(self, timeout=1, bstack1111l1lll1_opy_=1, bstack1111l1ll1l_opy_=None, bstack1111l1llll_opy_=None):
    ...
    def enqueue(self, job: Callable):
        if not callable(job):
            raise ValueError(bstack11111ll_opy_ (u"ࠦ࡮ࡴࡶࡢ࡮࡬ࡨࠥࡰ࡯ࡣ࠼ࠣࠦထ") + type(job))
        self.queue.put(job)
    def start(self):
        if self.threads:
            return
        ...
        while not self.bstack1111ll1111_opy_.is_set():
            try:
                ...
                try:
                    ...
                    if callable(self.bstack1111l1ll1l_opy_):
                        self.bstack1111l1ll1l_opy_(e, job)
                finally:
                    self.queue.task_done()
            except queue.Empty:
                pass
            except Exception as e:
                if callable(self.bstack1111l1llll_opy_):
                    self.bstack1111l1llll_opy_(e)