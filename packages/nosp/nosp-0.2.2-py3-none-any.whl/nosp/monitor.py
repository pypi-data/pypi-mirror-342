...
MONITOR_ENDPOINT = "http://127.0.0.1:12000/endpoint"
LOG_PATH = Path("C:\\logs") if sys.platform == "win32" else Path("/logs")
MAX_RETRY = 100
MONITOR_INTERVAL = 10

class SpiderInfo:
    def __init__(self, name=None, group_name=None, monitor=False, show_debug=False,monitor_endpoint='http://127.0.0.1:12000/endpoint'):
    ...
    def notice(self):
        try:
            self.debug('上报数据')
            print(self.to_dict())
            response = requests.post(url=self._monitor_endpoint, json=self.to_dict(), timeout=4)
    ...
    def start_monitor(self):
        self.debug('启动监控')
        monitor_thread = threading.Thread(target=self.__monitor)
