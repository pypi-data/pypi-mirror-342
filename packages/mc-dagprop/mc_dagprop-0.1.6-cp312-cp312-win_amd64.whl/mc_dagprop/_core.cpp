...
struct SimContext {
    ...
    SimContext(
        ...
        double md
    ): events(std::move(ev)), activities(std::move(am)), precedence_list(std::move(pl)), max_delay(md) {}
};
...
struct ExponentialDist {
    ...
    ExponentialDist(double lam, double mx): lambda(lam), max_scale(mx), dist(1.0 / lam) {}
...
class GenericDelayGenerator {
    ...
    void add_constant(int activity_type, double factor) {
        dist_map_[activity_type] = ConstantDist {
            factor
        };
    }

    void add_exponential(int activity_type, double lambda, double max_scale) {
        dist_map_[activity_type] = ExponentialDist {
            lambda,
            max_scale
        };
    }
...
class Simulator {
    ...
    public:
        Simulator(SimContext c, GenericDelayGenerator g): ctx_(std::move(c)), gen_(std::move(g)) {
            ...
            for (auto & kv: ctx_.activities) {
                acts_[idx] = kv.second;
                is_affected_[idx] = (kv.second.activity_type == 1 && kv.second.duration >= 0.01);
    ...
    SimResult run(int seed) {
        ...
        return SimResult {
            std::move(realized), std::move(compounded), std::move(cause_event)
        };
    }
