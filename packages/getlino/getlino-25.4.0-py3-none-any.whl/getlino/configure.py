...
RELOAD_NAME = '/usr/local/bin/reload_services.sh'
...
# HEALTHCHECK_CRON_JOB = """\
# #!/bin/bash
# /usr/local/bin/healthcheck.sh > /dev/null
# # we redirect STDOUT to null, but not STDERR because if there is an error we want cron to notify the admins.
# """
...
add('--sites-base', default_sites_base,
    'Base directory for Lino sites on this server')
add('--local-prefix', 'lino_local',
    "Prefix for local server-wide importable packages")
add('--shared-env', default_shared_env,
    "Root directory of your shared virtualenv")
...
add('--clone/--no-clone', False,
    "Clone all contributor repositories and install them to the shared-env")
# add('--branch', 'master', "The git branch to use for --clone")
add('--webdav/--no-webdav',
    True,
    "Whether to enable webdav on new sites",
    root_only=True)
add('--backups-base',
    '/var/backups/lino',
...
add('--usergroup',
    'www-data',
    "User group for files to be shared with the web server",
    root_only=True)
add('--supervisor-dir',
...
add('--repos-link', 'repositories',
    "link to code repositories (relative to virtualenv)")
add('--appy/--no-appy',
    ifroot,
    "Whether this server provides appypod and LibreOffice",
    root_only=True)
add('--weasyprint/--no-weasyprint',
    ifroot,
    "Whether this server provides weasyprint",
    root_only=True)
...
add('--https/--no-https',
    False,
    "Whether this server uses secure http",
    root_only=True)
add('--ldap/--no-ldap',
    False,
    "Whether this server works as an LDAP server",
    root_only=True)
add('--monit/--no-monit',
    True,
    "Whether this server uses monit",
    root_only=True)
add('--web-server',
    '',
    "Which web server to use here.",
    click.Choice([e.name for e in WEB_SERVERS] + ['']),
    root_only=True)
...
add(
    '--db-user', '',
    "Default database user name for new sites. Leave empty to use the project name."
)
add(
    '--db-password', '',
    "Default database password for new sites. Leave empty to generate a secure password."
)
...
def configure(ctx, batch, sites_base, local_prefix, shared_env, repos_base,
    ...
    i.ask_options(CONFIGURE_OPTIONS, locals())
    ...
    weasyprint = DEFAULTSECTION.getboolean('weasyprint')
    supervisor = web_server or appy
    devtools = DEFAULTSECTION.getboolean('devtools')
    ...
    if db_user and not db_password:
        raise click.Error(
            "If you set a shared --db-user you must also set a shared --db-password"
        )
    ...
    if not i.yes_or_no("Start installing Lino according to above settings?"):
        raise click.Abort()
    ...
    i.apt_install("build-essential")  # needed for installing mysqlclient
    i.apt_install("swig")  # required to install eidreader
    i.apt_install(
        "libldap2-dev libsasl2-dev")  # Needed e.g. by django_auth_ldap
    ...
    if weasyprint:
        ...
        i.apt_install(
            "python3-pip libpango-1.0-0 libpangoft2-1.0-0 libharfbuzz-subset0")
    ...
    if ifroot():
        for k in ("log_base", "backups_base"):
            ...
            if not pth:
                ...
                continue
            if not os.path.exists(pth):
                if batch or i.yes_or_no("Create {} {} ?".format(k, pth),
                                        default=True):
                    os.makedirs(pth, exist_ok=True)
    ...
    if ifroot() and DEFAULTSECTION.getboolean('monit'):
        ...
        if distro.id() == "debian" and distro.codename() == "buster":
            if batch or i.yes_or_no("Add backport for monit on Debian buster?",
                                    default=True):
    ...
    if clone:
        ...
        if not os.path.exists(repos_base):
            if batch or i.yes_or_no(
                    "Create base directory for repositories {} ?".format(
                        repos_base),
                    default=True):
        ...
        if batch or i.yes_or_no(
                "Clone repositories to {} ?".format(repos_base), default=True):
            with i.override_batch(True):
                for repo in repos:
                    i.clone_repo(repo)
        if batch or i.yes_or_no(
                "Install cloned repositories to {} ?".format(shared_env),
                default=True):
            # getlino must not reinstall itself because that causes at least
            # TemplateNotFound issues
            with i.override_batch(True):
                for repo in repos:
                    if repo.package_name:
                        if repo.package_name != "getlino":
                            i.install_repo(repo, shared_env)
        go_bases.append(repos_base)
        for repo in repos:
            if repo.package_name:
                name = repo.package_name.replace("-", "_")
                pth = Path(repos_base, repo.nickname, name, "projects")
                if pth.exists():
                    go_bases.append(str(pth))
                # else:
                #     print("20240708 no projects dir in", repo)

    pth = DEFAULTSECTION.get('sites_base')
    if not os.path.exists(pth):
        if batch or i.yes_or_no(
                "Create base directory for sites {} ?".format(pth),
                default=True):
            os.makedirs(pth, exist_ok=True)
    i.check_permissions(pth)
    ...
    if os.path.exists(pth):
        i.check_permissions(pth)
    elif batch or i.yes_or_no(
            "Create shared settings package {} ?".format(pth), default=True):
        os.makedirs(pth, exist_ok=True)
        i.check_permissions(pth)
    with i.override_batch(True):
        i.check_permissions(pth)
        fn = join(pth, '__init__.py')
        i.write_file(fn, '')
        i.check_permissions(fn)
    fn = join(pth, 'settings.py')
    i.write_file(fn, SHARED_SETTINGS.format(**DEFAULTSECTION))
    i.check_permissions(fn)
    go_bases.append(pth)
    ...
    i.write_file(pth, content)
    i.check_permissions(pth, writeable=False)
    click.echo(
        "Note: please add manually the following line to your .bashrc file:\nsource {}"
        .format(pth))

    if ifroot():
        if supervisor:
            i.write_logrotate_conf('supervisor.conf',
                                   '/var/log/supervisor/supervisord.log')
            i.must_restart('supervisor')

        i.jinja_write(RELOAD_NAME, **context)
        i.check_permissions(RELOAD_NAME, executable=True)
        ...
        # i.write_cron_job('healthcheck',
        #                  HEALTHCHECK_CRON_JOB.format(**context),
        #                  freq="hourly")
        ...
        if DEFAULTSECTION.getboolean('https'):
            ...
            if certbot_cmd:
                if batch or i.yes_or_no(
                        "Set up automatic certificate renewal?", default=True):
                    i.write_file('/etc/cron.d/getlino-certbot.conf',
                                 CERTBOT_AUTO_RENEW.format(certbot_cmd))
...
configure = click.Command('configure',
                          callback=configure,
                          params=params,
