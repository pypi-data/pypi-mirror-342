...
''')
    ...
    def test_simple_loop(self):
        ...
        if sys.version_info < (3,):
            self.assertEqual(
                '''Loop involving u'foo' while expanding "{foo}".''',
                str(cm.exception))
        else:
            self.assertEqual(
                '''Loop involving 'foo' while expanding "{foo}".''',
                str(cm.exception))
...
bfoo = {basename}
''')
        self.assertEqual('dir', stack.get('bfoo', expand=True))
...
bfoo = {basename}
''')
        self.assertEqual('subdir', stack.get('bfoo', expand=True))
...
''')
        self.assertEqual('loc-foo/dir', stack.get('lfoo', expand=True))
...
''')
    ...
    def test_expand_relpath_unknown_in_global(self):
...
''')
        ...
        self.assertEqual('loc-foo/dir', stack.get('gfoo', expand=True))
...
class TestSharedStores(unittest.TestCase):
    ...
    def test_file_store_is_shared(self):
    ...
    def test_cmdline_store_is_shared(self):
        cls1 = stacks.get_shared_store(stores.CommandLineStore())
        cls2 = stacks.get_shared_store(stores.CommandLineStore())
        self.assertIs(cls1, cls2)

    def test_unique_memory_stacks(self):
        s1 = stacks.MemoryStack(id='<string1>')
        s2 = stacks.MemoryStack(id='<string2>')
        self.assertIsNot(stacks.get_shared_store(s1.store),
                         stacks.get_shared_store(s2.store))

    def test_shared_memory_stack(self):
        s1 = stacks.MemoryStack()
        s2 = stacks.MemoryStack()
        self.assertIs(stacks.get_shared_store(s1.store),
                      stacks.get_shared_store(s2.store))
