...
def find_cImean_block(points_new, c_un, tik0, tik1):
    ...
    for i in c_un[tik0:tik1]:
        i_c_[j] = np.mean(points_new[points_new[:, -1] == i, -2])
        j += 1
    return i_c_
...
def find_CS_plus(txti_in,num_cpu=mp.cpu_count(),z_range=1):
    '环缝提取（普适版）'
    # 点云强度值离散化
    txti_in[:, -2] = zm.normalization(txti_in[:, -2], 255)  # 强度值离散化
    # 计算每个环的平均强度值
    num_C = len(np.unique(txti_in[:,4]))  # 圆环数量
    i_c = np.empty(num_C)  # 新建一个存储圆环平均强度值的容器
    # 并行计算准备
    ...
    for i in range(num_CS_0):
        # 求强度值极低点
        id_min_i_ = np.argmin(i_c[id_CS_0[i] - belong_i:id_CS_0[i] + belong_i]) + id_CS_0[i] - belong_i
        # 寻找最大X值与最小X值位置
        c_name_ins[i] = c_un[id_min_i_]  # 强度值极低点位置
        # name_min_i_ = xzrc[id_min_i_, -1]  # 强度值极低点位置
        names_c_ = np.arange(c_name_ins[i] - belong_i, c_name_ins[i] + belong_i)  # X的搜索范围
        id_x_max_ = np.argmax(txti_in[np.isin(txti_in[:, -1], names_c_), 0])  # 最大X的id
        id_x_min_ = np.argmin(txti_in[np.isin(txti_in[:, -1], names_c_), 0])  # 最小X的id
        id_c_x_max = txti_in[np.isin(txti_in[:, -1], names_c_), -1]
        id_c_x_min = txti_in[np.isin(txti_in[:, -1], names_c_), -1]
    ...
    c_xyzic = txti_in[np.isin(txti_in[:, -1], seams_all), :]  # 返回xyzic[:, -1]中有c[c_in]的行数
    # 精简衬砌表面
    id_del = np.isin(np.isin(txti_in[:, -1], seams_all), False)  # 除去环缝点云的点云下标
    txti_delC = txti_in[id_del, :]  # 去除环缝的点云
    return txti_delC, c_xyzic
